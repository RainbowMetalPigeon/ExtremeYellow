; wEventFlags bit flags

; Pallet Town events
	const_def
	const EVENT_FOLLOWED_OAK_INTO_LAB
	const EVENT_PRESSED_FOR_INFO_IN_STATUS_SCREEN ; new
	const EVENT_PRESSED_FOR_INFO_IN_OPTIONS ; new
	const EVENT_HALL_OF_FAME_DEX_RATING
	const EVENT_INFORMED_ABOUT_OPTIONS
	const EVENT_PLAYER_AT_RIGHT_EXIT_TO_PALLET_TOWN
	const EVENT_PALLET_AFTER_GETTING_POKEBALLS
	const EVENT_SECOND_HANDLING_RANDOM_CHART
	const EVENT_OPPONENT_SECONDARY_EFFECT_GUARANTEED ; new
	const EVENT_PLAYER_MINIMUM_ROLL ; new
	const EVENT_RANDOM_WILD_BOTH_GRASS_AND_WATER ; new
	const EVENT_REMATCHING_TRAINER ; new
	const EVENT_MOM_TOLD_US_ABOUT_RUNNING_SHOES ; new
	const EVENT_ALREADY_SPOKEN_WITH_DAISY_CLUES ; new
	const EVENT_ALREADY_SPOKEN_WITH_DARK_GUIDE ; new
	const EVENT_OBTAIN_ANY_MAP_PIECE ; new
	const EVENT_OBTAIN_MAP_PIECE_1_BIRD_FAN_CLUB ; new
	const EVENT_OBTAIN_MAP_PIECE_2_PIGEON ; new
	const EVENT_OBTAIN_MAP_PIECE_2_PIGEON_FAILED ; new
	const EVENT_OBTAIN_MAP_PIECE_3_TREASURE_HUNTER ; new
	const EVENT_OBTAIN_MAP_PIECE_3_TREASURE_HUNTER_FAILED ; new
	const EVENT_OBTAIN_MAP_PIECE_4_RESCUED_TRAVELER ; new
	const EVENT_OBTAIN_MAP_PIECE_4_RESCUED_TRAVELER_FAILED ; new
	const EVENT_USE_EXTRA_HIDESHOW ; new
	const EVENT_GOT_TOWN_MAP
	const EVENT_ENTERED_BLUES_HOUSE
	const EVENT_DAISY_WALKING
	const EVENT_SLIDING_ON_ICE ; new ; there were 5 empty events
	const EVENT_SOUND_PLAYING_DURING_TEXT ; new
	const EVENT_AI_SWITCH_MON ; new
	const EVENT_OAK_GIFTED_SHINY_CHARM ; new
	const EVENT_1ST_SPOKE_WITH_OAKSLAB_GIOVANNI ; new
	const EVENT_FOLLOWED_OAK_INTO_LAB_2
	const EVENT_OAK_ASKED_TO_CHOOSE_MON
	const EVENT_GOT_STARTER
	const EVENT_BATTLED_RIVAL_IN_OAKS_LAB
	const EVENT_GOT_POKEBALLS_FROM_OAK
	const EVENT_GOT_POKEDEX
	const EVENT_PALLET_AFTER_GETTING_POKEBALLS_2
	const EVENT_OAK_APPEARED_IN_PALLET

; Viridian City events
	const_next $28
	const EVENT_VIRIDIAN_GYM_ALREADY_INSTRUCTED_ABOUT_INTERIM_LEADER ; new
	const EVENT_BEAT_SEVII_ROUTE_35_TRAINER_1 ; new
	const EVENT_RESCUED_SELPHY ; new
	const EVENT_PLAYED_COLLISION_SOUND_WHILE_SLIDING_ALREADY_ONCE ; new
	const EVENT_02C
	const EVENT_02D
	const EVENT_02E
	const EVENT_02F
	const EVENT_DEFEATED_SEVII_SAGE_SANTRE ; new
	const EVENT_BEAT_OBSIDIAN_MINES_TRAINER_0 ; new
	const EVENT_SEVII_TICKET_UNLOCKED_UP_TO_5 ; new
	const EVENT_SEVII_TICKET_UNLOCKED_UP_TO_8 ; new
	const EVENT_BEAT_ROUTE_27_TRAINER_0 ; new
	const EVENT_BEAT_ROUTE_27_TRAINER_1 ; new
	const EVENT_BEAT_ROUTE_27_TRAINER_2 ; new
	const EVENT_PROPOSED_CATCHING_TUTORIAL ; new
	const EVENT_OAK_GOT_PARCEL
	const EVENT_GOT_OAKS_PARCEL
	const EVENT_GOT_TM42
	const EVENT_GOT_WATERFALL_FROM_SELPHY ; new
	const EVENT_DIVE_GO_UNDER ; new
	const EVENT_DIVE_GO_ABOVE ; new
	const EVENT_BEAT_ROUTE_28_TRAINER_0 ; new
	const EVENT_BEAT_ROUTE_28_TRAINER_1 ; new
	const EVENT_BEAT_ROUTE_28_TRAINER_2 ; new
	const EVENT_BEAT_OBSIDIAN_WOOD_TRAINER_0 ; new
	const EVENT_BEAT_OBSIDIAN_WOOD_TRAINER_1 ; new
	const EVENT_BEAT_OBSIDIAN_WOOD_TRAINER_2 ; new
	const EVENT_BEAT_OBSIDIAN_WOOD_TRAINER_3 ; new
	const EVENT_ALREADY_SPOKEN_WITH_OBSIDIAN_SCIENTIST ; new
	const EVENT_ALREADY_REWARDED_ORB_OBSIDIAN_SCIENTIST ; new
	const EVENT_ALREADY_REWARDED_MONEY_OBSIDIAN_SCIENTIST ; new
	const EVENT_OBSIDIAN_WAREHOUSE_SWITCH_CLICKED ; new
	const EVENT_BEAT_OBSIDIAN_WAREHOUSE_TRAINER_0 ; new
	const EVENT_BEAT_OBSIDIAN_WAREHOUSE_TRAINER_1 ; new
	const EVENT_BEAT_OBSIDIAN_WAREHOUSE_TRAINER_2 ; new
	const EVENT_BEAT_OBSIDIAN_WAREHOUSE_TRAINER_3 ; new
	const EVENT_GOT_AMULET_COIN ; new
	const EVENT_DOING_WATERFALL ; new
	const EVENT_DOING_ROCK_CLIMB ; new
	const EVENT_GOT_TM27 ; now useless
	const EVENT_BEAT_VIRIDIAN_GYM_GIOVANNI
;	const EVENT_BEAT_VIRIDIAN_GYM_TRAINER_0
	const EVENT_BEAT_VIRIDIAN_GYM_TRAINER_1
	const EVENT_BEAT_VIRIDIAN_GYM_TRAINER_2
	const EVENT_BEAT_VIRIDIAN_GYM_TRAINER_3
	const EVENT_BEAT_VIRIDIAN_GYM_TRAINER_4
	const EVENT_BEAT_VIRIDIAN_GYM_TRAINER_5
	const EVENT_BEAT_VIRIDIAN_GYM_TRAINER_6
	const EVENT_BEAT_VIRIDIAN_GYM_TRAINER_7
	const EVENT_BEAT_VIRIDIAN_GYM_TRAINER_8
	const EVENT_JUST_OPENED_TOWN_MAP ; new

; Pewter City events
	const_next $68
	const EVENT_BOUGHT_MUSEUM_TICKET
	const EVENT_GOT_OLD_AMBER
	const EVENT_BEAT_SEVII_ROUTE_34_TRAINER_1 ; new
	const EVENT_BEAT_SEVII_ROUTE_34_TRAINER_2 ; new
	const EVENT_BEAT_SEVII_ROUTE_34_TRAINER_3 ; new
	const EVENT_BEAT_SEVII_ROUTE_34_TRAINER_4 ; new
	const EVENT_GOT_PEWTER_REPELS ; new
	const EVENT_SEVII_ONE_ISLAND_CELIO_JENNY_DIALOGUE ; new
	const EVENT_GAVE_DRINK_TO_WEATHER_TUTOR ; new
	const EVENT_BEAT_BROCK_REMATCH ; new
	const EVENT_BEAT_PEWTER_GYM_TRAINER_0
	const EVENT_BEAT_PEWTER_GYM_TRAINER_1
	const EVENT_DEFEATED_SEVII_SAGE_ICHINO ; new
	const EVENT_DEFEATED_SEVII_SAGE_YOTTRO ; new
	const EVENT_GOT_TM34
	const EVENT_BEAT_BROCK

; Cerulean City events
	const_next $98 ; TBE
	const EVENT_BEAT_CERULEAN_RIVAL
	const EVENT_BEAT_ROUTE_29_TRAINER_0 ; new
	const EVENT_BEAT_ROUTE_29_TRAINER_1 ; new
	const EVENT_BEAT_ROUTE_29_TRAINER_2 ; new
	const EVENT_BEAT_ROUTE_29_TRAINER_3 ; new
	const EVENT_BEAT_ROUTE_29_TRAINER_4 ; new
	const EVENT_BEAT_ROUTE_29_TRAINER_5 ; new
	const EVENT_BEAT_ROUTE_29_TRAINER_6 ; new
	const EVENT_BEAT_ROUTE_29_TRAINER_7 ; new
	const EVENT_BEAT_ROUTE_29_TRAINER_8 ; new
	const EVENT_PLAYER_CANNOT_ATTACK_PSYCHIC_TERRAIN ; new
	const EVENT_ENEMY_CANNOT_ATTACK_PSYCHIC_TERRAIN ; new
	const EVENT_DEFEATED_SEVII_SAGE_NIUE ; new
	const EVENT_DEFEATED_SEVII_SAGE_GONQUE ; new
	const EVENT_SEVII_RESCUED_MAYOI ; new
	const EVENT_BEAT_CERULEAN_ROCKET_THIEF
	const EVENT_GOT_BULBASAUR_IN_CERULEAN
	const EVENT_SEVII_JUST_ENTERED_SEVEN_GYM ; new
	const EVENT_ENGAGED_ROKUSEI ; new
	const EVENT_DEFEATED_SEVII_SAGE_ROKUSEI ; new
	const EVENT_WEATHER_SUNNY_DAY ; new
	const EVENT_WEATHER_RAIN_DANCE ; new
	const EVENT_WEATHER_SANDSTORM ; new
	const EVENT_WEATHER_HAIL ; new
	const EVENT_ENABLE_WEATHER_DAMAGE ; new
	const EVENT_TERRAIN_GRASSY ; new
	const EVENT_TERRAIN_ELECTRIC ; new
	const EVENT_TERRAIN_MISTY ; new
	const EVENT_TERRAIN_PSYCHIC ; new
	const EVENT_ENABLE_TERRAIN_HEALING ; new
	const EVENT_TRICK_ROOM ; new
	const EVENT_HAZARDS_DAMAGING_PLAYER ; new
	const EVENT_FREED_ROUTE_4 ; new
	const EVENT_BEAT_MISTY_REMATCH ; new
	const EVENT_BEAT_CERULEAN_GYM_TRAINER_0
	const EVENT_BEAT_CERULEAN_GYM_TRAINER_1
	const EVENT_BEAT_CERULEAN_GYM_TRAINER_2
	const EVENT_BATTLE_CAN_BE_LOST ; new
	const EVENT_GOT_TM11
	const EVENT_BEAT_MISTY
	const EVENT_GOT_BICYCLE

; Lavender Town events
	const_next $F0 - 2 ; TBE?
	const EVENT_POKEMON_TOWER_RIVAL_ON_LEFT
	const EVENT_BEAT_POKEMON_TOWER_RIVAL
	const EVENT_SEVII_ALREADY_WARNED_ABOUT_ANOMALIES ; new
	const EVENT_BEAT_POKEMONTOWER_3_TRAINER_0
	const EVENT_BEAT_POKEMONTOWER_3_TRAINER_1
	const EVENT_BEAT_POKEMONTOWER_3_TRAINER_2
	const EVENT_SEVII_CLEARED_CHRONO_WAREHOUSE ; new
	const EVENT_BEAT_SEVII_FIVE_ISLAND_WAREHOUSE_2F_TRAINER_1 ; new
	const EVENT_SEVII_TANOBY_SOLVED_CHAMBER_1 ; new
	const EVENT_SEVII_TANOBY_TIME_PASSED ; new
	const EVENT_SEVII_TANOBY_TRACK_TIME ; new
	const EVENT_BEAT_POKEMONTOWER_4_TRAINER_0
	const EVENT_BEAT_POKEMONTOWER_4_TRAINER_1
	const EVENT_BEAT_POKEMONTOWER_4_TRAINER_2
	const EVENT_SEVII_TANOBY_CHAMBER_6_STARTED_PATH ; new
	const EVENT_SEVII_TANOBY_SOLVED_CHAMBER_6 ; new
	const EVENT_SEVII_TANOBY_SOLVED_CHAMBER_5 ; new
	const EVENT_SEVII_TANOBY_SOLVED_CHAMBER_4 ; new
	const EVENT_SEVII_TANOBY_SOLVED_CHAMBER_3 ; new
	const EVENT_SEVII_TANOBY_SOLVED_CHAMBER_2 ; new
	const EVENT_BEAT_POKEMONTOWER_5_TRAINER_0
	const EVENT_BEAT_POKEMONTOWER_5_TRAINER_1
	const EVENT_BEAT_POKEMONTOWER_5_TRAINER_2
	const EVENT_BEAT_POKEMONTOWER_5_TRAINER_3
	const EVENT_GOT_FUJI_MEGA_STONE_X ; new
	const EVENT_IN_PURIFIED_ZONE
	const EVENT_SEVII_JUST_ENTERED_SIX_GYM ; new
	const EVENT_SEVII_UNLOCKED_TANOBY_CHAMBERS ; new
	const EVENT_BEAT_POKEMONTOWER_6_TRAINER_0
	const EVENT_BEAT_POKEMONTOWER_6_TRAINER_1
	const EVENT_BEAT_POKEMONTOWER_6_TRAINER_2
	const EVENT_SEVII_UNLOCKED_PEARL_PC ; new
	const EVENT_SEVII_STOLEN_PEARL ; new
	const EVENT_BEAT_GHOST_MAROWAK
	const EVENT_SEVII_ROUTE_43_CAVES_SWITCH_5 ; new
	const EVENT_BEAT_POKEMONTOWER_7_TRAINER_0
	const EVENT_BEAT_POKEMONTOWER_7_TRAINER_1
	const EVENT_BEAT_POKEMONTOWER_7_TRAINER_2
	const EVENT_SEVII_ROUTE_43_CAVES_SWITCH_1 ; new
	const EVENT_SEVII_ROUTE_43_CAVES_SWITCH_2 ; new
	const EVENT_SEVII_ROUTE_43_CAVES_SWITCH_7 ; new
	const EVENT_RESCUED_MR_FUJI_2
	const EVENT_SEVII_ROUTE_43_CAVES_SWITCH_3 ; new
	const EVENT_SEVII_ROUTE_43_CAVES_SWITCH_4 ; new
	const EVENT_BEAT_OBSIDIAN_WAREHOUSE_FINAL_JESSIEJAMES
	const EVENT_BEAT_OBSIDIAN_WAREHOUSE_FINAL_TRAINER_0
	const EVENT_BEAT_OBSIDIAN_WAREHOUSE_FINAL_TRAINER_1
	const EVENT_BEAT_OBSIDIAN_WAREHOUSE_FINAL_TRAINER_2
	const EVENT_BEAT_OBSIDIAN_WAREHOUSE_FINAL_TRAINER_3
	const EVENT_BEAT_OBSIDIAN_WAREHOUSE_FINAL_TRAINER_4 ; Warehouse Giovanni
	const EVENT_ENHANCED_RODS ; new
	const EVENT_SEVII_FIVE_ISLAND_WAREHOUSE_BF_OPENED_DOOR_1 ; new
	const EVENT_SEVII_FIVE_ISLAND_WAREHOUSE_BF_OPENED_DOOR_2 ; new
	const EVENT_SEVII_FIVE_ISLAND_WAREHOUSE_BF_OPENED_DOOR_3 ; new
	const EVENT_SEVII_FIVE_ISLAND_WAREHOUSE_BF_OPENED_DOOR_4 ; new
	const EVENT_BEAT_SEVII_ROUTE_33_TRAINER_1 ; new
	const EVENT_BEAT_SEVII_ROUTE_33_TRAINER_2 ; new
	const EVENT_BEAT_SEVII_ROUTE_33_TRAINER_3 ; new
	const EVENT_BEAT_SEVII_ROUTE_33_TRAINER_4 ; new
	const EVENT_BEAT_SEVII_ROUTE_33_TRAINER_5 ; new
	const EVENT_SECOND_BATTLE_IN_SWAP_BATTLE ; new
	const EVENT_NON_DEFAULT_ENEMY_NICKNAME_LOADING ; new
	const EVENT_SKIP_FIRST_VICTORY_TEXT_IN_SWAP_BATTLE ; new
	const EVENT_BLACKOUT_FROM_SWAP_SECOND_BATTLE ; new
	const EVENT_SWAP_MODE_REPEAT_BATTLE ; new
	const EVENT_BEAT_SEVII_FIVE_ISLAND_CITY_TRAINER_1 ; new
	const EVENT_BEAT_SEVII_FIVE_ISLAND_CITY_TRAINER_2 ; new
	const EVENT_BEAT_SEVII_FIVE_ISLAND_CITY_TRAINER_3 ; new
	const EVENT_BEAT_SEVII_FIVE_ISLAND_CITY_TRAINER_4 ; new
	const EVENT_GOT_POKE_FLUTE
	const EVENT_GOT_SQUIRTLE_FROM_OFFICER_JENNY
	const EVENT_ALREADY_ENTERED_TYPE_CHART_MATCHFINDER ; new
	const EVENT_FLAG_DELTA_SPECIES_ENEMY_EVENT ; new
	const EVENT_SEVII_THREE_GOT_GIFT_FROM_KID ; new
	const EVENT_OBTAINED_SEEDS_BAG ; new
	const EVENT_AT_LEAST_ONE_EVOLUTION_TO_PRINT_IN_DEX ; new
	const EVENT_BEAT_SEVII_ROUTE_36_TRAINER_1 ; new
	const EVENT_BEAT_SEVII_ROUTE_36_TRAINER_2 ; new
	const EVENT_BEAT_SEVII_ROUTE_36_TRAINER_3 ; new
	const EVENT_BEAT_SEVII_ROUTE_36_TRAINER_4 ; new
	const EVENT_BEAT_SEVII_ROUTE_36_TRAINER_5 ; new
	const EVENT_BEAT_SEVII_ROUTE_36_TRAINER_6 ; new
	const EVENT_BEAT_SEVII_ROUTE_36_TRAINER_7 ; new
	const EVENT_BEAT_SEVII_ROUTE_36_TRAINER_8 ; new
	const EVENT_BEAT_SEVII_ROUTE_36_TRAINER_9 ; new
	const EVENT_BEAT_SEVII_ROUTE_36_TRAINER_10 ; new
	const EVENT_BEAT_SEVII_ROUTE_36_TRAINER_11 ; new
	const EVENT_BEAT_SEVII_ROUTE_36_TRAINER_12 ; new
	const EVENT_ALREADY_ENTERED_TYPE_CHART_MATCHFINDER_AGAIN ; new
	const EVENT_SEVII_BEAT_PINK_FOUR_ISLAND ; new
	const EVENT_SEVII_BEAT_ROCKET_BEASTS_TANOBY ; new
	const EVENT_BEAT_ARM_MEWTWO_TANOBY ; new

; Vermilion City events
	const_next $150
	const EVENT_SEVII_ROUTE_43_CAVES_SWITCH_6 ; new
	const EVENT_GOT_BIKE_VOUCHER
	const EVENT_152
	const EVENT_SEEL_FAN_BOAST
	const EVENT_PIKACHU_FAN_BOAST
	const EVENT_BEAT_SEVII_PATTERN_BUSH_TRAINER_1 ; new
	const EVENT_BEAT_SEVII_PATTERN_BUSH_TRAINER_2 ; new
	const EVENT_BEAT_SEVII_PATTERN_BUSH_TRAINER_3 ; new
	const EVENT_BEAT_SEVII_PATTERN_BUSH_TRAINER_4 ; new
	const EVENT_BEAT_SEVII_ROUTE_32_DIVE_TRAINER_1 ; new
	const EVENT_BEAT_SEVII_ROUTE_32_DIVE_TRAINER_2 ; new
	const EVENT_BEAT_SEVII_ROUTE_32_DIVE_TRAINER_3 ; new
	const EVENT_SEVII_BEAT_SIRD ; new
	const EVENT_SEVII_CLEARED_ROUTE_33 ; new
	const EVENT_ALREADY_GOT_INFO_BATTLE_FACILITY ; new
	const EVENT_BEAT_LT_SURGE_REMATCH ; new
	const EVENT_2ND_LOCK_OPENED
	const EVENT_1ST_LOCK_OPENED
	const EVENT_BEAT_VERMILION_GYM_TRAINER_0
	const EVENT_BEAT_VERMILION_GYM_TRAINER_1
	const EVENT_BEAT_VERMILION_GYM_TRAINER_2
	const EVENT_BEAT_VERMILION_GYM_TRAINER_3
	const EVENT_GOT_TM24
	const EVENT_BEAT_LT_SURGE

; Celadon City events
	const_next $180
	const EVENT_GOT_TM41
	const EVENT_TRIED_TO_GIFT_POKE_FLUTE ; new
	const EVENT_SEVII_FACE_PINK_CELIOS_HOUSE ; new
	const EVENT_OBTAINED_PRIZE_TM_1 ; new
	const EVENT_OBTAINED_PRIZE_TM_2 ; new
	const EVENT_OBTAINED_PRIZE_TM_3 ; new
	const EVENT_CELADON_DINER_ONIGIRI_OBTAINED ; new
	const EVENT_CELADON_HOTEL_GOT_GUTSCHEIN ; new
	const EVENT_CELADON_HOTEL_PAID_ROOM ; new
	const EVENT_UNI_QUIZ_ANSWERED_WRONGLY_1 ; new
	const EVENT_UNI_QUIZ_ANSWERED_WRONGLY_2 ; new
	const EVENT_1ST_SPOKE_WITH_PIGEON ; new
	const EVENT_GOT_TM13
	const EVENT_GOT_TM48
	const EVENT_GOT_TM49
	const EVENT_GOT_TM18
	const EVENT_SEVII_UNLOCKED_TANOBY_CHAMBER_MAJOR ; new
	const EVENT_BEAT_SEVII_SEVEN_ISLAND_GYM_2_TRAINER_1 ; new
	const EVENT_BEAT_SEVII_SEVEN_ISLAND_GYM_2_TRAINER_2 ; new
	const EVENT_BEAT_SEVII_SEVEN_ISLAND_GYM_2_TRAINER_3 ; new
	const EVENT_BEAT_SEVII_SEVEN_ISLAND_GYM_2_TRAINER_4 ; new
	const EVENT_BEAT_SEVII_SEVEN_ISLAND_GYM_2_TRAINER_5 ; new
	const EVENT_BEAT_SEVII_SEVEN_ISLAND_GYM_2_TRAINER_6 ; new
	const EVENT_BEAT_SEVII_SEVEN_ISLAND_GYM_2_TRAINER_7 ; new
	const EVENT_TRAVELING_WITH_FERRY ; new
	const EVENT_BEAT_SEVII_SEVEN_ISLAND_GYM_2_DIVE_TRAINER_1 ; new
	const EVENT_BEAT_SEVII_SEVEN_ISLAND_GYM_2_DIVE_TRAINER_2 ; new
	const EVENT_BEAT_SEVII_SEVEN_ISLAND_GYM_2_DIVE_TRAINER_3 ; new
	const EVENT_BEAT_SEVII_SEVEN_ISLAND_GYM_2_DIVE_TRAINER_4 ; new
	const EVENT_BEAT_SEVII_SEVEN_ISLAND_GYM_2_DIVE_TRAINER_5 ; new
	const EVENT_SEVII_SPOKEN_WITH_RIDERS ; new
	const EVENT_MAXED_BATTLE_FACILITY ; new
	const EVENT_COMPLETED_POKEDEX ; new
	const EVENT_FOUND_ALL_MEGA_STONES ; new
	const EVENT_ALREADY_SPOKEN_WITH_CELADON_PHD ; new
	const EVENT_ALREADY_REWARDED_ORB_CELADON_PHD ; new
	const EVENT_ALREADY_REWARDED_MONEY_CELADON_PHD ; new
	const EVENT_RETURNED_LUNAR_RELIC ; new
	const EVENT_MONK_NOTICED_RELIC ; new
	const EVENT_BEAT_ERIKA_REMATCH ; new
	const EVENT_GOT_TM21
	const EVENT_BEAT_ERIKA
	const EVENT_BEAT_CELADON_GYM_TRAINER_0
	const EVENT_BEAT_CELADON_GYM_TRAINER_1
	const EVENT_BEAT_CELADON_GYM_TRAINER_2
	const EVENT_BEAT_CELADON_GYM_TRAINER_3
	const EVENT_BEAT_CELADON_GYM_TRAINER_4
	const EVENT_BEAT_CELADON_GYM_TRAINER_5
	const EVENT_BEAT_CELADON_GYM_TRAINER_6
	const EVENT_BEAT_CELADON_GYM_TRAINER_7
	const EVENT_BEAT_SEVII_SEVEN_ISLAND_GYM_3_TRAINER_1 ; new
	const EVENT_BEAT_SEVII_SEVEN_ISLAND_GYM_3_TRAINER_2 ; new
	const EVENT_BEAT_SEVII_SEVEN_ISLAND_GYM_3_TRAINER_3 ; new
	const EVENT_BEAT_SEVII_SEVEN_ISLAND_GYM_3_TRAINER_4 ; new
	const EVENT_SEVII_SEVEN_ISLAND_GYM_3_BOULDER_ON_SWITCH ; new
	const EVENT_DEFEATED_SEVII_SAGE_NANETTE ; new
	const EVENT_1B8 ; ???
	const EVENT_FOUND_ROCKET_HIDEOUT
	const EVENT_GOT_10_COINS
	const EVENT_GOT_20_COINS
	const EVENT_GOT_20_COINS_2
	const EVENT_SCRAMBLE_INPUTS ; new
	const EVENT_SEVII_BEAT_TERRAIN_TUTOR ; new
	const EVENT_1BF ; ???
	const EVENT_SEVII_FINALIZED_HS_CELIO_HOUSE ; new
	const EVENT_BEAT_SEVII_ROUTE_37_TRAINER_1 ; new
	const EVENT_BEAT_SEVII_ROUTE_37_TRAINER_2 ; new
	const EVENT_BEAT_SEVII_ROUTE_37_TRAINER_3 ; new
	const EVENT_BEAT_SEVII_ROUTE_37_TRAINER_4 ; new
	const EVENT_BEAT_SEVII_ROUTE_37_TRAINER_5 ; new
	const EVENT_BEAT_SEVII_ROUTE_37_TRAINER_6 ; new
	const EVENT_BEAT_SEVII_ROUTE_37_TRAINER_7 ; new
	const EVENT_BEAT_SEVII_ROUTE_37_TRAINER_8 ; new
	const EVENT_BEAT_SEVII_ROUTE_37_TRAINER_9 ; new
	const EVENT_BEAT_SEVII_ROUTE_37_TRAINER_10 ; new
	const EVENT_BEAT_SEVII_ROUTE_37_TRAINER_11 ; new
	const EVENT_BEAT_SEVII_ROUTE_37_TRAINER_12 ; new
	const EVENT_LOAD_DELTA_SPECIES_TYPES ; new
	const EVENT_BIRB_PALLET ; new
	const EVENT_BIRB_VIRIDIAN ; new
	const EVENT_BIRB_PEWTER ; new
	const EVENT_BIRB_CERULEAN ; new
	const EVENT_BIRB_VERMILION ; new
	const EVENT_BIRB_LAVENDER ; new
	const EVENT_BIRB_CELADON ; new
	const EVENT_BIRB_SAFFRON ; new
	const EVENT_BIRB_OCHRE ; new
	const EVENT_BIRB_FUCHSIA ; new
	const EVENT_BIRB_OBSIDIAN ; new
	const EVENT_BIRB_CINNABAR ; new
	const EVENT_BIRB_INDIGO ; new
	const EVENT_BIRB_ATOLLSE ; new
	const EVENT_BIRB_ATOLLNE ; new
	const EVENT_BIRB_ATOLLNW ; new
	const EVENT_BIRB_ATOLLSW ; new
	const EVENT_BIRB_LAKEMIST ; new
	const EVENT_GOT_COIN_CASE ; new

; Fuchsia City events
	const_next $238
	const EVENT_GOT_LUNAR_RELIC ; edited
	const EVENT_BEAT_SEVII_TWO_ISLAND_CITY_DIVE_TRAINER_1 ; new
	const EVENT_BEAT_SEVII_TWO_ISLAND_CITY_DIVE_TRAINER_2 ; new
	const EVENT_BEAT_SEVII_TWO_ISLAND_CITY_DIVE_TRAINER_3 ; new
	const EVENT_BEAT_SEVII_TWO_ISLAND_CITY_DIVE_TRAINER_4 ; new
	const EVENT_GAVE_GOLD_TEETH
	const EVENT_USING_ROCK_SMASH_FROM_MENU ; new
	const EVENT_STARTED_PUSHING_BOULDER_FOR_ALTERING_CAVE ; new
	const EVENT_FINISHED_PUSHING_BOULDER_FOR_ALTERING_CAVE ; new
	const EVENT_GOT_TM51 ; new
	const EVENT_GOT_TM54 ; new
	const EVENT_SPOKEN_WITH_KOGA_IN_MEETING_ROOM ; new
	const EVENT_ALREADY_SPOKEN_WITH_OCHRE_POWER_ENGINEER ; new
	const EVENT_ALREADY_REWARDED_ORB_OCHRE_POWER_ENGINEER ; new
	const EVENT_ALREADY_REWARDED_MONEY_OCHRE_POWER_ENGINEER ; new
	const EVENT_OCHRE_CITY_FENCE_FORCED ; new
	const EVENT_OCHRE_REHABILITATION_SCOLDED_ONCE ; new
	const EVENT_OCHRE_REHABILITATION_SCOLDED_AGAIN ; new
	const EVENT_OCHRE_BIRD_FAN_CLUB_MEMBER ; new
	const EVENT_OCHRE_BIRD_FAN_CLUB_PRESIDENT ; new
	const EVENT_SPOKEN_WITH_ERIK ; new
	const EVENT_SPOKEN_WITH_SARA ; new
	const EVENT_SAFARI_GAME_OVER
	const EVENT_IN_SAFARI_ZONE
	const EVENT_SEVII_HAZARD_ROOM_PRESSED_TILE ; new
	const EVENT_BEAT_KOGA_REMATCH ; new
	const EVENT_GOT_TM06
	const EVENT_BEAT_SEVII_ROUTE_39_TRAINER_1 ; new
	const EVENT_BEAT_SEVII_ROUTE_39_TRAINER_2 ; new
	const EVENT_BEAT_SEVII_ROUTE_39_TRAINER_3 ; new
	const EVENT_BEAT_SEVII_ROUTE_39_TRAINER_4 ; new
	const EVENT_BEAT_SEVII_ROUTE_39_TRAINER_5 ; new
	const EVENT_BEAT_SEVII_ROUTE_39_TRAINER_6 ; new
	const EVENT_BEAT_KOGA
	const EVENT_BEAT_FUCHSIA_GYM_TRAINER_0
	const EVENT_BEAT_FUCHSIA_GYM_TRAINER_1
	const EVENT_BEAT_FUCHSIA_GYM_TRAINER_2
	const EVENT_BEAT_FUCHSIA_GYM_TRAINER_3
	const EVENT_BEAT_FUCHSIA_GYM_TRAINER_4
	const EVENT_BEAT_FUCHSIA_GYM_TRAINER_5
	const EVENT_BEAT_FUCHSIA_GYM_TRAINER_6

; Cinnabar Island events
	const_next $278
	const EVENT_MANSION_SWITCH_ON
	const EVENT_SEVII_HAZARD_ROOM_WON ; new
	const EVENT_BEAT_SEVII_ROUTE_40_TRAINER_1 ; new
	const EVENT_BEAT_SEVII_ROUTE_40_TRAINER_2 ; new
	const EVENT_BEAT_SEVII_ROUTE_40_TRAINER_3 ; new
	const EVENT_BEAT_SEVII_ROUTE_40_TRAINER_4 ; new
	const EVENT_BEAT_SEVII_ROUTE_40_TRAINER_5 ; new
	const EVENT_BEAT_SEVII_ROUTE_40_TRAINER_6 ; new
	const EVENT_BEAT_SEVII_ROUTE_40_TRAINER_7 ; new
	const EVENT_BEAT_SEVII_ROUTE_40_TRAINER_8 ; new
	const EVENT_TRAVELED_TO_SEVII_AT_LEAST_ONCE ; new
	const EVENT_GOT_HM06 ; new
	const EVENT_GOT_HM07 ; new
	const EVENT_GOT_CLIMB_CLUB_INVITATION ; new
	const EVENT_LAST_DIALOGUE_CINNABAR_ARCHEOLOGIST ; new
	const EVENT_GIVEN_CINNABAR_ARCHEOLOGIST_ARTIFACT ; new
	const EVENT_GIVEN_CINNABAR_ARCHEOLOGIST_ARTIFACT_FAILED ; new
	const EVENT_BEAT_MANSION_1_TRAINER_0
	const EVENT_BEAT_SEVII_ROUTE_33_DIVE_TRAINER_1 ; new
	const EVENT_BEAT_SEVII_ROUTE_33_DIVE_TRAINER_2 ; new
	const EVENT_BEAT_SEVII_ROUTE_33_DIVE_TRAINER_3 ; new
	const EVENT_BEAT_SEVII_ROUTE_33_DIVE_TRAINER_4 ; new
	const EVENT_MARK_TO_WALK_OUTSIDE_VERMILION_DOCK ; new
	const EVENT_VERMILION_DOCK_AUTOWALKING_SEVII_FERRY ; new
	const EVENT_FLASHED_SS_TICKET ; new
	const EVENT_PLACED_ALL_ORBS_IN_RECESSES ; new
	const EVENT_PLACED_ICE_ORB_IN_RECESS ; new
	const EVENT_PLACED_THUNDER_ORB_IN_RECESS ; new
	const EVENT_PLACED_FIRE_ORB_IN_RECESS ; new
	const EVENT_IN_HOT_SPRINGS ; new
	const EVENT_BEAT_MANSION_RIVAL ; new
	const EVENT_BEAT_BLAINE_REMATCH ; new
	const EVENT_GOT_TM38
	const EVENT_BEAT_BLAINE
	const EVENT_BEAT_CINNABAR_GYM_TRAINER_0
	const EVENT_BEAT_CINNABAR_GYM_TRAINER_1
	const EVENT_BEAT_CINNABAR_GYM_TRAINER_2
	const EVENT_BEAT_CINNABAR_GYM_TRAINER_3
	const EVENT_BEAT_CINNABAR_GYM_TRAINER_4
	const EVENT_BEAT_CINNABAR_GYM_TRAINER_5
	const EVENT_BEAT_CINNABAR_GYM_TRAINER_6
	const EVENT_BEAT_MOLTRES ; newly moved here from Victory Road
	const EVENT_BEAT_SEVII_ROUTE_32_TRAINER_1 ; new
	const EVENT_BEAT_SEVII_ROUTE_32_TRAINER_2 ; new
	const EVENT_BEAT_SEVII_ROUTE_32_TRAINER_3 ; new
	const EVENT_DIVE_FORCE_CONSIDERING_DUNGEON_TILESET ; new
	const EVENT_VERMILION_DOCK_AUTOWALKING_SEVII_FERRY_IN_DOCK ; new
	const EVENT_2A7 ; ???
	const EVENT_CINNABAR_GYM_GATE0_UNLOCKED
	const EVENT_CINNABAR_GYM_GATE1_UNLOCKED
	const EVENT_CINNABAR_GYM_GATE2_UNLOCKED
	const EVENT_CINNABAR_GYM_GATE3_UNLOCKED
	const EVENT_CINNABAR_GYM_GATE4_UNLOCKED
	const EVENT_CINNABAR_GYM_GATE5_UNLOCKED
	const EVENT_CINNABAR_GYM_GATE6_UNLOCKED
	const EVENT_SEVII_DOTTED_HOLE_OPENED ; new
	const EVENT_SEVII_DOTTED_HOLE_INSERTED_TOPAZ ; new
	const EVENT_SEVII_DOTTED_HOLE_INSERTED_TIGERS_EYE ; new
	const EVENT_BEAT_SEVII_ROUTE_42_TRAINER_1 ; new
	const EVENT_BEAT_SEVII_ROUTE_42_TRAINER_2 ; new
	const EVENT_BEAT_SEVII_ROUTE_42_TRAINER_3 ; new
	const EVENT_BEAT_SEVII_ROUTE_42_TRAINER_4 ; new
	const EVENT_BEAT_SEVII_ROUTE_42_TRAINER_5 ; new
	const EVENT_BEAT_SEVII_ROUTE_42_TRAINER_6 ; new
	const EVENT_BEAT_SEVII_ROUTE_42_TRAINER_7 ; new
	const EVENT_BEAT_SEVII_ROUTE_42_TRAINER_8 ; new
	const EVENT_BEAT_SEVII_ROUTE_42_TRAINER_9 ; new
	const EVENT_SUNKEN_SHIP_OPENED_DOOR_TOP_ROOM_4 ; new
	const EVENT_SUNKEN_SHIP_OPENED_DOOR_TOP_ROOM_7 ; new
	const EVENT_SUNKEN_SHIP_OPENED_DOOR_MID_ROOM_2 ; new
	const EVENT_SUNKEN_SHIP_OPENED_DOOR_MID_ROOM_5 ; new
	const EVENT_SUNKEN_SHIP_OPENED_DOOR_BOTTOM_ROOM_2 ; new
	const EVENT_SUNKEN_SHIP_OPENED_DOOR_BOTTOM_ROOM_8 ; new
	const EVENT_BEAT_ROUTE_19_DIVE_TRAINER_0 ; new
	const EVENT_BEAT_SEVII_ROUTE_43_TRAINER_1 ; new
	const EVENT_BEAT_SEVII_ROUTE_43_TRAINER_2 ; new
	const EVENT_BEAT_SEVII_ROUTE_43_TRAINER_3 ; new
	const EVENT_BEAT_SEVII_ROUTE_43_TRAINER_4 ; new
	const EVENT_BEAT_SEVII_ROUTE_43_TRAINER_5 ; new
	const EVENT_BEAT_SEVII_ROUTE_43_TRAINER_6 ; new
	const EVENT_BEAT_SEVII_ROUTE_43_TRAINER_7 ; new
	const EVENT_BEAT_SEVII_ROUTE_43_TRAINER_8 ; new
	const EVENT_BEAT_SEVII_ROUTE_43_TRAINER_9 ; new
	const EVENT_BEAT_SEVII_ROUTE_43_TRAINER_10 ; new
	const EVENT_SEVII_ALTERING_CAVE_2_SWITCH_1 ; new
	const EVENT_SEVII_ALTERING_CAVE_2_SWITCH_2 ; new
	const EVENT_SEVII_ALTERING_CAVE_3_SWITCH_1 ; new
	const EVENT_SEVII_ALTERING_CAVE_3_SWITCH_2 ; new
	const EVENT_CINNABAR_USED_SECRET_KEY ; new
	const EVENT_BEAT_ROUTE_20_DIVE_TRAINER_0 ; new
	const EVENT_BEAT_ROUTE_20_DIVE_TRAINER_1 ; new
	const EVENT_BEAT_ROUTE_20_DIVE_TRAINER_2 ; new
	const EVENT_SUNKEN_SHIP_GOT_TOPAZ ; new
	const EVENT_SUNKEN_SHIP_GOT_ROOM_KEY_34 ; new
	const EVENT_SUNKEN_SHIP_GOT_ROOM_KEY_37 ; new
	const EVENT_GOT_TM35
	const EVENT_SEVII_TANOBY_TRACK_PATH_0 ; new
	const EVENT_SEVII_TANOBY_TRACK_PATH_1 ; new
	const EVENT_SEVII_TANOBY_TRACK_PATH_2 ; new
	const EVENT_SEVII_TANOBY_TRACK_PATH_3 ; new
	const EVENT_SEVII_TANOBY_TRACK_PATH_4 ; new
	const EVENT_SEVII_TANOBY_TRACK_PATH_5 ; new
	const EVENT_SEVII_TANOBY_TRACK_PATH_6 ; new
	const EVENT_SEVII_TANOBY_TRACK_PATH_7 ; new
	const EVENT_GAVE_FOSSIL_TO_LAB
	const EVENT_LAB_STILL_REVIVING_FOSSIL
	const EVENT_LAB_HANDING_OVER_FOSSIL_MON

; Saffron City events
	const_next $340
	const EVENT_GOT_TM31
	const EVENT_DEFEATED_COPYCAT
	const EVENT_BEAT_SEVII_ROUTE_31_TRAINER_1 ; new
	const EVENT_BEAT_SEVII_ROUTE_31_TRAINER_2 ; new
	const EVENT_BEAT_SEVII_ROUTE_31_TRAINER_3 ; new
	const EVENT_BEAT_SEVII_ROUTE_31_TRAINER_4 ; new
	const EVENT_BEAT_SEVII_ROUTE_31_TRAINER_5 ; new
	const EVENT_BEAT_SEVII_ROUTE_31_TRAINER_6 ; new
	const EVENT_BEAT_SEVII_ROUTE_31_TRAINER_7 ; new
	const EVENT_BEAT_SEVII_ROUTE_31_TRAINER_8 ; new
	const EVENT_BEAT_SEVII_ROUTE_31_TRAINER_9 ; new
	const EVENT_BEAT_SEVII_ROUTE_31_TRAINER_10 ; new
	const EVENT_SEVII_SECRET_BEACH_HOUSE_WATERED_PLANT_1 ; new
	const EVENT_SEVII_SECRET_BEACH_HOUSE_WATERED_PLANT_2 ; new
	const EVENT_BIRB_SEVII_SECRET_BEACH ; new
	const EVENT_FED_ALL_BIRBS ; new
	const EVENT_CAUGHT_AT_LEAST_ONE_DELTA ; new
	const EVENT_DEFEATED_FIGHTING_DOJO
	const EVENT_BEAT_KARATE_MASTER
	const EVENT_BEAT_FIGHTING_DOJO_TRAINER_0
	const EVENT_BEAT_FIGHTING_DOJO_TRAINER_1
	const EVENT_BEAT_FIGHTING_DOJO_TRAINER_2
	const EVENT_BEAT_FIGHTING_DOJO_TRAINER_3
	const EVENT_GOT_A_HITMON ; new
	const EVENT_INDOOR_PALETTE ; new
	const EVENT_BEAT_SEVII_FOUR_ISLAND_CITY_DIVE_TRAINER_1 ; new
	const EVENT_BEAT_SEVII_FOUR_ISLAND_CITY_DIVE_TRAINER_2 ; new
	const EVENT_BEAT_SEVII_FOUR_ISLAND_CITY_DIVE_TRAINER_3 ; new
	const EVENT_BEAT_SEVII_FOUR_ISLAND_CITY_DIVE_TRAINER_4 ; new
	const EVENT_SEVII_BEAT_AT_LEAST_ONE_SHRINE_SAGE ; new
	const EVENT_SKIP_PRINTING_WHEN_FISHING ; new
	const EVENT_LAKE_OF_MIST_GAVE_FLAME_PLUME ; new
	const EVENT_GOT_TM46
	const EVENT_BEAT_SABRINA
	const EVENT_BEAT_SAFFRON_GYM_TRAINER_0
	const EVENT_BEAT_SAFFRON_GYM_TRAINER_1
	const EVENT_BEAT_SAFFRON_GYM_TRAINER_2
	const EVENT_BEAT_SAFFRON_GYM_TRAINER_3
	const EVENT_BEAT_SAFFRON_GYM_TRAINER_4
	const EVENT_BEAT_SAFFRON_GYM_TRAINER_5
	const EVENT_BEAT_SAFFRON_GYM_TRAINER_6
	const EVENT_BEAT_SAFFRON_GYM_TRAINER_7
	const EVENT_SHINY_RITUAL_ACTIVE ; new
	const EVENT_POKEDEX_DISPLAY_NIGHT_NEST ; new
	const EVENT_BIRB_SEVII_TWO_ISLET ; new
	const EVENT_BIRB_SEVII_ROCK_MAZE ; new
	const EVENT_SEVII_SECRET_BEACH_HOUSE_WATERED_PLANT_3 ; new
	const EVENT_SEVII_SECRET_BEACH_HOUSE_WATERED_PLANT_4 ; new
	const EVENT_DO_NOT_ALLOW_FOR_SAGE_WIN_RECORDING ; new
	const EVENT_BEAT_SEVII_ROUTE_38_DIVE_CAVERN_TRAINER_1 ; new
	const EVENT_BEAT_SEVII_ROUTE_38_DIVE_CAVERN_TRAINER_2 ; new
	const EVENT_BEAT_SEVII_ROUTE_38_DIVE_CAVERN_TRAINER_3 ; new
	const EVENT_BEAT_SEVII_ROUTE_38_DIVE_CAVERN_TRAINER_4 ; new
	const EVENT_BEAT_SEVII_ROUTE_38_DIVE_CAVERN_TRAINER_5 ; new
	const EVENT_BEAT_SEVII_ROUTE_38_DIVE_CAVERN_TRAINER_6 ; new
	const EVENT_BEAT_SEVII_ROUTE_38_DIVE_CAVERN_TRAINER_7 ; new
	const EVENT_BEAT_SEVII_ROUTE_38_DIVE_CAVERN_TRAINER_8 ; new
	const EVENT_BEAT_SEVII_ROUTE_38_DIVE_CAVERN_TRAINER_9 ; new
	const EVENT_BEAT_SEVII_ROUTE_38_DIVE_CAVERN_TRAINER_10 ; new
	const EVENT_BIRB_ROUTE1 ; new
	const EVENT_BIRB_ROUTE2 ; new
	const EVENT_BIRB_ROUTE3 ; new
	const EVENT_BIRB_ROUTE4 ; new
	const EVENT_BIRB_ROUTE5 ; new
	const EVENT_BIRB_ROUTE6 ; new
	const EVENT_BIRB_ROUTE7 ; new
	const EVENT_BIRB_ROUTE8 ; new
	const EVENT_BIRB_ROUTE9 ; new
	const EVENT_BIRB_ROUTE10 ; new
	const EVENT_BIRB_ROUTE11 ; new
	const EVENT_BIRB_ROUTE12 ; new
	const EVENT_BIRB_ROUTE13 ; new
	const EVENT_BIRB_ROUTE14 ; new
	const EVENT_BIRB_ROUTE15 ; new
	const EVENT_BIRB_ROUTE16 ; new
	const EVENT_BIRB_ROUTE17 ; new
	const EVENT_BIRB_ROUTE18 ; new
	const EVENT_BIRB_ROUTE19 ; new
	const EVENT_BIRB_ROUTE20 ; new
	const EVENT_BIRB_ROUTE21 ; new
	const EVENT_BIRB_ROUTE22 ; new
	const EVENT_BIRB_ROUTE23 ; new
	const EVENT_BIRB_ROUTE24 ; new
	const EVENT_BIRB_ROUTE25 ; new
	const EVENT_BIRB_ROUTE26 ; new
	const EVENT_BIRB_ROUTE27 ; new
	const EVENT_BEAT_SABRINA_REMATCH ; new
	const EVENT_RP_RELYED_MESSAGE_CAPTAIN ; new
	const EVENT_SEVII_TANOBY_GARDEN_ACTIVATED_MEW_2 ; new
	const EVENT_BEAT_SEVII_FIVE_ISLAND_CITY_DIVE_TRAINER_1 ; new
	const EVENT_BEAT_SEVII_FIVE_ISLAND_CITY_DIVE_TRAINER_2 ; new
	const EVENT_BEAT_SEVII_FIVE_ISLAND_CITY_DIVE_TRAINER_3 ; new
	const EVENT_BEAT_SEVII_FIVE_ISLAND_CITY_DIVE_TRAINER_4 ; new
	const EVENT_BIRB_SEVII_TANOBY_GARDEN ; new
	const EVENT_SEVII_TANOBY_GARDEN_COMPLETED_MEW_CHASE ; new
	const EVENT_SEVII_TANOBY_GARDEN_ACTIVATED_MEW_9 ; new
	const EVENT_BIRB_ROUTE28 ; new
	const EVENT_BIRB_ROUTE29 ; new
	const EVENT_BIRB_ROUTE30 ; new
	const EVENT_BEAT_BROCK_REMATCH_INVERSE ; new
	const EVENT_BEAT_MISTY_REMATCH_INVERSE ; new
	const EVENT_BEAT_LT_SURGE_REMATCH_INVERSE ; new
	const EVENT_BEAT_ERIKA_REMATCH_INVERSE ; new
	const EVENT_BEAT_KOGA_REMATCH_INVERSE ; new
	const EVENT_BEAT_SABRINA_REMATCH_INVERSE ; new
	const EVENT_BEAT_BLAINE_REMATCH_INVERSE ; new
	const EVENT_BEAT_ORAGE_REMATCH_INVERSE ; new
	const EVENT_BEAT_LORELEI_REMATCH_INVERSE ; new
	const EVENT_BEAT_BRUNO_REMATCH_INVERSE ; new
	const EVENT_BEAT_AGATHA_REMATCH_INVERSE ; new
	const EVENT_BEAT_LANCE_REMATCH_INVERSE ; new
	const EVENT_BEAT_ALL_REMATCH_INVERSE ; new
	const EVENT_GOT_TM29

; Route 1 events
	const_next $3C0
	const EVENT_GOT_POTION_SAMPLE
	const EVENT_SEVII_TANOBY_GARDEN_ACTIVATED_MEW_3 ; new
	const EVENT_BEAT_MEW_TANOBY ; new
	const EVENT_RP_BEAT_CHAMPION_PINK ; new
	const EVENT_RP_ELIMINATED_LORELEI ; new
	const EVENT_RP_ELIMINATED_BRUNO ; new
	const EVENT_RP_ELIMINATED_AGATHA ; new
	const EVENT_RP_ELIMINATED_LANCE ; new
	const EVENT_RP_ENTERED_LANCES_ROOM_ONCE ; new
	const EVENT_RP_GAVE_MEWTWO_TO_GIOVANNI ; new
	const EVENT_RP_GOT_A_STEAL_BALL ; new
	const EVENT_RP_USED_CELIOS_PC_FIRST_TIME ; new
	const EVENT_RP_REVEALED_MAYOI_FATE ; new

; Route 2 events
	const_next $3D8
	const EVENT_GOT_HM05
	const EVENT_RP_BEAT_HM05_AID ; new
	const EVENT_RP_BEAT_OBSIDIAN_BLUE ; new
	const EVENT_RP_BEAT_EXPALL_AID ; new
	const EVENT_RP_BEAT_ITEMFINDER_AID ; new
	const EVENT_RP_BEAT_DIVER_EXPERT ; new
	const EVENT_RP_BEAT_ORM_CELIOS_HOUSE ; new
	const EVENT_RP_BEAT_PINK_SIX_ISLAND ; new

; Route 3 events
	const_next $3E0
	const EVENT_SEVII_THREE_ISLAND_FOUND_SECRET_EXIT ; new
	const EVENT_SEVII_SHOWED_MONS_TO_TRICK_ROOM_TUTOR ; new
	const EVENT_BEAT_ROUTE_3_TRAINER_0
	const EVENT_BEAT_ROUTE_3_TRAINER_1
	const EVENT_BEAT_ROUTE_3_TRAINER_2
	const EVENT_BEAT_ROUTE_3_TRAINER_3
	const EVENT_BEAT_ROUTE_3_TRAINER_4
	const EVENT_BEAT_ROUTE_3_TRAINER_5
	const EVENT_BEAT_ROUTE_3_TRAINER_6
	const EVENT_BEAT_ROUTE_3_TRAINER_7

; Route 4 events
	const_next $3F0
	const EVENT_SEVII_BEAT_ROCKETS_DOTTED_HOLE ; new
	const EVENT_BEAT_MELTAN ; new
	const EVENT_FIXED_MATCHA_MACHINE ; new
	const EVENT_SPOKEN_WITH_REPAIR_PERSON ; new
	const EVENT_BEAT_ROUTE_4_TRAINER_0
	const EVENT_SPOKEN_WITH_COIN_CASE_GUY ; new
	const EVENT_BIRB_ELECTRICRAVINE ; new
	const EVENT_REMOVED_ROUTE_3_JENNY ; new
	const EVENT_GOT_FREE_MATCHA_TEA ; new
	const EVENT_BOUGHT_MAGIKARP
	const EVENT_BEAT_LAKE_OF_MIST_TRAINER_0 ; new
	const EVENT_BEAT_LAKE_OF_MIST_TRAINER_1 ; new
	const EVENT_BEAT_LAKE_OF_MIST_TRAINER_2 ; new
	const EVENT_BEAT_LAKE_OF_MIST_TRAINER_3 ; new
	const EVENT_BEAT_LAKE_OF_MIST_TRAINER_4 ; new

; Route 26 events, new, temp?
	const_next $400
	const EVENT_SPOKEN_WITH_REPAIR_BOSS ; new
	const EVENT_BEAT_CELADON_HOTEL_HALL_TRAINER_0 ; new
	const EVENT_BEAT_CELADON_HOTEL_HALL_TRAINER_1 ; new
	const EVENT_BEAT_CELADON_HOTEL_HALL_TRAINER_2 ; new
	const EVENT_BEAT_CELADON_HOTEL_HALL_TRAINER_3 ; new
	const EVENT_BEAT_ROUTE_26_TRAINER_0

; Route 6 events
	const_next $410
	const EVENT_DIVE_GOT_OXYGEN_TANK
	const EVENT_BEAT_ROUTE_6_TRAINER_0
	const EVENT_BEAT_ROUTE_6_TRAINER_1
	const EVENT_BEAT_ROUTE_6_TRAINER_2
	const EVENT_BEAT_ROUTE_6_TRAINER_3
	const EVENT_BEAT_ROUTE_6_TRAINER_4
	const EVENT_BEAT_ROUTE_6_TRAINER_5

; Route 8 events
	const_next $430
	const EVENT_SEVII_GAVE_TM_MIMIC_TO_IMPOSTER_TUTOR ; new
	const EVENT_BEAT_ROUTE_8_TRAINER_0
	const EVENT_BEAT_ROUTE_8_TRAINER_1
	const EVENT_BEAT_ROUTE_8_TRAINER_2
	const EVENT_BEAT_ROUTE_8_TRAINER_3
	const EVENT_BEAT_ROUTE_8_TRAINER_4
	const EVENT_BEAT_ROUTE_8_TRAINER_5
	const EVENT_BEAT_ROUTE_8_TRAINER_6
	const EVENT_BEAT_ROUTE_8_TRAINER_7
	const EVENT_BEAT_ROUTE_8_TRAINER_8

; Route 9 events
	const_next $440
	const EVENT_HID_UNDERGROUND_GUARDS ; new
	const EVENT_BEAT_ROUTE_9_TRAINER_0
	const EVENT_BEAT_ROUTE_9_TRAINER_1
	const EVENT_BEAT_ROUTE_9_TRAINER_2
	const EVENT_BEAT_ROUTE_9_TRAINER_3
	const EVENT_BEAT_ROUTE_9_TRAINER_4
	const EVENT_BEAT_ROUTE_9_TRAINER_5
	const EVENT_BEAT_ROUTE_9_TRAINER_6
	const EVENT_BEAT_ROUTE_9_TRAINER_7
	const EVENT_BEAT_ROUTE_9_TRAINER_8

; Route 10 events
	const_next $450
	const EVENT_GOT_HM09 ; new
	const EVENT_BEAT_ROUTE_10_TRAINER_0
	const EVENT_BEAT_ROUTE_10_TRAINER_1
	const EVENT_BEAT_ROUTE_10_TRAINER_2
	const EVENT_BEAT_ROUTE_10_TRAINER_3
	const EVENT_BEAT_ROUTE_10_TRAINER_4
	const EVENT_BEAT_ROUTE_10_TRAINER_5
	const EVENT_SEVII_OFFERED_LEMONADE_TO_GRAVE ; new
	const EVENT_GOT_SEVII_TICKET ; new
	const EVENT_BEAT_ROCK_TUNNEL_1_TRAINER_0
	const EVENT_BEAT_ROCK_TUNNEL_1_TRAINER_1
	const EVENT_BEAT_ROCK_TUNNEL_1_TRAINER_2
	const EVENT_BEAT_ROCK_TUNNEL_1_TRAINER_3
	const EVENT_BEAT_ROCK_TUNNEL_1_TRAINER_4
	const EVENT_BEAT_ROCK_TUNNEL_1_TRAINER_5
	const EVENT_BEAT_ROCK_TUNNEL_1_TRAINER_6
	const EVENT_ENABLED_IMPOSTER ; new
	const EVENT_BEAT_POWER_PLANT_VOLTORB_0
	const EVENT_BEAT_POWER_PLANT_VOLTORB_1
	const EVENT_BEAT_POWER_PLANT_VOLTORB_2
	const EVENT_BEAT_POWER_PLANT_VOLTORB_3
	const EVENT_BEAT_POWER_PLANT_VOLTORB_4
	const EVENT_BEAT_POWER_PLANT_VOLTORB_5
	const EVENT_BEAT_POWER_PLANT_VOLTORB_6
	const EVENT_BEAT_POWER_PLANT_VOLTORB_7
	const EVENT_BEAT_ZAPDOS

; Route 11 events
	const_next $470
	const EVENT_USING_SMASH_BALL ; new
	const EVENT_BEAT_ROUTE_11_TRAINER_0
	const EVENT_BEAT_ROUTE_11_TRAINER_1
	const EVENT_BEAT_ROUTE_11_TRAINER_2
	const EVENT_BEAT_ROUTE_11_TRAINER_3
	const EVENT_BEAT_ROUTE_11_TRAINER_4
	const EVENT_BEAT_ROUTE_11_TRAINER_5
	const EVENT_BEAT_ROUTE_11_TRAINER_6
	const EVENT_BEAT_ROUTE_11_TRAINER_7
	const EVENT_BEAT_ROUTE_11_TRAINER_8
	const EVENT_BEAT_ROUTE_11_TRAINER_9
	const EVENT_SEVII_UNDERGROUND_OPENED ; new
	const EVENT_SEVII_UNDERGROUND_BUTTON_PRESSED_ONE ; new
	const EVENT_SEVII_UNDERGROUND_BUTTON_PRESSED_TWO ; new
	const EVENT_SEVII_UNDERGROUND_BUTTON_PRESSED_THREE ; new
	const EVENT_GOT_ITEMFINDER

; Route 12 events
	const_next $480
	const EVENT_GOT_TM39
	const EVENT_PIKACHU_SASH_TRIGGERS ; new
	const EVENT_PIKACHU_SASH_ENABLED ; new
	const EVENT_BEAT_ROUTE_12_TRAINER_0
	const EVENT_BEAT_ROUTE_12_TRAINER_1
	const EVENT_BEAT_ROUTE_12_TRAINER_2
	const EVENT_BEAT_ROUTE_12_TRAINER_3
	const EVENT_BEAT_ROUTE_12_TRAINER_4
	const EVENT_BEAT_ROUTE_12_TRAINER_5
	const EVENT_BEAT_ROUTE_12_TRAINER_6
	const EVENT_SEVII_UNDERGROUND_BUTTON_PRESSED_FOUR ; new
	const EVENT_SEVII_EIGHT_ISLAND_CAVE_ENTRANCE_TO_BE_CLOSED ; new
	const EVENT_SEVII_EIGHT_ISLAND_CAVE_ENTRANCE_ALLOWED ; new
	const EVENT_FACED_SNORLAX_RIVAL ; new
	const EVENT_FIGHT_ROUTE12_SNORLAX
	const EVENT_BEAT_ROUTE12_SNORLAX

; Route 13 events
	const_next $490
	const EVENT_EIGHT_ISLAND_CAVE_BEAT_BOTH_BLUE_AND_PINK ; new
	const EVENT_BEAT_ROUTE_13_TRAINER_0
	const EVENT_BEAT_ROUTE_13_TRAINER_1
	const EVENT_BEAT_ROUTE_13_TRAINER_2
	const EVENT_BEAT_ROUTE_13_TRAINER_3
	const EVENT_BEAT_ROUTE_13_TRAINER_4
	const EVENT_BEAT_ROUTE_13_TRAINER_5
	const EVENT_BEAT_ROUTE_13_TRAINER_6
	const EVENT_BEAT_ROUTE_13_TRAINER_7
	const EVENT_BEAT_ROUTE_13_TRAINER_8
	const EVENT_BEAT_ROUTE_13_TRAINER_9
	const EVENT_PIKACHU_SASH_CAN_TRIGGER ; new

; Route 14 events
	const_next $4A0
	const EVENT_SEVII_UNDERGROUND_BUTTON_PRESSED_FIVE ; new
	const EVENT_BEAT_ROUTE_14_TRAINER_0
	const EVENT_BEAT_ROUTE_14_TRAINER_1
	const EVENT_BEAT_ROUTE_14_TRAINER_2
	const EVENT_BEAT_ROUTE_14_TRAINER_3
	const EVENT_BEAT_ROUTE_14_TRAINER_4
	const EVENT_BEAT_ROUTE_14_TRAINER_5
	const EVENT_BEAT_ROUTE_14_TRAINER_6
	const EVENT_BEAT_ROUTE_14_TRAINER_7
	const EVENT_BEAT_ROUTE_14_TRAINER_8
	const EVENT_BEAT_ROUTE_14_TRAINER_9

; Route 15 events
	const_next $4B0
	const EVENT_GOT_EXP_ALL
	const EVENT_BEAT_ROUTE_15_TRAINER_0
	const EVENT_BEAT_ROUTE_15_TRAINER_1
	const EVENT_BEAT_ROUTE_15_TRAINER_2
	const EVENT_BEAT_ROUTE_15_TRAINER_3
	const EVENT_BEAT_ROUTE_15_TRAINER_4
	const EVENT_BEAT_ROUTE_15_TRAINER_5
	const EVENT_BEAT_ROUTE_15_TRAINER_6
	const EVENT_BEAT_ROUTE_15_TRAINER_7
	const EVENT_BEAT_ROUTE_15_TRAINER_8
	const EVENT_BEAT_ROUTE_15_TRAINER_9

; Route 16 events
	const_next $4C0
	const EVENT_SEVII_UNDERGROUND_BUTTON_PRESSED_SIX ; new
	const EVENT_BEAT_ROUTE_16_TRAINER_0
	const EVENT_BEAT_ROUTE_16_TRAINER_1
	const EVENT_BEAT_ROUTE_16_TRAINER_2
	const EVENT_BEAT_ROUTE_16_TRAINER_3
	const EVENT_BEAT_ROUTE_16_TRAINER_4
	const EVENT_BEAT_ROUTE_16_TRAINER_5
	const EVENT_SEVII_UNDERGROUND_BUTTON_PRESSED_SEVEN ; new
	const EVENT_FIGHT_ROUTE16_SNORLAX
	const EVENT_BEAT_ROUTE16_SNORLAX
	const EVENT_SEVII_TRADED_SNORLAX_FOR_ONIGIRI_BOX ; new
	const EVENT_SPRITE_CHECK_FROM_START_OPENING ; new
	const EVENT_BEAT_STRONG_TRAINER_EARLY_ROUTE ; new
	const EVENT_TOWN_MAP_ZOOMED_AT_LEAST_ONCE ; new
	const EVENT_GOT_HM02
	const EVENT_RESCUED_MR_FUJI

; Route 17 events
	const_next $4D0
	const EVENT_TOWN_MAP_TWO_ISLET ; new
	const EVENT_BEAT_ROUTE_17_TRAINER_0
	const EVENT_BEAT_ROUTE_17_TRAINER_1
	const EVENT_BEAT_ROUTE_17_TRAINER_2
	const EVENT_BEAT_ROUTE_17_TRAINER_3
	const EVENT_BEAT_ROUTE_17_TRAINER_4
	const EVENT_BEAT_ROUTE_17_TRAINER_5
	const EVENT_BEAT_ROUTE_17_TRAINER_6
	const EVENT_BEAT_ROUTE_17_TRAINER_7
	const EVENT_BEAT_ROUTE_17_TRAINER_8
	const EVENT_BEAT_ROUTE_17_TRAINER_9

; Route 18 events
	const_next $4E0
	const EVENT_PERFORMING_SEVII_KANTO_SPECIAL_WARP ; new
	const EVENT_BEAT_ROUTE_18_TRAINER_0
	const EVENT_BEAT_ROUTE_18_TRAINER_1
	const EVENT_BEAT_ROUTE_18_TRAINER_2

; Route 19 events
	const_next $4F0
	const EVENT_SEVII_EIGHT_ISLAND_CAVE_TO_BE_CLOSED_AFTER_LOSS_VS_SUUJERO ; new
	const EVENT_BEAT_ROUTE_19_TRAINER_0
	const EVENT_BEAT_ROUTE_19_TRAINER_1
	const EVENT_BEAT_ROUTE_19_TRAINER_2
	const EVENT_BEAT_ROUTE_19_TRAINER_3
	const EVENT_BEAT_ROUTE_19_TRAINER_4
	const EVENT_BEAT_ROUTE_19_TRAINER_5
	const EVENT_BEAT_ROUTE_19_TRAINER_6
	const EVENT_BEAT_ROUTE_19_TRAINER_7
	const EVENT_BEAT_ROUTE_19_TRAINER_8
	const EVENT_BEAT_ROUTE_19_TRAINER_9

; Route 20 events
	const_next $500
	const EVENT_GOT_SLEEP_BAG ; new
	const EVENT_IN_SEAFOAM_ISLANDS
	const EVENT_BEAT_ROUTE_20_TRAINER_0
	const EVENT_BEAT_ROUTE_20_TRAINER_1
	const EVENT_BEAT_ROUTE_20_TRAINER_2
	const EVENT_BEAT_ROUTE_20_TRAINER_3
	const EVENT_BEAT_ROUTE_20_TRAINER_4
	const EVENT_BEAT_ROUTE_20_TRAINER_5
	const EVENT_BEAT_ROUTE_20_TRAINER_6
	const EVENT_BEAT_ROUTE_20_TRAINER_7
	const EVENT_BEAT_ROUTE_20_TRAINER_8
	const EVENT_BEAT_ROUTE_20_TRAINER_9
	const EVENT_SEVII_TANOBY_GARDEN_ACTIVATED_MEW_7 ; new
	const EVENT_SEVII_TANOBY_GARDEN_ACTIVATED_MEW_8 ; new
	const EVENT_SEAFOAM1_BOULDER1_DOWN_HOLE
	const EVENT_SEAFOAM1_BOULDER2_DOWN_HOLE

; Route 21 events
	const_next $510
	const EVENT_RECEIVING_MON ; new
	const EVENT_1ST_SPOKE_WITH_ROUTE21_OAK
	const EVENT_BEAT_ROUTE_21_TRAINER_0
	const EVENT_BEAT_ROUTE_21_TRAINER_1
	const EVENT_BEAT_ROUTE_21_TRAINER_2
	const EVENT_BEAT_ROUTE_21_TRAINER_3
	const EVENT_BEAT_ROUTE_21_TRAINER_4
	const EVENT_BEAT_ROUTE_21_TRAINER_5
	const EVENT_BEAT_ROUTE_21_TRAINER_6
	const EVENT_BEAT_ROUTE_21_TRAINER_7
	const EVENT_BEAT_ROUTE_21_TRAINER_8

; Route 22 events
	const_next $520
	const EVENT_1ST_ROUTE22_RIVAL_BATTLE
	const EVENT_2ND_ROUTE22_RIVAL_BATTLE
	const EVENT_2ND_ROUTE22_RIVAL_BATTLE_BEATEN ; new
	const EVENT_LOAD_RANDOMIZED_1_TYPES ; new
	const EVENT_USING_PORTABLE_PC ; new
	const EVENT_BEAT_ROUTE22_RIVAL_1ST_BATTLE
	const EVENT_BEAT_ROUTE22_RIVAL_2ND_BATTLE
	const EVENT_ROUTE22_RIVAL_WANTS_BATTLE

; Route 23 events
	const_next $530
	const EVENT_PASSED_CASCADEBADGE_CHECK
	const EVENT_PASSED_THUNDERBADGE_CHECK
	const EVENT_PASSED_RAINBOWBADGE_CHECK
	const EVENT_PASSED_SOULBADGE_CHECK
	const EVENT_PASSED_MARSHBADGE_CHECK
	const EVENT_PASSED_VOLCANOBADGE_CHECK
	const EVENT_PASSED_EARTHBADGE_CHECK
	const EVENT_LOAD_RANDOMIZED_2_TYPES ; new
	const EVENT_VICTORY_ROAD_2_BOULDER_ON_SWITCH1
	const EVENT_BEAT_VICTORY_ROAD_2_TRAINER_0
	const EVENT_BEAT_VICTORY_ROAD_2_TRAINER_1
	const EVENT_BEAT_VICTORY_ROAD_2_TRAINER_2
	const EVENT_BEAT_VICTORY_ROAD_2_TRAINER_3
	const EVENT_BEAT_VICTORY_ROAD_2_TRAINER_4
	const EVENT_BEAT_VICTORY_ROAD_2_TRAINER_5 ; new
	const EVENT_VICTORY_ROAD_2_BOULDER_ON_SWITCH2

; Route 24 events
	const_next $540
	const EVENT_GOT_NUGGET
	const EVENT_BEAT_ROUTE24_ROCKET
	const EVENT_BEAT_ROUTE_24_TRAINER_0
	const EVENT_BEAT_ROUTE_24_TRAINER_1
	const EVENT_BEAT_ROUTE_24_TRAINER_2
	const EVENT_BEAT_ROUTE_24_TRAINER_3
	const EVENT_BEAT_ROUTE_24_TRAINER_4
	const EVENT_BEAT_ROUTE_24_TRAINER_5
	const EVENT_GIVING_RANDOM_MON ; new
	const EVENT_NUGGET_REWARD_AVAILABLE

; Route 25 events
	const_next $550 - 1
	const EVENT_GOT_GIFT_CHARMANDER
	const EVENT_MET_BILL
	const EVENT_BEAT_ROUTE_25_TRAINER_0
	const EVENT_BEAT_ROUTE_25_TRAINER_1
	const EVENT_BEAT_ROUTE_25_TRAINER_2
	const EVENT_BEAT_ROUTE_25_TRAINER_3
	const EVENT_BEAT_ROUTE_25_TRAINER_4
	const EVENT_BEAT_ROUTE_25_TRAINER_5
	const EVENT_BEAT_ROUTE_25_TRAINER_6
	const EVENT_BEAT_ROUTE_25_TRAINER_7
	const EVENT_BEAT_ROUTE_25_TRAINER_8
	const EVENT_SHOWN_COMPLETE_MAP_TO_BILL ; new
	const EVENT_USED_CELL_SEPARATOR_ON_BILL
	const EVENT_GOT_SS_TICKET
	const EVENT_MET_BILL_2
	const EVENT_BILL_SAID_USE_CELL_SEPARATOR
	const EVENT_LEFT_BILLS_HOUSE_AFTER_HELPING

; Viridian Forest events
	const_next $560
	const EVENT_CHECK_SHINY_FOR_TRADING_STATIC_SMASH ; new
	const EVENT_RECEIVED_RANDOMIZED_POKEMON_1 ; new
	const EVENT_RECEIVED_RANDOMIZED_POKEMON_2 ; new
	const EVENT_BEAT_VIRIDIAN_FOREST_TRAINER_0
	const EVENT_BEAT_VIRIDIAN_FOREST_TRAINER_1
	const EVENT_BEAT_VIRIDIAN_FOREST_TRAINER_2
	const EVENT_BEAT_VIRIDIAN_FOREST_TRAINER_3
	const EVENT_BEAT_VIRIDIAN_FOREST_TRAINER_4

; Mt. Moon events
	const_next $570
	const EVENT_ROCKET_PATH ; new
	const EVENT_BEAT_MT_MOON_1_TRAINER_0
	const EVENT_BEAT_MT_MOON_1_TRAINER_1
	const EVENT_BEAT_MT_MOON_1_TRAINER_2
	const EVENT_BEAT_MT_MOON_1_TRAINER_3
	const EVENT_BEAT_MT_MOON_1_TRAINER_4
	const EVENT_BEAT_MT_MOON_1_TRAINER_5
	const EVENT_BEAT_MT_MOON_1_TRAINER_6
	const EVENT_GOT_DOME_FOSSIL
	const EVENT_BEAT_MT_MOON_EXIT_SUPER_NERD
	const EVENT_BEAT_MT_MOON_3_TRAINER_0
	const EVENT_BEAT_MT_MOON_3_TRAINER_1
	const EVENT_BEAT_MT_MOON_3_TRAINER_2
	const EVENT_BEAT_MT_MOON_3_TRAINER_3
	const EVENT_57E
	const EVENT_GOT_HELIX_FOSSIL
	const EVENT_BEAT_MT_MOON_1_RIVAL ; new, testing

; S.S. Anne events
	const_next $5C0
	const EVENT_RP_GOT_HM01 ; new
	const EVENT_RP_USE_VANILLA_BATTLE_MESSAGES ; new
	const EVENT_JUST_CAUGHT_TRAINER_MON ; new
	const EVENT_BEAT_RIVAL_SS_ANNE ; new
	const EVENT_BEAT_SS_ANNE_5_TRAINER_0
	const EVENT_BEAT_SS_ANNE_5_TRAINER_1
	const EVENT_DEFEATED_MISSINGNO ; new
	const EVENT_FACED_MISSINGNO_AT_LEAST_ONCE ; new
	const EVENT_SEVII_TANOBY_GARDEN_ACTIVATED_MEW_4 ; new
	const EVENT_BEAT_MZYGARDE
	const EVENT_BEAT_UNECROZMA
	const EVENT_BEAT_MRAYQUAZA
	const EVENT_BEAT_EETERNATUS
	const EVENT_BEAT_ARCEUS
	const EVENT_RP_EEVOLUTIONS_STOLEN_ALL ; new
	const EVENT_RP_EEVOLUTIONS_STOLEN_FAILED_SYLVEON ; new
	const EVENT_RP_SPECIAL_BILL_RENAMING ; new
	const EVENT_BEAT_SNORLAX_DEN ; new, testing
	const EVENT_HAUNTED_HOUSE_BETWEEN_BEDS ; new
	const EVENT_INTERACTED_WITH_DAD_PICTURE ; new
	const EVENT_SPOKEN_WITH_HAUNTED_MOM_1 ; new
	const EVENT_SPOKEN_WITH_HAUNTED_MOM_2 ; new
	const EVENT_SPOKEN_WITH_HAUNTED_MOM_3 ; new
	const EVENT_SPOKEN_WITH_HAUNTED_MOM_4 ; new
	const EVENT_SPOKEN_WITH_HAUNTED_MOM_5 ; new
	const EVENT_ROUTE_25_DISPLAY_HAUNTED_HOUSE_MESSAGE_1 ; new
	const EVENT_ROUTE_25_DISPLAY_HAUNTED_HOUSE_MESSAGE_2 ; new
	const EVENT_ROUTE_25_DISPLAY_HAUNTED_HOUSE_MESSAGE_3 ; new
	const EVENT_HAUNTED_HOUSE_FAKE_POISON ; new
	const EVENT_WALKED_ALL_STEPS_SECRET_GARDEN ; new
	const EVENT_IN_TALL_GRASS_IN_BILLS_SECRET_GARDEN ; new
	const EVENT_UNLOCKED_PATH_TO_HAUNTED_HOUSE ; new
	const EVENT_GOT_HM01
	const EVENT_RUBBED_CAPTAINS_BACK
	const EVENT_SS_ANNE_LEFT
	const EVENT_WALKED_PAST_GUARD_AFTER_SS_ANNE_LEFT
	const EVENT_STARTED_WALKING_OUT_OF_DOCK
	const EVENT_WALKED_OUT_OF_DOCK
	const EVENT_ENGAGED_WITH_FUCHSIA_TRAINER_0 ; new
	const EVENT_ENGAGED_WITH_FUCHSIA_TRAINER_1 ; new
	const EVENT_ENGAGED_WITH_FUCHSIA_TRAINER_2 ; new
	const EVENT_ENGAGED_WITH_FUCHSIA_TRAINER_3 ; new
	const EVENT_ENGAGED_WITH_FUCHSIA_TRAINER_4 ; new
	const EVENT_ENGAGED_WITH_FUCHSIA_TRAINER_5 ; new
	const EVENT_ENGAGED_WITH_FUCHSIA_TRAINER_6 ; new
	const EVENT_HAUNTED_HOUSE_4_MOM_IN_ROOM_SW ; new
	const EVENT_HAUNTED_HOUSE_4_MOM_IN_ROOM_SE ; new
	const EVENT_HAUNTED_HOUSE_4_MOM_IN_ROOM_NE ; new
	const EVENT_HAUNTED_HOUSE_4_MOM_IN_ROOM_NW ; new
	const EVENT_BEAT_SS_ANNE_8_TRAINER_0
	const EVENT_BEAT_SS_ANNE_8_TRAINER_1
	const EVENT_BEAT_SS_ANNE_8_TRAINER_2
	const EVENT_BEAT_SS_ANNE_8_TRAINER_3
	const_skip 11
	const EVENT_ENGAGED_WITH_FUCHSIA_TRAINER_7 ; new
	const EVENT_BEAT_SS_ANNE_9_TRAINER_0
	const EVENT_BEAT_SS_ANNE_9_TRAINER_1
	const EVENT_BEAT_SS_ANNE_9_TRAINER_2
	const EVENT_BEAT_SS_ANNE_9_TRAINER_3
	const_skip 12
	const EVENT_BEAT_SS_ANNE_10_TRAINER_0
	const EVENT_BEAT_SS_ANNE_10_TRAINER_1
	const EVENT_BEAT_SS_ANNE_10_TRAINER_2
	const EVENT_BEAT_SS_ANNE_10_TRAINER_3
	const EVENT_BEAT_SS_ANNE_10_TRAINER_4
	const EVENT_BEAT_SS_ANNE_10_TRAINER_5

; Victory Road 3F events
	const_next $660
	const EVENT_VICTORY_ROAD_3_BOULDER_ON_SWITCH1
	const EVENT_BEAT_VICTORY_ROAD_3_TRAINER_0
	const EVENT_BEAT_VICTORY_ROAD_3_TRAINER_1
	const EVENT_BEAT_VICTORY_ROAD_3_TRAINER_2
	const EVENT_BEAT_VICTORY_ROAD_3_TRAINER_3
	const EVENT_BEAT_VICTORY_ROAD_3_TRAINER_4 ; new
	const EVENT_BEAT_VICTORY_ROAD_3_TRAINER_5 ; new
	const EVENT_BEAT_VICTORY_ROAD_3_TRAINER_6 ; new
	const EVENT_BEAT_VICTORY_ROAD_3_TRAINER_7 ; new
	const EVENT_RP_EEVOLUTIONS_STOLEN_FAILED_GLACEON ; new
	const EVENT_VICTORY_ROAD_3_BOULDER_ON_SWITCH2

; Rocket Hideout events
	const_next $670
	const EVENT_RP_EEVOLUTIONS_STOLEN_FAILED_LEAFEON ; new
	const EVENT_BEAT_ROCKET_HIDEOUT_1_TRAINER_0
	const EVENT_BEAT_ROCKET_HIDEOUT_1_TRAINER_1
	const EVENT_BEAT_ROCKET_HIDEOUT_1_TRAINER_2
	const EVENT_BEAT_ROCKET_HIDEOUT_1_TRAINER_3
	const EVENT_BEAT_ROCKET_HIDEOUT_1_TRAINER_4
	const EVENT_RP_EEVOLUTIONS_STOLEN_FAILED_UMBREON ; new
	const EVENT_SS_ANNE_RETURNED ; new, disentagled for RP
	const EVENT_67F ; ???
	const EVENT_RP_EEVOLUTIONS_STOLEN_FAILED_ESPEON ; new
	const EVENT_BEAT_ROUTE_21_DIVE_TRAINER_0 ; new
	const EVENT_BEAT_ROUTE_21_DIVE_TRAINER_1 ; new
	const EVENT_BEAT_ROUTE_21_DIVE_TRAINER_2 ; new
	const EVENT_BEAT_ROUTE_21_DIVE_TRAINER_3 ; new
	const EVENT_BEAT_ROUTE_21_DIVE_TRAINER_4 ; new
	const EVENT_BEAT_ROUTE_21_DIVE_TRAINER_5 ; new
	const EVENT_BEAT_ROUTE_21_DIVE_TRAINER_6 ; new
	const EVENT_BEAT_ROCKET_HIDEOUT_2_TRAINER_0
	const_skip 15
	const EVENT_BEAT_ROCKET_HIDEOUT_3_TRAINER_0
	const EVENT_BEAT_ROCKET_HIDEOUT_3_TRAINER_1
	const_skip 13
	const EVENT_ENGAGED_JESSIE_JAMES_GAME_CORNER
	const EVENT_ROCKET_USED_LIFT_KEY ; new
	const EVENT_BEAT_ROCKET_HIDEOUT_4_JESSIE_JAMES
	const EVENT_ROCKET_HIDEOUT_4_JESSIE_JAMES_ON_LEFT
	const EVENT_BEAT_ROCKET_HIDEOUT_4_TRAINER_2
	const EVENT_ROCKET_HIDEOUT_4_DOOR_UNLOCKED
	const EVENT_ROCKET_DROPPED_LIFT_KEY
	const EVENT_BEAT_ROCKET_HIDEOUT_GIOVANNI

; Silph Co. events
	const_next $6F0
	const EVENT_SEVII_TANOBY_GARDEN_ACTIVATED_MEW_5 ; new
	const EVENT_USED_CARD_KEY ; new
	const EVENT_BEAT_SILPH_CO_2F_TRAINER_0
	const EVENT_BEAT_SILPH_CO_2F_TRAINER_1
	const EVENT_BEAT_SILPH_CO_2F_TRAINER_2
	const EVENT_BEAT_SILPH_CO_2F_TRAINER_3
	const_skip 7
	const EVENT_SILPH_CO_2_UNLOCKED_DOOR1
	const EVENT_SILPH_CO_2_UNLOCKED_DOOR2
	const EVENT_GOT_TM36
	const EVENT_RP_GOT_BULBASAUR ; new
	const EVENT_RP_BEAT_SS_ANNE_AGENT ; new
	const EVENT_BEAT_SILPH_CO_3F_TRAINER_0
	const EVENT_BEAT_SILPH_CO_3F_TRAINER_1
	const_skip 4
	const EVENT_SILPH_CO_3_UNLOCKED_DOOR1
	const EVENT_SILPH_CO_3_UNLOCKED_DOOR2
	const_skip 6
	const EVENT_GOT_DUBIOUS_DISK	; new
	const EVENT_GOT_UPGRADE			; new
	const EVENT_BEAT_SILPH_CO_4F_TRAINER_0
	const EVENT_BEAT_SILPH_CO_4F_TRAINER_1
	const EVENT_BEAT_SILPH_CO_4F_TRAINER_2
	const_skip 1
	const EVENT_BIRB_SEVII_PARKOUR_PATH ; new
	const EVENT_PARKOUR_TIME_TO_BE_PRINTED ; new
	const EVENT_SILPH_CO_4_UNLOCKED_DOOR1
	const EVENT_SILPH_CO_4_UNLOCKED_DOOR2
	const_skip 9
	const EVENT_BEAT_SILPH_CO_5F_TRAINER_0
	const EVENT_BEAT_SILPH_CO_5F_TRAINER_1
	const EVENT_BEAT_SILPH_CO_5F_TRAINER_2
	const EVENT_BEAT_SILPH_CO_5F_TRAINER_3
	const EVENT_SEVII_FIVE_ISLAND_PINK_ROCKET_FIGHT_OVER ; new
	const EVENT_SILPH_CO_5_UNLOCKED_DOOR1
	const EVENT_SILPH_CO_5_UNLOCKED_DOOR2
	const EVENT_SILPH_CO_5_UNLOCKED_DOOR3
	const_skip 11
	const EVENT_BEAT_SILPH_CO_6F_TRAINER_0
	const EVENT_BEAT_SILPH_CO_6F_TRAINER_1
	const EVENT_BEAT_SILPH_CO_6F_TRAINER_2
	const_skip 6
	const EVENT_SILPH_CO_6_UNLOCKED_DOOR
	const EVENT_BEAT_SILPH_CO_RIVAL
	const_skip 4
	const EVENT_BEAT_SILPH_CO_7F_TRAINER_0
	const EVENT_BEAT_SILPH_CO_7F_TRAINER_1
	const EVENT_BEAT_SILPH_CO_7F_TRAINER_2
	const EVENT_BEAT_SILPH_CO_7F_TRAINER_3
	const EVENT_USING_TM_CASE ; new
	const EVENT_CURRENTLY_RUNNING ; new
	const EVENT_DO_NOT_WARN_ABOUT_CHAOS_MODE ; new
	const EVENT_SILPH_CO_7_UNLOCKED_DOOR1
	const EVENT_SILPH_CO_7_UNLOCKED_DOOR2
	const EVENT_SILPH_CO_7_UNLOCKED_DOOR3
	const EVENT_OPENED_PALLET_FIELDS ; new
	const EVENT_CAN_OPEN_PALLET_FIELDS ; new
	const EVENT_RUN_TOGGLE ; new
	const EVENT_BEAT_SILPH_CO_8F_TRAINER_0
	const EVENT_BEAT_SILPH_CO_8F_TRAINER_1
	const EVENT_BEAT_SILPH_CO_8F_TRAINER_2
	const_skip 2
	const EVENT_GOT_TM_DIG ; new
	const EVENT_SILPH_CO_8_UNLOCKED_DOOR
	const_skip 9
	const EVENT_BEAT_SILPH_CO_9F_TRAINER_0
	const EVENT_BEAT_SILPH_CO_9F_TRAINER_1
	const EVENT_BEAT_SILPH_CO_9F_TRAINER_2
	const EVENT_FUSION_MACHINE_SHINY ; new
	const EVENT_FUSION_MACHINE_DELTA ; new
	const EVENT_BIRB_SEVII_MT_EMBER ; new
	const EVENT_SILPH_CO_9_UNLOCKED_DOOR1
	const EVENT_SILPH_CO_9_UNLOCKED_DOOR2
	const EVENT_SILPH_CO_9_UNLOCKED_DOOR3
	const EVENT_SILPH_CO_9_UNLOCKED_DOOR4
	const EVENT_BIRB_FORLORNVALLEY ; new
	const EVENT_BIRB_SEVII_ONE ; new
	const EVENT_BIRB_SEVII_TWO ; new
	const EVENT_BIRB_SEVII_THREE ; new
	const EVENT_BIRB_SEVII_FOUR ; new
	const EVENT_BEAT_SILPH_CO_10F_TRAINER_0
	const EVENT_BEAT_SILPH_CO_10F_TRAINER_1
	const EVENT_BIRB_SEVII_FIVE ; new
	const EVENT_BIRB_SEVII_SIX ; new
	const EVENT_BIRB_SEVII_SEVEN ; new
	const EVENT_BIRB_SEVII_ROUTE31 ; new
	const EVENT_BIRB_SEVII_ROUTE32 ; new
	const EVENT_SILPH_CO_10_UNLOCKED_DOOR
	const EVENT_BIRB_SEVII_ROUTE33 ; new
	const EVENT_BIRB_SEVII_ROUTE34 ; new
	const EVENT_BIRB_SEVII_ROUTE35 ; new
	const EVENT_BIRB_SEVII_ROUTE36 ; new
	const EVENT_BIRB_SEVII_ROUTE37 ; new
	const EVENT_BIRB_SEVII_ROUTE38 ; new
	const EVENT_BIRB_SEVII_ROUTE39 ; new
	const EVENT_780
	const EVENT_781
	const EVENT_782
	const EVENT_BIRB_SEVII_ROUTE40 ; new
	const EVENT_BEAT_SILPH_CO_11F_TRAINER_0
	const EVENT_BEAT_SILPH_CO_11F_TRAINER_1
	const EVENT_BIRB_SEVII_ROUTE41 ; new
	const EVENT_BIRB_SEVII_ROUTE42 ; new
	const EVENT_SILPH_CO_11_UNLOCKED_DOOR
	const EVENT_BIRB_SEVII_ROUTE43 ; new
	const EVENT_BIRB_SEVII_ROUTE44 ; new
	const EVENT_BIRB_SEVII_EIGHT ; new
	const EVENT_BIRB_SEVII_DESOLATED ; new
	const EVENT_GOT_MASTER_BALL
	const EVENT_BIRB_SEVII_TEN ; new
	const EVENT_BEAT_SILPH_CO_GIOVANNI

; Pokémon Mansion events
	const_next $800
	const EVENT_SEVII_BEAT_PINK_ROUTE_23 ; new
	const EVENT_SEVII_BEAT_PINK_PINKS_HOUSE ; new
	const EVENT_BEAT_MANSION_2_TRAINER_0
	const_skip 14 ; edited for rematch Blaine
	const EVENT_BEAT_MANSION_3_TRAINER_0
	const EVENT_BEAT_MANSION_3_TRAINER_1
	const_skip 14
	const EVENT_BEAT_MANSION_4_TRAINER_0
	const EVENT_BEAT_MANSION_4_TRAINER_1
	const EVENT_BEAT_MANSION_4_TRAINER_2 ; new

; Safari Zone events
	const_next $880
	const EVENT_RP_EEVOLUTIONS_STOLEN_FAILED_JOLTEON ; new
	const EVENT_BEAT_SAFARI_ZONE_NORTH_TRAINER_0 ; new, for Giovanni
	const EVENT_GOT_HM03

; Cerulean Cave events
	const_next $8C0
	const EVENT_RP_EEVOLUTIONS_STOLEN_FAILED_VAPOREON ; new
	const EVENT_BEAT_MEWTWO
	const EVENT_RP_DEFEAT_JENNY_CERULEAN ; new
	const EVENT_RP_DEFEAT_JENNY_VERMILION ; new
	const EVENT_RP_SPOKEN_WITH_GAME_CORNER_GIOVANNI ; new
	const EVENT_RP_RECEIVED_SILPH_SCOPE ; new
	const EVENT_RP_BEAT_ROCKET_HIDEOUT_4_RIVAL ; new
	const EVENT_RP_CONVINCED_MX_FUJI ; new
	const EVENT_RP_SPOKEN_WITH_GIOVANNI_POST_FUJI ; new
	const EVENT_RP_RECEIVED_SEVII_TICKET_1_3 ; new
	const EVENT_RP_GOT_ALL_ROOF_TMS ; new
	const EVENT_RP_BRIDGE_COMPLETED ; new
	const EVENT_RP_BEAT_WEATHER_BALL_TUTOR ; new
	const EVENT_RP_BEAT_TERRAIN_PULSE_TUTOR ; new
	const EVENT_RP_BEAT_PINK_CHRONO_WAREHOUSE ; new
	const EVENT_RP_UNLOCKED_SILPH ; new
	const EVENT_RP_GOT_PORYGON ; new
	const EVENT_RP_ROBBED_ANTIQUE_SHOP ; new
	const EVENT_RP_RETRIEVED_OTHER_FOSSIL ; new
	const EVENT_RP_SPECIAL_BIRDKEEPER_SPOKE_AS_HERO ; new
	const EVENT_RP_BEAT_ROUTE_3_JENNY ; new
	const EVENT_RP_BEAT_LUNAR_TEMPLE_BLUE ; new
	const EVENT_OBTAINED_HM04 ; new
	const EVENT_RP_FULLY_DESTROYS_SHRINE ; new
	const EVENT_RP_BEAT_ARCHEOLOGIST_GRANKID_MUSEUM ; new
	const EVENT_RP_RIPPED_DADS_PHOTO ; new
	const EVENT_RP_CANT_SURF_ON_CYCLING_ROAD ; new
	const EVENT_RP_GAVE_BIRBS_TO_GIOVANNI ; new
	const EVENT_RP_GAVE_BERSERK_GENE ; new
	const EVENT_RP_BEAT_HOF_OAK ; new
	const EVENT_RP_CREDITS ; new
	const EVENT_RP_KILLED_GIOVANNI ; new

; Indigo Plateau events
	const_next $8E0
DEF INDIGO_PLATEAU_EVENTS_START EQU const_value
	const EVENT_RP_EEVOLUTIONS_STOLEN_FAILED_FLAREON ; new
	const EVENT_BEAT_LORELEIS_ROOM_TRAINER_0
	const EVENT_BEAT_LORELEIS_ROOM_TRAINER_1 ; new
	const EVENT_BEAT_LORELEIS_ROOM_TRAINER_2 ; new
	const EVENT_BEAT_LORELEIS_ROOM_TRAINER_3 ; new
	const EVENT_BEAT_LORELEIS_ROOM_TRAINER_4 ; new
	const EVENT_AUTOWALKED_INTO_LORELEIS_ROOM
	const_skip 2
	const EVENT_BEAT_BRUNOS_ROOM_TRAINER_0
	const EVENT_BEAT_BRUNOS_ROOM_TRAINER_1 ; new
	const EVENT_BEAT_BRUNOS_ROOM_TRAINER_2 ; new
	const EVENT_BEAT_BRUNOS_ROOM_TRAINER_3 ; new
	const EVENT_BEAT_BRUNOS_ROOM_TRAINER_4 ; new
	const EVENT_AUTOWALKED_INTO_BRUNOS_ROOM
	const_skip 2
	const EVENT_BEAT_AGATHAS_ROOM_TRAINER_0
	const EVENT_BEAT_AGATHAS_ROOM_TRAINER_1 ; new
	const EVENT_BEAT_AGATHAS_ROOM_TRAINER_2 ; new
	const EVENT_BEAT_AGATHAS_ROOM_TRAINER_3 ; new
	const EVENT_BEAT_AGATHAS_ROOM_TRAINER_4 ; new
	const EVENT_AUTOWALKED_INTO_AGATHAS_ROOM
	const_skip 2
	const EVENT_BEAT_LANCES_ROOM_TRAINER_0
	const EVENT_BEAT_LANCES_ROOM_TRAINER_1 ; new
	const EVENT_BEAT_LANCES_ROOM_TRAINER_2 ; new
	const EVENT_BEAT_LANCES_ROOM_TRAINER_3 ; new
	const EVENT_BEAT_LANCES_ROOM_TRAINER_4 ; new
	const EVENT_BEAT_LANCE
	const EVENT_LANCES_ROOM_LOCK_DOOR
	const_skip 1
	const EVENT_BEAT_CHAMPION_RIVAL
	const_skip 4 ; edited, was 5
	const EVENT_ENGAGED_CHAMPION_FINAL_REMATCH ; new, ugly but whatever, we have plenty of events...
;	const_skip ; commented away
DEF INDIGO_PLATEAU_EVENTS_END EQU const_value - 1
	const EVENT_BEAT_LEAGUE_AT_LEAST_ONCE ; new
	const EVENT_BEAT_INTERDIMENSIONAL_TRAVELER ; new
	const EVENT_BEAT_ALL_GYMS_REMATCH ; new
	const EVENT_BEAT_CHAMPION_FINAL_REMATCH ; new

; Victory Road 1F events
	const_next $910
	const_skip
	const EVENT_BEAT_VICTORY_ROAD_1_TRAINER_0
	const EVENT_BEAT_VICTORY_ROAD_1_TRAINER_1
	const EVENT_BEAT_VICTORY_ROAD_1_TRAINER_2
	const EVENT_BEAT_VICTORY_ROAD_1_TRAINER_3
	const_skip 2
	const EVENT_VICTORY_ROAD_1_BOULDER_ON_SWITCH

; Rock Tunnel events
	const_next $9B0
	const EVENT_PARKOUR_OVERTIME ; new
	const EVENT_PARKOUR_TRACKING_TIME ; new
	const EVENT_BEAT_ROCK_TUNNEL_2_TRAINER_0
	const EVENT_BEAT_ROCK_TUNNEL_2_TRAINER_1
	const EVENT_BEAT_ROCK_TUNNEL_2_TRAINER_2
	const EVENT_BEAT_ROCK_TUNNEL_2_TRAINER_3
	const EVENT_BEAT_ROCK_TUNNEL_2_TRAINER_4
	const EVENT_BEAT_ROCK_TUNNEL_2_TRAINER_5
	const EVENT_BEAT_ROCK_TUNNEL_2_TRAINER_6
	const EVENT_BEAT_ROCK_TUNNEL_2_TRAINER_7

; Seafoam Islands events
	const_next $9C0
	const EVENT_SEAFOAM2_BOULDER1_DOWN_HOLE
	const EVENT_SEAFOAM2_BOULDER2_DOWN_HOLE
	const_skip 6
	const EVENT_SEAFOAM3_BOULDER1_DOWN_HOLE
	const EVENT_SEAFOAM3_BOULDER2_DOWN_HOLE
	const_skip 6
	const EVENT_SEAFOAM4_BOULDER1_DOWN_HOLE
	const EVENT_SEAFOAM4_BOULDER2_DOWN_HOLE
	const_skip 5
	const EVENT_REACHED_FORBIDDEN_MERCHANT ; new
	const EVENT_UNLOCKED_SEVII ; new for sevii
	const EVENT_FLYING_BETWEEN_KANTO_AND_SEVII ; new for sevii
	const EVENT_BEAT_ARTICUNO
	const EVENT_RP_BEAT_OBSIDIAN_CADRES ; new
	const EVENT_RP_BEAT_OCHRE_BIRB_FAN_1 ; new
	const EVENT_RP_BEAT_OCHRE_BIRB_FAN_2 ; new
	const EVENT_RP_BEAT_OCHRE_BIRB_FAN_3 ; new
	const EVENT_RP_STOLE_BIG_NUGGET_OBSIDIAN_HIKER ; new

; Ochre City events ; new, and then scattered around
	const_next $9F0
	const EVENT_SEVII_TANOBY_GARDEN_ACTIVATED_MEW_6 ; new
	const EVENT_1ST_SPOKE_WITH_WONDERLAND_BLUE
	const EVENT_BEAT_OCHRE_GYM_TRAINER_0
	const EVENT_BEAT_OCHRE_GYM_TRAINER_1
	const EVENT_BEAT_OCHRE_GYM_TRAINER_2
	const EVENT_BEAT_OCHRE_GYM_TRAINER_3
	const EVENT_BEAT_OCHRE_GYM_TRAINER_4
	const EVENT_BEAT_OCHRE_GYM_TRAINER_5
	const EVENT_BEAT_OCHRE_GYM_TRAINER_6
	const EVENT_BEAT_OCHRE_GYM_ORAGE
	const EVENT_OCHRE_GYM_GOT_GIFT
	const EVENT_OCHRE_LOCK_TRY_1
	const EVENT_OCHRE_LOCK_TRY_2
	const EVENT_REACTIVATE_ALL_TRAINERS
	const EVENT_ALREADY_TALKED_WITH_TRAINER_REACTIVATER
	const EVENT_IN_SEVII

; End of events
	const_next $A00
DEF NUM_EVENTS EQU const_value
