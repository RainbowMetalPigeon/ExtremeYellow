; Yellow entry format:
;	db trainerclass, trainerid
;	repeat { db partymon location, partymon move, move id }
;	db $FE

SpecialTrainerMoves:
	db BUG_CATCHER, 15 ; updated
	db 1, 1, TACKLE
	db 1, 2, WRAP
	db $FE

	db BUG_CATCHER, 3 ; new
	; BUTTERFREE
	db 1, 2, TACKLE
	db 1, 3, STRING_SHOT
	db 1, 4, HARDEN
	; BEEDRILL
	db 2, 2, POISON_STING
	db 2, 3, STRING_SHOT
	db 2, 4, HARDEN
	db $FE

	db YOUNGSTER, 14
	db 6, 1, SLASH
	db 6, 2, DIG
	db 6, 3, DEFENSE_CURL
	db 6, 4, FISSURE
	db $FE

	db SUPER_NERD, 17
	; ELECTABUZZ
	db 1, 1, ELECTRIC_TERRAIN
	; TANGELA
	db 2, 1, GRASSY_TERRAIN
	; EXEGGUTOR
	db 3, 1, GRASSY_TERRAIN
	db 3, 1, PSYCHIC_TERRAIN
	; HYPNO
	db 4, 1, PSYCHIC_TERRAIN
	; MR_MIME
	db 5, 1, PSYCHIC_TERRAIN
	db 5, 1, MISTY_TERRAIN
	; WIGGLYTUFF
	db 6, 1, MISTY_TERRAIN
	db $FE

	db JUGGLER, 4 ; evil exploding Juggler in Fuchsia Gym :')
	; WEEZING
	db 1, 1, EXPLOSION
	db 1, 2, NO_MOVE
	db 1, 3, NO_MOVE
	db 1, 4, NO_MOVE
	; ELECTRODE
	db 2, 1, EXPLOSION
	db 2, 2, NO_MOVE
	db 2, 3, NO_MOVE
	db 2, 4, NO_MOVE
	; GOLEM
	db 3, 1, EXPLOSION
	db 3, 2, NO_MOVE
	db 3, 3, NO_MOVE
	db 3, 4, NO_MOVE
	; CLOYSTER
	db 4, 1, EXPLOSION
	db 4, 2, NO_MOVE
	db 4, 3, NO_MOVE
	db 4, 4, NO_MOVE
	; LICKILICKY
	db 5, 1, EXPLOSION
	db 5, 2, NO_MOVE
	db 5, 3, NO_MOVE
	db 5, 4, NO_MOVE
	; SNORLAX
	db 6, 1, SELFDESTRUCT
	db 6, 2, NO_MOVE
	db 6, 3, NO_MOVE
	db 6, 4, NO_MOVE
	db $FE

	db JUGGLER, 9 ; new
	; HYPNO
	db 1, 1, METRONOME
	db 1, 2, NO_MOVE
	db 1, 3, NO_MOVE
	db 1, 4, NO_MOVE
	; CLEFAIRY
	db 2, 1, METRONOME
	db 2, 2, NO_MOVE
	db 2, 3, NO_MOVE
	db 2, 4, NO_MOVE
	; ALAKAZAM
	db 3, 1, METRONOME
	db 3, 2, NO_MOVE
	db 3, 3, NO_MOVE
	db 3, 4, NO_MOVE
	; WIGGLYTUFF
	db 4, 1, METRONOME
	db 4, 2, NO_MOVE
	db 4, 3, NO_MOVE
	db 4, 4, NO_MOVE
	; MR_MIME
	db 5, 1, METRONOME
	db 5, 2, NO_MOVE
	db 5, 3, NO_MOVE
	db 5, 4, NO_MOVE
	; GENGAR
	db 6, 1, METRONOME
	db 6, 2, NO_MOVE
	db 6, 3, NO_MOVE
	db 6, 4, NO_MOVE
	db $FE

	db GENTLEMAN, 6 ; new, SS Anne Captain
	; SCYTHER
	db 1, 1, CUT
	db 1, 2, AIR_SLASH
	db 1, 3, X_SCISSOR
	db 1, 4, AIR_CUTTER
	; KABUTOPS
	db 2, 1, CUT
	db 2, 2, SLASH
	db 2, 3, STONE_EDGE
	db 2, 4, SWORDS_DANCE
	; SCIZOR
	db 3, 1, CUT
	db 3, 2, SLASH
	db 3, 3, X_SCISSOR
	db 3, 4, PSYCHO_CUT
	; SANDSLASH
	db 4, 1, CUT
	db 4, 2, SLASH
	db 4, 3, STONE_EDGE
	db 4, 4, NIGHT_SLASH
	; KLEAVOR
	db 5, 1, CUT
	db 5, 2, SLASH
	db 5, 3, STONE_EDGE
	db 5, 4, X_SCISSOR
	; FARFETCHD
	db 6, 1, CUT
	db 6, 2, SLASH
	db 6, 3, SWORDS_DANCE
	db 6, 4, NIGHT_SLASH
	db $FE

	db COOLTRAINER, 64 ; pseudo-Green
	; JIGGLYPUFF
	db 1, 1, SING
	db 1, 2, DISABLE
	db 1, 3, DOUBLE_EDGE
	db 1, 4, DEFENSE_CURL
	; ALAKAZAM
	db 2, 1, THUNDERPUNCH
	db 2, 2, ICE_PUNCH
	db 2, 3, FIRE_PUNCH
	db 2, 4, PSYCHIC_M
	; DITTO
	; lol
	; CLEFABLE
	db 4, 1, MOONBLAST
	db 4, 2, LIGHT_SCREEN
	db 4, 3, REFLECT
	db 4, 4, METRONOME
	; NIDOQUEEN
	db 5, 1, CRUNCH
	db 5, 2, DOUBLE_KICK
	db 5, 3, ROCK_TOMB
	db 5, 4, SLUDGE_WAVE
	; BLASTOISE
	db 6, 1, HYDRO_CANNON
	db 6, 2, FLASH_CANNON
	db 6, 3, ICE_BEAM
	db 6, 4, EARTHQUAKE
	db $FE

	db COOLTRAINER, 65 ; pseudo-Red
	; POLIWRATH
	db 1, 1, CLOSE_COMBAT
	db 1, 2, SCALD
	db 1, 3, ICE_BEAM
	db 1, 4, HYPNOSIS
	; VENUSAUR
	db 2, 1, SLUDGE_WAVE
	db 2, 2, FRENZY_PLANT
	db 2, 3, SLEEP_POWDER
	db 2, 4, LEECH_SEED
	; PIKACHU
	db 3, 1, TOXIC
	db 3, 2, IRON_TAIL
	db 3, 3, SURF
	db 3, 4, VOLT_TACKLE
	; SNORLAX
	db 4, 1, AMNESIA
	db 4, 2, CRUNCH
	db 4, 3, REST
	db 4, 4, BODY_SLAM
	; GYARADOS
	db 5, 1, HYDRO_PUMP
	db 5, 2, HYPER_BEAM
	db 5, 3, DRAGON_DANCE
	db 5, 4, CRUNCH
	; AERODACTYL
	db 6, 1, FLY
	db 6, 2, HEAD_SMASH
	db 6, 3, DRAGON_CLAW
	db 6, 4, HYPER_BEAM
	db $FE

	db COOLTRAINER, 66 ; pseudo-Blue
	; SCIZOR
	db 1, 1, SLASH
	db 1, 2, STEEL_WING
	db 1, 3, X_SCISSOR
	db 1, 4, ROOST
	; CHARIZARD
	db 2, 1, BLAST_BURN
	db 2, 2, FLY
	db 2, 3, REFLECT
	db 2, 4, FIRE_SPIN
	; GOLDUCK
	db 3, 1, HYDRO_PUMP
	db 3, 2, CONFUSION
	db 3, 3, DIG
	db 3, 4, IRON_TAIL
	; MACHAMP
	db 4, 1, KARATE_CHOP
	db 4, 2, HYPER_BEAM
	db 4, 3, FOCUS_ENERGY
	db 4, 4, EARTHQUAKE
	; PORYGON2
	db 5, 1, TRI_ATTACK
	db 5, 2, CONVERSION
	db 5, 3, ZAP_CANNON
	db 5, 4, PSYBEAM
	; RHYPERIOR
	db 6, 1, EARTHQUAKE
	db 6, 2, HEAD_SMASH
	db 6, 3, MEGAHORN
	db 6, 4, HORN_DRILL
	db $FE

	db COOLTRAINER, 17 ; Vermilion Gym, new
	; PICHU
	db 1, 1, VOLT_TACKLE
	db 1, 2, FLY
	db 1, 3, SURF
	db 1, 4, PSYCHIC_M
	; ELEKID
	db 2, 1, IRON_TAIL
	db 2, 2, THUNDERBOLT
	db 2, 3, KARATE_CHOP
	db 2, 4, CLOSE_COMBAT
	db $FE

	db COOLTRAINER, 9 ; Cerulean Gym, new
	; KRABBY
	db 5, 1, METAL_CLAW
	db 5, 2, MUD_SHOT
	db 5, 3, LEER
	db 5, 4, CRABHAMMER
	db $FE

	db COOLTRAINER, 44 ; Saffron Gym, new
	; JYNX
	db 2, 1, LOVELY_KISS
;	db 2, 2, LIGHT_SCREEN
;	db 2, 3, WATER_PULSE
;	db 2, 4, ICE_PUNCH
	; EXEGGUTOR
;	db 3, 1, ENERGY_BALL
;	db 3, 2, CONFUSION
	db 3, 3, SLEEP_POWDER
;	db 3, 4, STOMP
	; HAUNTER
	db 2, 1, HYPNOSIS
;	db 2, 2, INFESTATION
;	db 2, 3, SHADOW_PUNCH
;	db 2, 4, NIGHT_SHADE
	db $FE

; GYM LEADERS ===================================================

; BROCK's teams -------------------------------------------------

	db BROCK, 1
	; GEODUDE
	db 1, 1, TACKLE
	db 1, 2, DEFENSE_CURL
	db 1, 3, MUD_SLAP
	db 1, 4, ROCK_THROW
	; OMANYTE
	db 2, 1, WATER_GUN
	db 2, 2, DEFENSE_CURL
	db 2, 3, WRAP
	db 2, 4, SAND_ATTACK
	; KABUTO
	db 3, 1, SCRATCH
	db 3, 2, HARDEN
	db 3, 3, WATER_GUN
	db 3, 4, ABSORB
	; RHYHORN
	db 4, 1, HORN_ATTACK
	db 4, 2, BIDE
	db 4, 3, MUD_SLAP
	db 4, 4, LEER
	; VULPIX
	db 5, 1, EMBER
	db 5, 2, LEER
	db 5, 3, DISABLE
	db 5, 4, QUICK_ATTACK
	; ONIX
	db 6, 1, WRAP
	db 6, 2, SCREECH
	db 6, 3, BIDE
	db 6, 4, MUD_SLAP
	db $FE

	db BROCK, 2
	; GEODUDE
	db 1, 1, MUD_SLAP
	db 1, 2, ROCK_THROW
	db 1, 3, SELFDESTRUCT
	db 1, 4, CURSE
	; OMANYTE
	db 2, 1, WRAP
	db 2, 2, SAND_ATTACK
	db 2, 3, WATER_GUN
	db 2, 4, SUPERSONIC
	; KABUTO
	db 3, 1, AQUA_JET
	db 3, 2, ROCK_THROW
	db 3, 3, CONFUSE_RAY
	db 3, 4, MEGA_DRAIN
	; RHYHORN
	db 4, 1, HORN_ATTACK
	db 4, 2, MUD_SLAP
	db 4, 3, SAND_ATTACK
	db 4, 4, ROCK_THROW
	; VULPIX
	db 5, 1, EMBER
	db 5, 2, DISABLE
	db 5, 3, CONFUSE_RAY
	db 5, 4, WILL_O_WISP
	; ONIX
	db 6, 1, MUD_SLAP
	db 6, 2, BODY_PRESS
	db 6, 3, WRAP
	db 6, 4, ROCK_SLIDE
	db $FE

	db BROCK, 3
	; GRAVELER
	db 1, 1, ROCK_SLIDE
	db 1, 2, MUD_SLAP
	db 1, 3, FIRE_BLAST
	db 1, 4, SELFDESTRUCT
	; OMANYTE
	db 2, 1, BUBBLEBEAM
	db 2, 2, SUPERSONIC
	db 2, 3, MUD_SHOT
	db 2, 4, ROCK_TOMB
	; KABUTO
	db 3, 1, GIGA_DRAIN
	db 3, 2, BUBBLEBEAM
	db 3, 3, ROCK_THROW
	db 3, 4, CONFUSE_RAY
	; RHYHORN
	db 4, 1, ROCK_THROW
	db 4, 2, IRON_TAIL
	db 4, 3, DIG
	db 4, 4, BODY_SLAM
	; NINETALES
	db 5, 1, CONFUSE_RAY
	db 5, 2, HYPNOSIS
	db 5, 3, FLAMETHROWER
	db 5, 4, DARK_PULSE
	; ONIX
	db 6, 1, DIG
	db 6, 2, BODY_PRESS
	db 6, 3, BODY_SLAM
	db 6, 4, ROCK_SLIDE
	db $FE

	db BROCK, 4
	; GRAVELER
	db 1, 1, ROCK_SLIDE
	db 1, 2, EARTHQUAKE
	db 1, 3, FIRE_BLAST
	db 1, 4, SELFDESTRUCT
	; OMASTAR
	db 2, 1, BUBBLEBEAM
	db 2, 2, ICE_BEAM
	db 2, 3, BITE
	db 2, 4, ROCK_SLIDE
	; KABUTOPS
	db 3, 1, GIGA_DRAIN
	db 3, 2, BODY_SLAM
	db 3, 3, ROCK_SLIDE
	db 3, 4, CONFUSE_RAY
	; RHYDON
	db 4, 1, MEGA_PUNCH
	db 4, 2, MEGAHORN
	db 4, 3, DIG
	db 4, 4, ROCK_SLIDE
	; NINETALES
	db 5, 1, CONFUSE_RAY
	db 5, 2, HYPNOSIS
	db 5, 3, FLAMETHROWER
	db 5, 4, DARK_PULSE
	; ONIX
	db 6, 1, EARTHBADGE
	db 6, 2, BODY_PRESS
	db 6, 3, EXPLOSION
	db 6, 4, ROCK_SLIDE
	db $FE

	db BROCK, 5
	; GOLEM
	db 1, 1, EARTHQUAKE
	db 1, 2, EXPLOSION
	db 1, 3, BODY_PRESS
	db 1, 4, GYRO_BALL
	; OMASTAR
	db 2, 1, SURF
	db 2, 2, ICE_BEAM
	db 2, 3, SUPERSONIC
	db 2, 4, ROCK_SLIDE
	; KABUTOPS
	db 3, 1, LEECH_LIFE
	db 3, 2, SLASH
	db 3, 3, ROCK_SLIDE
	db 3, 4, CONFUSE_RAY
	; RHYDON
	db 4, 1, EARTHQUAKE
	db 4, 2, MEGAHORN
	db 4, 3, IRON_TAIL
	db 4, 4, ROCK_SLIDE
	; NINETALES
	db 5, 1, CONFUSE_RAY
	db 5, 2, WILL_O_WISP
	db 5, 3, DARK_PULSE
	db 5, 4, SUBSTITUTE
	; KLEAVOR
	db 6, 1, AIR_SLASH
	db 6, 2, SLASH
	db 6, 3, SWORDS_DANCE
	db 6, 4, X_SCISSOR
	db $FE

	db BROCK, 6
	; GOLEM
	db 1, 1, EARTHQUAKE
	db 1, 2, CURSE
	db 1, 3, BODY_PRESS
	db 1, 4, GYRO_BALL
	; OMASTAR
	db 2, 1, SURF
	db 2, 2, ICE_BEAM
	db 2, 3, HYPER_BEAM
	db 2, 4, ROCK_SLIDE
	; KABUTOPS
	db 3, 1, LEECH_LIFE
	db 3, 2, SLASH
	db 3, 3, ROCK_SLIDE
	db 3, 4, CONFUSE_RAY
	; RHYDON
	db 4, 1, EARTHQUAKE
	db 4, 2, MEGAHORN
	db 4, 3, IRON_TAIL
	db 4, 4, STONE_EDGE
	; NINETALES
	db 5, 1, CONFUSE_RAY
	db 5, 2, WILL_O_WISP
	db 5, 3, FIRE_BLAST
	db 5, 4, DOUBLE_TEAM
	; KLEAVOR
	db 6, 1, AIR_SLASH
	db 6, 2, ROOST
	db 6, 3, CLOSE_COMBAT
	db 6, 4, X_SCISSOR
	db $FE

	db BROCK, 7
	; GOLEM
	db 1, 1, EARTHQUAKE
	db 1, 2, EXPLOSION
	db 1, 3, SUPERPOWER
	db 1, 4, GYRO_BALL
	; OMASTAR
	db 2, 1, HYDRO_PUMP
	db 2, 2, BLIZZARD
	db 2, 3, HYPER_BEAM
	db 2, 4, STONE_EDGE
	; KABUTOPS
	db 3, 1, LEECH_LIFE
	db 3, 2, SLASH
	db 3, 3, STONE_EDGE
	db 3, 4, PSYCHO_CUT
	; RHYPERIOR
	db 4, 1, EARTHQUAKE
	db 4, 2, STONE_EDGE
	db 4, 3, SHADOW_CLAW
	db 4, 4, BODY_PRESS
	; KLEAVOR
	db 6, 1, AIR_SLASH
	db 6, 2, STONE_EDGE
	db 6, 3, CLOSE_COMBAT
	db 6, 4, X_SCISSOR
	; AERODACTYL
	db 6, 1, AIR_CUTTER
	db 6, 2, ROOST
	db 6, 3, STONE_EDGE
	db 6, 4, EARTHQUAKE
	db $FE

	db BROCK, 8
	; GOLEM
	db 1, 1, EARTHQUAKE
	db 1, 2, EXPLOSION
	db 1, 3, BODY_PRESS
	db 1, 4, STONE_EDGE
	; OMASTAR
	db 2, 1, HYDRO_PUMP
	db 2, 2, BLIZZARD
	db 2, 3, HYPER_BEAM
	db 2, 4, STONE_EDGE
	; KABUTOPS
	db 3, 1, LEECH_LIFE
	db 3, 2, SLASH
	db 3, 3, STONE_EDGE
	db 3, 4, PSYCHO_CUT
	; RHYPERIOR
	db 4, 1, EARTHQUAKE
	db 4, 2, STONE_EDGE
	db 4, 3, HYPER_BEAM
	db 4, 4, BODY_PRESS
	; KLEAVOR
	db 6, 1, AIR_SLASH
	db 6, 2, STONE_EDGE
	db 6, 3, CLOSE_COMBAT
	db 6, 4, X_SCISSOR
	; AERODACTYL
	db 6, 1, AIR_CUTTER
	db 6, 2, ROOST
	db 6, 3, STONE_EDGE
	db 6, 4, EARTHQUAKE
	db $FE

	db BROCK, 9
	; GOLEM
	db 1, 1, EARTHQUAKE
	db 1, 2, GYRO_BALL
	db 1, 3, BODY_PRESS
	db 1, 4, STONE_EDGE
	; OMASTAR
	db 2, 1, HYDRO_PUMP
	db 2, 2, BLIZZARD
	db 2, 3, HYPER_BEAM
	db 2, 4, STONE_EDGE
	; KABUTOPS
	db 3, 1, LEECH_LIFE
	db 3, 2, SLASH
	db 3, 3, STONE_EDGE
	db 3, 4, PSYCHO_CUT
	; RHYPERIOR
	db 4, 1, EARTHQUAKE
	db 4, 2, STONE_EDGE
	db 4, 3, HYPER_BEAM
	db 4, 4, BODY_PRESS
	; KLEAVOR
	db 6, 1, AIR_SLASH
	db 6, 2, STONE_EDGE
	db 6, 3, CLOSE_COMBAT
	db 6, 4, X_SCISSOR
	; MARODACTYL
	db 6, 1, AIR_CUTTER
	db 6, 2, ROOST
	db 6, 3, STONE_EDGE
	db 6, 4, EARTHQUAKE
	db $FE

; MISTY's teams -------------------------------------------------

	db MISTY, 1
	; GOLDEEN
	db 1, 1, PECK
	db 1, 2, SUPERSONIC
	db 1, 3, LEER
	db 1, 4, WATER_PULSE
	; HORSEA
	db 2, 1, BUBBLE
	db 2, 2, SMOKESCREEN
	db 2, 3, TWISTER
	db 2, 4, NO_MOVE
	; PSYDUCK
	db 3, 1, LEER
	db 3, 2, WATER_GUN
	db 3, 3, CONFUSION
	db 3, 4, FURY_SWIPES
	; POLIWAG
	db 4, 1, BUBBLE
	db 4, 2, POUND
	db 4, 3, DEFENSE_CURL
	db 4, 4, MUD_SHOT
	; SQUIRTLE
	db 5, 1, TACKLE
	db 5, 2, LEER
	db 5, 3, BUBBLE
	db 5, 4, BITE
	; STARYU
	db 6, 1, TACKLE
	db 6, 2, WATER_GUN
	db 6, 3, CONFUSE_RAY
	db 6, 4, MINIMIZE
	db $FE

	db MISTY, 2
	; STARYU
	db 1, 1, CONFUSE_RAY
	db 1, 2, BUBBLEBEAM
	db 1, 3, MINIMIZE
	db 1, 4, SWIFT
	; PSYDUCK
	db 2, 1, AMNESIA
	db 2, 2, DISABLE
	db 2, 3, ZEN_HEADBUTT
	db 2, 4, HEADBUTT
	; POLIWHIRL
	db 3, 1, MUD_SHOT
	db 3, 2, HYPNOSIS
	db 3, 3, LOW_KICK
	db 3, 4, BUBBLEBEAM
	; GYARADOS
	db 4, 1, BITE
	db 4, 2, TWISTER
	db 4, 3, BUBBLEBEAM
	db 4, 4, LEER
	; VAPOREON
	db 5, 1, ACID_ARMOR
	db 5, 2, REST ; AMNESIA?
	db 5, 3, WATER_PULSE
	db 5, 4, AURORA_BEAM
	; STARMIE
	db 6, 1, RECOVER
	db 6, 2, BUBBLEBEAM
	db 6, 3, MINIMIZE
	db 6, 4, PSYBEAM
	db $FE

	db MISTY, 3
	; WARTORTLE
	db 1, 1, HEADBUTT
	db 1, 2, MUD_SLAP
	db 1, 3, WATER_PULSE
	db 1, 4, CRUNCH
	; GOLDUCK
	db 2, 1, AMNESIA
	db 2, 2, DISABLE
	db 2, 3, ZEN_HEADBUTT
	db 2, 4, HEADBUTT
	; POLIWRATH
	db 3, 1, BODY_SLAM
	db 3, 2, HYPNOSIS
	db 3, 3, SUBMISSION
	db 3, 4, BUBBLEBEAM
	; GYARADOS
	db 4, 1, FOCUS_ENERGY
	db 4, 2, FLAIL
	db 4, 3, BUBBLEBEAM
	db 4, 4, WRAP
	; VAPOREON
	db 5, 1, AMNESIA
	db 5, 2, REST
	db 5, 3, WATER_PULSE
	db 5, 4, AURORA_BEAM
	; STARMIE
	db 6, 1, RECOVER
	db 6, 2, BUBBLEBEAM
	db 6, 3, MINIMIZE
	db 6, 4, LIGHT_SCREEN
	db $FE

	db MISTY, 9
	; GYARADOS
	db 1, 1, DRAGON_DANCE
	db 1, 2, HURRICANE
	db 1, 3, STONE_EDGE
	db 1, 4, HYDRO_PUMP
	; LAPRAS
	db 2, 1, HYDRO_PUMP
	db 2, 2, FREEZE_DRY
	db 2, 3, CONFUSE_RAY
	db 2, 4, THUNDERBOLT
	; POLITOED
	db 3, 1, AMNESIA
	db 3, 2, LOVELY_KISS
	db 3, 3, SCALD
	db 3, 4, PSYCHIC_M
	; VAPOREON
	db 4, 1, SAND_ATTACK
	db 4, 2, ACID_ARMOR
	db 4, 3, HYDRO_PUMP
	db 4, 4, REST
	; STARMIE
	db 5, 1, PSYCHO_CUT
	db 5, 2, MINIMIZE
	db 5, 3, HYDRO_PUMP
	db 5, 4, RECOVER
	; MBLASTOISE
	db 6, 1, FLASH_CANNON
	db 6, 2, BULLET_SEED
	db 6, 3, HYDRO_CANNON
	db 6, 4, OUTRAGE
	db $FE

; LT_SURGE's teams -------------------------------------------------

	db LT_SURGE, 1
	; VOLTORB
	db 2, 1, THUNDERSHOCK
	db 2, 2, TACKLE
	db 2, 3, SCREECH
	db 2, 4, NO_MOVE
	; MAGNEMITE
	db 2, 1, THUNDERSHOCK
	db 2, 2, TACKLE
	db 2, 3, THUNDER_WAVE
	db 2, 4, NO_MOVE
	; ELEKID
	db 3, 1, CHARM
	db 3, 2, SWIFT
	db 3, 3, MUD_SLAP
	db 3, 4, SHOCK_WAVE
	; POLIWAG
	db 4, 1, BUBBLE
	db 4, 2, POUND
	db 4, 3, DEFENSE_CURL
	db 4, 4, MUD_SHOT
	; PICHU
	db 5, 1, CHARM
	db 5, 2, THUNDER_WAVE
	db 5, 3, DOUBLE_TEAM
	db 5, 4, LEER
	; PIKACHU
	db 6, 1, CHARM
	db 6, 2, THUNDER_WAVE
	db 6, 3, DOUBLE_TEAM
	db 6, 4, QUICK_ATTACK
	db $FE

	db LT_SURGE, 2
	; VOLTORB
	db 2, 1, SONICBOOM
	db 2, 2, THUNDER_WAVE
	db 2, 3, SELFDESTRUCT
	db 2, 4, THUNDERSHOCK
	; MAGNEMITE
	db 2, 1, THUNDERSHOCK
	db 2, 2, THUNDER_WAVE
	db 2, 3, LIGHT_SCREEN
	db 2, 4, SONICBOOM
	; ELECTABUZZ
	db 3, 1, FIRE_PUNCH
	db 3, 2, ICE_PUNCH
	db 3, 3, ROCK_TOMB
	db 3, 4, THUNDERSHOCK
	; POLIWHIRL
	db 4, 1, MUD_SHOT
	db 4, 2, HYPNOSIS
	db 4, 3, BUBBLEBEAM
	db 4, 4, LOW_KICK
	; JOLTEON
	db 5, 1, THUNDERSHOCK
	db 5, 2, THUNDER_WAVE
	db 5, 3, DIG
	db 5, 4, BITE
	; PIKACHU
	db 6, 1, FLY
	db 6, 2, SURF
	db 6, 3, SING
	db 6, 4, VOLT_TACKLE
	db $FE

	db LT_SURGE, 3
	; ELECTRODE
	db 1, 1, SONICBOOM
	db 1, 2, THUNDER_WAVE
	db 1, 3, SELFDESTRUCT
	db 1, 4, THUNDERBOLT
	; MAGNETON
	db 2, 1, SONICBOOM
	db 2, 2, FLASH_CANNON
	db 2, 3, SUPERSONIC
	db 2, 4, THUNDERBOLT
	; ELECTABUZZ
	db 3, 1, THUNDER_WAVE
	db 3, 2, BARRIER
	db 3, 3, THUNDERPUNCH
	db 3, 4, SEISMIC_TOSS
	; TANGELA
	db 4, 1, LEECH_SEED
	db 4, 2, INFESTATION
	db 4, 3, GIGA_DRAIN
	db 4, 4, POISONPOWDER
	; JOLTEON
	db 5, 1, THUNDERBOLT
	db 5, 2, THUNDER_WAVE
	db 5, 3, DIG
	db 5, 4, BITE
	; RAICHU
	db 6, 1, ZAP_CANNON
	db 6, 2, MEGA_PUNCH
	db 6, 3, IRON_TAIL
	db 6, 4, SUBMISSION
	db $FE

	db LT_SURGE, 4
	; ELECTRODE
	db 1, 1, SONICBOOM
	db 1, 2, THUNDER_WAVE
	db 1, 3, EXPLOSION
	db 1, 4, THUNDERBOLT
	; MAGNETON
	db 2, 1, REFLECT
	db 2, 2, FLASH_CANNON
	db 2, 3, SUPERSONIC
	db 2, 4, THUNDERBOLT
	; ELECTABUZZ
	db 3, 1, IRON_TAIL
	db 3, 2, THUNDERBOLT
	db 3, 3, KARATE_CHOP
	db 3, 4, FLAMETHROWER
	; TANGELA
	db 4, 1, LEECH_SEED
	db 4, 2, INFESTATION
	db 4, 3, GIGA_DRAIN
	db 4, 4, TOXIC
	; JOLTEON
	db 5, 1, THUNDERBOLT
	db 5, 2, THUNDER_WAVE
	db 5, 3, DIG
	db 5, 4, BITE
	; RAICHU
	db 6, 1, THUNDERBOLT
	db 6, 2, SURF
	db 6, 3, EXTREMESPEED
	db 6, 4, SUBMISSION
	db $FE

	db LT_SURGE, 5
	; ELECTRODE
	db 1, 1, TAKE_DOWN
	db 1, 2, THUNDER_WAVE
	db 1, 3, EXPLOSION
	db 1, 4, THUNDERBOLT
	; MAGNEZONE
	db 2, 1, REFLECT
	db 2, 2, FLASH_CANNON
	db 2, 3, SUBSTITUTE
	db 2, 4, THUNDERBOLT
	; ELECTABUZZ
	db 3, 1, ROCK_TOMB
	db 3, 2, THUNDER
	db 3, 3, FLAMETHROWER
	db 3, 4, CLOSE_COMBAT
	; TANGROWTH
	db 4, 1, LEECH_SEED
	db 4, 2, INFESTATION
	db 4, 3, GIGA_DRAIN
	db 4, 4, SLEEP_POWDER
	; JOLTEON
	db 5, 1, THUNDERBOLT
	db 5, 2, DOUBLE_KICK
	db 5, 3, PIN_MISSILE
	db 5, 4, IRON_TAIL
	; RAICHU
	db 6, 1, VOLT_TACKLE
	db 6, 2, SURF
	db 6, 3, PETAL_DANCE
	db 6, 4, PSYCHIC_M
	db $FE

	db LT_SURGE, 6
	; ELECTRODE
	db 1, 1, HYPER_BEAM
	db 1, 2, THUNDER_WAVE
	db 1, 3, EXPLOSION
	db 1, 4, THUNDERBOLT
	; MAGNEZONE
	db 2, 1, BODY_PRESS
	db 2, 2, FLASH_CANNON
	db 2, 3, SUBSTITUTE
	db 2, 4, THUNDERBOLT
	; ELECTIVIRE
	db 3, 1, ROCK_SLIDE
	db 3, 2, THUNDERBOLT
	db 3, 3, ICE_PUNCH
	db 3, 4, PSYCHIC_M
	; TANGROWTH
	db 4, 1, LEECH_SEED
	db 4, 2, INFESTATION
	db 4, 3, ENERGY_BALL
	db 4, 4, SLEEP_POWDER
	; JOLTEON
	db 5, 1, THUNDERBOLT
	db 5, 2, DOUBLE_KICK
	db 5, 3, PIN_MISSILE
	db 5, 4, SHADOW_BALL
	; RAICHU
	db 6, 1, VOLT_TACKLE
	db 6, 2, SURF
	db 6, 3, PLAY_ROUGH
	db 6, 4, PSYCHIC_M
	db $FE

	db LT_SURGE, 7
	; ELECTRODE
	db 1, 1, HYPER_BEAM
	db 1, 2, THUNDER_WAVE
	db 1, 3, EXPLOSION
	db 1, 4, THUNDERBOLT
	; MAGNEZONE
	db 2, 1, THUNDER_WAVE
	db 2, 2, FLASH_CANNON
	db 2, 3, EXPLOSION
	db 2, 4, THUNDERBOLT
	; ELECTIVIRE
	db 3, 1, EARTHQUAKE
	db 3, 2, THUNDERBOLT
	db 3, 3, ICE_PUNCH
	db 3, 4, PSYCHIC_M
	; TANGROWTH
	db 4, 1, CURSE
	db 4, 2, INFESTATION
	db 4, 3, ENERGY_BALL
	db 4, 4, SLEEP_POWDER
	; JOLTEON
	db 5, 1, THUNDERBOLT
	db 5, 2, DOUBLE_KICK
	db 5, 3, HYPER_BEAM
	db 5, 4, SHADOW_BALL
	; RAICHU
	db 6, 1, VOLT_TACKLE
	db 6, 2, SURF
	db 6, 3, PLAY_ROUGH
	db 6, 4, PSYCHIC_M
	db $FE

	db LT_SURGE, 8
	; ELECTRODE
	db 1, 1, HYPER_BEAM
	db 1, 2, THUNDER_WAVE
	db 1, 3, EXPLOSION
	db 1, 4, THUNDER
	; MAGNEZONE
	db 2, 1, THUNDER_WAVE
	db 2, 2, FLASH_CANNON
	db 2, 3, EXPLOSION
	db 2, 4, THUNDER
	; ELECTIVIRE
	db 3, 1, EARTHQUAKE
	db 3, 2, THUNDER
	db 3, 3, ICE_PUNCH
	db 3, 4, CLOSE_COMBAT
	; TANGROWTH
	db 4, 1, EARTHQUAKE
	db 4, 2, HYPER_BEAM
	db 4, 3, LEAF_STORM
	db 4, 4, ROCK_SLIDE
	; JOLTEON
	db 5, 1, THUNDER
	db 5, 2, DOUBLE_KICK
	db 5, 3, HYPER_BEAM
	db 5, 4, SHADOW_BALL
	; RAICHU
	db 6, 1, VOLT_TACKLE
	db 6, 2, SURF
	db 6, 3, MEGA_PUNCH
	db 6, 4, PSYCHIC_M
	db $FE

	db LT_SURGE, 9
	; ELECTRODE
	db 1, 1, HYPER_BEAM
	db 1, 2, THUNDER_WAVE
	db 1, 3, EXPLOSION
	db 1, 4, THUNDER
	; MAGNEZONE
	db 2, 1, THUNDER_WAVE
	db 2, 2, FLASH_CANNON
	db 2, 3, EXPLOSION
	db 2, 4, THUNDER
	; ELECTIVIRE
	db 3, 1, EARTHQUAKE
	db 3, 2, THUNDER
	db 3, 3, ICE_PUNCH
	db 3, 4, CLOSE_COMBAT
	; TANGROWTH
	db 4, 1, EARTHQUAKE
	db 4, 2, HYPER_BEAM
	db 4, 3, LEAF_STORM
	db 4, 4, ROCK_SLIDE
	; JOLTEON
	db 5, 1, THUNDER
	db 5, 2, DOUBLE_KICK
	db 5, 3, HYPER_BEAM
	db 5, 4, SHADOW_BALL
	; RAICHU
	db 6, 1, VOLT_TACKLE
	db 6, 2, SURF
	db 6, 3, MEGA_PUNCH
	db 6, 4, PSYCHIC_M
	db $FE

; ERIKA's teams -------------------------------------------------

	db ERIKA, 2
	; PARAS
	db 1, 1, MEGA_DRAIN
	db 1, 2, LEECH_SEED
	db 1, 3, INFESTATION
	db 1, 4, SPORE
	; GLOOM
	db 2, 1, LEECH_SEED
	db 2, 2, POISONPOWDER
	db 2, 3, STUN_SPORE
	db 2, 4, GIGA_DRAIN
	; EXEGGCUTE
	db 3, 1, PSYWAVE
	db 3, 2, LEECH_SEED
	db 3, 3, LIGHT_SCREEN
	db 3, 4, ENERGY_BALL
	; WEEPINBELL
	db 4, 1, WRAP
	db 4, 2, STUN_SPORE
	db 4, 3, GIGA_DRAIN
	db 4, 4, ACID
	; LEAFEON
	db 5, 1, SAND_ATTACK
	db 5, 2, QUICK_ATTACK
	db 5, 3, FOCUS_ENERGY
	db 5, 4, DOUBLE_KICK
	; TANGELA
	db 6, 1, LEECH_SEED
	db 6, 2, POISONPOWDER
	db 6, 3, WRAP
	db 6, 4, GIGA_DRAIN
	db $FE

	db ERIKA, 3
	; PARASECT
	db 1, 1, GIGA_DRAIN
	db 1, 2, LEECH_SEED
	db 1, 3, INFESTATION
	db 1, 4, SPORE
	; VILEPLUME
	db 2, 1, STUN_SPORE
	db 2, 2, MOONBLAST
	db 2, 3, SLUDGE_WAVE
	db 2, 4, ENERGY_BALL
	; EXEGGUTOR
	db 3, 1, CONFUSION
	db 3, 2, LEECH_SEED
	db 3, 3, BULLET_SEED
	db 3, 4, STUN_SPORE
	; VICTREEBEL
	db 4, 1, WRAP
	db 4, 2, STUN_SPORE
	db 4, 3, MAGICAL_LEAF
	db 4, 4, LEECH_LIFE
	; LEAFEON
	db 5, 1, DOUBLE_EDGE
	db 5, 2, X_SCISSOR
	db 5, 3, SHADOW_BALL
	db 5, 4, RAZOR_LEAF
	; TANGELA
	db 6, 1, LEECH_SEED
	db 6, 2, LIGHT_SCREEN
	db 6, 3, WRAP
	db 6, 4, GIGA_DRAIN
	db $FE

	db ERIKA, 4
	; PARASECT
	db 1, 1, SPORE
	db 1, 2, INFESTATION
	db 1, 3, LEECH_LIFE
	db 1, 4, SLASH
	; VILEPLUME
	db 2, 1, STUN_SPORE
	db 2, 2, MOONBLAST
	db 2, 3, GUNK_SHOT
	db 2, 4, PETAL_DANCE
	; BELLOSSOM
	db 3, 1, STUN_SPORE
	db 3, 2, MOONBLAST
	db 3, 3, DRAIN_PUNCH
	db 3, 4, PETAL_DANCE
	; VICTREEBEL
	db 4, 1, WRAP
	db 4, 2, HYPER_BEAM
	db 4, 3, SLEEP_POWDER
	db 4, 4, RAZOR_LEAF
	; LEAFEON
	db 5, 1, SWORDS_DANCE
	db 5, 2, HYPER_BEAM
	db 5, 3, SAND_ATTACK
	db 5, 4, LEAF_STORM
	; TANGROWTH
	db 6, 1, LIGHT_SCREEN
	db 6, 2, REFLECT
	db 6, 3, LEECH_SEED
	db 6, 4, ROCK_SLIDE
	db $FE

	db ERIKA, 5
	; VILEPLUME
	db 1, 1, STUN_SPORE
	db 1, 2, MOONBLAST
	db 1, 3, GUNK_SHOT
	db 1, 4, PETAL_DANCE
	; BELLOSSOM
	db 2, 1, STUN_SPORE
	db 2, 2, MOONBLAST
	db 2, 3, DRAIN_PUNCH
	db 2, 4, PETAL_DANCE
	; VICTREEBEL
	db 3, 1, WRAP
	db 3, 2, HYPER_BEAM
	db 3, 3, SLEEP_POWDER
	db 3, 4, RAZOR_LEAF
	; LEAFEON
	db 4, 1, SWORDS_DANCE
	db 4, 2, HYPER_BEAM
	db 4, 3, SHADOW_BALL
	db 4, 4, RAZOR_LEAF
	; VENUSAUR
	db 5, 1, GUNK_SHOT
	db 5, 2, LEECH_SEED
	db 5, 3, RAZOR_LEAF
	db 5, 4, EARTHQUAKE
	; TANGROWTH
	db 6, 1, AMNESIA
	db 6, 2, CURSE
	db 6, 3, GIGA_DRAIN
	db 6, 4, ROCK_SLIDE
	db $FE

	db ERIKA, 6
	; VILEPLUME
	db 1, 1, STUN_SPORE
	db 1, 2, MOONBLAST
	db 1, 3, GUNK_SHOT
	db 1, 4, PETAL_DANCE
	; EXEGGUTOR
	db 2, 1, PSYCHIC_M
	db 2, 2, SLEEP_POWDER
	db 2, 3, OUTRAGE
	db 2, 4, EXPLOSION
	; VICTREEBEL
	db 3, 1, WRAP
	db 3, 2, HYPER_BEAM
	db 3, 3, LOVELY_KISS
	db 3, 4, RAZOR_LEAF
	; LEAFEON
	db 4, 1, SWORDS_DANCE
	db 4, 2, HYPER_BEAM
	db 4, 3, SHADOW_BALL
	db 4, 4, X_SCISSOR
	; VENUSAUR
	db 5, 1, GUNK_SHOT
	db 5, 2, LEECH_SEED
	db 5, 3, RAZOR_LEAF
	db 5, 4, EARTHQUAKE
	; TANGROWTH
	db 6, 1, AMNESIA
	db 6, 2, CURSE
	db 6, 3, GIGA_DRAIN
	db 6, 4, REST
	db $FE

	db ERIKA, 7
	; VILEPLUME
	db 1, 1, STUN_SPORE
	db 1, 2, MOONBLAST
	db 1, 3, GUNK_SHOT
	db 1, 4, PETAL_DANCE
	; EXEGGUTOR
	db 2, 1, ZEN_HEADBUTT
	db 2, 2, SLEEP_POWDER
	db 2, 3, OUTRAGE
	db 2, 4, EXPLOSION
	; VICTREEBEL
	db 3, 1, WRAP
	db 3, 2, GUNK_SHOT
	db 3, 3, STUN_SPORE
	db 3, 4, RAZOR_LEAF
	; LEAFEON
	db 4, 1, LEAF_STORM
	db 4, 2, HYPER_BEAM
	db 4, 3, SHADOW_BALL
	db 4, 4, X_SCISSOR
	; VENUSAUR
	db 5, 1, GUNK_SHOT
	db 5, 2, LEECH_SEED
	db 5, 3, LEAF_STORM
	db 5, 4, EARTHQUAKE
	; TANGROWTH
	db 6, 1, AMNESIA
	db 6, 2, CURSE
	db 6, 3, GIGA_DRAIN
	db 6, 4, REST
	db $FE

	db ERIKA, 8
	; VILEPLUME
	db 1, 1, STUN_SPORE
	db 1, 2, MOONBLAST
	db 1, 3, GUNK_SHOT
	db 1, 4, ENERGY_BALL
	; EXEGGUTOR
	db 2, 1, PSYCHO_CUT
	db 2, 2, SLEEP_POWDER
	db 2, 3, OUTRAGE
	db 2, 4, EXPLOSION
	; VICTREEBEL
	db 3, 1, WRAP
	db 3, 2, GUNK_SHOT
	db 3, 3, STUN_SPORE
	db 3, 4, RAZOR_LEAF
	; LEAFEON
	db 4, 1, LEAF_STORM
	db 4, 2, HYPER_BEAM
	db 4, 3, SHADOW_BALL
	db 4, 4, X_SCISSOR
	; VENUSAUR
	db 5, 1, GUNK_SHOT
	db 5, 2, LEECH_SEED
	db 5, 3, LEAF_STORM
	db 5, 4, EARTHQUAKE
	; TANGROWTH
	db 6, 1, AMNESIA
	db 6, 2, CURSE
	db 6, 3, GIGA_DRAIN
	db 6, 4, REST
	db $FE

	db ERIKA, 9
	; VICTREEBEL
	db 1, 1, WRAP
	db 1, 2, GUNK_SHOT
	db 1, 3, STUN_SPORE
	db 1, 4, RAZOR_LEAF
	; VILEPLUME
	db 2, 1, STUN_SPORE
	db 2, 2, MOONBLAST
	db 2, 3, GUNK_SHOT
	db 2, 4, ENERGY_BALL
	; EXEGGUTOR
	db 3, 1, PSYCHO_CUT
	db 3, 2, SLEEP_POWDER
	db 3, 3, OUTRAGE
	db 3, 4, EXPLOSION
	; LEAFEON
	db 4, 1, LEAF_STORM
	db 4, 2, HYPER_BEAM
	db 4, 3, SHADOW_BALL
	db 4, 4, X_SCISSOR
	; TANGROWTH
	db 5, 1, AMNESIA
	db 5, 2, CURSE
	db 5, 3, GIGA_DRAIN
	db 5, 4, REST
	; MVENUSAUR
	db 6, 1, EARTHQUAKE
	db 6, 2, AMNESIA
	db 6, 3, GUNK_SHOT
	db 6, 4, FRENZY_PLANT
	db $FE

; KOGA's teams -------------------------------------------------

	db KOGA, 2
	; GRIMER
	db 1, 1, TOXIC
	db 1, 2, SLUDGE
	db 1, 3, MINIMIZE
	db 1, 4, LICK
	; BEEDRILL
	db 2, 1, FURY_ATTACK
	db 2, 2, FOCUS_ENERGY
	db 2, 3, POISON_STING
	db 2, 4, TWINEEDLE
	; VENONAT
	db 3, 1, SUPERSONIC
	db 3, 2, STUN_SPORE
	db 3, 3, PSYBEAM
	db 3, 4, FLASH
	; TENTACOOL
	db 4, 1, REFLECT
	db 4, 2, SUPERSONIC
	db 4, 3, WRAP
	db 4, 4, BUBBLEBEAM
	; GOLBAT
	db 5, 1, LEECH_LIFE
	db 5, 2, CONFUSE_RAY
	db 5, 3, DOUBLE_TEAM
	db 5, 4, MEGA_DRAIN
	; WEEZING
	db 6, 1, SMOKESCREEN
	db 6, 2, INFESTATION
	db 6, 3, TOXIC
	db 6, 4, SLUDGE_WAVE
	db $FE

	db KOGA, 3
	; MUK
	db 1, 1, TOXIC
	db 1, 2, SLUDGE
	db 1, 3, MINIMIZE
	db 1, 4, SHADOW_SNEAK
	; BEEDRILL
	db 2, 1, AGILITY
	db 2, 2, AERIAL_ACE
	db 2, 3, SLUDGE_WAVE
	db 2, 4, X_SCISSOR
	; VENOMOTH
	db 3, 1, STICKY_WEB
	db 3, 2, PSYBEAM
	db 3, 3, GIGA_DRAIN
	db 3, 4, SLEEP_POWDER
	; TENTACOOL
	db 4, 1, REFLECT
	db 4, 2, TOXIC
	db 4, 3, GIGA_DRAIN
	db 4, 4, WATER_PULSE
	; CROBAT
	db 5, 1, CONFUSE_RAY
	db 5, 2, AIR_CUTTER
	db 5, 3, SUPER_FANG
	db 5, 4, SLUDGE
	; WEEZING
	db 6, 1, SHADOW_BALL
	db 6, 2, SUBSTITUTE
	db 6, 3, TOXIC
	db 6, 4, SLUDGE
	db $FE

	db KOGA, 4
	; MUK
	db 1, 1, TOXIC
	db 1, 2, SLUDGE_WAVE
	db 1, 3, MINIMIZE
	db 1, 4, FIRE_BLAST
	; BEEDRILL
	db 2, 1, SWORDS_DANCE
	db 2, 2, X_SCISSOR
	db 2, 3, AERIAL_ACE
	db 2, 4, SLUDGE_WAVE
	; VENOMOTH
	db 3, 1, STICKY_WEB
	db 3, 2, ZEN_HEADBUTT
	db 3, 3, GIGA_DRAIN
	db 3, 4, SLEEP_POWDER
	; TENTACRUEL
	db 4, 1, REFLECT
	db 4, 2, TOXIC
	db 4, 3, ICE_BEAM
	db 4, 4, SURF
	; CROBAT
	db 5, 1, CONFUSE_RAY
	db 5, 2, ROOST
	db 5, 3, SUPER_FANG
	db 5, 4, LEECH_LIFE
	; WEEZING
	db 6, 1, CURSE
	db 6, 2, SUBSTITUTE
	db 6, 3, TOXIC
	db 6, 4, GYRO_BALL
	db $FE

	db KOGA, 5
	; MUK
	db 1, 1, TOXIC
	db 1, 2, MINIMIZE
	db 1, 3, DIG
	db 1, 4, GUNK_SHOT
	; ARBOK
	db 2, 1, GLARE
	db 2, 2, SLUDGE_WAVE
	db 2, 3, EARTHQUAKE
	db 2, 4, ROCK_SLIDE
	; VENOMOTH
	db 3, 1, GIGA_DRAIN
	db 3, 2, SLEEP_POWDER
	db 3, 3, STICKY_WEB
	db 3, 4, PSYCHIC_M
	; TENTACRUEL
	db 4, 1, TOXIC
	db 4, 2, ICE_BEAM
	db 4, 3, SURF
	db 4, 4, SLUDGE_WAVE
	; CROBAT
	db 5, 1, CONFUSE_RAY
	db 5, 2, SUPER_FANG
	db 5, 3, DOUBLE_TEAM
	db 5, 4, LEECH_LIFE ; ROOST?
	; WEEZING
	db 6, 1, SLUDGE_WAVE
	db 6, 2, SMOKESCREEN
	db 6, 3, FLAMETHROWER
	db 6, 4, EXPLOSION
	db $FE

	db KOGA, 6
	; MUK
	db 1, 1, FIRE_BLAST
	db 1, 2, MINIMIZE
	db 1, 3, REST
	db 1, 4, GUNK_SHOT
	; NIDOQUEEN
	db 2, 1, SHADOW_BALL
	db 2, 2, BLIZZARD
	db 2, 3, THUNDERBOLT
	db 2, 4, STONE_EDGE
	; VENOMOTH
	db 3, 1, STICKY_WEB
	db 3, 2, SLEEP_POWDER
	db 3, 3, LEECH_LIFE
	db 3, 4, PSYCHO_CUT ; illegal lol
	; TENTACRUEL
	db 4, 1, ACID_ARMOR
	db 4, 2, BLIZZARD
	db 4, 3, SCALD
	db 4, 4, REST
	; CROBAT
	db 5, 1, CONFUSE_RAY
	db 5, 2, SUPER_FANG
	db 5, 3, DOUBLE_TEAM
	db 5, 4, BRAVE_BIRD
	; WEEZING
	db 6, 1, GUNK_SHOT
	db 6, 2, ZAP_CANNON
	db 6, 3, FIRE_BLAST
	db 6, 4, EXPLOSION
	db $FE

	db KOGA, 7
	; NIDOQUEEN
	db 1, 1, SUPER_FANG
	db 1, 2, TOXIC
	db 1, 3, EARTHQUAKE
	db 1, 4, REST
	; TENTACRUEL
	db 2, 1, ACID_ARMOR
	db 2, 2, BLIZZARD
	db 2, 3, SCALD
	db 2, 4, REST
	; NIDOKING
	db 3, 1, EARTHQUAKE
	db 3, 2, GUNK_SHOT
	db 3, 3, MEGAHORN
	db 3, 4, STONE_EDGE
	; CROBAT
	db 4, 1, CONFUSE_RAY
	db 4, 2, SUPER_FANG
	db 4, 3, DOUBLE_TEAM
	db 4, 4, BRAVE_BIRD
	; GENGAR
	db 5, 1, CURSE
	db 5, 2, HYPNOSIS
	db 5, 3, DARK_PULSE
	db 5, 4, ENERGY_BALL
	; WEEZING
	db 6, 1, GUNK_SHOT
	db 6, 2, SHADOW_BALL
	db 6, 3, FIRE_BLAST
	db 6, 4, EXPLOSION
	db $FE

	db KOGA, 8
	; NIDOQUEEN
	db 1, 1, SUPER_FANG
	db 1, 2, TOXIC
	db 1, 3, EARTHQUAKE
	db 1, 4, LOVELY_KISS
	; TENTACRUEL
	db 2, 1, HYPER_BEAM
	db 2, 2, BLIZZARD
	db 2, 3, HYDRO_PUMP
	db 2, 4, MIRROR_COAT
	; NIDOKING
	db 3, 1, EARTHQUAKE
	db 3, 2, GUNK_SHOT
	db 3, 3, MEGAHORN
	db 3, 4, STONE_EDGE
	; CROBAT
	db 4, 1, CONFUSE_RAY
	db 4, 2, SUPER_FANG
	db 4, 3, DOUBLE_TEAM
	db 4, 4, BRAVE_BIRD
	; GENGAR
	db 5, 1, CURSE
	db 5, 2, HYPNOSIS
	db 5, 3, DARK_PULSE
	db 5, 4, ENERGY_BALL
	; WEEZING
	db 6, 1, GUNK_SHOT
	db 6, 2, SHADOW_BALL
	db 6, 3, FIRE_BLAST
	db 6, 4, EXPLOSION
	db $FE

	db KOGA, 9
	; MUK
	db 1, 1, SHADOW_SNEAK
	db 1, 2, MINIMIZE
	db 1, 3, REST
	db 1, 4, GUNK_SHOT
	; GENGAR
	db 2, 1, PSYCHIC_M
	db 2, 2, HYPNOSIS
	db 2, 3, DARK_PULSE
	db 2, 4, ENERGY_BALL
	; TENTACRUEL
	db 3, 1, HYPER_BEAM
	db 3, 2, BLIZZARD
	db 3, 3, HYDRO_PUMP
	db 3, 4, TOXIC
	; CROBAT
	db 4, 1, CONFUSE_RAY
	db 4, 2, SUPER_FANG
	db 4, 3, DOUBLE_TEAM
	db 4, 4, BRAVE_BIRD
	; WEEZING
	db 5, 1, GUNK_SHOT
	db 5, 2, SHADOW_BALL
	db 5, 3, FIRE_BLAST
	db 5, 4, EXPLOSION
	; MBEEDRILL
	db 6, 1, HYPER_BEAM
	db 6, 2, MEGAHORN
	db 6, 3, GUNK_SHOT
	db 6, 4, STEEL_WING
	db $FE

; SABRINA's teams -------------------------------------------------

	db SABRINA, 2
	; MIME_JR
	db 1, 1, CONFUSE_RAY
	db 1, 2, INFESTATION
	db 1, 3, PSYBEAM
	db 1, 4, MIMIC
	; DROWZEE
	db 2, 1, CONFUSION
	db 2, 2, HEADBUTT
	db 2, 3, POISON_GAS
	db 2, 4, HYPNOSIS
	; SMOOCHUM
	db 3, 1, WATER_PULSE
	db 3, 2, PSYCHIC_M
	db 3, 3, ICICLE_SPEAR
	db 3, 4, LOVELY_KISS
	; SLOWPOKE
	db 4, 1, WATER_PULSE
	db 4, 2, BODY_SLAM
	db 4, 3, ZEN_HEADBUTT
	db 4, 4, SHADOW_BALL
	; HAUNTER
	db 5, 1, HYPNOSIS
	db 5, 2, NIGHT_SHADE
	db 5, 3, CURSE
	db 5, 4, CONFUSE_RAY
	; KADABRA
	db 6, 1, PSYBEAM
	db 6, 2, NIGHT_SHADE
	db 6, 3, RECOVER
	db 6, 4, REFLECT
	db $FE

	db SABRINA, 3
	; MR_MIME
	db 1, 1, SUBSTITUTE
	db 1, 2, PLAY_ROUGH
	db 1, 3, MIMIC
	db 1, 4, PSYCHIC_M
	; DROWZEE
	db 2, 1, TAKE_DOWN
	db 2, 2, DREAM_EATER
	db 2, 3, BARRIER
	db 2, 4, HYPNOSIS
	; JYNX
	db 3, 1, WATER_PULSE
	db 3, 2, PSYCHIC_M
	db 3, 3, ICICLE_SPEAR
	db 3, 4, LOVELY_KISS
	; EXEGGCUTE
	db 4, 1, LEECH_SEED
	db 4, 2, PSYCHIC_M
	db 4, 3, STUN_SPORE
	db 4, 4, ENERGY_BALL
	; HAUNTER
	db 5, 1, HYPNOSIS
	db 5, 2, DARK_PULSE
	db 5, 3, CURSE
	db 5, 4, CONFUSE_RAY
	; KADABRA
	db 6, 1, PSYCHIC_M
	db 6, 2, THUNDER_WAVE
	db 6, 3, RECOVER
	db 6, 4, REFLECT
	db $FE

	db SABRINA, 4
	; MR_MIME
	db 1, 1, SUBSTITUTE
	db 1, 2, PLAY_ROUGH
	db 1, 3, MIMIC
	db 1, 4, BARRIER
	; HYPNO
	db 2, 1, REFLECT
	db 2, 2, DREAM_EATER
	db 2, 3, THUNDERPUNCH
	db 2, 4, HYPNOSIS
	; ESPEON
	db 3, 1, SAND_ATTACK
	db 3, 2, MUD_SLAP
	db 3, 3, FOCUS_ENERGY
	db 3, 4, PSYCHIC_M
	; SLOWKING
	db 4, 1, ZEN_HEADBUTT
	db 4, 2, AMNESIA
	db 4, 3, SCALD
	db 4, 4, ICE_BEAM
	; HAUNTER
	db 5, 1, HYPNOSIS
	db 5, 2, DARK_PULSE
	db 5, 3, CURSE
	db 5, 4, CONFUSE_RAY
	; ALAKAZAM
	db 6, 1, PSYCHIC_M
	db 6, 2, THUNDER_WAVE
	db 6, 3, RECOVER
	db 6, 4, REFLECT
	db $FE

	db SABRINA, 5
	; SLOWKING
	db 1, 1, PSYCHO_CUT
	db 1, 2, AMNESIA
	db 1, 3, SCALD
	db 1, 4, BLIZZARD
	; EXEGGUTOR
	db 2, 1, LEECH_SEED
	db 2, 2, SLEEP_POWDER
	db 2, 3, PSYCHO_CUT
	db 2, 4, EXPLOSION
	; ESPEON
	db 3, 1, SAND_ATTACK
	db 3, 2, ZAP_CANNON
	db 3, 3, FOCUS_ENERGY
	db 3, 4, PSYCHIC_M
	; STARMIE
	db 4, 1, THUNDERBOLT
	db 4, 2, RECOVER
	db 4, 3, SCALD
	db 4, 4, PSYCHIC_M
	; HAUNTER
	db 5, 1, HYPNOSIS
	db 5, 2, DARK_PULSE
	db 5, 3, CURSE
	db 5, 4, CONFUSE_RAY
	; ALAKAZAM
	db 6, 1, PSYCHIC_M
	db 6, 2, THUNDER_WAVE
	db 6, 3, RECOVER
	db 6, 4, REFLECT
	db $FE

	db SABRINA, 6
	; MR_MIME
	db 1, 1, SUBSTITUTE
	db 1, 2, PLAY_ROUGH
	db 1, 3, MIMIC
	db 1, 4, METRONOME ; PSYCHIC_M?
	; JYNX
	db 2, 1, BLIZZARD
	db 2, 2, PSYCHIC_M
	db 2, 3, LOVELY_KISS
	db 2, 4, DRAININGKISS
	; ESPEON
	db 3, 1, ZAP_CANNON
	db 3, 2, PSYCHIC_M
	db 3, 3, BITE
	db 3, 4, FOCUS_ENERGY ; HYPER_BEAM?
	; EXEGGUTOR
	db 4, 1, GIGA_DRAIN
	db 4, 2, PSYCHIC_M
	db 4, 3, SLEEP_POWDER
	db 4, 4, EXPLOSION
	; HAUNTER
	db 5, 1, ICE_PUNCH
	db 5, 2, SHADOW_BALL
	db 5, 3, DARK_PULSE
	db 5, 4, CONFUSE_RAY
	; ALAKAZAM
	db 6, 1, THUNDERPUNCH
	db 6, 2, ICE_PUNCH
	db 6, 3, FIRE_PUNCH
	db 6, 4, PSYCHO_CUT
	db $FE

	db SABRINA, 7
	; SLOWBRO
	db 1, 1, AMNESIA
	db 1, 2, PSYCHIC_M
	db 1, 3, SCALD
	db 1, 4, REST
	; JYNX
	db 2, 1, BLIZZARD
	db 2, 2, PSYCHO_CUT
	db 2, 3, LOVELY_KISS
	db 2, 4, DRAININGKISS
	; ESPEON
	db 3, 1, ZAP_CANNON
	db 3, 2, PSYCHO_CUT
	db 3, 3, BITE
	db 3, 4, HYPER_BEAM
	; EXEGGUTOR
	db 4, 1, GIGA_DRAIN
	db 4, 2, PSYCHO_CUT
	db 4, 3, SLEEP_POWDER
	db 4, 4, EXPLOSION
	; GENGAR
	db 5, 1, THUNDERBOLT
	db 5, 2, CURSE
	db 5, 3, DARK_PULSE
	db 5, 4, CONFUSE_RAY
	; ALAKAZAM
	db 6, 1, REFLECT
	db 6, 2, THUNDER_WAVE
	db 6, 3, FIRE_PUNCH
	db 6, 4, PSYCHO_CUT
	db $FE

	db SABRINA, 8
	; JYNX
	db 1, 1, BLIZZARD
	db 1, 2, PSYCHO_CUT
	db 1, 3, LOVELY_KISS
	db 1, 4, DRAININGKISS
	; ESPEON
	db 2, 1, ZAP_CANNON
	db 2, 2, PSYCHO_CUT
	db 2, 3, TOXIC
	db 2, 4, HYPER_BEAM
	; EXEGGUTOR
	db 3, 1, LEAF_STORM
	db 3, 2, PSYCHO_CUT
	db 3, 3, SLEEP_POWDER
	db 3, 4, EXPLOSION
	; STARMIE
	db 4, 1, PSYCHO_CUT
	db 4, 2, HYDRO_PUMP
	db 4, 3, BLIZZARD
	db 4, 4, RECOVER
	; GENGAR
	db 5, 1, THUNDER
	db 5, 2, CURSE
	db 5, 3, EXPLOSION
	db 5, 4, CONFUSE_RAY
	; ALAKAZAM
	db 6, 1, REFLECT
	db 6, 2, THUNDER_WAVE
	db 6, 3, FIRE_PUNCH
	db 6, 4, PSYCHO_CUT
	db $FE

	db SABRINA, 9
	; JYNX
	db 1, 1, BLIZZARD
	db 1, 2, PSYCHO_CUT
	db 1, 3, LOVELY_KISS
	db 1, 4, DRAININGKISS
	; ESPEON
	db 2, 1, ZAP_CANNON
	db 2, 2, PSYCHO_CUT
	db 2, 3, SAND_ATTACK
	db 2, 4, REFLECT
	; SLOWKING
	db 3, 1, THUNDER_WAVE
	db 3, 2, AMNESIA
	db 3, 3, REST
	db 3, 4, HYDRO_PUMP
	; EXEGGUTOR
	db 4, 1, LEAF_STORM
	db 4, 2, PSYCHO_CUT
	db 4, 3, SLEEP_POWDER
	db 4, 4, EXPLOSION
	; STARMIE
	db 5, 1, PSYCHO_CUT
	db 5, 2, HYDRO_PUMP
	db 5, 3, BLIZZARD
	db 5, 4, RECOVER
	; MALAKAZAM
	db 6, 1, ICE_PUNCH
	db 6, 2, THUNDERPUNCH
	db 6, 3, FIRE_PUNCH
	db 6, 4, PSYCHO_CUT
	db $FE

; BLAINE's teams -------------------------------------------------

	db BLAINE, 3
	; VULPIX
	db 1, 1, CONFUSE_RAY
	db 1, 2, HYPNOSIS
	db 1, 3, FLAMETHROWER
	db 1, 4, ENERGY_BALL
	; PONYTA
	db 2, 1, WILL_O_WISP
	db 2, 2, STOMP
	db 2, 3, IRON_TAIL
	db 2, 4, FIRE_SPIN
	; MAGMAR
	db 3, 1, ROCK_TOMB
	db 3, 2, MACH_PUNCH
	db 3, 3, FLARE_BLITZ
	db 3, 4, BULLET_SEED
	; FLAREON
	db 4, 1, DOUBLE_KICK
	db 4, 2, SHADOW_BALL
	db 4, 3, FIRE_SPIN
	db 4, 4, IRON_TAIL
	; CHARMELEON
	db 5, 1, DRAGONBREATH
	db 5, 2, SLASH
	db 5, 3, SMOKESCREEN
	db 5, 4, FLAMETHROWER
	; ARCANINE
	db 6, 1, BODY_SLAM
	db 6, 2, DOUBLE_KICK
	db 6, 3, CRUNCH
	db 6, 4, AERIAL_ACE
	db $FE

	db BLAINE, 4
	; NINETALES
	db 1, 1, CONFUSE_RAY
	db 1, 2, HYPNOSIS
	db 1, 3, FLAMETHROWER
	db 1, 4, ENERGY_BALL
	; RAPIDASH
	db 2, 1, SWORDS_DANCE
	db 2, 2, MEGAHORN
	db 2, 3, IRON_TAIL
	db 2, 4, HYPER_BEAM
	; MAGMAR
	db 3, 1, THUNDERPUNCH
	db 3, 2, KARATE_CHOP
	db 3, 3, FLAMETHROWER
	db 3, 4, BULLET_SEED
	; FLAREON
	db 4, 1, DOUBLE_KICK
	db 4, 2, SHADOW_BALL
	db 4, 3, FLAMETHROWER
	db 4, 4, IRON_TAIL
	; RHYDON
	db 5, 1, MUD_SHOT
	db 5, 2, ROCK_SLIDE
	db 5, 3, BODY_PRESS
	db 5, 4, BODY_SLAM
	; ARCANINE
	db 6, 1, BODY_SLAM
	db 6, 2, DOUBLE_KICK
	db 6, 3, CRUNCH
	db 6, 4, FLAMETHROWER
	db $FE

	db BLAINE, 5
	; NINETALES
	db 1, 1, CONFUSE_RAY
	db 1, 2, HYPNOSIS
	db 1, 3, FLAMETHROWER
	db 1, 4, ENERGY_BALL
	; RAPIDASH
	db 2, 1, WILL_O_WISP
	db 2, 2, MEGAHORN
	db 2, 3, IRON_TAIL
	db 2, 4, FIRE_BLAST
	; MAGMORTAR
	db 3, 1, THUNDERPUNCH
	db 3, 2, KARATE_CHOP
	db 3, 3, FLAMETHROWER
	db 3, 4, BULLET_SEED
	; FLAREON
	db 4, 1, DOUBLE_KICK
	db 4, 2, SHADOW_BALL
	db 4, 3, FLAMETHROWER
	db 4, 4, IRON_TAIL
	; CHARIZARD
	db 5, 1, AIR_SLASH
	db 5, 2, THUNDERPUNCH
	db 5, 3, SHADOW_CLAW
	db 5, 4, DRAGON_DANCE
	; ARCANINE
	db 6, 1, EXTREMESPEED
	db 6, 2, DRAGONBREATH
	db 6, 3, FLARE_BLITZ
	db 6, 4, SUPERPOWER
	db $FE

	db BLAINE, 6
	; NINETALES
	db 1, 1, CONFUSE_RAY
	db 1, 2, HYPNOSIS
	db 1, 3, FIRE_BLAST
	db 1, 4, ENERGY_BALL
	; RAPIDASH
	db 2, 1, WILL_O_WISP
	db 2, 2, MEGAHORN
	db 2, 3, IRON_TAIL
	db 2, 4, FIRE_BLAST
	; MAGMORTAR
	db 3, 1, THUNDERBOLT
	db 3, 2, PSYCHIC_M
	db 3, 3, FLAMETHROWER
	db 3, 4, BULLET_SEED
	; FLAREON
	db 4, 1, SUPERPOWER
	db 4, 2, SHADOW_BALL
	db 4, 3, FIRE_BLAST
	db 4, 4, IRON_TAIL
	; RHYDON
	db 5, 1, STONE_EDGE
	db 5, 2, HYPER_BEAM
	db 5, 3, EARTHQUAKE
	db 5, 4, SUPERPOWER
	; ARCANINE
	db 6, 1, EXTREMESPEED
	db 6, 2, DRAGONBREATH
	db 6, 3, FLARE_BLITZ
	db 6, 4, SUPERPOWER
	db $FE

	db BLAINE, 7
	; NINETALES
	db 1, 1, CONFUSE_RAY
	db 1, 2, ENERGY_BALL
	db 1, 3, HYPNOSIS
	db 1, 4, FIRE_BLAST
	; RAPIDASH
	db 2, 1, WILL_O_WISP
	db 2, 2, FIRE_SPIN
	db 2, 3, MEGAHORN
	db 2, 4, FLARE_BLITZ
	; MAGMORTAR
	db 3, 1, THUNDERBOLT
	db 3, 2, MACH_PUNCH
	db 3, 3, FLAMETHROWER
	db 3, 4, BULLET_SEED
	; FLAREON
	db 4, 1, IRON_TAIL
	db 4, 2, FLAMETHROWER
	db 4, 3, SUPERPOWER
	db 4, 4, SHADOW_BALL
	; RHYPERIOR
	db 5, 1, STONE_EDGE
	db 5, 2, MEGAHORN
	db 5, 3, EARTHQUAKE
	db 5, 4, SUPERPOWER
	; ARCANINE
	db 6, 1, EXTREMESPEED
	db 6, 2, FLARE_BLITZ
	db 6, 3, CLOSE_COMBAT
	db 6, 4, HYPER_BEAM
	db $FE

	db BLAINE, 8
	; NINETALES
	db 1, 1, CONFUSE_RAY
	db 1, 2, ENERGY_BALL
	db 1, 3, HYPNOSIS
	db 1, 4, FIRE_BLAST
	; RAPIDASH
	db 2, 1, WILL_O_WISP
	db 2, 2, FIRE_SPIN
	db 2, 3, MEGAHORN
	db 2, 4, FLARE_BLITZ
	; MAGMORTAR
	db 3, 1, THUNDERBOLT
	db 3, 2, PSYCHIC_M
	db 3, 3, FIRE_BLAST
	db 3, 4, EARTHQUAKE
	; FLAREON
	db 4, 1, IRON_TAIL
	db 4, 2, FIRE_BLAST
	db 4, 3, SUPERPOWER
	db 4, 4, SHADOW_BALL
	; RHYPERIOR
	db 5, 1, HEAD_SMASH
	db 5, 2, MEGAHORN
	db 5, 3, EARTHQUAKE
	db 5, 4, SUPERPOWER
	; ARCANINE
	db 6, 1, EXTREMESPEED
	db 6, 2, FLARE_BLITZ
	db 6, 3, CLOSE_COMBAT
	db 6, 4, HYPER_BEAM
	db $FE

	db BLAINE, 9
	; NINETALES
	db 1, 1, CONFUSE_RAY
	db 1, 2, ENERGY_BALL
	db 1, 3, HYPNOSIS
	db 1, 4, FIRE_BLAST
	; MAGMORTAR
	db 2, 1, THUNDERBOLT
	db 2, 2, PSYCHIC_M
	db 2, 3, FIRE_BLAST
	db 2, 4, EARTHQUAKE
	; RAPIDASH
	db 3, 1, WILL_O_WISP
	db 3, 2, FIRE_SPIN
	db 3, 3, MEGAHORN
	db 3, 4, FLARE_BLITZ
	; FLAREON
	db 4, 1, IRON_TAIL
	db 4, 2, FIRE_BLAST
	db 4, 3, SUPERPOWER
	db 4, 4, SHADOW_BALL
	; ARCANINE
	db 5, 1, EXTREMESPEED
	db 5, 2, FLARE_BLITZ
	db 5, 3, CLOSE_COMBAT
	db 5, 4, HYPER_BEAM
	; MCHARZARDY
	db 6, 1, DRAGON_CLAW
	db 6, 2, ROOST
	db 6, 3, BLAST_BURN
	db 6, 4, THUNDERPUNCH
	db $FE

; ===============================================================

	db GIOVANNI, 1
	; ONIX
	db 1, 1, ROCK_SLIDE
	db 1, 2, DIG
	db 1, 3, WRAP
	db 1, 4, TWISTER
	; RHYHORN
	db 2, 1, FURY_ATTACK
	db 2, 2, ROCK_TOMB
	db 2, 3, MUD_SLAP
	db 2, 4, IRON_TAIL
	; DUGTRIO
	db 3, 1, DIG
	db 3, 2, SLASH
	db 3, 3, SCREECH
	db 3, 4, SAND_ATTACK
	; KANGASKHAN
	db 4, 1, THUNDERBOLT
	db 4, 2, AERIAL_ACE
	db 4, 3, DRAIN_PUNCH
	db 4, 4, DIZZY_PUNCH
	; MAROWAK
	db 5, 1, FURY_ATTACK
	db 5, 2, WATER_GUN
	db 5, 3, ROCK_TOMB
	db 5, 4, BONEMERANG
	; PERSIAN
	db 6, 1, SHADOW_BALL
	db 6, 2, BUBBLEBEAM
	db 6, 3, PAY_DAY
	db 6, 4, SLASH
	db $FE

	db GIOVANNI, 2
	; STEELIX
	db 1, 1, IRON_TAIL
	db 1, 2, EARTHQUAKE
	db 1, 3, DRAGONBREATH
	db 1, 4, HEADBUTT
	; NIDOKING
	db 2, 1, ICE_PUNCH
	db 2, 2, EARTHQUAKE
	db 2, 3, THRASH
	db 2, 4, TOXIC
	; RHYDON
	db 3, 1, BODY_SLAM
	db 3, 2, STONE_EDGE
	db 3, 3, EARTHQUAKE
	db 3, 4, IRON_TAIL
	; NIDOQUEEN
	db 4, 1, THUNDERBOLT
	db 4, 2, ICE_BEAM
	db 4, 3, LOVELY_KISS
	db 4, 4, SUPER_FANG
	; GOLEM
	db 5, 1, SELFDESTRUCT
	db 5, 2, EARTHQUAKE
	db 5, 3, ROCK_TOMB
	db 5, 4, SEISMIC_TOSS
	; PERSIAN
	db 6, 1, SLASH
	db 6, 2, PAY_DAY
	db 6, 3, DREAM_EATER
	db 6, 4, HYPNOSIS
	db $FE

	db GIOVANNI, 3
	; SANDSLASH
	db 1, 1, ROCK_TOMB
	db 1, 2, EARTHQUAKE
	db 1, 3, SLASH
	db 1, 4, SHADOW_CLAW
	; DUGTRIO
	db 2, 1, EARTHQUAKE
	db 2, 2, SLUDGE_WAVE
	db 2, 3, FISSURE
	db 2, 4, STONE_EDGE
	; NIDOKING
	db 3, 1, THUNDERBOLT
	db 3, 2, ICE_BEAM
	db 3, 3, LOVELY_KISS
	db 3, 4, SUPER_FANG
	; NIDOQUEEN
	db 4, 1, EARTHQUAKE
	db 4, 2, SHADOW_CLAW
	db 4, 3, TOXIC
	db 4, 4, FIRE_BLAST
	; KANGASKHAN
	db 5, 1, DRAIN_PUNCH
	db 5, 2, SHADOW_CLAW
	db 5, 3, DIZZY_PUNCH
	db 5, 4, AERIAL_ACE
	; PERSIAN
	db 6, 1, SLASH
	db 6, 2, HYPER_BEAM
	db 6, 3, THUNDERBOLT
	db 6, 4, PAY_DAY
	db $FE

	db GIOVANNI, 4
	; DUGTRIO
	db 1, 1, EARTHQUAKE
	db 1, 2, SLUDGE_WAVE
	db 1, 3, FISSURE
	db 1, 4, STONE_EDGE
	; GOLEM
	db 2, 1, SUPERPOWER
	db 2, 2, EXPLOSION
	db 2, 3, EARTHQUAKE
	db 2, 4, STONE_EDGE
	; NIDOQUEEN
	db 3, 1, THUNDER
	db 3, 2, BLIZZARD
	db 3, 3, LOVELY_KISS
	db 3, 4, SUPER_FANG
	; NIDOKING
	db 4, 1, MEGAHORN
	db 4, 2, OUTRAGE
	db 4, 3, EARTHQUAKE
	db 4, 4, GUNK_SHOT
	; RHYPERIOR
	db 5, 1, HEAD_SMASH
	db 5, 2, MEGAHORN
	db 5, 3, EARTHQUAKE
	db 5, 4, FLASH_CANNON
	; PERSIAN
	db 6, 1, HYPNOSIS ; SLASH?
	db 6, 2, AERIAL_ACE ; HYPER_BEAM?
	db 6, 3, SHADOW_CLAW ; THUNDERBOLT?
	db 6, 4, SLASH ; TOXIC? PAY_DAY?
	db $FE

; ===============================================================

	db LORELEI, 1
	; DEWGONG
	db 1, 1, GRASSY_TERRAIN ; HAIL
	db 1, 2, ELECTRIC_TERRAIN ; AQUA_JET
	db 1, 3, MISTY_TERRAIN ; REST
	db 1, 4, BLIZZARD ; BLIZZARD
	; CLOYSTER
	db 2, 1, EXPLOSION
	db 2, 2, PIN_MISSILE
	db 2, 3, SPIKE_CANNON
	db 2, 4, ICICLE_SPEAR
	; JYNX
	db 3, 1, BLIZZARD
	db 3, 2, LOVELY_KISS
	db 3, 3, DRAININGKISS
	db 3, 4, PSYCHO_CUT
	; GLACEON
	db 4, 1, BLIZZARD
	db 4, 2, HAIL ; MIRROR_COAT?
	db 4, 3, WATER_PULSE
	db 4, 4, BARRIER
	; LAPRAS
	db 5, 1, PSYCHIC_M
	db 5, 2, BLIZZARD
	db 5, 3, HYDRO_PUMP
	db 5, 4, REST
	; MSLOWBRO
	db 6, 1, AMNESIA
	db 6, 2, PSYCHIC_M
	db 6, 3, REST
	db 6, 4, SCALD ; THUNDER_WAVE?
	db $FE

	db LORELEI, 2
	; JYNX
	db 1, 1, BLIZZARD
	db 1, 2, LOVELY_KISS
	db 1, 3, DRAININGKISS
	db 1, 4, PSYCHO_CUT
	; DEWGONG
	db 2, 1, AQUA_JET
	db 2, 2, REST
	db 2, 3, IRON_TAIL
	db 2, 4, BLIZZARD
	; CLOYSTER
	db 3, 1, EXPLOSION
	db 3, 2, PIN_MISSILE
	db 3, 3, HYDRO_PUMP
	db 3, 4, ICICLE_SPEAR
	; GLACEON
	db 4, 1, FREEZE_DRY
	db 4, 2, CURSE
	db 4, 3, WATER_PULSE
	db 4, 4, REST
	; LAPRAS
	db 5, 1, THUNDER
	db 5, 2, BLIZZARD
	db 5, 3, HYDRO_PUMP
	db 5, 4, CONFUSE_RAY
	; MSLOWBRO
	db 6, 1, AMNESIA
	db 6, 2, PSYCHIC_M
	db 6, 3, REST
	db 6, 4, HYDRO_PUMP
	db $FE

; ===============================================================

	db BRUNO, 1
	; HITMONTOP
	db 1, 1, FOCUS_ENERGY
	db 1, 2, STONE_EDGE
	db 1, 3, EARTHQUAKE
	db 1, 3, HI_JUMP_KICK
	; HITMONCHAN
	db 2, 1, DRAIN_PUNCH
	db 2, 2, SHADOW_PUNCH
	db 2, 3, DIZZY_PUNCH
	db 2, 4, BULLET_PUNCH
	; HITMONLEE
	db 3, 1, HI_JUMP_KICK
	db 3, 2, CLOSE_COMBAT
	db 3, 3, ROLLING_KICK
	db 3, 4, STONE_EDGE
	; POLIWRATH
	db 4, 1, ROCK_SLIDE
	db 4, 2, CLOSE_COMBAT
	db 4, 3, SCALD
	db 4, 4, LOVELY_KISS
	; MACHAMP
 	db 5, 1, STONE_EDGE
	db 5, 2, FISSURE
	db 5, 3, CLOSE_COMBAT
	db 5, 4, DRAIN_PUNCH
	; MSTEELIX
 	db 6, 1, METEOR_MASH
	db 6, 2, EARTHQUAKE
	db 6, 3, EXPLOSION
	db 6, 4, DRAGON_DANCE
	db $FE

	db BRUNO, 2
	; HITMONLEE
	db 1, 1, HI_JUMP_KICK
	db 1, 2, EARTHQUAKE
	db 1, 3, BULLET_PUNCH
	db 1, 3, STONE_EDGE
	; POLIWRATH
	db 2, 1, ROCK_SLIDE
	db 2, 2, CLOSE_COMBAT
	db 2, 3, SCALD
	db 2, 4, LOVELY_KISS
	; HITMONTOP
	db 3, 1, FOCUS_ENERGY
	db 3, 2, STONE_EDGE
	db 3, 3, EARTHQUAKE
	db 3, 4, HI_JUMP_KICK
	; ANNIHILAPE
	db 4, 1, CLOSE_COMBAT
	db 4, 2, SHADOW_SNEAK
	db 4, 3, STONE_EDGE
	db 4, 4, GUNK_SHOT
	; MACHAMP
 	db 5, 1, STONE_EDGE
	db 5, 2, EARTHQUAKE
	db 5, 3, BULLET_PUNCH
	db 5, 4, DRAIN_PUNCH
	; MSTEELIX
 	db 6, 1, GYRO_BALL
	db 6, 2, EARTHQUAKE
	db 6, 3, BODY_PRESS
	db 6, 4, STONE_EDGE
	db $FE

; ===============================================================

	db AGATHA, 1
	; GENGAR 1
	db 1, 1, THUNDER_WAVE
	db 1, 2, CONFUSE_RAY
	db 1, 3, MINIMIZE
	db 1, 4, DARK_PULSE
	; CROBAT
	db 2, 1, AIR_SLASH
	db 2, 2, ROOST
	db 2, 3, GUNK_SHOT
	db 2, 4, LEECH_LIFE
	; GENGAR 2
	db 3, 1, PSYCHIC_M
	db 3, 2, NIGHT_SLASH
	db 3, 3, WILL_O_WISP ; THUNDERBOLT?
	db 3, 4, CURSE ; GIGA_DRAIN?
	; ANNIHILAPE
	db 4, 1, CLOSE_COMBAT
	db 4, 2, SHADOW_PUNCH
	db 4, 3, STONE_EDGE
	db 4, 4, GUNK_SHOT
	; UMBREON
	db 5, 1, REST
	db 5, 2, DARK_PULSE
	db 5, 3, DOUBLE_TEAM
	db 5, 4, TOXIC
	; MGENGAR
	db 6, 1, DRAININGKISS
	db 6, 2, NIGHT_SLASH
	db 6, 3, THUNDERBOLT
	db 6, 4, ENERGY_BALL
	db $FE

	db AGATHA, 2
	; GENGAR 1
	db 1, 1, THUNDERBOLT
	db 1, 2, GIGA_DRAIN
	db 1, 3, WILL_O_WISP
	db 1, 4, CURSE
	; ANNIHILAPE
	db 2, 1, CLOSE_COMBAT
	db 2, 2, SHADOW_BALL
	db 2, 3, STONE_EDGE
	db 2, 4, GUNK_SHOT
	; CROBAT
	db 3, 1, AIR_SLASH
	db 3, 2, ROOST
	db 3, 3, TOXIC
	db 3, 4, LEECH_LIFE
	; GENGAR 2
	db 4, 1, THUNDER_WAVE
	db 4, 2, CONFUSE_RAY
	db 4, 3, MINIMIZE
	db 4, 4, DARK_PULSE
	; UMBREON
	db 5, 1, REST
	db 5, 2, DARK_PULSE
	db 5, 3, DOUBLE_TEAM
	db 5, 4, CONFUSE_RAY
	; MGENGAR
	db 6, 1, DRAININGKISS
	db 6, 2, NIGHT_SLASH
	db 6, 3, THUNDERBOLT
	db 6, 4, ENERGY_BALL
	db $FE

; ===============================================================

	db LANCE, 1 ; TBE?
	; GYARADOS
	db 1, 1, HURRICANE
	db 1, 2, FIRE_BLAST ; DRAGON_DANCE?
	db 1, 3, OUTRAGE ; STONE_EDGE?
	db 1, 4, HYPER_BEAM ; EARTHQUAKE?
	; DRAGONITE 1
	db 2, 1, DRAGON_DANCE
	db 2, 2, AIR_SLASH
	db 2, 3, STEEL_WING
	db 2, 4, EARTHQUAKE
	; AERODACTYL
	db 3, 1, AIR_CUTTER
	db 3, 2, HEAD_SMASH
	db 3, 3, EARTHQUAKE
	db 3, 4, HYPER_BEAM
	; KINGDRA
	db 4, 1, FLASH_CANNON
	db 4, 2, OUTRAGE
	db 4, 3, HYPER_BEAM
	db 4, 4, HYDRO_PUMP
	; DRAGONITE 2
	db 5, 1, ROOST
	db 5, 2, THUNDER_WAVE
	db 5, 3, WRAP
	db 5, 4, OUTRAGE
	; MCHARZARDX
	db 6, 1, HYPER_BEAM
	db 6, 2, SHADOW_CLAW
	db 6, 3, DRAGON_DANCE
	db 6, 4, STEEL_WING
	db $FE

	db LANCE, 2 ; TBE?
	; AERODACTYL
	db 1, 1, AIR_CUTTER
	db 1, 2, HEAD_SMASH
	db 1, 3, EARTHQUAKE
	db 1, 4, HYPER_BEAM
	; DRAGONITE 1
	db 2, 1, FIRE_BLAST
	db 2, 2, THUNDER
	db 2, 3, BLIZZARD
	db 2, 4, OUTRAGE
	; GYARADOS
	db 3, 1, HURRICANE
	db 3, 2, DRAGON_DANCE
	db 3, 3, STONE_EDGE
	db 3, 4, EARTHQUAKE
	; KINGDRA
	db 4, 1, FLASH_CANNON
	db 4, 2, DRAGONBREATH
	db 4, 3, DOUBLE_TEAM
	db 4, 4, HYDRO_PUMP
	; DRAGONITE 2
	db 5, 1, AIR_CUTTER
	db 5, 2, STONE_EDGE
	db 5, 3, EXTREMESPEED
	db 5, 4, SUPERPOWER
	; MCHARZARDX
	db 6, 1, HYPER_BEAM
	db 6, 2, SHADOW_CLAW
	db 6, 3, DRAGON_DANCE
	db 6, 4, ROCK_SLIDE
	db $FE

; ===============================================================

	db RIVAL1, 3
	; PIDGEOTTO
	db 1, 1, SAND_ATTACK ; old move
	db 1, 2, QUICK_ATTACK
	db 1, 3, TWISTER
	db 1, 4, WING_ATTACK
	; NIDOKING
	db 2, 1, DOUBLE_KICK
	db 2, 2, MEGA_PUNCH ; TM!
	db 2, 3, BUBBLEBEAM ; TM!
	db 2, 4, MUD_SLAP
	; CLEFABLE
	db 3, 1, SING
	db 3, 2, MINIMIZE ; old move
	db 3, 3, MUD_SLAP
	db 3, 4, METRONOME ; old move
	; SANDSHREW
	db 4, 1, SAND_ATTACK
	db 4, 2, DEFENSE_CURL
	db 4, 3, MUD_SLAP
	db 4, 4, SLASH
	; MANKEY
	db 5, 1, METRONOME ; TM!
	db 5, 2, MUD_SLAP
	db 5, 3, SEISMIC_TOSS
	db 5, 4, KARATE_CHOP
	; EEVEE
	db 6, 1, SAND_ATTACK
	db 6, 2, QUICK_ATTACK
	db 6, 3, BIDE ; TM!
	db 6, 4, MUD_SLAP
	db $FE

; ===============================================================

	db RIVAL2, 1 ; SS Anne
	; NIDOKING
	db 1, 1, DOUBLE_KICK
	db 1, 2, DIG
	db 1, 3, BUBBLEBEAM
	db 1, 4, BODY_SLAM
	; PIDGEOTTO
	db 2, 1, SAND_ATTACK
	db 2, 2, QUICK_ATTACK
	db 2, 3, TWISTER
	db 2, 4, WING_ATTACK
	; CLEFABLE
	db 3, 1, SING
	db 3, 2, MUD_SLAP
	db 3, 3, MINIMIZE
	db 3, 4, METRONOME
	; SANDSLASH
	db 4, 1, DEFENSE_CURL
	db 4, 2, MUD_SLAP
	db 4, 3, SLASH
	db 4, 4, SAND_ATTACK
	; RATICATE
	db 5, 1, BUBBLEBEAM
	db 5, 2, HYPER_FANG
	db 5, 3, FOCUS_ENERGY
	db 5, 4, SUPER_FANG
	; EEVEE
	db 6, 1, SAND_ATTACK
	db 6, 2, QUICK_ATTACK
	db 6, 3, BIDE
	db 6, 4, BITE
	db $FE

	db RIVAL2, 2 ; Pokemon Tower
	; PINSIR
	db 1, 1, SEISMIC_TOSS
	db 1, 2, X_SCISSOR
	db 1, 3, ROCK_SLIDE
	db 1, 4, SWORDS_DANCE ; DIG?
	; PORYGONZ
	db 2, 1, AMNESIA
	db 2, 2, TRI_ATTACK
	db 2, 3, DARK_PULSE
	db 2, 4, PSYCHIC_M
	; KLEAVOR
	db 3, 1, X_SCISSOR
	db 3, 2, AIR_SLASH
	db 3, 3, STONE_EDGE
	db 3, 4, SWORDS_DANCE
	; CLOYSTER
	db 4, 1, LIGHT_SCREEN
	db 4, 2, CLAMP
	db 4, 3, AURORA_BEAM
	db 4, 4, MUD_SHOT
	; NINETALES
	db 5, 1, FIRE_SPIN
	db 5, 2, CONFUSE_RAY
	db 5, 4, WILL_O_WISP
	db 5, 4, HYPNOSIS
	; UMBREON
	db 6, 1, DARK_PULSE
	db 6, 2, DOUBLE_TEAM
	db 6, 3, CONFUSE_RAY
	db 6, 4, REST
	db $FE

	db RIVAL2, 3 ; Silph Co
	; ANNIHILAPE
	db 1, 1, CLOSE_COMBAT ; DRAIN_PUNCH?
	db 1, 2, SHADOW_CLAW
	db 1, 3, STONE_EDGE
	db 1, 4, GUNK_SHOT ; COUNTER?
	; ELECTIVIRE
	db 2, 1, ICE_PUNCH
	db 2, 2, ROCK_TOMB
	db 2, 3, THUNDERPUNCH
	db 2, 4, KARATE_CHOP
	; JYNX
	db 3, 1, ICE_BEAM
	db 3, 2, DRAININGKISS
	db 3, 3, LOVELY_KISS
	db 3, 4, PSYCHIC_M
	; MAGMORTAR
	db 4, 1, ROCK_SLIDE
	db 4, 2, FIRE_BLAST
	db 4, 3, KARATE_CHOP
	db 4, 4, BULLET_SEED
	; SNORLAX
	db 5, 1, BODY_SLAM
	db 5, 2, SELFDESTRUCT
	db 5, 3, SHADOW_BALL
	db 5, 4, PLAY_ROUGH
	; UMBREON
	db 6, 1, TOXIC
	db 6, 2, DOUBLE_TEAM
	db 6, 3, REST
	db 6, 4, DARK_PULSE
	db $FE

	db RIVAL2, 4 ; Route 22
	; JOLTEON
	db 1, 1, DOUBLE_KICK
	db 1, 2, THUNDER_WAVE
	db 1, 3, BITE
	db 1, 4, ZAP_CANNON
	; VAPOREON
	db 2, 1, TOXIC
	db 2, 2, REST
	db 2, 3, ACID_ARMOR
	db 2, 4, SCALD
	; FLAREON
	db 3, 1, IRON_TAIL
	db 3, 2, WILL_O_WISP
	db 3, 3, SUPERPOWER
	db 3, 4, HEAT_WAVE
	; LEAFEON
	db 4, 1, BODY_SLAM
	db 4, 2, LEECH_SEED
	db 4, 3, AERIAL_ACE
	db 4, 4, LEAF_STORM
	; GLACEON
	db 5, 1, SAND_ATTACK
	db 5, 2, REST
	db 5, 3, BLIZZARD
	db 5, 4, SUBSTITUTE
	; UMBREON
	db 6, 1, CONFUSE_RAY
	db 6, 2, DARK_PULSE
	db 6, 3, REST
	db 6, 4, DOUBLE_TEAM
	db $FE

	db RIVAL2, 5 ; Snorlaxs
	; AERODACTYL
	db 1, 1, DRAGON_RAGE
	db 1, 2, AERIAL_ACE
	db 1, 3, ROCK_SLIDE
	db 1, 4, IRON_TAIL
	; STARMIE
	db 2, 1, PSYCHIC_M
	db 2, 2, WATER_PULSE
	db 2, 3, THUNDERBOLT
	db 2, 4, RECOVER
	; TANGROWTH
	db 3, 1, SLEEP_POWDER
	db 3, 2, LEECH_SEED
	db 3, 3, INFESTATION
	db 3, 4, GIGA_DRAIN
	; MAGNEZONE
	db 4, 1, THUNDER_WAVE
	db 4, 2, THUNDERBOLT
	db 4, 3, BODY_PRESS
	db 4, 4, FLASH_CANNON
	; KINGDRA
	db 5, 1, DRAGONBREATH
	db 5, 2, BUBBLEBEAM
	db 5, 3, ICE_BEAM
	db 5, 4, SMOKESCREEN
	; UMBREON
	db 6, 1, DARK_PULSE
	db 6, 2, DOUBLE_TEAM
	db 6, 3, CONFUSE_RAY
	db 6, 4, REST
	db $FE

	db RIVAL2, 6 ; Mt Moon
	; ALAKAZAM
	db 1, 1, PSYCHIC_M
	db 1, 2, RECOVER
	db 1, 3, THUNDER_WAVE
	db 1, 4, REFLECT
	; BLISSEY
	db 2, 1, SOFTBOILED
	db 2, 2, ICE_BEAM
	db 2, 3, THUNDER_WAVE
	db 2, 4, COUNTER
	; RHYPERIOR
	db 3, 1, STONE_EDGE
	db 3, 2, EARTHQUAKE
	db 3, 3, SUBSTITUTE
	db 3, 4, HYPER_BEAM
	; EXEGGUTOR
	db 4, 1, EXPLOSION
	db 4, 2, PSYCHIC_M
	db 4, 3, SLEEP_POWDER
	db 4, 4, GIGA_DRAIN
	; TAUROS
	db 5, 1, HYPER_BEAM
	db 5, 2, BODY_SLAM
	db 5, 3, EARTHQUAKE
	db 5, 4, BLIZZARD
	; UMBREON
	db 6, 1, DARK_PULSE
	db 6, 2, DOUBLE_TEAM
	db 6, 3, CONFUSE_RAY
	db 6, 4, REST
	db $FE

	db RIVAL2, 7 ; Pokemon Mansion
	; LAPRAS
	db 1, 1, HYDRO_PUMP
	db 1, 2, BLIZZARD
	db 1, 3, CONFUSE_RAY
	db 1, 4, THUNDERBOLT
	; MACHAMP
	db 2, 1, CLOSE_COMBAT
	db 2, 2, STONE_EDGE
	db 2, 3, BULLET_PUNCH
	db 2, 4, EARTHQUAKE
	; GENGAR
	db 3, 1, CONFUSE_RAY
	db 3, 2, MINIMIZE
	db 3, 3, NIGHT_SLASH
	db 3, 4, WILL_O_WISP
	; DRAGONITE
	db 4, 1, DRAGON_DANCE
	db 4, 2, AIR_SLASH
	db 4, 3, EARTHQUAKE
	db 4, 4, OUTRAGE
	; GYARADOS
	db 5, 1, DRAGON_DANCE
	db 5, 2, HURRICANE
	db 5, 3, HYPER_BEAM
	db 5, 4, EARTHQUAKE
	; UMBREON
	db 6, 1, DARK_PULSE
	db 6, 2, DOUBLE_TEAM
	db 6, 3, CONFUSE_RAY
	db 6, 4, TOXIC
	db $FE

; ===============================================================

	db RIVAL3, 1
	; MPIDGEOT
	db 1, 1, THUNDER
	db 1, 2, BLIZZARD
	db 1, 3, HEAT_WAVE
	db 1, 4, HURRICANE
	; MALAKAZAM
	db 2, 1, PSYCHIC_M
	db 2, 2, DARK_PULSE
	db 2, 3, MOONBLAST
	db 2, 4, ZAP_CANNON
	; MSCIZOR
	db 3, 1, METEOR_MASH
	db 3, 2, STEEL_WING
	db 3, 3, LEECH_LIFE
	db 3, 4, CLOSE_COMBAT
	; MVENUSAUR
	db 4, 1, DOUBLE_EDGE
	db 4, 2, GUNK_SHOT
	db 4, 3, EARTHQUAKE
	db 4, 4, FRENZY_PLANT
	; MCHARZARDY
	db 5, 1, HURRICANE ; ?
	db 5, 2, DRAGON_CLAW
	db 5, 3, CRUNCH
	db 5, 4, BLAST_BURN
	; MBLASTOISE
	db 6, 1, ZAP_CANNON
	db 6, 2, OUTRAGE
	db 6, 3, BLIZZARD
	db 6, 4, HYDRO_CANNON
	db $FE

	db RIVAL3, 2
	; ARTICUNO
	db 1, 1, ROOST
	db 1, 2, BRAVE_BIRD
	db 1, 3, BLIZZARD
	db 1, 4, WATER_PULSE
	; ZAPDOS
	db 2, 1, ROOST
	db 2, 2, BRAVE_BIRD
	db 2, 3, THUNDER
	db 2, 4, HEAT_WAVE
	; MOLTRES
	db 3, 1, ROOST
	db 3, 2, BRAVE_BIRD
	db 3, 3, FIRE_BLAST
	db 3, 4, STEEL_WING
	db $FE

	db RIVAL3, 3
	; MPIDGEOT
	db 1, 1, THUNDER
	db 1, 2, BLIZZARD
	db 1, 3, HEAT_WAVE
	db 1, 4, ROOST
	; MALAKAZAM
	db 2, 1, PSYCHIC_M
	db 2, 2, REFLECT
	db 2, 3, MOONBLAST
	db 2, 4, RECOVER
	; MVENUSAUR
	db 3, 1, LEECH_SEED
	db 3, 2, GUNK_SHOT
	db 3, 3, EARTHQUAKE
	db 3, 4, FRENZY_PLANT
	; MCHARZARDY
	db 4, 1, ROOST
	db 4, 2, DRAGON_CLAW
	db 4, 3, THUNDERPUNCH
	db 4, 4, BLAST_BURN
	; MBLASTOISE
	db 5, 1, REST
	db 5, 2, OUTRAGE
	db 5, 3, BLIZZARD
	db 5, 4, HYDRO_CANNON
	; UMBREON
	db 6, 1, DARK_PULSE
	db 6, 2, DOUBLE_TEAM
	db 6, 3, CONFUSE_RAY
	db 6, 4, REST
	db $FE

	db RIVAL3, 4
	; ARTICUNO
	db 1, 1, ROOST
	db 1, 2, BRAVE_BIRD
	db 1, 3, ICE_BEAM
	db 1, 4, WATER_PULSE
	; ZAPDOS
	db 2, 1, ROOST
	db 2, 2, BRAVE_BIRD
	db 2, 3, THUNDERBOLT
	db 2, 4, HEAT_WAVE
	; MOLTRES
	db 3, 1, ROOST
	db 3, 2, BRAVE_BIRD
	db 3, 3, FLAMETHROWER
	db 3, 4, STEEL_WING
	db $FE

	db RIVAL3, 5
	; MPINSIR
	db 1, 1, AIR_SLASH
	db 1, 2, MEGAHORN
	db 1, 3, SWORDS_DANCE
	db 1, 4, CLOSE_COMBAT
	; MPIDGEOT
	db 2, 1, THUNDER
	db 2, 2, BLIZZARD
	db 2, 3, HEAT_WAVE
	db 2, 4, ROOST
	; MALAKAZAM
	db 3, 1, PSYCHIC_M
	db 3, 2, REFLECT
	db 3, 3, MOONBLAST
	db 3, 4, RECOVER
	; MVENUSAUR
	db 4, 1, LEECH_SEED
	db 4, 2, GUNK_SHOT
	db 4, 3, EARTHQUAKE
	db 4, 4, FRENZY_PLANT
	; MCHARZARDY
	db 5, 1, ROOST
	db 5, 2, DRAGON_CLAW
	db 5, 3, THUNDERPUNCH
	db 5, 4, BLAST_BURN
	; MBLASTOISE
	db 6, 1, REST
	db 6, 2, OUTRAGE
	db 6, 3, BLIZZARD
	db 6, 4, HYDRO_CANNON
	db $FE

	db RIVAL3, 6
	; ARTICUNO
	db 1, 1, ROOST
	db 1, 2, BRAVE_BIRD
	db 1, 3, BLIZZARD
	db 1, 4, WATER_PULSE
	; ZAPDOS
	db 2, 1, ROOST
	db 2, 2, BRAVE_BIRD
	db 2, 3, THUNDERBOLT
	db 2, 4, HEAT_WAVE
	; MOLTRES
	db 3, 1, ROOST
	db 3, 2, BRAVE_BIRD
	db 3, 3, FLAMETHROWER
	db 3, 4, STEEL_WING
	; MEWTWO
	db 4, 1, PSYSTRIKE
	db 4, 2, RECOVER
	db 4, 3, BLIZZARD
	db 4, 4, FIRE_BLAST
	; MEW
	db 5, 1, TRI_ATTACK
	db 5, 2, CRABHAMMER
	db 5, 3, VOLT_TACKLE
	db 5, 4, SOFTBOILED
	; UMBREON
	db 6, 1, DARK_PULSE
	db 6, 2, DOUBLE_TEAM
	db 6, 3, TOXIC
	db 6, 4, REST
	db $FE

; ===============================================================

	db JESSIEJAMES, 1
	; EKANS
	db 1, 1, WRAP
	db 1, 2, POISON_STING
	db 1, 3, BITE
	db 1, 4, GLARE
	; KOFFING
	db 2, 1, TACKLE
	db 2, 2, SMOG
	db 2, 3, SMOKESCREEN
	db 2, 4, INFESTATION
	; MEOWTH
	db 3, 1, SCRATCH
	db 3, 2, LEER
	db 3, 3, DEFENSE_CURL
	db 3, 4, BITE
	db $FE

	db JESSIEJAMES, 2
	; ARBOK
	db 1, 1, GLARE
	db 1, 2, SCREECH
	db 1, 3, ACID
	db 1, 4, WRAP
	; WEEZING
	db 2, 1, SMOKESCREEN
	db 2, 2, HEADBUTT
	db 2, 3, SLUDGE
	db 2, 4, INFESTATION
	; MEOWTH
	db 3, 1, PAY_DAY
	db 3, 2, MUD_SLAP
	db 3, 3, SWIFT
	db 3, 4, HEADBUTT
	db $FE

	db JESSIEJAMES, 3
	; ARBOK
	db 1, 1, GLARE
	db 1, 2, CRUNCH
	db 1, 3, SLUDGE
	db 1, 4, HEADBUTT
	; LICKITUNG
	db 2, 1, SLAM
	db 2, 2, LICK
	db 2, 3, WRAP
	db 2, 4, ROCK_TOMB
	; WEEZING
	db 3, 1, SMOKESCREEN
	db 3, 2, INFESTATION
	db 3, 3, SELFDESTRUCT
	db 3, 4, SLUDGE_WAVE
	; WEEPINBELL
	db 4, 1, WRAP
	db 4, 2, ACID
	db 4, 3, STUN_SPORE
	db 4, 4, MAGICAL_LEAF
	; MEOWTH
	db 5, 1, FURY_SWIPES
	db 5, 2, PAY_DAY
	db 5, 3, CRUNCH
	db 5, 4, WATER_PULSE
	db $FE

	db JESSIEJAMES, 4
	; ARBOK
	db 1, 1, DARK_PULSE
	db 1, 2, EARTHQUAKE
	db 1, 3, GIGA_DRAIN
	db 1, 4, SLUDGE_WAVE
	; LICKILICKY
	db 2, 1, ICE_BEAM
	db 2, 2, THUNDERBOLT
	db 2, 3, FIRE_BLAST
	db 2, 4, AMNESIA
	; WEEZING
	db 3, 1, SHADOW_BALL
	db 3, 2, EXPLOSION
	db 3, 3, SLUDGE_WAVE
	db 3, 4, HEAT_WAVE
	; VICTREEBEL
	db 4, 1, TOXIC
	db 4, 2, GROWTH
	db 4, 3, GIGA_DRAIN
	db 4, 4, LEECH_LIFE
	; MEOWTH
	db 5, 1, PAY_DAY
	db 5, 2, SLASH
	db 5, 3, SING
	db 5, 4, DREAM_EATER
	db $FE

	db JESSIEJAMES, 5
	; ARBOK
	db 1, 1, DARK_PULSE
	db 1, 2, EARTHQUAKE
	db 1, 3, GIGA_DRAIN
	db 1, 4, SLUDGE_WAVE
	; LICKILICKY
	db 2, 1, ICE_BEAM
	db 2, 2, THUNDERBOLT
	db 2, 3, FIRE_BLAST
	db 2, 4, AMNESIA
	; WEEZING
	db 3, 1, SHADOW_BALL
	db 3, 2, EXPLOSION
	db 3, 3, SLUDGE_WAVE
	db 3, 4, HEAT_WAVE
	; VICTREEBEL
	db 4, 1, TOXIC
	db 4, 2, GROWTH
	db 4, 3, GIGA_DRAIN
	db 4, 4, LEECH_LIFE
	; MEOWTH
	db 5, 1, PAY_DAY
	db 5, 2, SLASH
	db 5, 3, SING
	db 5, 4, BUBBLEBEAM
	db $FE

	db ROCKET, 55 ; Lunar Shrine Rocket Sibling 1
	; PARASECT
	db 1, 1, SPORE
	db 1, 2, INFESTATION
	db 1, 3, STICKY_WEB
	db 1, 4, LEECH_SEED
	; VENOMOTH
	db 2, 1, TOXIC
	db 2, 2, SLEEP_POWDER
	db 2, 3, PSYCHIC_M
	db 2, 4, LEECH_LIFE
	; ARBOK
	db 3, 1, GLARE
	db 3, 2, WRAP
	db 3, 3, TOXIC
	db 3, 4, GUNK_SHOT
	; CLOYSTER
	db 4, 1, CLAMP
	db 4, 2, BLIZZARD
	db 4, 3, EXPLOSION
	db 4, 4, NO_MOVE
	; WEEZING
	db 5, 1, WILL_O_WISP
	db 5, 2, TOXIC
	db 5, 3, FIRE_BLAST
	db 5, 4, SHADOW_BALL
	db $FE

	db ROCKET, 56 ; Lunar Shrine Rocket Sibling 2
	; DUGTRIO
	db 1, 1, EARTHQUAKE
	db 1, 2, STONE_EDGE
	db 1, 3, SHADOW_CLAW
	db 1, 4, HYPER_BEAM
	; PERSIAN
	db 2, 1, SLASH
	db 2, 2, NIGHT_SLASH
	db 2, 3, PLAY_ROUGH
	db 2, 4, IRON_TAIL
	; CROBAT
	db 3, 1, SUPER_FANG
	db 3, 2, GUNK_SHOT
	db 3, 3, AIR_SLASH
	db 3, 4, HEAT_WAVE
	; ALAKAZAM
	db 4, 1, PSYCHO_CUT
	db 4, 2, ENERGY_BALL
	db 4, 3, ICE_PUNCH
	db 4, 4, THUNDERPUNCH
	; TAUROS
	db 5, 1, BODY_SLAM
	db 5, 2, HYPER_BEAM
	db 5, 3, EARTHQUAKE
	db 5, 4, BLIZZARD
	db $FE

	db ROCKET, 57 ; Lunar Shrine Rocket Sibling 3
	; ELECTRODE
	db 1, 1, EXPLOSION
	db 1, 2, THUNDERBOLT
	db 1, 3, NO_MOVE
	db 1, 4, NO_MOVE
	; WEEZING
	db 2, 1, EXPLOSION
	db 2, 2, GUNK_SHOT
	db 2, 3, NO_MOVE
	db 2, 4, NO_MOVE
	; GOLEM
	db 3, 1, EXPLOSION
	db 3, 2, STONE_EDGE
	db 3, 3, EARTHQUAKE
	db 3, 4, NO_MOVE
	; CLOYSTER
	db 4, 1, EXPLOSION
	db 4, 2, BLIZZARD
	db 4, 3, HYDRO_PUMP
	db 4, 4, NO_MOVE
	; LICKILICKY
	db 5, 1, EXPLOSION
	db 5, 2, SHADOW_BALL
	db 5, 3, NO_MOVE
	db 5, 4, NO_MOVE
	db $FE

	db ROCKET, 58 ; Lunar Shrine Rocket Sibling 4
	; MUK
	db 1, 1, MINIMIZE
	db 1, 2, REST
	db 1, 3, SUBSTITUTE
	db 1, 4, FLAMETHROWER
	; VILEPLUME
	db 2, 1, LEECH_SEED
	db 2, 2, REST
	db 2, 3, PETAL_DANCE
	db 2, 4, GUNK_SHOT
	; BLISSEY
	db 3, 1, REFLECT
	db 3, 2, SOFTBOILED
	db 3, 3, ICE_BEAM
	db 3, 4, PSYCHIC_M
	; STEELIX
	db 4, 1, REST
	db 4, 2, DOUBLE_TEAM
	db 4, 3, FLASH_CANNON
	db 4, 4, EARTHQUAKE
	; UMBREON
	db 5, 1, DOUBLE_TEAM
	db 5, 2, SUBSTITUTE
	db 5, 3, REST
	db 5, 4, DARK_PULSE
	db $FE

; ===============================================================

	db PROF_OAK, 1
	; TAUROS
	db 1, 1, HYPER_BEAM
	db 1, 2, BODY_SLAM
	db 1, 3, EARTHQUAKE
	db 1, 4, BLIZZARD
	; EXEGGUTOR
	db 2, 1, SLEEP_POWDER
	db 2, 2, LEECH_SEED
	db 2, 3, GRASSY_TERRAIN
	db 2, 4, GIGA_DRAIN
	; ARCANINE
	db 3, 1, FLARE_BLITZ
	db 3, 2, EXTREMESPEED
	db 3, 3, OUTRAGE
	db 3, 4, CLOSE_COMBAT
	; RAICHU
	db 4, 1, PETAL_DANCE
	db 4, 2, PSYCHIC_M
	db 4, 3, VOLT_TACKLE
	db 4, 4, SURF
	; ESPEON
	db 5, 1, PSYCHIC_M
	db 5, 2, PSYCHIC_TERRAIN
	db 5, 3, REFLECT
	db 5, 4, ZAP_CANNON
	; GYARADOS
	db 6, 1, IRON_TAIL
	db 6, 2, HYDRO_PUMP
	db 6, 3, HYPER_BEAM
	db 6, 4, EARTHQUAKE
	db $FE

; ===============================================================

	db ORAGE, 2
	; BLISSEY
	db 1, 1, SOFTBOILED
	db 1, 2, ICE_BEAM
	db 1, 3, REFLECT
	db 1, 4, THUNDER_WAVE
	; DODRIO
	db 2, 1, BRAVE_BIRD
	db 2, 2, SWORDS_DANCE
	db 2, 3, ROOST
	db 2, 4, STEEL_WING
	; PINSIR
	db 3, 1, EARTHQUAKE
	db 3, 2, STONE_EDGE
	db 3, 3, SWORDS_DANCE
	db 3, 4, X_SCISSOR
	; SCIZOR
	db 4, 1, BULLET_PUNCH
	db 4, 2, ROOST ; ?
	db 4, 3, X_SCISSOR
	db 4, 4, SWORDS_DANCE ; ?
	; UMBREON
	db 5, 1, PSYCHIC_M
	db 5, 2, CONFUSE_RAY
	db 5, 3, DARK_PULSE
	db 5, 4, REST
	; SYLVEON
	db 6, 1, REFLECT
	db 6, 2, DRAININGKISS
	db 6, 3, MAGICAL_LEAF
	db 6, 4, DIG
	db $FE

; -----------------

	db ORAGE, 3
	; BLISSEY
	db 1, 1, SOFTBOILED
	db 1, 2, ICE_BEAM
	db 1, 3, REFLECT
	db 1, 4, THUNDER_WAVE
	; DODRIO
	db 2, 1, BRAVE_BIRD
	db 2, 2, SWORDS_DANCE
	db 2, 3, ROOST
	db 2, 4, STEEL_WING
	; PINSIR
	db 3, 1, EARTHQUAKE
	db 3, 2, STONE_EDGE
	db 3, 3, SWORDS_DANCE
	db 3, 4, X_SCISSOR
	; SCIZOR
	db 4, 1, BULLET_PUNCH
	db 4, 2, ROOST ; ?
	db 4, 3, X_SCISSOR
	db 4, 4, SWORDS_DANCE ; ?
	; UMBREON
	db 5, 1, PSYCHIC_M
	db 5, 2, CONFUSE_RAY
	db 5, 3, DARK_PULSE
	db 5, 4, REST
	; SYLVEON
	db 6, 1, REFLECT
	db 6, 2, DRAININGKISS
	db 6, 3, MAGICAL_LEAF
	db 6, 4, DIG
	db $FE

; -----------------

	db ORAGE, 4
	; BLISSEY
	db 1, 1, SOFTBOILED
	db 1, 2, ICE_BEAM
	db 1, 3, REFLECT
	db 1, 4, THUNDER_WAVE
	; DODRIO
	db 2, 1, BRAVE_BIRD
	db 2, 2, SWORDS_DANCE
	db 2, 3, ROOST
	db 2, 4, STEEL_WING
	; PINSIR
	db 3, 1, EARTHQUAKE
	db 3, 2, STONE_EDGE
	db 3, 3, SWORDS_DANCE
	db 3, 4, X_SCISSOR
	; SCIZOR
	db 4, 1, BULLET_PUNCH
	db 4, 2, ROOST ; ?
	db 4, 3, X_SCISSOR
	db 4, 4, SWORDS_DANCE ; ?
	; UMBREON
	db 5, 1, PSYCHIC_M
	db 5, 2, CONFUSE_RAY
	db 5, 3, DARK_PULSE
	db 5, 4, REST
	; SYLVEON
	db 6, 1, REFLECT
	db 6, 2, DRAININGKISS
	db 6, 3, MAGICAL_LEAF
	db 6, 4, DIG
	db $FE

; -----------------

	db ORAGE, 5
	; BLISSEY
	db 1, 1, SOFTBOILED
	db 1, 2, ICE_BEAM
	db 1, 3, REFLECT
	db 1, 4, THUNDER_WAVE
	; DODRIO
	db 2, 1, BRAVE_BIRD
	db 2, 2, SWORDS_DANCE
	db 2, 3, ROOST
	db 2, 4, STEEL_WING
	; PINSIR
	db 3, 1, EARTHQUAKE
	db 3, 2, STONE_EDGE
	db 3, 3, SWORDS_DANCE
	db 3, 4, X_SCISSOR
	; SCIZOR
	db 4, 1, BULLET_PUNCH
	db 4, 2, ROOST ; ?
	db 4, 3, X_SCISSOR
	db 4, 4, SWORDS_DANCE ; ?
	; UMBREON
	db 5, 1, PSYCHIC_M
	db 5, 2, CONFUSE_RAY
	db 5, 3, DARK_PULSE
	db 5, 4, REST
	; SYLVEON
	db 6, 1, REFLECT
	db 6, 2, DRAININGKISS
	db 6, 3, MAGICAL_LEAF
	db 6, 4, DIG
	db $FE

; -----------------

	db ORAGE, 6
	; BLISSEY
	db 1, 1, SOFTBOILED
	db 1, 2, ICE_BEAM
	db 1, 3, REFLECT
	db 1, 4, THUNDER_WAVE
	; DODRIO
	db 2, 1, BRAVE_BIRD
	db 2, 2, SWORDS_DANCE
	db 2, 3, ROOST
	db 2, 4, STEEL_WING
	; PINSIR
	db 3, 1, EARTHQUAKE
	db 3, 2, STONE_EDGE
	db 3, 3, SWORDS_DANCE
	db 3, 4, X_SCISSOR
	; SCIZOR
	db 4, 1, BULLET_PUNCH
	db 4, 2, ROOST ; ?
	db 4, 3, X_SCISSOR
	db 4, 4, SWORDS_DANCE ; ?
	; UMBREON
	db 5, 1, PSYCHIC_M
	db 5, 2, CONFUSE_RAY
	db 5, 3, DARK_PULSE
	db 5, 4, REST
	; SYLVEON
	db 6, 1, REFLECT
	db 6, 2, DRAININGKISS
	db 6, 3, MAGICAL_LEAF
	db 6, 4, DIG
	db $FE

; -----------------

	db ORAGE, 7
	; BLISSEY
	db 1, 1, SOFTBOILED
	db 1, 2, ICE_BEAM
	db 1, 3, REFLECT
	db 1, 4, THUNDER_WAVE
	; DODRIO
	db 2, 1, BRAVE_BIRD
	db 2, 2, SWORDS_DANCE
	db 2, 3, ROOST
	db 2, 4, STEEL_WING
	; PINSIR
	db 3, 1, EARTHQUAKE
	db 3, 2, STONE_EDGE
	db 3, 3, SWORDS_DANCE
	db 3, 4, X_SCISSOR
	; SCIZOR
	db 4, 1, BULLET_PUNCH
	db 4, 2, ROOST ; ?
	db 4, 3, X_SCISSOR
	db 4, 4, SWORDS_DANCE ; ?
	; UMBREON
	db 5, 1, PSYCHIC_M
	db 5, 2, CONFUSE_RAY
	db 5, 3, DARK_PULSE
	db 5, 4, REST
	; SYLVEON
	db 6, 1, REFLECT
	db 6, 2, DRAININGKISS
	db 6, 3, MAGICAL_LEAF
	db 6, 4, DIG
	db $FE

; -----------------

	db ORAGE, 8
	; BLISSEY
	db 1, 1, SOFTBOILED
	db 1, 2, ICE_BEAM
	db 1, 3, REFLECT
	db 1, 4, THUNDER_WAVE
	; DODRIO
	db 2, 1, BRAVE_BIRD
	db 2, 2, SWORDS_DANCE
	db 2, 3, ROOST
	db 2, 4, STEEL_WING
	; PINSIR
	db 3, 1, EARTHQUAKE
	db 3, 2, STONE_EDGE
	db 3, 3, SWORDS_DANCE
	db 3, 4, X_SCISSOR
	; SCIZOR
	db 4, 1, BULLET_PUNCH
	db 4, 2, ROOST ; ?
	db 4, 3, X_SCISSOR
	db 4, 4, SWORDS_DANCE ; ?
	; UMBREON
	db 5, 1, PSYCHIC_M
	db 5, 2, CONFUSE_RAY
	db 5, 3, DARK_PULSE
	db 5, 4, REST
	; SYLVEON
	db 6, 1, REFLECT
	db 6, 2, DRAININGKISS
	db 6, 3, MAGICAL_LEAF
	db 6, 4, DIG
	db $FE

; -----------------

	db ORAGE, 9
	; DODRIO
	db 1, 1, AIR_CUTTER
	db 1, 2, SWORDS_DANCE
	db 1, 3, BODY_SLAM
	db 1, 4, LOW_KICK
	; BLISSEY
	db 2, 1, SOFTBOILED
	db 2, 2, BLIZZARD
	db 2, 3, REFLECT
	db 2, 4, PSYCHIC_M
	; KLEAVOR
	db 3, 1, AIR_CUTTER
	db 3, 2, STONE_EDGE
	db 3, 3, SWORDS_DANCE
	db 3, 4, X_SCISSOR
	; MAGNEZONE
	db 4, 1, EXPLOSION
	db 4, 2, FLASH_CANNON
	db 4, 3, THUNDER_WAVE
	db 4, 4, THUNDERBOLT
	; SYLVEON
	db 5, 1, REFLECT
	db 5, 2, DRAININGKISS
	db 5, 3, MAGICAL_LEAF
	db 5, 4, SAND_ATTACK
	; MGYARADOS
	db 6, 1, NIGHT_SLASH
	db 6, 2, HYDRO_PUMP
	db 6, 3, HYPER_BEAM
	db 6, 4, EARTHQUAKE
	db $FE

; ===============================================================

	db TRAVELER, 1
	; MZYGARDE
	db 1, 1, DRAGON_CLAW
	db 1, 2, GLARE
	db 1, 3, EXTREMESPEED
	db 1, 4, THOUSANDARROWS
	; UNECROZMA
	db 2, 1, FLASH_CANNON
	db 2, 2, OUTRAGE
	db 2, 3, EARTHQUAKE
	db 2, 4, PHOTONGEYSER
	; MRAYQUAZA
	db 3, 1, DRAGON_DANCE
	db 3, 2, IRON_TAIL
	db 3, 3, FIRE_BLAST
	db 3, 4, DRAGONASCENT
	; EETERNATUS
	db 4, 1, RECOVER
	db 4, 2, GUNK_SHOT
	db 4, 3, FLAMETHROWER
	db 4, 4, ETERNABEAM
	; ARCEUS
	db 5, 1, RECOVER
	db 5, 2, SWORDS_DANCE
	db 5, 3, WILL_O_WISP
	db 5, 4, JUDGMENT
	db $FE

	db TRAVELER, 2
	; MZYGARDE
	db 1, 1, DRAGON_CLAW
	db 1, 2, GLARE
	db 1, 3, EXTREMESPEED
	db 1, 4, THOUSANDARROWS
	; UNECROZMA
	db 2, 1, FLASH_CANNON
	db 2, 2, OUTRAGE
	db 2, 3, EARTHQUAKE
	db 2, 4, PHOTONGEYSER
	; MRAYQUAZA
	db 3, 1, DRAGON_DANCE
	db 3, 2, IRON_TAIL
	db 3, 3, FIRE_BLAST
	db 3, 4, DRAGONASCENT
	; EETERNATUS
	db 4, 1, RECOVER
	db 4, 2, GUNK_SHOT
	db 4, 3, FLAMETHROWER
	db 4, 4, ETERNABEAM
	; ARCEUS
	db 5, 1, RECOVER
	db 5, 2, SWORDS_DANCE
	db 5, 3, WILL_O_WISP
	db 5, 4, JUDGMENT
	; MMEWTWOX
	db 6, 1, CLOSE_COMBAT
	db 6, 2, STONE_EDGE
	db 6, 3, LEECH_LIFE
	db 6, 4, METEOR_MASH
	db $FE

; ===============================================================

	db PIGEON, 1
	; MPIDGEOT
	db 1, 1, THUNDER
	db 1, 2, BLIZZARD
	db 1, 3, HEAT_WAVE
	db 1, 4, METRONOME
	; DRAGONAIR
	db 2, 1, OUTRAGE
	db 2, 2, THUNDER_WAVE
	db 2, 3, ZAP_CANNON
	db 2, 4, METRONOME
	; HAUNTER
	db 3, 1, CURSE
	db 3, 2, HYPNOSIS
	db 3, 3, CONFUSE_RAY
	db 3, 4, METRONOME
	; PORYGON
	db 4, 1, CONVERSION
	db 4, 2, TRI_ATTACK
	db 4, 3, PSYWAVE
	db 4, 4, METRONOME
	; MMEWTWOY
	db 5, 1, PSYCHO_CUT
	db 5, 2, MOONBLAST
	db 5, 3, FIRE_BLAST
	db 5, 4, METRONOME
	; MAGIKARP
	db 6, 1, ANCESTOR_PWR
	db 6, 2, NO_MOVE
	db 6, 3, NO_MOVE
	db 6, 4, NO_MOVE
	db $FE

; ===============================================================

	db ICHINO, 1
	; MRAICHUY
	db 1, 1, THUNDER
	db 1, 2, OUTRAGE ; PPS
	db 1, 3, SURF
	db 1, 4, DRAININGKISS
	; MCLEFABLE
	db 2, 1, NIGHT_SLASH
	db 2, 2, PSYCHIC_M
	db 2, 3, MINIMIZE
	db 2, 4, THUNDER_WAVE
	; MVICTREBEL
	db 3, 1, RAZOR_LEAF
	db 3, 2, LOVELY_KISS
	db 3, 3, GUNK_SHOT
	db 3, 4, EARTHQUAKE
	; MSTARMIE
	db 4, 1, RECOVER
	db 4, 2, PSYCHO_CUT
	db 4, 3, HYDRO_PUMP
	db 4, 4, CLOSE_COMBAT ; BLIZZARD?
	; MRAICHUX
	db 5, 1, METEOR_MASH
	db 5, 2, VOLT_TACKLE
	db 5, 3, EXTREMESPEED
	db 5, 4, CLOSE_COMBAT
	; MDRAGONITE
	db 6, 1, FREEZE_DRY ; AIR_CUTTER?
	db 6, 2, MOONBLAST
	db 6, 3, OUTRAGE
	db 6, 4, FIRE_BLAST
	db $FE

; ===============================================================

	db NIUE, 1
	; MVENUSAUR
	db 1, 1, SUNNY_DAY
	db 1, 2, SYNTHESIS
	db 1, 3, SOLARBEAM
	db 1, 4, WEATHER_BALL
	; MGYARADOS
	db 2, 1, FIRE_BLAST
	db 2, 2, DARK_PULSE
	db 2, 3, GUNK_SHOT
	db 2, 4, EARTHQUAKE
	; MCLEFABLE
	db 3, 1, DRAININGKISS
	db 3, 2, SUBSTITUTE
	db 3, 3, MINIMIZE
	db 3, 4, THUNDER_WAVE
	; MPINSIR
	db 4, 1, AIR_SLASH
	db 4, 2, MEGAHORN
	db 4, 3, STONE_EDGE
	db 4, 4, CLOSE_COMBAT
	; MARODACTYL
	db 5, 1, BRAVE_BIRD
	db 5, 2, HEAD_SMASH
	db 5, 3, IRON_TAIL
	db 5, 4, ROAR
	; MALAKAZAM
	db 6, 1, REFLECT
	db 6, 2, ENERGY_BALL
	db 6, 3, PSYCHO_CUT
	db 6, 4, NIGHT_SLASH
	db $FE

; --------------------------

	db NIUE, 2
	; MBEEDRILL
	db 1, 1, MEGAHORN
	db 1, 2, GUNK_SHOT
	db 1, 3, STICKY_WEB
	db 1, 4, DRILL_RUN
	; MCHARZARDX
	db 2, 1, EARTHQUAKE
	db 2, 2, STEEL_WING
	db 2, 3, DRAGON_DANCE
	db 2, 4, BLAST_BURN
	; MKANGASKAN
	db 3, 1, SHADOW_CLAW
	db 3, 2, BONEMERANG
	db 3, 3, METEOR_MASH
	db 3, 4, LOVELY_KISS
	; MSLOWBRO
	db 4, 1, AMNESIA
	db 4, 2, PSYCHIC_M
	db 4, 3, REST
	db 4, 4, SCALD
	; MSCIZOR
	db 5, 1, SWORDS_DANCE
	db 5, 2, BULLET_PUNCH
	db 5, 3, METEOR_MASH
	db 5, 4, X_SCISSOR
	; MDRAGONITE
	db 6, 1, DRAGON_DANCE
	db 6, 2, HURRICANE
	db 6, 3, LOVELY_KISS
	db 6, 4, EARTHQUAKE
	db $FE

; --------------------------

	db NIUE, 3
	; MGENGAR
	db 1, 1, CONFUSE_RAY
	db 1, 2, DARK_PULSE
	db 1, 3, TOXIC_SPIKES
	db 1, 4, PERISH_SONG
	; MCHARZARDY
	db 2, 1, OUTRAGE
	db 2, 2, BLAST_BURN
	db 2, 3, SUNNY_DAY
	db 2, 4, SOLARBEAM
	; MVICTREBEL
	db 3, 1, STUN_SPORE
	db 3, 2, WEATHER_BALL
	db 3, 3, SOLARBEAM
	db 3, 4, WRAP
	; MRAICHUX
	db 4, 1, VOLT_TACKLE
	db 4, 2, EXTREMESPEED
	db 4, 3, CLOSE_COMBAT
	db 4, 4, METEOR_MASH
	; MSCIZOR
	db 5, 1, SUBSTITUTE
	db 5, 2, BULLET_PUNCH
	db 5, 3, METEOR_MASH
	db 5, 4, X_SCISSOR
	; MSTARMIE
	db 6, 1, MINIMIZE
	db 6, 2, RECOVER
	db 6, 3, PSYCHO_CUT
	db 6, 4, CLOSE_COMBAT
	db $FE

; --------------------------

	db NIUE, 4
	; MBLASTOISE
	db 1, 1, RAIN_DANCE
	db 1, 2, HYDRO_CANNON
	db 1, 3, EARTHQUAKE
	db 1, 4, BLIZZARD
	; MSTARMIE
	db 2, 1, THUNDER
	db 2, 2, HYDRO_PUMP
	db 2, 3, METEOR_MASH
	db 2, 4, CLOSE_COMBAT
	; MPIDGEOT
	db 3, 1, HURRICANE
	db 3, 2, THUNDER
	db 3, 3, ROOST
	db 3, 4, BLIZZARD
	; MDRAGONITE
	db 4, 1, OUTRAGE
	db 4, 2, THUNDER_WAVE
	db 4, 3, SUBSTITUTE
	db 4, 4, DRAININGKISS
	; MSTEELIX
	db 5, 1, STONE_EDGE
	db 5, 2, EARTHQUAKE
	db 5, 3, GYRO_BALL
	db 5, 4, BODY_PRESS
	; MALAKAZAM
	db 6, 1, MOONBLAST
	db 6, 2, DARK_PULSE
	db 6, 3, PSYCHO_CUT
	db 6, 4, ENERGY_BALL
	db $FE

; --------------------------

	db NIUE, 5
	; MBEEDRILL
	db 1, 1, STICKY_WEB
	db 1, 2, SPIKES
	db 1, 3, GUNK_SHOT
	db 1, 4, DRILL_RUN
	; MGENGAR
	db 2, 1, TOXIC_SPIKES
	db 2, 2, PERISH_SONG
	db 2, 3, CURSE
	db 2, 4, DARK_PULSE
	; MSTEELIX
	db 3, 1, STEALTH_ROCK
	db 3, 2, ROAR
	db 3, 3, EARTHQUAKE
	db 3, 4, STONE_EDGE
	; MKANGASKAN
	db 4, 1, LOVELY_KISS
	db 4, 2, ROAR
	db 4, 3, ICICLE_SPEAR
	db 4, 4, BONEMERANG
	; MVICTREBEL
	db 5, 1, LOVELY_KISS
	db 5, 2, EARTHQUAKE
	db 5, 3, GUNK_SHOT
	db 5, 4, RAZOR_LEAF
	; MSTARMIE
	db 6, 1, RECOVER
	db 6, 2, MINIMIZE
	db 6, 3, BLIZZARD
	db 6, 4, CLOSE_COMBAT
	db $FE

; --------------------------

	db NIUE, 6
	; MSLOWBRO
	db 1, 1, RAIN_DANCE
	db 1, 2, SCALD
	db 1, 3, PSYCHIC_M
	db 1, 4, BLIZZARD
	; MPIDGEOT
	db 2, 1, THUNDER
	db 2, 2, HURRICANE
	db 2, 3, BLIZZARD
	db 2, 4, ROOST
	; MGYARADOS
	db 3, 1, HURRICANE
	db 3, 2, HYDRO_PUMP
	db 3, 3, BLIZZARD
	db 3, 4, STONE_EDGE
	; MCLEFABLE ; TBE?
	db 4, 1, DRAININGKISS
	db 4, 2, MINIMIZE
	db 4, 3, THUNDER_WAVE
	db 4, 4, SUBSTITUTE
	; MVICTREBEL
	db 5, 1, LOVELY_KISS
	db 5, 2, EARTHQUAKE
	db 5, 3, GUNK_SHOT
	db 5, 4, RAZOR_LEAF
	; MDRAGONITE
	db 6, 1, EXTREMESPEED
	db 6, 2, FIRE_BLAST
	db 6, 3, THUNDER
	db 6, 4, BLIZZARD
	db $FE

; --------------------------

	db NIUE, 7
	; MGENGAR
	db 1, 1, TOXIC_SPIKES
	db 1, 2, CURSE
	db 1, 3, DARK_PULSE
	db 1, 4, DRAININGKISS
	; MARODACTYL
	db 2, 1, HEAD_SMASH
	db 2, 2, HURRICANE
	db 2, 3, WHIRLWIND
	db 2, 4, STEALTH_ROCK
	; MALAKAZAM
	db 3, 1, REFLECT
	db 3, 2, RECOVER
	db 3, 3, THUNDER_WAVE
	db 3, 4, PSYCHO_CUT
	; MPINSIR
	db 4, 1, MEGAHORN
	db 4, 2, SUPERPOWER
	db 4, 3, EARTHQUAKE
	db 4, 4, EXPLOSION
	; MGYARADOS
	db 5, 1, HYPER_BEAM
	db 5, 2, STONE_EDGE
	db 5, 3, EARTHQUAKE
	db 5, 4, HURRICANE
	; MRAICHUY
	db 6, 1, FIRE_BLAST
	db 6, 2, OUTRAGE
	db 6, 3, VOLT_TACKLE
	db 6, 4, PETAL_DANCE
	db $FE

; --------------------------

	db NIUE, 8
	; MVENUSAUR
	db 1, 1, GRASSY_TERRAIN
	db 1, 2, LEECH_SEED
	db 1, 3, RAZOR_LEAF
	db 1, 4, SLEEP_POWDER
	; MRAICHUX
	db 2, 1, METEOR_MASH
	db 2, 2, VOLT_TACKLE
	db 2, 3, EXTREMESPEED
	db 2, 4, CLOSE_COMBAT
	; MRAICHUY
	db 3, 1, FIRE_BLAST
	db 3, 2, OUTRAGE
	db 3, 3, VOLT_TACKLE
	db 3, 4, PETAL_DANCE
	; MBLASTOISE
	db 4, 1, ZAP_CANNON
	db 4, 2, EARTHQUAKE
	db 4, 3, BLIZZARD
	db 4, 4, HYDRO_CANNON
	; MCHARZARDX
	db 5, 1, HYPER_BEAM
	db 5, 2, SHADOW_CLAW
	db 5, 3, DRAGON_DANCE
	db 5, 4, STEEL_WING
	; MCHARZARDY
	db 6, 1, OUTRAGE
	db 6, 2, ROOST
	db 6, 3, THUNDERPUNCH
	db 6, 4, BLAST_BURN
	db $FE

; ===============================================================

	db SANTRE, 1 ; vs NORMAL
	; MACHAMP
	db 1, 1, STONE_EDGE
	db 1, 2, EARTHQUAKE
	db 1, 3, CLOSE_COMBAT
	db 1, 4, HYPER_BEAM
	; MGENGAR
	db 2, 1, EXPLOSION
	db 2, 2, CURSE
	db 2, 3, DARK_PULSE
	db 2, 4, ZAP_CANNON
	; MARODACTYL
	db 3, 1, HURRICANE
	db 3, 2, HEAD_SMASH
	db 3, 3, HYPER_BEAM
	db 3, 4, IRON_TAIL
	; ANNIHILAPE
	db 4, 1, CLOSE_COMBAT
	db 4, 2, COUNTER
	db 4, 3, STONE_EDGE
	db 4, 4, GUNK_SHOT
	; MSTARMIE
	db 5, 1, CLOSE_COMBAT
	db 5, 2, BLIZZARD
	db 5, 3, RECOVER
	db 5, 4, MINIMIZE
	; MSCIZOR
	db 6, 1, ROOST
	db 6, 2, SWORDS_DANCE
	db 6, 3, CLOSE_COMBAT
	db 6, 4, BULLET_PUNCH
	db $FE

; --------------------------

	db SANTRE, 2 ; vs FIGHTING
	; SYLVEON
	db 1, 1, DRAININGKISS
	db 1, 2, SUBSTITUTE
	db 1, 3, REFLECT
	db 1, 4, DOUBLE_TEAM
	; MGENGAR
	db 2, 1, EXPLOSION
	db 2, 2, PSYCHIC_M
	db 2, 3, DRAININGKISS
	db 2, 4, TOXIC_SPIKES
	; MSTARMIE
	db 3, 1, PSYCHO_CUT
	db 3, 2, HYDRO_PUMP
	db 3, 3, RECOVER
	db 3, 4, THUNDER_WAVE
	; MCLEFABLE
	db 4, 1, DRAININGKISS
	db 4, 2, AIR_SLASH
	db 4, 3, REFLECT
	db 4, 4, MINIMIZE
	; CROBAT
	db 5, 1, BRAVE_BIRD
	db 5, 2, ROOST
	db 5, 3, ZEN_HEADBUTT
	db 5, 4, CONFUSE_RAY
	; MALAKAZAM
	db 6, 1, PSYCHO_CUT
	db 6, 2, RECOVER
	db 6, 3, REFLECT ; ?
	db 6, 4, THUNDER_WAVE ; ?
	db $FE

; --------------------------

	db SANTRE, 3 ; vs FLYING
	; MARODACTYL
	db 1, 1, HEAD_SMASH
	db 1, 2, STEALTH_ROCK
	db 1, 3, SANDSTORM
	db 1, 4, NO_MOVE
	; RHYPERIOR
	db 2, 1, STONE_EDGE
	db 2, 2, STEALTH_ROCK
	db 2, 3, SANDSTORM
	db 2, 4, NO_MOVE
	; MRAICHUY
	db 3, 1, VOLT_TACKLE ; PPS THUNDER
	db 3, 2, DRAININGKISS
	db 3, 3, FIRE_BLAST
	db 3, 4, SURF
	; ARTICUNO
	db 4, 1, BLIZZARD
	db 4, 2, FREEZE_DRY
	db 4, 3, ROOST
	db 4, 4, WATER_PULSE
	; ZAPDOS
	db 5, 1, THUNDER
	db 5, 2, HEAT_WAVE
	db 5, 3, ROOST
	db 5, 4, WHIRLWIND
	; MARODACTYL
	db 6, 1, HEAD_SMASH
	db 6, 2, NO_MOVE
	db 6, 3, NO_MOVE
	db 6, 4, NO_MOVE
	db $FE

; --------------------------

	db SANTRE, 4 ; vs POISON
	; NIDOKING
	db 1, 1, LOVELY_KISS
	db 1, 2, DRILL_RUN
	db 1, 3, STONE_EDGE
	db 1, 4, FIRE_BLAST
	; MSTARMIE
	db 2, 1, PSYCHO_CUT
	db 2, 2, RECOVER
	db 2, 3, HYDRO_PUMP
	db 2, 4, MINIMIZE
	; MSCIZOR ?
	db 3, 1, SWORDS_DANCE
	db 3, 2, SUBSTITUTE
	db 3, 3, BULLET_PUNCH
	db 3, 4, BODY_SLAM ; PPS PSYCHO_CUT
	; MSTEELIX
	db 4, 1, EARTHQUAKE
	db 4, 2, IRON_TAIL
	db 4, 3, STONE_EDGE
	db 4, 4, NO_MOVE
	; DUGTRIO
	db 5, 1, EARTHQUAKE
	db 5, 2, STONE_EDGE
	db 5, 3, NO_MOVE
	db 5, 4, NO_MOVE
	; MALAKAZAM
	db 6, 1, PSYCHO_CUT
	db 6, 2, REFLECT
	db 6, 3, RECOVER
	db 6, 4, THUNDER_WAVE
	db $FE

; --------------------------

	db SANTRE, 5 ; vs GROUND
	; MVENUSAUR
	db 1, 1, SLEEP_POWDER
	db 1, 2, FRENZY_PLANT
	db 1, 3, SYNTHESIS
	db 1, 4, GUNK_SHOT
	; MBLASTOISE
	db 2, 1, RAIN_DANCE
	db 2, 2, HYDRO_CANNON
	db 2, 3, BLIZZARD
	db 2, 4, BULLET_SEED
	; KINGDRA
	db 3, 1, HYDRO_PUMP
	db 3, 2, BLIZZARD
	db 3, 3, BULLET_PUNCH
	db 3, 4, HURRICANE
	; MDRAGONITE
	db 4, 1, HYDRO_PUMP
	db 4, 2, OUTRAGE
	db 4, 3, BLIZZARD
	db 4, 4, EXTREMESPEED
	; ARTICUNO
	db 5, 1, BLIZZARD
	db 5, 2, WATER_PULSE
	db 5, 3, ROOST
	db 5, 4, HURRICANE
	; MSLOWBRO
	db 6, 1, PSYCHIC_M
	db 6, 2, HYDRO_PUMP
	db 6, 3, AMNESIA
	db 6, 4, REST
	db $FE

; --------------------------

	db SANTRE, 6 ; vs ROCK
	; DUGTRIO
	db 1, 1, STONE_EDGE
	db 1, 2, EARTHQUAKE
	db 1, 3, SHADOW_CLAW
	db 1, 4, FEINT_ATTACK ; PPS NIGHT_SLASH
	; MACHAMP
	db 2, 1, STONE_EDGE
	db 2, 2, EARTHQUAKE
	db 2, 3, CLOSE_COMBAT
	db 2, 4, COUNTER
	; MSCIZOR
	db 3, 1, SWORDS_DANCE
	db 3, 2, METEOR_MASH
	db 3, 3, SUPERPOWER
	db 3, 4, ROOST
	; MSTEELIX
	db 4, 1, IRON_TAIL
	db 4, 2, EARTHQUAKE
	db 4, 3, STONE_EDGE
	db 4, 4, EXPLOSION
	; MVICTREBEL
	db 5, 1, RAZOR_LEAF
	db 5, 2, LOVELY_KISS
	db 5, 3, EARTHQUAKE
	db 5, 4, SYNTHESIS
	; MBLASTOISE
	db 6, 1, HYDRO_CANNON
	db 6, 2, BLIZZARD
	db 6, 3, BULLET_SEED
	db 6, 4, FLASH_CANNON
	db $FE

; --------------------------

	db SANTRE, 7 ; vs BIRD
	; VENUSTOISE
	db 1, 1, HYDRO_CANNON
	db 1, 2, FRENZY_PLANT
	db 1, 3, SLEEP_POWDER
	db 1, 4, ROCK_TOMB
	; THU_FI_ZER
	db 2, 1, TRI_ATTACK
	db 2, 2, ROOST
	db 2, 3, BRAVE_BIRD
	db 2, 4, FIRE_SPIN
	; ARM_MEWTWO
	db 3, 1, PSYSTRIKE
	db 3, 2, RECOVER
	db 3, 3, AMNESIA
	db 3, 4, THUNDER_WAVE
	; MAGIKARP
	db 4, 1, FLAIL
	db 4, 2, NO_MOVE
	db 4, 3, NO_MOVE
	db 4, 4, NO_MOVE
	; PIKACHU
	db 5, 1, VOLT_TACKLE
	db 5, 2, SURF
	db 5, 3, PSYCHIC_M
	db 5, 4, FLY
	; DITTO
	db 6, 1, TRANSFORM
	db 6, 2, NO_MOVE
	db 6, 3, NO_MOVE
	db 6, 4, NO_MOVE
	db $FE

; --------------------------

	db SANTRE, 8 ; vs BUG
	; MARODACTYL
	db 1, 1, STEALTH_ROCK
	db 1, 2, HEAD_SMASH
	db 1, 3, HURRICANE
	db 1, 4, ROOST
	; MCHARZARDX
	db 2, 1, OUTRAGE
	db 2, 2, BLAST_BURN
	db 2, 3, AIR_SLASH
	db 2, 4, ROCK_SLIDE
	; DODRIO
	db 3, 1, BRAVE_BIRD
	db 3, 2, ROOST
	db 3, 3, STEEL_WING
	db 3, 4, HYPER_BEAM
	; MCHARZARDY
	db 4, 1, BLAST_BURN
	db 4, 2, HURRICANE
	db 4, 3, STEEL_WING
	db 4, 4, EARTHQUAKE
	; MPIDGEOT
	db 5, 1, BRAVE_BIRD ; HURRICANE
	db 5, 2, ROOST
	db 5, 3, THUNDER
	db 5, 4, HEAT_WAVE
	; MPINSIR
	db 6, 1, AIR_CUTTER
	db 6, 2, STONE_EDGE
	db 6, 3, MEGAHORN
	db 6, 4, ROOST
	db $FE

; --------------------------

	db SANTRE, 9 ; vs GHOST
	; MGENGAR
	db 1, 1, NIGHT_SLASH ; PPS DARK_PULSE
	db 1, 2, PSYCHIC_M ; PPS SHADOW_BALL
	db 1, 3, HYPNOSIS
	db 1, 4, CONFUSE_RAY
	; UMBREON
	db 2, 1, DARK_PULSE
	db 2, 2, REST
	db 2, 3, PSYCHIC_M
	db 2, 4, CONFUSE_RAY
	; MGYARADOS
	db 3, 1, NIGHT_SLASH
	db 3, 2, HURRICANE
	db 3, 3, HYDRO_PUMP
	db 3, 4, EARTHQUAKE
	; MSLOWBRO
	db 4, 1, AMNESIA
	db 4, 2, PSYCHIC_M
	db 4, 3, HYDRO_PUMP
	db 4, 4, REST
	; SNORLAX
	db 5, 1, PSYCHIC_M ; PPS ZEN_HEADBUTT
	db 5, 2, EARTHQUAKE
	db 5, 3, SHADOW_BALL ; PPS CRUNCH
	db 5, 4, LOVELY_KISS
	; MALAKAZAM
	db 6, 1, PSYCHO_CUT ; PPS PSYCHIC_M
	db 6, 2, NIGHT_SLASH ; PPS SHADOW_BALL
	db 6, 3, MOONBLAST
	db 6, 4, RECOVER
	db $FE

; --------------------------

	db SANTRE, 10 ; vs STEEL
	; MCHARZARDX
	db 1, 1, BLAST_BURN
	db 1, 2, EARTHQUAKE
	db 1, 3, THUNDERPUNCH
	db 1, 4, ROOST
	; MACHAMP
	db 2, 1, CLOSE_COMBAT
	db 2, 2, EARTHQUAKE
	db 2, 3, FIRE_PUNCH
	db 2, 4, DRAIN_PUNCH
	; MDRAGONITE
	db 3, 1, FIRE_BLAST
	db 3, 2, THUNDER
	db 3, 3, EARTHQUAKE
	db 3, 4, SUPERPOWER
	; DUGTRIO
	db 4, 1, EARTHQUAKE
	db 4, 2, STONE_EDGE
	db 4, 3, SHADOW_CLAW
	db 4, 4, NIGHT_SLASH
	; MSTARMIE
	db 5, 1, CLOSE_COMBAT
	db 5, 2, HYDRO_PUMP
	db 5, 3, THUNDER
	db 5, 4, RECOVER
	; MOLTRES
	db 6, 1, FIRE_BLAST
	db 6, 2, SUNNY_DAY
	db 6, 3, SOLARBEAM
	db 6, 4, HURRICANE
	db $FE

; --------------------------

	db SANTRE, 11 ; vs FIRE
	; MARODACTYL
	db 1, 1, STEALTH_ROCK
	db 1, 2, EARTHQUAKE
	db 1, 3, HEAD_SMASH
	db 1, 4, NO_MOVE
	; RHYPERIOR
	db 2, 1, STEALTH_ROCK
	db 2, 2, EARTHQUAKE
	db 2, 3, STONE_EDGE
	db 2, 4, NO_MOVE
	; MBLASTOISE
	db 3, 1, HYDRO_CANNON
	db 3, 2, EARTHQUAKE
	db 3, 3, ROCK_TOMB
	db 3, 4, RAIN_DANCE
	; MDRAGONITE
	db 4, 1, HYDRO_PUMP
	db 4, 2, THUNDER
	db 4, 3, EARTHQUAKE
	db 4, 4, STONE_EDGE
	; MSTARMIE
	db 5, 1, PSYCHO_CUT
	db 5, 2, HYDRO_PUMP
	db 5, 3, RECOVER
	db 5, 4, MINIMIZE
	; MGYARADOS
	db 6, 1, STONE_EDGE
	db 6, 2, EARTHQUAKE
	db 6, 3, HYDRO_PUMP
	db 6, 4, HYPER_BEAM
	db $FE

; --------------------------

	db SANTRE, 12 ; vs WATER
	; ZAPDOS
	db 1, 1, THUNDER
	db 1, 2, ROOST
	db 1, 3, HURRICANE
	db 1, 4, THUNDER_WAVE
	; MVENUSAUR
	db 2, 1, FRENZY_PLANT
	db 2, 2, EARTHQUAKE
	db 2, 3, GUNK_SHOT
	db 2, 4, SLEEP_POWDER
	; MVICTREBEL
	db 3, 1, RAZOR_LEAF
	db 3, 2, EARTHQUAKE
	db 3, 3, LOVELY_KISS
	db 3, 4, GUNK_SHOT
	; MRAICHUX
	db 4, 1, ELECTRIC_TERRAIN
	db 4, 2, THUNDER
	db 4, 3, EARTHQUAKE
	db 4, 4, PETAL_DANCE
	; MRAICHUY
	db 5, 1, PETAL_DANCE
	db 5, 2, THUNDER
	db 5, 3, OUTRAGE
	db 5, 4, NO_MOVE
	; MDRAGONITE ; delta
	db 6, 1, THUNDER
	db 6, 2, MEGAHORN
	db 6, 3, LOVELY_KISS
	db 6, 4, PLAY_ROUGH
	db $FE

; --------------------------

	db SANTRE, 13 ; vs GRASS
	; MBEEDRILL
	db 1, 1, STICKY_WEB
	db 1, 2, MEGAHORN
	db 1, 3, GUNK_SHOT
	db 1, 4, AERIAL_ACE
	; MCHARZARDY
	db 2, 1, BLAST_BURN
	db 2, 2, HURRICANE
	db 2, 3, ROOST
	db 2, 4, CRUNCH
	; MPIDGEOT
	db 3, 1, HURRICANE
	db 3, 2, BLIZZARD
	db 3, 3, ROOST
	db 3, 4, HEAT_WAVE
	; ARTICUNO
	db 4, 1, BLIZZARD
	db 4, 2, BRAVE_BIRD
	db 4, 3, ROOST
	db 4, 4, MIRROR_COAT
	; JYNX
	db 5, 1, BLIZZARD
	db 5, 2, PSYCHO_CUT ; PPS PSYCHIC_M
	db 5, 3, LOVELY_KISS
	db 5, 4, DRAININGKISS
	; MOLTRES
	db 6, 1, FIRE_BLAST
	db 6, 2, BRAVE_BIRD
	db 6, 3, ROOST
	db 6, 4, HYPER_BEAM
	db $FE

; --------------------------

	db SANTRE, 14 ; vs ELECTRIC
	; MPINSIR ; delta
	db 1, 1, EARTHQUAKE
	db 1, 2, STONE_EDGE
	db 1, 3, MEGAHORN
	db 1, 4, EXPLOSION
	; MSTEELIX
	db 2, 1, EARTHQUAKE
	db 2, 2, HEAD_SMASH
	db 2, 3, BODY_PRESS
	db 2, 4, SELFDESTRUCT
	; ELECTIVIRE ; delta
	db 3, 1, EARTHQUAKE
	db 3, 2, CLOSE_COMBAT
	db 3, 3, FLAMETHROWER
	db 3, 4, HYPER_BEAM
	; ARTICUNO
	db 4, 1, BLIZZARD
	db 4, 2, WATER_PULSE
	db 4, 3, ROOST
	db 4, 4, MIRROR_COAT
	; RHYPERIOR
	db 5, 1, DRILL_RUN
	db 5, 2, STONE_EDGE
	db 5, 3, MEGAHORN
	db 5, 4, CLOSE_COMBAT
	; MDRAGONITE
	db 6, 1, EARTHQUAKE
	db 6, 2, BLIZZARD
	db 6, 3, FIRE_BLAST
	db 6, 4, HYPER_BEAM
	db $FE

; --------------------------

	db SANTRE, 15 ; vs PSYCHIC
	; MBEEDRILL ; delta
	db 1, 1, STICKY_WEB
	db 1, 2, GUNK_SHOT
	db 1, 3, MEGAHORN
	db 1, 4, DRILL_RUN
	; UMBREON
	db 2, 1, DARK_PULSE
	db 2, 2, REST
	db 2, 3, TOXIC
	db 2, 4, DOUBLE_TEAM
	; MGYARADOS
	db 3, 1, NIGHT_SLASH
	db 3, 2, HYDRO_PUMP
	db 3, 3, HYPER_BEAM
	db 3, 4, PLAY_ROUGH
	; MGENGAR
	db 4, 1, DARK_PULSE
	db 4, 2, CONFUSE_RAY
	db 4, 3, SHADOW_BALL
	db 4, 4, HYPNOSIS
	; MCHARZARDY ; delta
	db 5, 1, CRUNCH
	db 5, 2, BLAST_BURN
	db 5, 3, BRAVE_BIRD
	db 5, 4, OUTRAGE
	; NIDOKING ; delta
	db 6, 1, DRILL_RUN
	db 6, 2, GUNK_SHOT
	db 6, 3, MEGAHORN
	db 6, 4, SHADOW_BALL
	db $FE

; --------------------------

	db SANTRE, 16 ; vs ICE
	; MSTEELIX
	db 1, 1, STEALTH_ROCK
	db 1, 2, STONE_EDGE
	db 1, 3, GYRO_BALL
	db 1, 4, BODY_PRESS
	; MARODACTYL
	db 2, 1, STEALTH_ROCK
	db 2, 2, HEAD_SMASH
	db 2, 3, EARTHQUAKE
	db 2, 4, IRON_TAIL
	; MGYARADOS ; delta
	db 3, 1, FIRE_BLAST
	db 3, 2, THUNDERBOLT
	db 3, 3, SUNNY_DAY
	db 3, 4, STONE_EDGE
	; MACHAMP
	db 4, 1, STONE_EDGE
	db 4, 2, EARTHQUAKE
	db 4, 3, CLOSE_COMBAT
	db 4, 4, COUNTER
	; MPINSIR ; delta
	db 5, 1, CLOSE_COMBAT
	db 5, 2, STONE_EDGE
	db 5, 3, EARTHQUAKE
	db 5, 4, MEGAHORN
	; MSCIZOR
	db 6, 1, METEOR_MASH
	db 6, 2, LEECH_LIFE
	db 6, 3, BULLET_PUNCH
	db 6, 4, CLOSE_COMBAT
	db $FE

; --------------------------

	db SANTRE, 17 ; vs DRAGON
	; SYLVEON
	db 1, 1, DRAININGKISS
	db 1, 2, SUBSTITUTE
	db 1, 3, REFLECT
	db 1, 4, DOUBLE_TEAM
	; ARTICUNO
	db 2, 1, BLIZZARD
	db 2, 2, BRAVE_BIRD
	db 2, 3, ROOST
	db 2, 4, HYPER_BEAM
	; MSTARMIE ; delta
	db 3, 1, BLIZZARD
	db 3, 2, MOONBLAST
	db 3, 3, RECOVER
	db 3, 4, EARTHQUAKE
	; MCLEFABLE
	db 4, 1, MOONBLAST
	db 4, 2, MINIMIZE
	db 4, 3, SUBSTITUTE
	db 4, 4, THUNDER_WAVE
	; VILEPLUME ; delta
	db 5, 1, MOONBLAST
	db 5, 2, GUNK_SHOT
	db 5, 3, SYNTHESIS
	db 5, 4, SLEEP_POWDER
	; MDRAGONITE
	db 6, 1, MOONBLAST
	db 6, 2, OUTRAGE
	db 6, 3, BLIZZARD
	db 6, 4, EXTREMESPEED
	db $FE

; --------------------------

	db SANTRE, 18 ; vs DARK
	; MBEEDRILL
	db 1, 1, MEGAHORN
	db 1, 2, STICKY_WEB
	db 1, 3, GUNK_SHOT
	db 1, 4, DRILL_RUN
	; MACHAMP
	db 2, 1, STONE_EDGE
	db 2, 2, EARTHQUAKE
	db 2, 3, CLOSE_COMBAT
	db 2, 4, COUNTER
	; MPINSIR
	db 3, 1, AIR_CUTTER
	db 3, 2, STONE_EDGE
	db 3, 3, MEGAHORN
	db 3, 4, ROOST
	; SYLVEON
	db 4, 1, DRAININGKISS
	db 4, 2, SUBSTITUTE
	db 4, 3, REFLECT
	db 4, 4, DOUBLE_TEAM
	; MCLEFABLE
	db 5, 1, MOONBLAST
	db 5, 2, MINIMIZE
	db 5, 3, SUBSTITUTE
	db 5, 4, THUNDER_WAVE
	; CROBAT ; delta
	db 6, 1, LEECH_LIFE
	db 6, 2, GUNK_SHOT
	db 6, 3, CONFUSE_RAY
	db 6, 4, AIR_SLASH
	db $FE

; --------------------------

	db SANTRE, 19 ; vs FAIRY
	; MBEEDRILL
	db 1, 1, STEEL_WING
	db 1, 2, STICKY_WEB
	db 1, 3, GUNK_SHOT
	db 1, 4, DRILL_RUN
	; MSTEELIX
	db 2, 1, STONE_EDGE
	db 2, 2, EARTHQUAKE
	db 2, 3, IRON_TAIL
	db 2, 4, SELFDESTRUCT
	; MVICTREBEL
	db 3, 1, GUNK_SHOT
	db 3, 2, RAZOR_LEAF
	db 3, 3, LOVELY_KISS
	db 3, 4, SYNTHESIS
	; MSCIZOR
	db 4, 1, METEOR_MASH
	db 4, 2, AIR_SLASH
	db 4, 3, SUBSTITUTE
	db 4, 4, SWORDS_DANCE
	; MVENUSAUR
	db 5, 1, SYNTHESIS
	db 5, 2, SLEEP_POWDER
	db 5, 3, LEECH_SEED
	db 5, 4, GUNK_SHOT
	; MSTARMIE ; delta
	db 6, 1, METEOR_MASH
	db 6, 2, PSYCHO_CUT
	db 6, 3, BLIZZARD
	db 6, 4, RECOVER
	db $FE

; ===============================================================

	db YOTTRO, 1
	; MVENUSAUR
	db 1, 1, SYNTHESIS
	db 1, 2, SUNNY_DAY
	db 1, 3, RAZOR_LEAF
	db 1, 4, WEATHER_BALL
	; MSTARMIE
	db 2, 1, RAPID_SPIN
	db 2, 2, CLOSE_COMBAT
	db 2, 3, PSYCHO_CUT
	db 2, 4, RECOVER
	; MGENGAR
	db 3, 1, REST
	db 3, 2, TOXIC
	db 3, 3, DARK_PULSE
	db 3, 4, CURSE
	; MBLASTOISE
	db 4, 1, RAPID_SPIN
	db 4, 2, HYDRO_CANNON
	db 4, 3, BLIZZARD
	db 4, 4, REST
	; MSTEELIX
	db 5, 1, DRAGON_DANCE
	db 5, 2, STONE_EDGE
	db 5, 3, EARTHQUAKE
	db 5, 4, REST
	; MALAKAZAM
	db 6, 1, MOONBLAST
	db 6, 2, PSYCHO_CUT
	db 6, 3, ENERGY_BALL
	db 6, 4, RECOVER
	db $FE

; ===============================================================

	db GONQUE, 1
	; MBEEDRILL - BUG/POISON
	db 1, 1, MEGAHORN
	db 1, 2, STICKY_WEB
	db 1, 3, GUNK_SHOT
	db 1, 4, DRILL_RUN
	; MSLOWBRO - WATER/PSYCHIC
	db 2, 1, HYDRO_PUMP
	db 2, 2, PSYCHIC_M
	db 2, 3, THUNDER_WAVE
	db 2, 4, AMNESIA
	; MCHARZARDX ; delta - ELECTRIC/DARK
	db 3, 1, THUNDERPUNCH
	db 3, 2, EARTHQUAKE
	db 3, 3, BLAST_BURN
	db 3, 4, OUTRAGE
	; MSTARMIE ; delta - ICE/STEEL
	db 4, 1, RECOVER
	db 4, 2, HYDRO_PUMP
	db 4, 3, METEOR_MASH
	db 4, 4, BLIZZARD
	; MPIDGEOT - NORMAL/FLYING
	db 5, 1, BRAVE_BIRD
	db 5, 2, THUNDER
	db 5, 3, HEAT_WAVE
	db 5, 4, DOUBLE_EDGE
	; MPINSIR ; delta - GROUND/FIGHTING
	db 6, 1, CLOSE_COMBAT
	db 6, 2, EARTHQUAKE
	db 6, 3, AIR_SLASH
	db 6, 4, MEGAHORN
	db $FE

; --------------------------

	db GONQUE, 2
	; MBLASTOISE ; delta - ROCK/STEEL
	db 1, 1, COUNTER
	db 1, 2, MIRROR_COAT
	db 1, 3, FLASH_CANNON
	db 1, 4, ROCK_SLIDE
	; MVENUSAUR ; delta - ICE/GHOST
	db 2, 1, SLEEP_POWDER
	db 2, 2, CURSE
	db 2, 3, LEECH_SEED
	db 2, 4, FRENZY_PLANT
	; MCHARZARDX - FIRE/DRAGON
	db 3, 1, FIRE_BLAST
	db 3, 2, OUTRAGE
	db 3, 3, STEEL_WING
	db 3, 4, AIR_SLASH
	; MVICTREBEL - GRASS/POISON
	db 4, 1, GUNK_SHOT
	db 4, 2, EARTHQUAKE
	db 4, 3, SHADOW_BALL
	db 4, 4, RAZOR_LEAF
	; MCLEFABLE - FAIRY/FLYING
	db 5, 1, MOONBLAST
	db 5, 2, REFLECT
	db 5, 3, RECOVER
	db 5, 4, PSYCHIC_M
	; MDRAGONITE ; delta - ELECTRIC/GRASS
	db 6, 1, THUNDER
	db 6, 2, HYDRO_PUMP
	db 6, 3, EXTREMESPEED
	db 6, 4, FREEZE_DRY
	db $FE

; ===============================================================

	db ROKUSEI, 1
	; MGENGAR
	db 1, 1, DARK_PULSE
	db 1, 2, CONFUSE_RAY
	db 1, 3, SUBSTITUTE
	db 1, 4, DOUBLE_TEAM
	; MVENUSAUR ; delta
	db 2, 1, SLEEP_POWDER
	db 2, 2, CURSE
	db 2, 3, SYNTHESIS
	db 2, 4, FRENZY_PLANT
	; BLISSEY
	db 3, 1, MINIMIZE
	db 3, 2, SOFTBOILED
	db 3, 3, BLIZZARD
	db 3, 4, THUNDER_WAVE
	; MSTEELIX
	db 4, 1, HEAD_SMASH
	db 4, 2, EARTHQUAKE
	db 4, 3, IRON_TAIL
	db 4, 4, REST
	; MDRAGONITE
	db 5, 1, OUTRAGE
	db 5, 2, FIRE_BLAST
	db 5, 3, HYDRO_PUMP
	db 5, 4, EXTREMESPEED
	; MSTARMIE
	db 6, 1, METEOR_MASH
	db 6, 2, CLOSE_COMBAT
	db 6, 3, RECOVER
	db 6, 4, PSYCHO_CUT
	db $FE

; ===============================================================

	db NANETTE, 1
	; ARTICUNO
	db 1, 1, BRAVE_BIRD
	db 1, 2, ROOST
	db 1, 3, BLIZZARD
	db 1, 4, MIRROR_COAT
	; ZAPDOS
	db 2, 1, BRAVE_BIRD
	db 2, 2, ROOST
	db 2, 3, THUNDER
	db 2, 4, THUNDER_WAVE
	; MOLTRES
	db 3, 1, BRAVE_BIRD
	db 3, 2, ROOST
	db 3, 3, FIRE_BLAST
	db 3, 4, HYPER_BEAM
	; MVENUSAUR ; delta
	db 4, 1, FRENZY_PLANT
	db 4, 2, CURSE
	db 4, 3, REST
	db 4, 4, SLEEP_POWDER
	; MCHARZARDY ; delta
	db 5, 1, BLAST_BURN
	db 5, 2, CRUNCH
	db 5, 3, THUNDERPUNCH
	db 5, 4, DRAGON_CLAW
	; MBLASTOISE ; delta
	db 6, 1, HYDRO_CANNON
	db 6, 2, FLASH_CANNON
	db 6, 3, BULLET_SEED
	db 6, 4, BODY_PRESS
	db $FE

; ===============================================================

	db SUUJERO, 1
	; VENUSTOISE ; delta - GHOST/ROCK
	db 1, 1, SYNTHESIS
	db 1, 2, HYDRO_CANNON
	db 1, 3, FRENZY_PLANT
	db 1, 4, ROCK_SLIDE
	; MEW ; delta - FIRE/WATER
	db 2, 1, SOFTBOILED
	db 2, 2, BONEMERANG
	db 2, 3, CRABHAMMER
	db 2, 4, BLAST_BURN
	; MMEWTWOX ; delta - FIRE/ELECTRIC
	db 3, 1, RECOVER
	db 3, 2, STONE_EDGE
	db 3, 3, EARTHQUAKE
	db 3, 4, CLOSE_COMBAT
	; MMEWTWOY ; delta - FIRE/ELECTRIC
	db 4, 1, RECOVER
	db 4, 2, PSYSTRIKE
	db 4, 3, FIRE_BLAST
	db 4, 4, THUNDER
	; THU_FI_ZER
	db 5, 1, TRI_ATTACK
	db 5, 2, ROOST
	db 5, 3, BRAVE_BIRD
	db 5, 4, WHIRLWIND
	; MELMETAL
	db 6, 1, THUNDER_WAVE
	db 6, 2, DRAIN_PUNCH
	db 6, 3, DOUBLE_IRON_BASH
	db 6, 4, BODY_PRESS
	db $FE

; ===============================================================

	db PINK, 2
	; GYARADOS
	db 1, 1, WATER_PULSE
	db 1, 2, BODY_SLAM
	db 1, 3, DRAGON_DANCE
	db 1, 4, THUNDERBOLT
	; KANGASKHAN
	db 2, 1, FOCUS_ENERGY
	db 2, 2, IRON_TAIL
	db 2, 3, DRAIN_PUNCH
	db 2, 4, DIZZY_PUNCH
	; PRIMEAPE
	db 3, 1, KARATE_CHOP
	db 3, 2, AERIAL_ACE
	db 3, 3, ROCK_TOMB
	db 3, 4, DOUBLE_EDGE
	; DRAGONAIR
	db 4, 1, WATER_PULSE
	db 4, 2, FLAMETHROWER
	db 4, 3, DRAGON_RAGE
	db 4, 4, DRAGON_DANCE
	; CLEFAIRY
	db 5, 1, METRONOME
	db 5, 2, MAGICAL_LEAF
	db 5, 3, AIR_SLASH
	db 5, 4, PLAY_ROUGH
	db $FE

; --------------------------

	db PINK, 3
	; GYARADOS
	db 1, 1, EARTHQUAKE
	db 1, 2, STONE_EDGE
	db 1, 3, HYDRO_PUMP
	db 1, 4, HYPER_BEAM
	; KANGASKHAN
	db 2, 1, SHADOW_CLAW
	db 2, 2, SING
	db 2, 3, COUNTER
	db 2, 4, BODY_SLAM
	; ANNIHILAPE
	db 3, 1, SHADOW_CLAW
	db 3, 2, CLOSE_COMBAT
	db 3, 3, STONE_EDGE
	db 3, 4, EARTHQUAKE
	; BLISSEY
	db 4, 1, SOFTBOILED
	db 4, 2, REFLECT
	db 4, 3, THUNDER_WAVE
	db 4, 4, BLIZZARD
	; DRAGONITE
	db 5, 1, OUTRAGE
	db 5, 2, STEEL_WING
	db 5, 3, MEGAHORN
	db 5, 4, AIR_SLASH
	; CLEFABLE
	db 6, 1, PLAY_ROUGH ; PPS MOONBLAST
	db 6, 2, METEOR_MASH
	db 6, 3, ICE_BEAM
	db 6, 4, PSYCHIC_M
	db $FE

; --------------------------

	db PINK, 4
	; MGYARADOS
	db 1, 1, EARTHQUAKE
	db 1, 2, STONE_EDGE
	db 1, 3, HYDRO_PUMP
	db 1, 4, DARK_PULSE
	; MKANGASKAN
	db 2, 1, SHADOW_CLAW
	db 2, 2, ICICLE_SPEAR
	db 2, 3, BONEMERANG
	db 2, 4, BODY_SLAM
	; ANNIHILAPE
	db 3, 1, SHADOW_CLAW
	db 3, 2, CLOSE_COMBAT
	db 3, 3, STONE_EDGE
	db 3, 4, EARTHQUAKE
	; BLISSEY
	db 4, 1, SOFTBOILED
	db 4, 2, REFLECT
	db 4, 3, THUNDER_WAVE
	db 4, 4, BLIZZARD
	; MDRAGONITE
	db 5, 1, OUTRAGE
	db 5, 2, FIRE_BLAST
	db 5, 3, ROOST
	db 5, 4, LOVELY_KISS
	; MCLEFABLE
	db 6, 1, DRAININGKISS
	db 6, 2, THUNDER_WAVE
	db 6, 3, MINIMIZE
	db 6, 4, SUBSTITUTE
	db $FE

; --------------------------

	db PINK, 5
	; MGYARADOS
	db 1, 1, EARTHQUAKE
	db 1, 2, STONE_EDGE
	db 1, 3, HYDRO_PUMP
	db 1, 4, DARK_PULSE
	; MKANGASKAN
	db 2, 1, SHADOW_CLAW
	db 2, 2, ICICLE_SPEAR
	db 2, 3, BONEMERANG
	db 2, 4, BODY_SLAM
	; ANNIHILAPE
	db 3, 1, SHADOW_CLAW
	db 3, 2, CLOSE_COMBAT
	db 3, 3, STONE_EDGE
	db 3, 4, EARTHQUAKE
	; MDRAGONITE
	db 4, 1, OUTRAGE
	db 4, 2, FIRE_BLAST
	db 4, 3, ROOST
	db 4, 4, LOVELY_KISS
	; MELMETAL
	db 5, 1, DOUBLE_IRON_BASH
	db 5, 2, SELFDESTRUCT
	db 5, 3, BODY_PRESS
	db 5, 4, DRAIN_PUNCH
	; MCLEFABLE
	db 6, 1, DRAININGKISS
	db 6, 2, THUNDER_WAVE
	db 6, 3, MINIMIZE
	db 6, 4, SUBSTITUTE
	db $FE

; ===============================================================

	db YOUNG_OAK, 1
	; TAUROS
	db 1, 1, EARTHQUAKE
	db 1, 2, BODY_SLAM
	db 1, 3, BLIZZARD
	db 1, 4, HYPER_BEAM
	; MGYARADOS
	db 2, 1, NIGHT_SLASH
	db 2, 2, HYDRO_PUMP
	db 2, 3, GUNK_SHOT
	db 2, 4, EARTHQUAKE
	; MMEWTWOX
	db 3, 1, CLOSE_COMBAT
	db 3, 2, STONE_EDGE
	db 3, 3, LEECH_LIFE
	db 3, 4, SHADOW_CLAW ; PPS PSYCHO_CUT
	; MMEWTWOY
	db 4, 1, PSYSTRIKE
	db 4, 2, FIRE_BLAST
	db 4, 3, MOONBLAST
	db 4, 4, THUNDERBOLT
	; VENUSTOISE
	db 5, 1, FRENZY_PLANT
	db 5, 2, HYDRO_CANNON
	db 5, 3, COUNTER
	db 5, 4, MIRROR_COAT
	; THU_FI_ZER
	db 6, 1, TRI_ATTACK
	db 6, 2, ROOST
	db 6, 3, FREEZE_DRY
	db 6, 4, BRAVE_BIRD
	db $FE

; ===============================================================

	db CHALLENGER, 4
	; SNORLAX
	db 1, 1, BODY_SLAM
	db 1, 2, REFLECT
	db 1, 3, REST
	db 1, 4, SELFDESTRUCT
	; BLISSEY
	db 2, 1, THUNDERBOLT
	db 2, 2, ICE_BEAM
	db 2, 3, SOFTBOILED
	db 2, 4, THUNDER_WAVE
	; TAUROS
	db 3, 1, BODY_SLAM
	db 3, 2, HYPER_BEAM
	db 3, 3, BLIZZARD
	db 3, 4, EARTHQUAKE
	; ALAKAZAM
	db 4, 1, PSYCHIC_M
	db 4, 2, REFLECT
	db 4, 3, THUNDER_WAVE
	db 4, 4, RECOVER
	; RHYPERIOR
	db 5, 1, STONE_EDGE
	db 5, 2, EARTHQUAKE
	db 5, 3, BODY_SLAM
	db 5, 4, SUBSTITUTE
	; MEW
	db 6, 1, SWORDS_DANCE
	db 6, 2, EARTHQUAKE
	db 6, 3, HYPER_BEAM
	db 6, 4, SOFTBOILED
	db $FE

; --------------------------

	db CHALLENGER, 5
	; MSTARMIE
	db 1, 1, METEOR_MASH
	db 1, 2, PSYCHO_CUT
	db 1, 3, BLIZZARD
	db 1, 4, RECOVER
	; MGENGAR
	db 2, 1, DARK_PULSE
	db 2, 2, CONFUSE_RAY
	db 2, 3, SUBSTITUTE
	db 2, 4, DOUBLE_TEAM
	; MSCIZOR
	db 3, 1, METEOR_MASH
	db 3, 2, AIR_SLASH
	db 3, 3, LEECH_LIFE
	db 3, 4, SWORDS_DANCE
	; MGYARADOS
	db 4, 1, NIGHT_SLASH
	db 4, 2, HYDRO_PUMP
	db 4, 3, GUNK_SHOT
	db 4, 4, EARTHQUAKE
	; MPINSIR
	db 5, 1, AIR_CUTTER
	db 5, 2, STONE_EDGE
	db 5, 3, MEGAHORN
	db 5, 4, ROOST
	; MDRAGONITE
	db 6, 1, OUTRAGE
	db 6, 2, FIRE_BLAST
	db 6, 3, HYDRO_PUMP
	db 6, 4, EXTREMESPEED
	db $FE

; --------------------------

	db CHALLENGER, 6
	; MALAKAZAM
	db 1, 1, PSYCHIC_M
	db 1, 2, DOUBLE_TEAM
	db 1, 3, RECOVER
	db 1, 4, ZAP_CANNON
	; THU_FI_ZER
	db 2, 1, WATER_PULSE
	db 2, 2, BLIZZARD
	db 2, 3, DOUBLE_TEAM
	db 2, 4, ROOST
	; VENUSTOISE
	db 3, 1, SLEEP_POWDER
	db 3, 2, SYNTHESIS
	db 3, 3, ZEN_HEADBUTT
	db 3, 4, WATER_PULSE
	; MARODACTYL
	db 4, 1, ROCK_SLIDE
	db 4, 2, BODY_SLAM
	db 4, 3, SUPERSONIC
	db 4, 4, NO_MOVE
	; MMEWTWOY
	db 5, 1, BLIZZARD
	db 5, 2, ZAP_CANNON
	db 5, 3, RECOVER
	db 5, 4, ZEN_HEADBUTT
	; MMEWTWOX
	db 6, 1, DOUBLE_TEAM
	db 6, 2, ROCK_SLIDE
	db 6, 3, RECOVER
	db 6, 4, ZEN_HEADBUTT
	db $FE

; ===============================================================

	db PROTON, 1
	; CROBAT
	db 1, 1, SUPER_FANG
	db 1, 2, GUNK_SHOT
	db 1, 3, CONFUSE_RAY
	db 1, 4, ROOST
	; WEEZING
	db 2, 1, EXPLOSION
	db 2, 2, FIRE_BLAST
	db 2, 3, THUNDERBOLT
	db 2, 4, GUNK_SHOT
	db $FE

; --------------------------

	db PROTON, 2
	; VENOMOTH
	db 1, 1, STICKY_WEB
	db 1, 2, SLEEP_POWDER
	db 1, 3, LEECH_LIFE
	db 1, 4, PSYCHIC_M
	; TENTACRUEL
	db 2, 1, HYDRO_PUMP
	db 2, 2, TOXIC_SPIKES
	db 2, 3, GIGA_DRAIN
	db 2, 4, BLIZZARD
	; NIDOKING
	db 3, 1, DRILL_RUN
	db 3, 2, MEGAHORN
	db 3, 3, GUNK_SHOT
	db 3, 4, THUNDERBOLT
	; WEEZING
	db 4, 1, EXPLOSION
	db 4, 2, TOXIC
	db 4, 3, GUNK_SHOT
	db 4, 4, SHADOW_BALL
	; CROBAT
	db 5, 1, BRAVE_BIRD
	db 5, 2, GUNK_SHOT
	db 5, 3, ROOST
	db 5, 4, CONFUSE_RAY
	; MBEEDRILL
	db 6, 1, DRILL_RUN
	db 6, 2, MEGAHORN
	db 6, 3, GUNK_SHOT
	db 6, 4, STEEL_WING
	db $FE

; ===============================================================

	db PETREL, 1
	; KOFFING
	db 1, 1, EXPLOSION
	db 1, 2, NO_MOVE
	db 1, 3, NO_MOVE
	db 1, 4, NO_MOVE
	; KOFFING
	db 2, 1, EXPLOSION
	db 2, 2, NO_MOVE
	db 2, 3, NO_MOVE
	db 2, 4, NO_MOVE
	; KOFFING
	db 3, 1, EXPLOSION
	db 3, 2, NO_MOVE
	db 3, 3, NO_MOVE
	db 3, 4, NO_MOVE
	; KOFFING
	db 4, 1, EXPLOSION
	db 4, 2, NO_MOVE
	db 4, 3, NO_MOVE
	db 4, 4, NO_MOVE
	; KOFFING
	db 5, 1, EXPLOSION
	db 5, 2, NO_MOVE
	db 5, 3, NO_MOVE
	db 5, 4, NO_MOVE
	; WEEZING
	db 6, 1, EXPLOSION
	db 6, 2, SHADOW_BALL
	db 6, 3, SLUDGE_WAVE
	db 6, 4, HEAT_WAVE
	db $FE

; --------------------------

	db PETREL, 2
	; WEEZING
	db 1, 1, EXPLOSION
	db 1, 2, SHADOW_BALL
	db 1, 3, GUNK_SHOT
	db 1, 4, FIRE_BLAST
	; WEEZING
	db 2, 1, EXPLOSION
	db 2, 2, SHADOW_BALL
	db 2, 3, GUNK_SHOT
	db 2, 4, FIRE_BLAST
	; WEEZING
	db 3, 1, EXPLOSION
	db 3, 2, SHADOW_BALL
	db 3, 3, GUNK_SHOT
	db 3, 4, FIRE_BLAST
	; WEEZING
	db 4, 1, EXPLOSION
	db 4, 2, SHADOW_BALL
	db 4, 3, GUNK_SHOT
	db 4, 4, FIRE_BLAST
	; RATICATE
	db 5, 1, SUPER_FANG
	db 5, 2, HYPER_FANG
	db 5, 3, TOXIC
	db 5, 4, SURF
	; WEEZING
	db 6, 1, EXPLOSION
	db 6, 2, SHADOW_BALL
	db 6, 3, GUNK_SHOT
	db 6, 4, FIRE_BLAST
	db $FE

; ===============================================================

	db ARIANA, 1
	; ARBOK
	db 1, 1, GUNK_SHOT
	db 1, 2, WRAP
	db 1, 3, GLARE
	db 1, 4, EARTHQUAKE
	; VILEPLUME
	db 2, 1, SLEEP_POWDER
	db 2, 2, MOONBLAST
	db 2, 3, SYNTHESIS
	db 2, 4, RAZOR_LEAF
	; MUK
	db 3, 1, BODY_SLAM
	db 3, 2, GUNK_SHOT
	db 3, 3, MINIMIZE
	db 3, 4, SHADOW_PUNCH
	; UMBREON
	db 4, 1, CONFUSE_RAY
	db 4, 2, DARK_PULSE
	db 4, 3, TOXIC
	db 4, 4, REST
	db $FE

; --------------------------

	db ARIANA, 2
	; ARBOK
	db 1, 1, GUNK_SHOT
	db 1, 2, WRAP
	db 1, 3, GLARE
	db 1, 4, EARTHQUAKE
	; VILEPLUME
	db 2, 1, SLEEP_POWDER
	db 2, 2, MOONBLAST
	db 2, 3, SYNTHESIS
	db 2, 4, RAZOR_LEAF
	; LICKILICKY
	db 3, 1, COUNTER
	db 3, 2, BODY_SLAM
	db 3, 3, BLIZZARD
	db 3, 4, THUNDERBOLT
	; MUK
	db 4, 1, BODY_SLAM
	db 4, 2, GUNK_SHOT
	db 4, 3, MINIMIZE
	db 4, 4, SHADOW_PUNCH
	; UMBREON
	db 5, 1, CONFUSE_RAY
	db 5, 2, DARK_PULSE
	db 5, 3, TOXIC
	db 5, 4, REST
	; MVICTREBEL
	db 6, 1, GUNK_SHOT
	db 6, 2, RAZOR_LEAF
	db 6, 3, LOVELY_KISS
	db 6, 4, SYNTHESIS
	db $FE

; ===============================================================

	db ARCHER, 1
	; PARASECT
	db 1, 1, SPORE
	db 1, 2, LEECH_LIFE
	db 1, 3, LEECH_SEED
	db 1, 4, STICKY_WEB
	; WEEZING
	db 2, 1, EXPLOSION
	db 2, 2, SLUDGE_WAVE
	db 2, 3, FIRE_BLAST
	db 2, 4, WILL_O_WISP
	; ELECTRODE
	db 3, 1, EXPLOSION
	db 3, 2, SUBSTITUTE
	db 3, 3, THUNDER_WAVE
	db 3, 4, THUNDERBOLT
	; CROBAT
	db 4, 1, CONFUSE_RAY
	db 4, 2, TOXIC
	db 4, 3, BRAVE_BIRD
	db 4, 4, GUNK_SHOT
	; MAGMORTAR
	db 5, 1, FIRE_BLAST
	db 5, 2, FEINT_ATTACK
	db 5, 3, BULLET_SEED
	db 5, 4, KARATE_CHOP
	; UMBREON
	db 6, 1, REST
	db 6, 2, DARK_PULSE
	db 6, 3, PSYCHIC_M
	db 6, 4, IRON_TAIL
	db $FE

; --------------------------

	db ARCHER, 2
	; PARASECT
	db 1, 1, SPORE
	db 1, 2, LEECH_LIFE
	db 1, 3, LEECH_SEED
	db 1, 4, STICKY_WEB
	; PERSIAN
	db 2, 1, SLASH
	db 2, 2, SHADOW_CLAW
	db 2, 3, DREAM_EATER
	db 2, 4, HYPNOSIS
	; RHYPERIOR
	db 3, 1, DRILL_RUN
	db 3, 2, STONE_EDGE
	db 3, 3, MEGAHORN
	db 3, 4, CLOSE_COMBAT
	; MAGMORTAR
	db 4, 1, FIRE_BLAST
	db 4, 2, THUNDERBOLT
	db 4, 3, BULLET_SEED
	db 4, 4, KARATE_CHOP
	; UMBREON
	db 5, 1, PSYCHIC_M
	db 5, 2, CONFUSE_RAY
	db 5, 3, DARK_PULSE
	db 5, 4, REST
	; MGENGAR
	db 6, 1, DARK_PULSE
	db 6, 2, DRAININGKISS
	db 6, 3, THUNDERBOLT
	db 6, 4, ENERGY_BALL
	db $FE

; ===============================================================

	db ORM, 1
	; WEEZING
	db 1, 1, EXPLOSION
	db 1, 2, TOXIC_SPIKES
	db 1, 3, FIRE_BLAST
	db 1, 4, WILL_O_WISP
	; PARASECT
	db 2, 1, SPORE
	db 2, 2, LEECH_LIFE
	db 2, 3, LEECH_SEED
	db 2, 4, STICKY_WEB
	; VENOMOTH
	db 3, 1, QUIVER_DANCE
	db 3, 2, PSYCHIC_M
	db 3, 3, GIGA_DRAIN
	db 3, 4, REFLECT
	; KLEAVOR
	db 4, 1, AIR_CUTTER
	db 4, 2, STONE_EDGE
	db 4, 3, SWORDS_DANCE
	db 4, 4, X_SCISSOR
	; TANGROWTH
	db 5, 1, GIGA_DRAIN
	db 5, 2, BODY_SLAM
	db 5, 3, ROCK_SLIDE
	db 5, 4, GRASSY_TERRAIN
	; EXEGGUTOR
	db 6, 1, LEECH_SEED
	db 6, 2, SYNTHESIS
	db 6, 3, SLEEP_POWDER
	db 6, 4, DREAM_EATER
	db $FE

; --------------------------

	db ORM, 2
	; PARASECT
	db 1, 1, SPORE
	db 1, 2, LEECH_LIFE
	db 1, 3, LEECH_SEED
	db 1, 4, STICKY_WEB
	; WEEZING
	db 2, 1, EXPLOSION
	db 2, 2, TOXIC_SPIKES
	db 2, 3, FIRE_BLAST
	db 2, 4, WILL_O_WISP
	; KLEAVOR
	db 3, 1, AIR_CUTTER
	db 3, 2, STONE_EDGE
	db 3, 3, SWORDS_DANCE
	db 3, 4, X_SCISSOR
	; TANGROWTH
	db 4, 1, GIGA_DRAIN
	db 4, 2, BODY_SLAM
	db 4, 3, ROCK_SLIDE
	db 4, 4, EARTHQUAKE
	; EXEGGUTOR
	db 5, 1, SELFDESTRUCT
	db 5, 2, SYNTHESIS
	db 5, 3, SLEEP_POWDER
	db 5, 4, DREAM_EATER
	; MPINSIR
	db 6, 1, AIR_CUTTER
	db 6, 2, STONE_EDGE
	db 6, 3, MEGAHORN
	db 6, 4, ROOST
	db $FE

; ===============================================================

	db CARR, 1
	; ELECTRODE
	db 1, 1, EXPLOSION
	db 1, 2, SUBSTITUTE
	db 1, 3, THUNDER_WAVE
	db 1, 4, THUNDERBOLT
	; STEELIX
	db 2, 1, STONE_EDGE
	db 2, 2, EARTHQUAKE
	db 2, 3, IRON_TAIL
	db 2, 4, SELFDESTRUCT
	; SCIZOR
	db 3, 1, BULLET_PUNCH
	db 3, 2, AIR_SLASH
	db 3, 3, LEECH_LIFE
	db 3, 4, ROOST
	; MAGNEZONE
	db 4, 1, THUNDER_WAVE
	db 4, 2, THUNDERBOLT
	db 4, 3, BODY_PRESS
	db 4, 4, FLASH_CANNON
	; RHYPERIOR
	db 5, 1, DRILL_RUN
	db 5, 2, STONE_EDGE
	db 5, 3, MEGAHORN
	db 5, 4, CLOSE_COMBAT
	; FEAROW
	db 6, 1, DRILL_RUN
	db 6, 2, BRAVE_BIRD
	db 6, 3, ROOST
	db 6, 4, HYPER_BEAM
	db $FE

; --------------------------

	db CARR, 2
	; ELECTRODE
	db 1, 1, EXPLOSION
	db 1, 2, SUBSTITUTE
	db 1, 3, THUNDER_WAVE
	db 1, 4, THUNDERBOLT
	; SCIZOR
	db 2, 1, METEOR_MASH
	db 2, 2, AIR_SLASH
	db 2, 3, LEECH_LIFE
	db 2, 4, SWORDS_DANCE
	; MAGNEZONE
	db 3, 1, THUNDER_WAVE
	db 3, 2, THUNDERBOLT
	db 3, 3, EXPLOSION
	db 3, 4, FLASH_CANNON
	; RHYPERIOR
	db 4, 1, DRILL_RUN
	db 4, 2, STONE_EDGE
	db 4, 3, MEGAHORN
	db 4, 4, CLOSE_COMBAT
	; FEAROW
	db 5, 1, DRILL_RUN
	db 5, 2, BRAVE_BIRD
	db 5, 3, ROOST
	db 5, 4, STEEL_WING
	; MSTEELIX
	db 6, 1, STONE_EDGE
	db 6, 2, EARTHQUAKE
	db 6, 3, IRON_TAIL
	db 6, 4, DRAGON_DANCE
	db $FE

; ===============================================================

	db SIRD, 1
	; PERSIAN
	db 1, 1, SLASH
	db 1, 2, NIGHT_SLASH
	db 1, 3, PLAY_ROUGH
	db 1, 4, IRON_TAIL
	; STARMIE
	db 2, 1, MINIMIZE
	db 2, 2, RECOVER
	db 2, 3, PSYCHO_CUT ; PPS PSYCHIC_M
	db 2, 4, HYDRO_PUMP
	; GENGAR
	db 3, 1, PERISH_SONG
	db 3, 2, HYPNOSIS
	db 3, 3, CURSE
	db 3, 4, TOXIC_SPIKES
	; UMBREON
	db 4, 1, CONFUSE_RAY
	db 4, 2, DOUBLE_TEAM
	db 4, 3, REST
	db 4, 4, DARK_PULSE
	; ALAKAZAM
	db 5, 1, PSYCHO_CUT ; PPS PSYCHIC_M
	db 5, 2, THUNDER_WAVE
	db 5, 3, RECOVER
	db 5, 4, REFLECT
	; ANNIHILAPE
	db 6, 1, CLOSE_COMBAT
	db 6, 2, SHADOW_CLAW
	db 6, 3, EARTHQUAKE
	db 6, 4, STONE_EDGE
	db $FE

; --------------------------

	db SIRD, 2
	; PERSIAN
	db 1, 1, SLASH
	db 1, 2, NIGHT_SLASH
	db 1, 3, PLAY_ROUGH
	db 1, 4, IRON_TAIL
	; GENGAR
	db 2, 1, PERISH_SONG
	db 2, 2, HYPNOSIS
	db 2, 3, CURSE
	db 2, 4, TOXIC_SPIKES
	; UMBREON
	db 3, 1, CONFUSE_RAY
	db 3, 2, DOUBLE_TEAM
	db 3, 3, REST
	db 3, 4, DARK_PULSE
	; ALAKAZAM
	db 4, 1, PSYCHO_CUT ; PPS PSYCHIC_M
	db 4, 2, THUNDER_WAVE
	db 4, 3, RECOVER
	db 4, 4, REFLECT
	; ANNIHILAPE
	db 5, 1, CLOSE_COMBAT
	db 5, 2, SHADOW_CLAW
	db 5, 3, EARTHQUAKE
	db 5, 4, STONE_EDGE
	; MSTARMIE
	db 6, 1, MINIMIZE
	db 6, 2, RECOVER
	db 6, 3, CLOSE_COMBAT
	db 6, 4, METEOR_MASH
	db $FE

; ===============================================================

; ===============================================================

; ===============================================================

	db -1 ; end
