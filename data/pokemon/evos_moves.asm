; See constants/pokemon_data_constants.asm
; The max number of evolutions per monster is MAX_EVOLUTIONS

EvosMovesPointerTable:
	table_width 2, EvosMovesPointerTable
	dw BulbasaurEvosMoves
	dw IvysaurEvosMoves
	dw VenusaurEvosMoves
	dw MVenusaurEvosMoves
	dw CharmanderEvosMoves
	dw CharmeleonEvosMoves
	dw CharizardEvosMoves
	dw MCharzardXEvosMoves
	dw MCharzardYEvosMoves
	dw SquirtleEvosMoves
	dw WartortleEvosMoves
	dw BlastoiseEvosMoves
	dw MBlastoiseEvosMoves
	dw CaterpieEvosMoves
	dw MetapodEvosMoves
	dw ButterfreeEvosMoves
	dw WeedleEvosMoves
	dw KakunaEvosMoves
	dw BeedrillEvosMoves
	dw MBeedrillEvosMoves
	dw PidgeyEvosMoves
	dw PidgeottoEvosMoves
	dw PidgeotEvosMoves
	dw MPidgeotEvosMoves
	dw RattataEvosMoves
	dw RaticateEvosMoves
	dw SpearowEvosMoves
	dw FearowEvosMoves
	dw EkansEvosMoves
	dw ArbokEvosMoves
	dw PichuEvosMoves
	dw PikachuEvosMoves
	dw RaichuEvosMoves
	dw MRaichuXEvosMoves
	dw MRaichuYEvosMoves
	dw SandshrewEvosMoves
	dw SandslashEvosMoves
	dw NidoranFEvosMoves
	dw NidorinaEvosMoves
	dw NidoqueenEvosMoves
	dw NidoranMEvosMoves
	dw NidorinoEvosMoves
	dw NidokingEvosMoves
	dw CleffaEvosMoves
	dw ClefairyEvosMoves
	dw ClefableEvosMoves
	dw MClefableEvosMoves
	dw VulpixEvosMoves
	dw NinetalesEvosMoves
	dw IgglybuffEvosMoves
	dw JigglypuffEvosMoves
	dw WigglytuffEvosMoves
	dw ZubatEvosMoves
	dw GolbatEvosMoves
	dw CrobatEvosMoves
	dw OddishEvosMoves
	dw GloomEvosMoves
	dw VileplumeEvosMoves
	dw BellossomEvosMoves
	dw ParasEvosMoves
	dw ParasectEvosMoves
	dw VenonatEvosMoves
	dw VenomothEvosMoves
	dw DiglettEvosMoves
	dw DugtrioEvosMoves
	dw MeowthEvosMoves
	dw PersianEvosMoves
	dw PsyduckEvosMoves
	dw GolduckEvosMoves
	dw MankeyEvosMoves
	dw PrimeapeEvosMoves
	dw AnnihilapeEvosMoves
	dw GrowlitheEvosMoves
	dw ArcanineEvosMoves
	dw PoliwagEvosMoves
	dw PoliwhirlEvosMoves
	dw PoliwrathEvosMoves
	dw PolitoedEvosMoves
	dw AbraEvosMoves
	dw KadabraEvosMoves
	dw AlakazamEvosMoves
	dw MAlakazamEvosMoves
	dw MachopEvosMoves
	dw MachokeEvosMoves
	dw MachampEvosMoves
	dw BellsproutEvosMoves
	dw WeepinbellEvosMoves
	dw VictreebelEvosMoves
	dw MVictrebelEvosMoves
	dw TentacoolEvosMoves
	dw TentacruelEvosMoves
	dw GeodudeEvosMoves
	dw GravelerEvosMoves
	dw GolemEvosMoves
	dw PonytaEvosMoves
	dw RapidashEvosMoves
	dw SlowpokeEvosMoves
	dw SlowbroEvosMoves
	dw MSlowbroEvosMoves
	dw SlowkingEvosMoves
	dw MagnemiteEvosMoves
	dw MagnetonEvosMoves
	dw MagnezoneEvosMoves
	dw FarfetchdEvosMoves
	dw DoduoEvosMoves
	dw DodrioEvosMoves
	dw SeelEvosMoves
	dw DewgongEvosMoves
	dw GrimerEvosMoves
	dw MukEvosMoves
	dw ShellderEvosMoves
	dw CloysterEvosMoves
	dw GastlyEvosMoves
	dw HaunterEvosMoves
	dw GengarEvosMoves
	dw MGengarEvosMoves
	dw OnixEvosMoves
	dw SteelixEvosMoves
	dw MSteelixEvosMoves
	dw DrowzeeEvosMoves
	dw HypnoEvosMoves
	dw KrabbyEvosMoves
	dw KinglerEvosMoves
	dw VoltorbEvosMoves
	dw ElectrodeEvosMoves
	dw ExeggcuteEvosMoves
	dw ExeggutorEvosMoves
	dw CuboneEvosMoves
	dw MarowakEvosMoves
	dw TyrogueEvosMoves
	dw HitmonleeEvosMoves
	dw HitmonchanEvosMoves
	dw HitmontopEvosMoves
	dw LickitungEvosMoves
	dw LickilickyEvosMoves
	dw KoffingEvosMoves
	dw WeezingEvosMoves
	dw RhyhornEvosMoves
	dw RhydonEvosMoves
	dw RhyperiorEvosMoves
	dw HappinyEvosMoves
	dw ChanseyEvosMoves
	dw BlisseyEvosMoves
	dw TangelaEvosMoves
	dw TangrowthEvosMoves
	dw KangaskhanEvosMoves
	dw MKangaskanEvosMoves
	dw HorseaEvosMoves
	dw SeadraEvosMoves
	dw KingdraEvosMoves
	dw GoldeenEvosMoves
	dw SeakingEvosMoves
	dw StaryuEvosMoves
	dw StarmieEvosMoves
	dw MStarmieEvosMoves
	dw MimeJrEvosMoves
	dw MrMimeEvosMoves
	dw ScytherEvosMoves
	dw ScizorEvosMoves
	dw MScizorEvosMoves
	dw KleavorEvosMoves
	dw SmoochumEvosMoves
	dw JynxEvosMoves
	dw ElekidEvosMoves
	dw ElectabuzzEvosMoves
	dw ElectivireEvosMoves
	dw MagbyEvosMoves
	dw MagmarEvosMoves
	dw MagmortarEvosMoves
	dw PinsirEvosMoves
	dw MPinsirEvosMoves
	dw TaurosEvosMoves
	dw MagikarpEvosMoves
	dw GyaradosEvosMoves
	dw MGyaradosEvosMoves
	dw LaprasEvosMoves
	dw DittoEvosMoves
	dw EeveeEvosMoves
	dw VaporeonEvosMoves
	dw JolteonEvosMoves
	dw FlareonEvosMoves
	dw EspeonEvosMoves
	dw UmbreonEvosMoves
	dw LeafeonEvosMoves
	dw GlaceonEvosMoves
	dw SylveonEvosMoves
	dw PorygonEvosMoves
	dw Porygon2EvosMoves
	dw PorygonZEvosMoves
	dw OmanyteEvosMoves
	dw OmastarEvosMoves
	dw KabutoEvosMoves
	dw KabutopsEvosMoves
	dw AerodactylEvosMoves
	dw MArodactylEvosMoves
	dw MunchlaxEvosMoves
	dw SnorlaxEvosMoves
	dw ArticunoEvosMoves
	dw ZapdosEvosMoves
	dw MoltresEvosMoves
	dw DratiniEvosMoves
	dw DragonairEvosMoves
	dw DragoniteEvosMoves
	dw MDragoniteEvosMoves
	dw ArmMewtwoEvosMoves
	dw MewtwoEvosMoves
	dw MMewtwoXEvosMoves
	dw MMewtwoYEvosMoves
	dw MewEvosMoves
	dw MeltanEvosMoves
	dw MelmetalEvosMoves
	dw VenustoiseEvosMoves
	dw ThuFiZerEvosMoves
	dw MZygardeEvosMoves
	dw UNecrozmaEvosMoves
	dw MRayquazaEvosMoves
	dw EEternatusEvosMoves
	dw ArceusEvosMoves
	dw MissingNoEvosMoves
	dw BluesDadEvosMoves
	dw BluesMomEvosMoves
	dw DadEvosMoves
	dw FossilKabutopsEvosMoves
	dw FossilAerodactylEvosMoves
	dw MonGhostEvosMoves
; map pieces, new
	dw MapPiecesEvosMoves
	dw MapPiecesEvosMoves
	dw MapPiecesEvosMoves
	dw MapPiecesEvosMoves
	dw MapPiecesEvosMoves
	dw MapPiecesEvosMoves
	dw MapPiecesEvosMoves
	dw MapPiecesEvosMoves
	dw MapPiecesEvosMoves
	dw MapPiecesEvosMoves
	dw MapPiecesEvosMoves
	dw MapPiecesEvosMoves
	dw MapPiecesEvosMoves
	dw MapPiecesEvosMoves
	dw MapPiecesEvosMoves
	assert_table_length NUM_POKEMON_INDEXES

BulbasaurEvosMoves:
; Evolutions
	db EV_LEVEL, 16, IVYSAUR
	db 0
; Learnset
	db 7, LEECH_SEED
	db 9, VINE_WHIP
	db 13, POISONPOWDER
	db 15, SLEEP_POWDER
	db 18, RAZOR_LEAF
	db 21, MAGICAL_LEAF
	db 25, GROWTH
	db 28, DOUBLE_EDGE
	db 30, PETAL_DANCE
	db 32, SLUDGE_WAVE
	db 35, LEAF_STORM
	db 39, ENERGY_BALL
	db 42, SOLARBEAM
	db 0

IvysaurEvosMoves:
; Evolutions
	db EV_LEVEL, 32, VENUSAUR
	db 0
; Learnset
	db 7, LEECH_SEED
	db 9, VINE_WHIP
	db 13, POISONPOWDER
	db 15, SLEEP_POWDER
	db 20, RAZOR_LEAF
	db 24, MAGICAL_LEAF
	db 28, GROWTH
	db 31, DOUBLE_EDGE
	db 33, PETAL_DANCE
	db 35, SLUDGE_WAVE
	db 38, LEAF_STORM
	db 42, ENERGY_BALL
	db 45, ROAR
	db 47, SOLARBEAM
	db 0

VenusaurEvosMoves:
; Evolutions
	db EV_ITEM, MEGA_STONE_X, 1, MVENUSAUR
	db EV_ITEM, MEGA_STONE_Y, 1, MVENUSAUR
	db 0
; Learnset
	db 7, LEECH_SEED
	db 9, VINE_WHIP
	db 13, POISONPOWDER
	db 15, SLEEP_POWDER
	db 20, RAZOR_LEAF
	db 24, MAGICAL_LEAF
	db 28, GROWTH
	db 31, DOUBLE_EDGE
	db 34, PETAL_DANCE
	db 38, SLUDGE_WAVE
	db 42, LEAF_STORM
	db 48, ENERGY_BALL
	db 50, ROAR
	db 53, SOLARBEAM
	db 60, FRENZY_PLANT
	db 0

MVenusaurEvosMoves:
; Evolutions
	db 0
; Learnset
	db 12, LEECH_SEED
	db 14, VINE_WHIP
	db 18, POISONPOWDER
	db 20, SLEEP_POWDER
	db 25, RAZOR_LEAF
	db 29, MAGICAL_LEAF
	db 33, GROWTH
	db 36, DOUBLE_EDGE
	db 39, PETAL_DANCE
	db 43, SLUDGE_WAVE
	db 45, AMNESIA
	db 47, LEAF_STORM
	db 53, ENERGY_BALL
	db 55, ROAR
	db 56, GUNK_SHOT
	db 58, SOLARBEAM
	db 65, FRENZY_PLANT
	db 0

CharmanderEvosMoves:
; Evolutions
	db EV_LEVEL, 16, CHARMELEON
	db 0
; Learnset
	db 9, EMBER
	db 12, DRAGONBREATH
	db 15, LEER
	db 17, FIRE_PUNCH
	db 20, SLASH
	db 24, SMOKESCREEN
	db 27, FLAMETHROWER
	db 29, IRON_TAIL
	db 31, CRUNCH
	db 33, ROAR
	db 35, FIRE_SPIN
	db 38, DRAGON_DANCE
	db 40, FLARE_BLITZ
	db 0

CharmeleonEvosMoves:
; Evolutions
	db EV_LEVEL, 36, CHARIZARD
	db 0
; Learnset
	db 9, EMBER
	db 12, DRAGONBREATH
	db 15, LEER
	db 19, FIRE_PUNCH
	db 24, SLASH
	db 27, SMOKESCREEN
	db 30, FLAMETHROWER
	db 33, IRON_TAIL
	db 35, CRUNCH
	db 37, ROAR
	db 41, FIRE_SPIN
	db 45, DRAGON_DANCE
	db 49, FLARE_BLITZ
	db 0

CharizardEvosMoves:
; Evolutions
	db EV_ITEM, MEGA_STONE_X, 1, MCHARZARDX
	db EV_ITEM, MEGA_STONE_Y, 1, MCHARZARDY
	db 0
; Learnset
	db 9, EMBER
	db 12, DRAGONBREATH
	db 15, LEER
	db 19, FIRE_PUNCH
	db 24, SLASH
	db 27, SMOKESCREEN
	db 30, FLAMETHROWER
	db 33, IRON_TAIL
	db 35, CRUNCH
	db 37, ROAR
	db 38, AIR_SLASH
	db 42, STEEL_WING
	db 46, FIRE_SPIN
	db 51, ROOST
	db 54, DRAGON_DANCE
	db 57, FLARE_BLITZ
	db 60, BLAST_BURN
	db 0

MCharzardXEvosMoves:
; Evolutions
	db 0
; Learnset
	db 14, EMBER
	db 17, DRAGONBREATH
	db 20, LEER
	db 22, DRAGON_RAGE
	db 24, FIRE_PUNCH
	db 29, SLASH
	db 32, SMOKESCREEN
	db 35, FLAMETHROWER
	db 40, CRUNCH
	db 42, ROAR
	db 43, AIR_SLASH
	db 45, DRAGON_CLAW
	db 47, STEEL_WING
	db 51, FIRE_SPIN
	db 56, ROOST
	db 57, SHADOW_CLAW
	db 59, DRAGON_DANCE
	db 60, OUTRAGE
	db 62, FLARE_BLITZ
	db 65, BLAST_BURN
	db 0

MCharzardYEvosMoves:
; Evolutions
	db 0
; Learnset
	db 14, EMBER
	db 17, DRAGONBREATH
	db 20, LEER
	db 22, DRAGON_RAGE
	db 24, FIRE_PUNCH
	db 29, SLASH
	db 32, SMOKESCREEN
	db 35, FLAMETHROWER
	db 40, CRUNCH
	db 42, ROAR
	db 43, AIR_SLASH
	db 45, HURRICANE
	db 47, STEEL_WING
	db 51, FIRE_SPIN
	db 56, ROOST
	db 57, FIRE_BLAST
	db 59, DRAGON_DANCE
	db 62, FLARE_BLITZ
	db 65, BLAST_BURN
	db 0

SquirtleEvosMoves:
; Evolutions
	db EV_LEVEL, 16, WARTORTLE
	db 0
; Learnset
	db 8, BUBBLE
	db 12, BITE
	db 15, WATER_GUN
	db 17, HAZE
	db 20, MUD_SLAP
	db 21, RAPID_SPIN
	db 23, WATER_PULSE
	db 24, DEFENSE_CURL
	db 26, HEADBUTT
	db 28, CRUNCH
	db 30, SUBMISSION
	db 33, RAIN_DANCE
	db 34, IRON_TAIL
	db 35, ICE_PUNCH
	db 37, SCALD
	db 40, AQUA_JET
	db 42, ZEN_HEADBUTT
	db 45, ROCK_TOMB
	db 47, HYDRO_PUMP
	db 0

WartortleEvosMoves:
; Evolutions
	db EV_LEVEL, 36, BLASTOISE
	db 0
; Learnset
	db 8, BUBBLE
	db 12, BITE
	db 15, WATER_GUN
	db 18, HAZE
	db 23, MUD_SLAP
	db 25, RAPID_SPIN
	db 26, WATER_PULSE
	db 28, DEFENSE_CURL
	db 30, HEADBUTT
	db 32, CRUNCH
	db 34, SUBMISSION
	db 37, RAIN_DANCE
	db 38, IRON_TAIL
	db 39, ICE_PUNCH
	db 41, SCALD
	db 44, AQUA_JET
	db 46, ZEN_HEADBUTT
	db 49, ROCK_TOMB
	db 51, HYDRO_PUMP
	db 0

BlastoiseEvosMoves:
; Evolutions
	db EV_ITEM, MEGA_STONE_X, 1, MBLASTOISE
	db EV_ITEM, MEGA_STONE_Y, 1, MBLASTOISE
	db 0
; Learnset
	db 8, BUBBLE
	db 12, BITE
	db 15, WATER_GUN
	db 18, HAZE
	db 23, MUD_SLAP
	db 25, RAPID_SPIN
	db 26, WATER_PULSE
	db 28, DEFENSE_CURL
	db 30, HEADBUTT
	db 32, CRUNCH
	db 34, SUBMISSION
	db 36, FLASH_CANNON
	db 41, IRON_TAIL
	db 42, RAIN_DANCE
	db 43, ICE_PUNCH
	db 45, SCALD
	db 48, AQUA_JET
	db 50, ZEN_HEADBUTT
	db 51, ROAR
	db 53, ROCK_TOMB
	db 55, HYDRO_PUMP
	db 60, HYDRO_CANNON
	db 0

MBlastoiseEvosMoves:
; Evolutions
	db 0
; Learnset
	db 13, BUBBLE
	db 17, BITE
	db 20, WATER_GUN
	db 23, HAZE
	db 28, MUD_SLAP
	db 30, RAPID_SPIN
	db 31, WATER_PULSE
	db 33, DEFENSE_CURL
	db 35, HEADBUTT
	db 37, CRUNCH
	db 39, SUBMISSION
	db 41, FLASH_CANNON
	db 43, ZAP_CANNON
	db 46, IRON_TAIL
	db 47, RAIN_DANCE
	db 48, ICE_PUNCH
	db 50, SCALD
	db 53, AQUA_JET
	db 55, ZEN_HEADBUTT
	db 56, ROAR
	db 57, BULLET_SEED
	db 58, ROCK_TOMB
	db 60, HYDRO_PUMP
	db 65, HYDRO_CANNON
	db 0

CaterpieEvosMoves:
; Evolutions
	db EV_LEVEL, 7, METAPOD
	db 0
; Learnset
	db 15, STICKY_WEB
	db 0

MetapodEvosMoves:
; Evolutions
	db EV_LEVEL, 10, BUTTERFREE
	db 0
; Learnset
	db 7, HARDEN
	db 0

ButterfreeEvosMoves:
; Evolutions
	db 0
; Learnset
	db 10, CONFUSION
	db 11, SUPERSONIC
	db 12, POISONPOWDER
	db 14, STUN_SPORE
	db 15, SLEEP_POWDER
	db 16, PSYBEAM
	db 17, SWIFT
	db 18, GUST
	db 20, WHIRLWIND
	db 22, AIR_SLASH
	db 25, MEGA_DRAIN
	db 28, TWISTER
	db 31, DREAM_EATER
	db 33, INFESTATION
	db 35, ENERGY_BALL
	db 37, ROOST
	db 39, AIR_CUTTER
	db 41, AERIAL_ACE
	db 44, HURRICANE
	db 0

WeedleEvosMoves:
; Evolutions
	db EV_LEVEL, 7, KAKUNA
	db 0
; Learnset
	db 15, STICKY_WEB
	db 0

KakunaEvosMoves:
; Evolutions
	db EV_LEVEL, 10, BEEDRILL
	db 0
; Learnset
	db 7, HARDEN
	db 0

BeedrillEvosMoves:
; Evolutions
	db EV_ITEM, MEGA_STONE_X, 1, MBEEDRILL
	db EV_ITEM, MEGA_STONE_Y, 1, MBEEDRILL
	db 0
; Learnset
	db 10, FURY_ATTACK
	db 13, FOCUS_ENERGY
	db 15, POISON_STING
	db 18, TWINEEDLE
	db 21, SWIFT
	db 24, PIN_MISSILE
	db 25, TOXIC_SPIKES
	db 26, MEGA_DRAIN
	db 29, ROOST
	db 32, AGILITY
	db 34, INFESTATION
	db 36, AERIAL_ACE
	db 42, SLUDGE_WAVE
	db 45, SOLARBEAM
	db 50, DRILL_RUN
	db 0

MBeedrillEvosMoves:
; Evolutions
	db 0
; Learnset
	db 15, FURY_ATTACK
	db 18, FOCUS_ENERGY
	db 20, POISON_STING
	db 23, TWINEEDLE
	db 26, SWIFT
	db 29, PIN_MISSILE
	db 30, TOXIC_SPIKES
	db 31, MEGA_DRAIN
	db 34, ROOST
	db 37, AGILITY
	db 39, INFESTATION
	db 40, STEEL_WING
	db 41, AERIAL_ACE
	db 42, X_SCISSOR
	db 47, SLUDGE_WAVE
	db 50, SOLARBEAM
	db 53, GUNK_SHOT
	db 55, DRILL_RUN
	db 0

PidgeyEvosMoves:
; Evolutions
	db EV_LEVEL, 18, PIDGEOTTO
	db 0
; Learnset
	db 5, SAND_ATTACK
	db 7, PECK
	db 9, GUST
	db 12, QUICK_ATTACK
	db 15, TWISTER
	db 17, WING_ATTACK
	db 19, SWIFT
	db 21, WHIRLWIND
	db 23, AGILITY
	db 28, ROOST
	db 32, STEEL_WING
	db 31, MIRROR_MOVE
	db 34, AERIAL_ACE
	db 37, AIR_SLASH
	db 40, AIR_CUTTER
	db 44, HURRICANE
	db 0

PidgeottoEvosMoves:
; Evolutions
	db EV_LEVEL, 36, PIDGEOT
	db 0
; Learnset
	db 5, SAND_ATTACK
	db 7, PECK
	db 9, GUST
	db 12, QUICK_ATTACK
	db 15, TWISTER
	db 19, WING_ATTACK
	db 23, SWIFT
	db 25, WHIRLWIND
	db 27, AGILITY
	db 32, ROOST
	db 36, STEEL_WING
	db 35, MIRROR_MOVE
	db 38, AERIAL_ACE
	db 41, AIR_SLASH
	db 44, AIR_CUTTER
	db 48, HURRICANE
	db 0

PidgeotEvosMoves:
; Evolutions
	db EV_ITEM, MEGA_STONE_X, 1, MPIDGEOT
	db EV_ITEM, MEGA_STONE_Y, 1, MPIDGEOT
	db 0
; Learnset
	db 5, SAND_ATTACK
	db 7, PECK
	db 9, GUST
	db 12, QUICK_ATTACK
	db 15, TWISTER
	db 19, WING_ATTACK
	db 23, SWIFT
	db 25, WHIRLWIND
	db 27, AGILITY
	db 32, ROOST
	db 36, STEEL_WING
	db 39, MIRROR_MOVE
	db 42, AERIAL_ACE
	db 45, AIR_SLASH
	db 48, AIR_CUTTER
	db 52, HURRICANE
	db 0

MPidgeotEvosMoves:
; Evolutions
	db 0
; Learnset
	db 10, SAND_ATTACK
	db 12, PECK
	db 14, GUST
	db 17, QUICK_ATTACK
	db 20, TWISTER
	db 24, WING_ATTACK
	db 28, SWIFT
	db 32, AGILITY
	db 37, ROOST
	db 39, HEAT_WAVE
	db 41, STEEL_WING
	db 43, THUNDER
	db 44, MIRROR_MOVE
	db 45, BLIZZARD
	db 47, AERIAL_ACE
	db 50, AIR_SLASH
	db 53, AIR_CUTTER
	db 57, HURRICANE
	db 0

RattataEvosMoves:
; Evolutions
	db EV_LEVEL, 20, RATICATE
	db 0
; Learnset
	db 4, QUICK_ATTACK
	db 7, FOCUS_ENERGY
	db 10, BITE
	db 12, DEFENSE_CURL
	db 14, HYPER_FANG
	db 16, TAKE_DOWN
	db 20, SUPER_FANG
	db 23, SCREECH
	db 26, SWIFT
	db 30, CRUNCH
	db 32, IRON_TAIL
	db 35, DOUBLE_EDGE
	db 39, SWORDS_DANCE
	db 43, HEADBUTT
	db 0

RaticateEvosMoves:
; Evolutions
	db 0
; Learnset
	db 4, QUICK_ATTACK
	db 7, FOCUS_ENERGY
	db 10, BITE
	db 12, DEFENSE_CURL
	db 14, HYPER_FANG
	db 16, TAKE_DOWN
	db 24, SUPER_FANG
	db 25, SCREECH
	db 29, SWIFT
	db 34, CRUNCH
	db 37, IRON_TAIL
	db 39, DOUBLE_EDGE
	db 43, SWORDS_DANCE
	db 45, ROAR
	db 47, HEADBUTT
	db 0

SpearowEvosMoves:
; Evolutions
	db EV_LEVEL, 20, FEAROW
	db 0
; Learnset
	db 4, LEER
	db 9, FURY_ATTACK
	db 14, WING_ATTACK
	db 18, AERIAL_ACE
	db 21, TAKE_DOWN
	db 23, MIRROR_MOVE
	db 25, AGILITY
	db 27, SWIFT
	db 29, FOCUS_ENERGY
	db 31, DRILL_PECK
	db 33, ROOST
	db 36, STEEL_WING
	db 40, WHIRLWIND
	db 45, DRILL_RUN
	db 0

FearowEvosMoves:
; Evolutions
	db 0
; Learnset
	db 4, LEER
	db 9, FURY_ATTACK
	db 14, WING_ATTACK
	db 18, AERIAL_ACE
	db 23, TAKE_DOWN
	db 25, MIRROR_MOVE
	db 27, AGILITY
	db 30, SWIFT
	db 32, FOCUS_ENERGY
	db 34, DRILL_PECK
	db 36, ROOST
	db 39, STEEL_WING
	db 45, WHIRLWIND
	db 50, DRILL_RUN
	db 0

EkansEvosMoves:
; Evolutions
	db EV_LEVEL, 22, ARBOK
	db 0
; Learnset
	db 4, POISON_STING
	db 9, BITE
	db 12, GLARE
	db 17, SCREECH
	db 20, ACID
	db 23, DISABLE
	db 24, CRUNCH
	db 26, SLUDGE
	db 28, HEADBUTT
	db 30, MEGA_DRAIN
	db 32, SLUDGE_WAVE
	db 34, SLAM
	db 36, IRON_TAIL
	db 39, TAKE_DOWN
	db 43, HAZE
	db 46, GUNK_SHOT
	db 0

ArbokEvosMoves:
; Evolutions
	db 0
; Learnset
	db 4, POISON_STING
	db 9, BITE
	db 12, GLARE
	db 17, SCREECH
	db 20, ACID
	db 24, DISABLE
	db 26, CRUNCH
	db 28, SLUDGE
	db 31, HEADBUTT
	db 33, MEGA_DRAIN
	db 35, SLUDGE_WAVE
	db 37, SLAM
	db 39, IRON_TAIL
	db 43, TAKE_DOWN
	db 47, HAZE
	db 52, GUNK_SHOT
	db 0

PichuEvosMoves:
; Evolutions
	db EV_LEVEL, 10, PIKACHU
	db 0
; Learnset
	db 10, THUNDER_WAVE
	db 11, LEER
	db 12, DOUBLE_TEAM
	db 13, DEFENSE_CURL
	db 14, QUICK_ATTACK
	db 15, DOUBLE_KICK
	db 16, SLAM
	db 17, AGILITY
	db 18, SWIFT
	db 19, MUD_SLAP
	db 20, SING
	db 21, THUNDERBOLT
	db 22, TAKE_DOWN
	db 23, THUNDERPUNCH
	db 24, HEADBUTT
	db 25, VOLT_TACKLE
	db 26, IRON_TAIL
	db 27, LIGHT_SCREEN
	db 28, SHOCK_WAVE
	db 29, PLAY_ROUGH
	db 30, THUNDER
	db 31, EXTREMESPEED
	db 32, SUBMISSION
	db 33, ZAP_CANNON
	db 34, PETAL_DANCE
	db 35, PSYCHIC_M
	db 0

PikachuEvosMoves:
; Evolutions
	db EV_ITEM, THUNDER_STONE, 1, RAICHU
	db 0
; Learnset
	db 6, THUNDER_WAVE
	db 7, LEER
	db 8, DOUBLE_TEAM
	db 9, DEFENSE_CURL
	db 11, QUICK_ATTACK
	db 13, DOUBLE_KICK
	db 16, SLAM
	db 19, AGILITY
	db 21, SWIFT
	db 23, MUD_SLAP
	db 25, SING
	db 27, THUNDERBOLT
	db 29, TAKE_DOWN
	db 31, THUNDERPUNCH
	db 34, HEADBUTT
	db 36, VOLT_TACKLE
	db 37, IRON_TAIL
	db 39, LIGHT_SCREEN
	db 41, SHOCK_WAVE
	db 44, PLAY_ROUGH
	db 47, THUNDER
	db 50, EXTREMESPEED
	db 55, SUBMISSION
	db 60, ZAP_CANNON
	db 81, PETAL_DANCE
	db 0

RaichuEvosMoves:
; Evolutions
	db EV_ITEM, MEGA_STONE_X, 1, MRAICHUX
	db EV_ITEM, MEGA_STONE_Y, 1, MRAICHUY
	db 0
; Learnset
	db 11, THUNDER_WAVE
	db 12, LEER
	db 13, DOUBLE_TEAM
	db 14, DEFENSE_CURL
	db 16, QUICK_ATTACK
	db 18, DOUBLE_KICK
	db 21, SLAM
	db 24, AGILITY
	db 26, SWIFT
	db 28, MUD_SLAP
	db 30, SING
	db 32, THUNDERBOLT
	db 34, TAKE_DOWN
	db 36, THUNDERPUNCH
	db 39, HEADBUTT
	db 41, VOLT_TACKLE
	db 42, IRON_TAIL
	db 44, LIGHT_SCREEN
	db 46, SHOCK_WAVE
	db 49, PLAY_ROUGH
	db 52, THUNDER
	db 55, EXTREMESPEED
	db 60, SUBMISSION
	db 65, ZAP_CANNON
	db 86, PETAL_DANCE
	db 0

MRaichuXEvosMoves:
; Evolutions
	db 0
; Learnset
	db 16, THUNDER_WAVE
	db 17, LEER
	db 18, DOUBLE_TEAM
	db 19, DEFENSE_CURL
	db 21, QUICK_ATTACK
	db 23, DOUBLE_KICK
	db 26, SLAM
	db 29, AGILITY
	db 31, SWIFT
	db 33, MUD_SLAP
	db 35, SING
	db 37, THUNDERBOLT
	db 39, TAKE_DOWN
	db 41, THUNDERPUNCH
	db 44, HEADBUTT
	db 46, VOLT_TACKLE
	db 47, IRON_TAIL
	db 49, LIGHT_SCREEN
	db 51, SHOCK_WAVE
	db 54, PLAY_ROUGH
	db 57, THUNDER
	db 60, EXTREMESPEED
	db 65, SUBMISSION
	db 70, ZAP_CANNON
	db 91, PETAL_DANCE
	db 95, CLOSE_COMBAT
	db 0

MRaichuYEvosMoves:
; Evolutions
	db 0
; Learnset
	db 16, THUNDER_WAVE
	db 17, LEER
	db 18, DOUBLE_TEAM
	db 19, DEFENSE_CURL
	db 21, QUICK_ATTACK
	db 23, DOUBLE_KICK
	db 26, SLAM
	db 29, AGILITY
	db 31, SWIFT
	db 33, MUD_SLAP
	db 35, SING
	db 37, THUNDERBOLT
	db 39, TAKE_DOWN
	db 41, THUNDERPUNCH
	db 44, HEADBUTT
	db 46, VOLT_TACKLE
	db 47, IRON_TAIL
	db 49, LIGHT_SCREEN
	db 51, SHOCK_WAVE
	db 54, PLAY_ROUGH
	db 57, THUNDER
	db 60, EXTREMESPEED
	db 65, SUBMISSION
	db 70, ZAP_CANNON
	db 91, PETAL_DANCE
	db 95, OUTRAGE
	db 0

SandshrewEvosMoves:
; Evolutions
	db EV_LEVEL, 22, SANDSLASH
	db 0
; Learnset
	db 10, SAND_ATTACK
	db 13, DEFENSE_CURL
	db 15, MUD_SLAP
	db 17, RAPID_SPIN
	db 19, SLASH
	db 21, SWIFT
	db 23, AGILITY
	db 24, FURY_SWIPES
	db 25, POISON_STING
	db 27, ICE_PUNCH
	db 29, LEECH_LIFE
	db 32, HEADBUTT
	db 34, ROCK_TOMB
	db 35, DIG
	db 37, SANDSTORM
	db 39, NIGHT_SLASH
	db 42, SHADOW_CLAW
	db 44, PIN_MISSILE
	db 46, SUPER_FANG
	db 48, SWORDS_DANCE
	db 50, SUBMISSION
	db 52, EARTHQUAKE
	db 0

SandslashEvosMoves:
; Evolutions
	db 0
; Learnset
	db 10, SAND_ATTACK
	db 13, DEFENSE_CURL
	db 15, MUD_SLAP
	db 17, RAPID_SPIN
	db 19, SLASH
	db 21, SWIFT
	db 24, AGILITY
	db 26, FURY_SWIPES
	db 27, POISON_STING
	db 30, ICE_PUNCH
	db 33, LEECH_LIFE
	db 36, HEADBUTT
	db 39, ROCK_TOMB
	db 41, DIG
	db 42, SANDSTORM
	db 43, NIGHT_SLASH
	db 45, SHADOW_CLAW
	db 47, PIN_MISSILE
	db 49, SUPER_FANG
	db 51, SWORDS_DANCE
	db 54, SUBMISSION
	db 59, EARTHQUAKE
	db 64, DRILL_RUN
	db 0

NidoranFEvosMoves:
; Evolutions
	db EV_LEVEL, 16, NIDORINA
	db 0
; Learnset
	db 7, LEER
	db 8, TACKLE
	db 9, DEFENSE_CURL
	db 10, FURY_SWIPES
	db 12, DOUBLE_KICK
	db 15, POISON_STING
	db 17, MUD_SLAP
	db 18, SUPERSONIC
	db 20, BODY_SLAM
	db 22, BITE
	db 24, FOCUS_ENERGY
	db 25, MUD_SHOT
	db 27, HEADBUTT
	db 28, SUPER_FANG
	db 29, TOXIC_SPIKES
	db 30, TAKE_DOWN
	db 31, TOXIC
	db 33, CRUNCH
	db 35, IRON_TAIL
	db 37, ROCK_TOMB
	db 39, SLUDGE_WAVE
	db 41, AERIAL_ACE
	db 0

NidorinaEvosMoves:
; Evolutions
	db EV_ITEM, MOON_STONE, 1, NIDOQUEEN
	db 0
; Learnset
	db 7, LEER
	db 8, TACKLE
	db 9, DEFENSE_CURL
	db 10, FURY_SWIPES
	db 12, DOUBLE_KICK
	db 15, POISON_STING
	db 18, MUD_SLAP
	db 20, SUPERSONIC
	db 22, BODY_SLAM
	db 24, BITE
	db 26, FOCUS_ENERGY
	db 28, MUD_SHOT
	db 30, HEADBUTT
	db 32, SUPER_FANG
	db 33, TOXIC_SPIKES
	db 34, TAKE_DOWN
	db 37, TOXIC
	db 39, CRUNCH
	db 41, IRON_TAIL
	db 43, ROCK_TOMB
	db 45, SLUDGE_WAVE
	db 47, AERIAL_ACE
	db 0

NidoqueenEvosMoves:
; Evolutions
	db 0
; Learnset
	db 7, LEER
	db 8, TACKLE
	db 9, DEFENSE_CURL
	db 10, FURY_SWIPES
	db 12, DOUBLE_KICK
	db 15, POISON_STING
	db 18, MUD_SLAP
	db 20, SUPERSONIC
	db 22, BODY_SLAM
	db 24, BITE
	db 26, FOCUS_ENERGY
	db 28, MUD_SHOT
	db 30, HEADBUTT
	db 32, SUPER_FANG
	db 33, TOXIC_SPIKES
	db 34, TAKE_DOWN
	db 36, SUPERPOWER
	db 37, FIRE_PUNCH
	db 38, THUNDERPUNCH
	db 39, ICE_PUNCH
	db 40, SHADOW_CLAW
	db 42, TOXIC
	db 44, CRUNCH
	db 45, ROAR
	db 46, IRON_TAIL
	db 48, ROCK_TOMB
	db 50, SLUDGE_WAVE
	db 52, SUBMISSION
	db 54, AERIAL_ACE
	db 56, LOVELY_KISS
	db 60, HORN_DRILL
	db 65, DRILL_RUN
	db 0

NidoranMEvosMoves:
; Evolutions
	db EV_LEVEL, 16, NIDORINO
	db 0
; Learnset
	db 7, GROWL
	db 8, TACKLE
	db 9, DEFENSE_CURL
	db 10, FURY_SWIPES
	db 11, PECK
	db 12, DOUBLE_KICK
	db 13, HORN_ATTACK
	db 15, POISON_STING
	db 17, MUD_SLAP
	db 19, SUPERSONIC
	db 20, BODY_SLAM
	db 22, BITE
	db 24, FOCUS_ENERGY
	db 26, MUD_SHOT
	db 28, HEADBUTT
	db 29, TOXIC_SPIKES
	db 30, SUPER_FANG
	db 32, TAKE_DOWN
	db 34, TOXIC
	db 36, CRUNCH
	db 38, IRON_TAIL
	db 40, ROCK_TOMB
	db 42, SLUDGE_WAVE
	db 44, AERIAL_ACE
	db 0

NidorinoEvosMoves:
; Evolutions
	db EV_ITEM, MOON_STONE, 1, NIDOKING
	db 0
; Learnset
	db 7, GROWL
	db 8, TACKLE
	db 9, DEFENSE_CURL
	db 10, FURY_SWIPES
	db 11, PECK
	db 12, DOUBLE_KICK
	db 13, HORN_ATTACK
	db 15, POISON_STING
	db 18, MUD_SLAP
	db 20, SUPERSONIC
	db 22, BODY_SLAM
	db 24, BITE
	db 26, FOCUS_ENERGY
	db 28, MUD_SHOT
	db 30, HEADBUTT
	db 32, SUPER_FANG
	db 33, TOXIC_SPIKES
	db 34, TAKE_DOWN
	db 37, TOXIC
	db 39, CRUNCH
	db 41, IRON_TAIL
	db 43, ROCK_TOMB
	db 45, SLUDGE_WAVE
	db 47, AERIAL_ACE
	db 0

NidokingEvosMoves:
; Evolutions
	db 0
; Learnset
	db 7, GROWL
	db 8, TACKLE
	db 9, DEFENSE_CURL
	db 10, FURY_SWIPES
	db 12, DOUBLE_KICK
	db 13, HORN_ATTACK
	db 15, POISON_STING
	db 18, MUD_SLAP
	db 20, SUPERSONIC
	db 22, BODY_SLAM
	db 24, BITE
	db 26, FOCUS_ENERGY
	db 28, MUD_SHOT
	db 30, HEADBUTT
	db 32, SUPER_FANG
	db 33, TOXIC_SPIKES
	db 34, TAKE_DOWN
	db 36, SUPERPOWER
	db 37, FIRE_PUNCH
	db 38, THUNDERPUNCH
	db 39, ICE_PUNCH
	db 40, SHADOW_CLAW
	db 41, THRASH
	db 42, TOXIC
	db 44, CRUNCH
	db 45, ROAR
	db 46, IRON_TAIL
	db 48, MEGAHORN
	db 50, ROCK_TOMB
	db 52, SLUDGE_WAVE
	db 54, SUBMISSION
	db 56, AERIAL_ACE
	db 58, LOVELY_KISS
	db 60, HORN_DRILL
	db 63, HEAD_SMASH
	db 65, DRILL_RUN
	db 0

CleffaEvosMoves:
; Evolutions
	db EV_LEVEL, 10, CLEFAIRY
	db 0
; Learnset
	db 10, MINIMIZE
	db 11, METRONOME
	db 12, SING
	db 13, TELEPORT
	db 14, MUD_SLAP
	db 15, DEFENSE_CURL
	db 16, SWIFT
	db 17, DRAIN_PUNCH
	db 18, IRON_TAIL
	db 19, MAGICAL_LEAF
	db 20, METEOR_MASH
	db 21, FLAMETHROWER
	db 22, MOONBLAST
	db 23, LIGHT_SCREEN
	db 24, ICE_PUNCH
	db 25, THUNDERPUNCH
	db 26, FIRE_PUNCH
	db 27, AMNESIA
	db 28, HEADBUTT
	db 29, ZEN_HEADBUTT
	db 30, DREAM_EATER
	db 31, PETAL_DANCE
	db 32, PLAY_ROUGH
	db 33, NIGHT_SLASH
	db 0

ClefairyEvosMoves:
; Evolutions
	db EV_ITEM, MOON_STONE, 1, CLEFABLE
	db 0
; Learnset
	db 8, MINIMIZE
	db 10, METRONOME
	db 12, SING
	db 14, TELEPORT
	db 16, MUD_SLAP
	db 19, DEFENSE_CURL
	db 22, SWIFT
	db 24, DRAIN_PUNCH
	db 27, IRON_TAIL
	db 29, MAGICAL_LEAF
	db 32, METEOR_MASH
	db 34, FLAMETHROWER
	db 36, MOONBLAST
	db 40, LIGHT_SCREEN
	db 42, ICE_PUNCH
	db 43, THUNDERPUNCH
	db 44, FIRE_PUNCH
	db 46, AMNESIA
	db 48, HEADBUTT
	db 49, ZEN_HEADBUTT
	db 51, DREAM_EATER
	db 54, PETAL_DANCE
	db 57, PLAY_ROUGH
	db 0

ClefableEvosMoves:
; Evolutions
	db EV_ITEM, MEGA_STONE_X, 1, MCLEFABLE
	db EV_ITEM, MEGA_STONE_Y, 1, MCLEFABLE
	db 0
; Learnset
	db 8, MINIMIZE
	db 10, METRONOME
	db 12, SING
	db 14, TELEPORT
	db 16, MUD_SLAP
	db 19, DEFENSE_CURL
	db 22, SWIFT
	db 27, DRAIN_PUNCH
	db 30, IRON_TAIL
	db 32, MAGICAL_LEAF
	db 35, METEOR_MASH
	db 37, FLAMETHROWER
	db 39, MOONBLAST
	db 43, LIGHT_SCREEN
	db 45, ICE_PUNCH
	db 46, THUNDERPUNCH
	db 47, FIRE_PUNCH
	db 49, AMNESIA
	db 51, HEADBUTT
	db 52, ZEN_HEADBUTT
	db 54, DREAM_EATER
	db 57, PETAL_DANCE
	db 60, PLAY_ROUGH
	db 0

MClefableEvosMoves:
; Evolutions
	db 0
; Learnset
	db 13, MINIMIZE
	db 15, METRONOME
	db 17, SING
	db 19, TELEPORT
	db 21, MUD_SLAP
	db 24, DEFENSE_CURL
	db 27, SWIFT
	db 32, DRAIN_PUNCH
	db 35, IRON_TAIL
	db 37, MAGICAL_LEAF
	db 40, METEOR_MASH
	db 42, FLAMETHROWER
	db 44, MOONBLAST
	db 48, LIGHT_SCREEN
	db 50, ICE_PUNCH
	db 51, THUNDERPUNCH
	db 52, FIRE_PUNCH
	db 54, AMNESIA
	db 56, HEADBUTT
	db 57, ZEN_HEADBUTT
	db 59, DREAM_EATER
	db 62, PETAL_DANCE
	db 65, PLAY_ROUGH
	db 70, AIR_CUTTER
	db 0

VulpixEvosMoves:
; Evolutions
	db EV_ITEM, FIRE_STONE, 1, NINETALES
	db 0
; Learnset
	db 4, DISABLE
	db 8, QUICK_ATTACK
	db 14, SWIFT
	db 18, CONFUSE_RAY
	db 20, ROAR
	db 22, WILL_O_WISP
	db 25, HYPNOSIS
	db 27, FLAMETHROWER
	db 30, AGILITY
	db 32, HEADBUTT
	db 35, FIRE_SPIN
	db 38, IRON_TAIL
	db 41, ENERGY_BALL
	db 44, ZEN_HEADBUTT
	db 47, FLARE_BLITZ
	db 50, HEAT_WAVE
	db 53, FIRE_BLAST
	db 0

NinetalesEvosMoves:
; Evolutions
	db 0
; Learnset
	db 4, DISABLE
	db 8, QUICK_ATTACK
	db 14, SWIFT
	db 18, CONFUSE_RAY
	db 20, ROAR
	db 22, WILL_O_WISP
	db 25, HYPNOSIS
	db 29, FLAMETHROWER
	db 33, AGILITY
	db 35, HEADBUTT
	db 37, DREAM_EATER
	db 40, FIRE_SPIN
	db 43, IRON_TAIL
	db 47, ENERGY_BALL
	db 49, ZEN_HEADBUTT
	db 52, FLARE_BLITZ
	db 54, HEAT_WAVE
	db 56, SOLARBEAM
	db 59, FIRE_BLAST
	db 0

IgglybuffEvosMoves:
; Evolutions
	db EV_LEVEL, 10, JIGGLYPUFF
	db 0
; Learnset
	db 10, POUND
	db 11, DREAM_EATER
	db 12, SCREECH
	db 13, DISABLE
	db 14, WATER_GUN
	db 15, SONICBOOM
	db 16, TELEPORT
	db 17, DEFENSE_CURL
	db 18, REST
	db 19, BODY_SLAM
	db 20, MIMIC
	db 21, SHOCK_WAVE
	db 22, HEADBUTT
	db 23, DOUBLE_EDGE
	db 24, PLAY_ROUGH
	db 25, WATER_PULSE
	db 26, TAKE_DOWN
	db 27, FIRE_PUNCH
	db 28, ICE_PUNCH
	db 29, THUNDERPUNCH
	db 30, DRAIN_PUNCH
	db 31, FLAMETHROWER
	db 32, LIGHT_SCREEN
	db 33, MINIMIZE
	db 34, MOONBLAST
	db 0

JigglypuffEvosMoves:
; Evolutions
	db EV_ITEM, MOON_STONE, 1, WIGGLYTUFF
	db 0
; Learnset
	db 7, POUND
	db 10, DREAM_EATER
	db 12, SCREECH
	db 14, DISABLE
	db 17, WATER_GUN
	db 20, SONICBOOM
	db 22, TELEPORT
	db 23, DEFENSE_CURL
	db 24, REST
	db 27, BODY_SLAM
	db 29, MIMIC
	db 32, SHOCK_WAVE
	db 35, HEADBUTT
	db 38, DOUBLE_EDGE
	db 40, PLAY_ROUGH
	db 43, WATER_PULSE
	db 46, TAKE_DOWN
	db 48, FIRE_PUNCH
	db 49, ICE_PUNCH
	db 50, THUNDERPUNCH
	db 51, DRAIN_PUNCH
	db 54, FLAMETHROWER
	db 59, LIGHT_SCREEN
	db 61, MINIMIZE
	db 0

WigglytuffEvosMoves:
; Evolutions
	db 0
; Learnset
	db 7, POUND
	db 10, DREAM_EATER
	db 12, SCREECH
	db 14, DISABLE
	db 17, WATER_GUN
	db 20, SONICBOOM
	db 22, TELEPORT
	db 26, DEFENSE_CURL
	db 27, REST
	db 30, BODY_SLAM
	db 33, MIMIC
	db 36, SHOCK_WAVE
	db 39, HEADBUTT
	db 42, DOUBLE_EDGE
	db 44, PLAY_ROUGH
	db 47, WATER_PULSE
	db 49, TAKE_DOWN
	db 52, FIRE_PUNCH
	db 53, ICE_PUNCH
	db 54, THUNDERPUNCH
	db 55, DRAIN_PUNCH
	db 57, FLAMETHROWER
	db 61, LIGHT_SCREEN
	db 65, MINIMIZE
	db 0

ZubatEvosMoves:
; Evolutions
	db EV_LEVEL, 22, GOLBAT
	db 0
; Learnset
	db 10, SUPERSONIC
	db 13, ABSORB
	db 15, BITE
	db 17, LEECH_LIFE
	db 18, WHIRLWIND
	db 19, WING_ATTACK
	db 21, CONFUSE_RAY
	db 23, SWIFT
	db 24, AIR_CUTTER
	db 25, GUST
	db 27, SUPER_FANG
	db 29, SLUDGE_WAVE
	db 31, MEGA_DRAIN
	db 33, TWISTER
	db 35, AGILITY
	db 37, HAZE
	db 40, HEADBUTT
	db 42, ZEN_HEADBUTT
	db 45, CRUNCH
	db 48, ROOST
	db 50, STEEL_WING
	db 52, AERIAL_ACE
	db 55, AIR_SLASH
	db 0

GolbatEvosMoves:
; Evolutions
	db EV_LEVEL, 42, CROBAT
	db 0
; Learnset
	db 10, SUPERSONIC
	db 13, ABSORB
	db 15, BITE
	db 17, LEECH_LIFE
	db 18, WHIRLWIND
	db 19, WING_ATTACK
	db 21, CONFUSE_RAY
	db 24, SWIFT
	db 25, AIR_CUTTER
	db 26, GUST
	db 29, SUPER_FANG
	db 31, SLUDGE_WAVE
	db 33, MEGA_DRAIN
	db 35, TWISTER
	db 38, AGILITY
	db 41, HAZE
	db 44, HEADBUTT
	db 45, ZEN_HEADBUTT
	db 48, CRUNCH
	db 51, ROOST
	db 54, STEEL_WING
	db 57, AERIAL_ACE
	db 60, AIR_SLASH
	db 63, HEAT_WAVE
	db 0

CrobatEvosMoves:
; Evolutions
	db 0
; Learnset
	db 10, SUPERSONIC
	db 13, ABSORB
	db 15, BITE
	db 17, LEECH_LIFE
	db 18, WHIRLWIND
	db 19, WING_ATTACK
	db 21, CONFUSE_RAY
	db 24, SWIFT
	db 25, AIR_CUTTER
	db 26, GUST
	db 29, SUPER_FANG
	db 31, SLUDGE_WAVE
	db 33, MEGA_DRAIN
	db 35, TWISTER
	db 38, AGILITY
	db 41, HAZE
	db 42, X_SCISSOR
	db 48, HEADBUTT
	db 50, ZEN_HEADBUTT
	db 53, CRUNCH
	db 55, ROOST
	db 57, GUNK_SHOT
	db 59, STEEL_WING
	db 61, AERIAL_ACE
	db 64, AIR_SLASH
	db 66, HURRICANE
	db 68, HEAT_WAVE
	db 0

OddishEvosMoves:
; Evolutions
	db EV_LEVEL, 21, GLOOM
	db 0
; Learnset
	db 4, GROWTH
	db 7, ACID
	db 11, LEECH_SEED
	db 13, MEGA_DRAIN
	db 15, POISONPOWDER
	db 16, STUN_SPORE
	db 18, SLEEP_POWDER
	db 20, BULLET_SEED
	db 22, GIGA_DRAIN
	db 24, TOXIC
	db 26, ENERGY_BALL
	db 28, MOONBLAST
	db 30, INFESTATION
	db 32, GRASSY_TERRAIN
	db 34, PETAL_DANCE
	db 36, RAZOR_LEAF
	db 39, SLUDGE_WAVE
	db 44, SOLARBEAM
	db 0

GloomEvosMoves:
; Evolutions
	db EV_ITEM, LEAF_STONE, 1, VILEPLUME
	db EV_ITEM, SUN_STONE, 1, BELLOSSOM
	db 0
; Learnset
	db 4, GROWTH
	db 7, ACID
	db 11, LEECH_SEED
	db 13, MEGA_DRAIN
	db 15, POISONPOWDER
	db 16, STUN_SPORE
	db 18, SLEEP_POWDER
	db 20, BULLET_SEED
	db 25, GIGA_DRAIN
	db 27, TOXIC
	db 29, ENERGY_BALL
	db 31, MOONBLAST
	db 33, INFESTATION
	db 34, DRAIN_PUNCH
	db 36, GRASSY_TERRAIN
	db 37, PETAL_DANCE
	db 39, RAZOR_LEAF
	db 41, SLUDGE_WAVE
	db 44, TAKE_DOWN
	db 47, SOLARBEAM
	db 0

VileplumeEvosMoves:
; Evolutions
	db 0
; Learnset
	db 4, GROWTH
	db 7, ACID
	db 11, LEECH_SEED
	db 13, MEGA_DRAIN
	db 15, POISONPOWDER
	db 16, STUN_SPORE
	db 18, SLEEP_POWDER
	db 20, BULLET_SEED
	db 25, GIGA_DRAIN
	db 27, TOXIC
	db 29, ENERGY_BALL
	db 31, MOONBLAST
	db 33, INFESTATION
	db 35, DRAIN_PUNCH
	db 38, GRASSY_TERRAIN
	db 40, PETAL_DANCE
	db 42, RAZOR_LEAF
	db 44, SLUDGE_WAVE
	db 47, TAKE_DOWN
	db 51, SOLARBEAM
	db 0

BellossomEvosMoves:
; Evolutions
	db 0
; Learnset
	db 4, GROWTH
	db 7, ACID
	db 11, LEECH_SEED
	db 13, MEGA_DRAIN
	db 15, POISONPOWDER
	db 16, STUN_SPORE
	db 18, SLEEP_POWDER
	db 20, BULLET_SEED
	db 23, SUNNY_DAY
	db 25, GIGA_DRAIN
	db 27, TOXIC
	db 29, ENERGY_BALL
	db 31, MOONBLAST
	db 33, INFESTATION
	db 35, DRAIN_PUNCH
	db 36, GRASSY_TERRAIN
	db 37, PLAY_ROUGH
	db 40, PETAL_DANCE
	db 42, RAZOR_LEAF
	db 44, SLUDGE_WAVE
	db 47, TAKE_DOWN
	db 51, SOLARBEAM
	db 0

ParasEvosMoves:
; Evolutions
	db EV_LEVEL, 24, PARASECT
	db 0
; Learnset
	db 6, ABSORB
	db 9, LEECH_LIFE
	db 12, LEECH_SEED
	db 14, POISONPOWDER
	db 16, STUN_SPORE
	db 18, INFESTATION
	db 20, MEGA_DRAIN
	db 22, SPORE
	db 24, GROWTH
	db 25, STICKY_WEB
	db 27, SLASH
	db 29, BULLET_SEED
	db 32, SCREECH
	db 35, PSYBEAM
	db 38, GIGA_DRAIN
	db 40, AERIAL_ACE
	db 43, AGILITY
	db 45, METAL_CLAW
	db 47, X_SCISSOR
	db 49, ENERGY_BALL
	db 52, LIGHT_SCREEN
	db 55, SOLARBEAM
	db 0

ParasectEvosMoves:
; Evolutions
	db 0
; Learnset
	db 6, ABSORB
	db 9, LEECH_LIFE
	db 12, LEECH_SEED
	db 14, POISONPOWDER
	db 16, STUN_SPORE
	db 18, INFESTATION
	db 20, MEGA_DRAIN
	db 22, SPORE
	db 24, GROWTH
	db 27, TAKE_DOWN
	db 29, SLASH
	db 31, STICKY_WEB
	db 33, BULLET_SEED
	db 35, SCREECH
	db 38, PSYBEAM
	db 41, GIGA_DRAIN
	db 44, AERIAL_ACE
	db 46, AGILITY
	db 48, METAL_CLAW
	db 50, X_SCISSOR
	db 53, ENERGY_BALL
	db 56, LIGHT_SCREEN
	db 59, SOLARBEAM
	db 0

VenonatEvosMoves:
; Evolutions
	db EV_LEVEL, 31, VENOMOTH
	db 0
; Learnset
	db 5, SUPERSONIC
	db 11, CONFUSION
	db 13, POISONPOWDER
	db 15, PSYBEAM
	db 18, SCREECH
	db 21, SWIFT
	db 23, STUN_SPORE
	db 25, MEGA_DRAIN
	db 27, LEECH_LIFE
	db 29, SLEEP_POWDER
	db 32, TAKE_DOWN
	db 35, AGILITY
	db 37, ZEN_HEADBUTT
	db 43, PSYCHIC_M
	db 47, SOLARBEAM
	db 0

VenomothEvosMoves:
; Evolutions
	db 0
; Learnset
	db 5, SUPERSONIC
	db 11, CONFUSION
	db 13, POISONPOWDER
	db 15, PSYBEAM
	db 18, SCREECH
	db 21, SWIFT
	db 23, STUN_SPORE
	db 25, MEGA_DRAIN
	db 27, LEECH_LIFE
	db 29, SLEEP_POWDER
	db 32, GUST
	db 35, TAKE_DOWN
	db 37, AGILITY
	db 39, AERIAL_ACE
	db 41, ZEN_HEADBUTT
	db 44, WHIRLWIND
	db 46, ENERGY_BALL
	db 50, PSYCHIC_M
	db 52, ROOST
	db 55, SOLARBEAM
	db 0

DiglettEvosMoves:
; Evolutions
	db EV_LEVEL, 26, DUGTRIO
	db 0
; Learnset
	db 4, GROWL
	db 12, MUD_SLAP
	db 15, FURY_SWIPES
	db 18, DIG
	db 20, HEADBUTT
	db 22, SCREECH
	db 24, SAND_ATTACK
	db 27, SLASH
	db 28, SANDSTORM
	db 29, TAKE_DOWN
	db 33, AERIAL_ACE
	db 36, ROCK_TOMB
	db 38, AGILITY
	db 40, EARTHQUAKE
	db 43, THRASH
	db 46, SHADOW_CLAW
	db 51, FISSURE
	db 0

DugtrioEvosMoves:
; Evolutions
	db 0
; Learnset
	db 4, GROWL
	db 12, MUD_SLAP
	db 15, FURY_SWIPES
	db 18, DIG
	db 20, HEADBUTT
	db 22, SCREECH
	db 24, SAND_ATTACK
	db 28, SANDSTORM
	db 30, SLASH
	db 32, TAKE_DOWN
	db 36, AERIAL_ACE
	db 39, ROCK_TOMB
	db 41, AGILITY
	db 46, EARTHQUAKE
	db 49, THRASH
	db 52, SHADOW_CLAW
	db 54, NIGHT_SLASH
	db 57, SLUDGE_WAVE
	db 59, FISSURE
	db 0

MeowthEvosMoves:
; Evolutions
	db EV_LEVEL, 28, PERSIAN
	db 0
; Learnset
	db 8, LEER
	db 10, DEFENSE_CURL
	db 12, BITE
	db 15, PAY_DAY
	db 17, MUD_SLAP
	db 19, SWIFT
	db 22, HEADBUTT
	db 25, SCREECH
	db 27, FURY_SWIPES
	db 29, TAKE_DOWN
	db 30, CRUNCH
	db 31, WATER_PULSE
	db 33, SLASH
	db 35, SING
	db 36, DREAM_EATER
	db 37, HYPNOSIS
	db 39, METAL_CLAW
	db 41, AERIAL_ACE
	db 43, AMNESIA
	db 45, PLAY_ROUGH
	db 47, IRON_TAIL
	db 49, NIGHT_SLASH
	db 52, SHADOW_CLAW
	db 55, THRASH
	db 0

PersianEvosMoves:
; Evolutions
	db 0
; Learnset
	db 8, LEER
	db 10, DEFENSE_CURL
	db 12, BITE
	db 15, PAY_DAY
	db 17, MUD_SLAP
	db 19, SWIFT
	db 22, HEADBUTT
	db 25, SCREECH
	db 27, FURY_SWIPES
	db 33, TAKE_DOWN
	db 35, WATER_PULSE
	db 38, SLASH
	db 40, SING
	db 41, DREAM_EATER
	db 42, HYPNOSIS
	db 43, AERIAL_ACE
	db 45, ROAR
	db 47, AMNESIA
	db 49, PLAY_ROUGH
	db 52, IRON_TAIL
	db 55, NIGHT_SLASH
	db 58, SHADOW_CLAW
	db 0

PsyduckEvosMoves:
; Evolutions
	db EV_LEVEL, 33, GOLDUCK
	db 0
; Learnset
	db 3, LEER
	db 6, WATER_GUN
	db 8, CONFUSION
	db 10, FURY_SWIPES
	db 12, WATER_PULSE
	db 15, DISABLE
	db 17, ZEN_HEADBUTT
	db 20, SCREECH
	db 22, BUBBLE
	db 24, MUD_SLAP
	db 27, SWIFT
	db 28, HEADBUTT
	db 33, PSYBEAM
	db 34, AMNESIA
	db 37, CONFUSE_RAY
	db 40, LIGHT_SCREEN
	db 42, TAKE_DOWN
	db 45, IRON_TAIL
	db 48, HYPNOSIS
	db 50, SCALD
	db 53, SHADOW_CLAW
	db 55, ICE_PUNCH
	db 60, HYDRO_PUMP
	db 0

GolduckEvosMoves:
; Evolutions
	db 0
; Learnset
	db 3, LEER
	db 6, WATER_GUN
	db 8, CONFUSION
	db 10, FURY_SWIPES
	db 12, WATER_PULSE
	db 15, DISABLE
	db 17, ZEN_HEADBUTT
	db 20, SCREECH
	db 22, BUBBLE
	db 24, MUD_SLAP
	db 27, SWIFT
	db 28, HEADBUTT
	db 33, PSYBEAM
	db 35, AERIAL_ACE
	db 37, AMNESIA
	db 41, CONFUSE_RAY
	db 43, SUBMISSION
	db 44, LIGHT_SCREEN
	db 47, TAKE_DOWN
	db 50, IRON_TAIL
	db 52, HYPNOSIS
	db 54, SCALD
	db 56, LOW_KICK
	db 58, SHADOW_CLAW
	db 61, ICE_PUNCH
	db 65, HYDRO_PUMP
	db 0

MankeyEvosMoves:
; Evolutions
	db EV_LEVEL, 28, PRIMEAPE
	db 0
; Learnset
	db 7, DEFENSE_CURL
	db 9, FURY_SWIPES
	db 11, LOW_KICK
	db 13, MUD_SLAP
	db 16, SEISMIC_TOSS
	db 18, KARATE_CHOP
	db 21, HEADBUTT
	db 24, SWIFT
	db 27, FOCUS_ENERGY
	db 29, IRON_TAIL
	db 32, SCREECH
	db 35, THRASH
	db 37, AERIAL_ACE
	db 39, ROCK_TOMB
	db 41, TAKE_DOWN
	db 43, SUBMISSION
	db 45, ROAR
	db 47, NIGHT_SLASH
	db 49, ICE_PUNCH
	db 50, FIRE_PUNCH
	db 51, THUNDERPUNCH
	db 54, CLOSE_COMBAT
	db 57, OUTRAGE
	db 0

PrimeapeEvosMoves:
; Evolutions
	db EV_LEVEL, 38, ANNIHILAPE
	db 0
; Learnset
	db 7, DEFENSE_CURL
	db 9, FURY_SWIPES
	db 11, LOW_KICK
	db 13, MUD_SLAP
	db 16, SEISMIC_TOSS
	db 18, KARATE_CHOP
	db 21, HEADBUTT
	db 24, SWIFT
	db 27, FOCUS_ENERGY
	db 34, SCREECH
	db 37, THRASH
	db 39, AERIAL_ACE
	db 42, ROCK_TOMB
	db 45, TAKE_DOWN
	db 47, SUBMISSION
	db 49, ROAR
	db 51, NIGHT_SLASH
	db 53, ICE_PUNCH
	db 54, FIRE_PUNCH
	db 55, THUNDERPUNCH
	db 59, CLOSE_COMBAT
	db 62, OUTRAGE
	db 0

AnnihilapeEvosMoves:
; Evolutions
	db 0
; Learnset
	db 7, DEFENSE_CURL
	db 9, FURY_SWIPES
	db 11, LOW_KICK
	db 13, MUD_SLAP
	db 16, SEISMIC_TOSS
	db 18, KARATE_CHOP
	db 21, HEADBUTT
	db 24, SWIFT
	db 27, FOCUS_ENERGY
	db 34, SCREECH
	db 37, THRASH
	db 38, SHADOW_PUNCH
	db 40, AERIAL_ACE
	db 42, COUNTER
	db 44, ROCK_TOMB
	db 45, SHADOW_CLAW
	db 47, TAKE_DOWN
	db 49, NIGHT_SHADE
	db 51, SUBMISSION
	db 53, DRAIN_PUNCH
	db 54, ROAR
	db 55, NIGHT_SLASH
	db 57, ICE_PUNCH
	db 58, FIRE_PUNCH
	db 59, THUNDERPUNCH
	db 61, CLOSE_COMBAT
	db 65, OUTRAGE
	db 0

GrowlitheEvosMoves:
; Evolutions
	db EV_ITEM, FIRE_STONE, 1, ARCANINE
	db 0
; Learnset
	db 8, TACKLE
	db 13, BITE
	db 15, ROAR
	db 17, EMBER
	db 19, AGILITY
	db 21, MUD_SLAP
	db 23, LEER
	db 26, SWIFT
	db 28, DOUBLE_KICK
	db 30, TAKE_DOWN
	db 32, CRUNCH
	db 34, DRAGON_RAGE
	db 36, AERIAL_ACE
	db 38, HEADBUTT
	db 40, FLAMETHROWER
	db 43, THRASH
	db 45, IRON_TAIL
	db 48, PLAY_ROUGH
	db 51, FIRE_SPIN
	db 54, DRAGONBREATH
	db 57, HEAT_WAVE
	db 60, FLARE_BLITZ
	db 0

ArcanineEvosMoves:
; Evolutions
	db 0
; Learnset
	db 8, TACKLE
	db 13, BITE
	db 15, ROAR
	db 17, EMBER
	db 19, AGILITY
	db 21, MUD_SLAP
	db 23, LEER
	db 26, SWIFT
	db 28, DOUBLE_KICK
	db 30, TAKE_DOWN
	db 32, CRUNCH
	db 34, DRAGON_RAGE
	db 36, AERIAL_ACE
	db 40, HEADBUTT
	db 42, EXTREMESPEED
	db 44, FLAMETHROWER
	db 46, THRASH
	db 47, IRON_TAIL
	db 51, PLAY_ROUGH
	db 54, FIRE_SPIN
	db 56, DRAGONBREATH
	db 59, HEAT_WAVE
	db 62, FLARE_BLITZ
	db 64, SUPERPOWER
	db 66, SOLARBEAM
	db 0

PoliwagEvosMoves:
; Evolutions
	db EV_LEVEL, 25, POLIWHIRL
	db 0
; Learnset
	db 6, POUND
	db 9, DEFENSE_CURL
	db 12, MUD_SHOT
	db 15, HYPNOSIS
	db 17, WATER_GUN
	db 19, BUBBLEBEAM
	db 21, LOW_KICK
	db 23, HEADBUTT
	db 25, GROWTH
	db 27, RAIN_DANCE
	db 30, BODY_SLAM
	db 33, WATER_PULSE
	db 36, TAKE_DOWN
	db 38, AMNESIA
	db 42, HYDRO_PUMP
	db 45, HAZE
	db 49, DOUBLE_EDGE
	db 53, LOVELY_KISS
	db 56, SCALD
	db 0

PoliwhirlEvosMoves:
; Evolutions
	db EV_ITEM, WATER_STONE, 1, POLIWRATH
	db EV_ITEM, KINGS_ROCK, 1, POLITOED
	db 0
; Learnset
	db 6, POUND
	db 9, DEFENSE_CURL
	db 12, MUD_SHOT
	db 15, HYPNOSIS
	db 17, WATER_GUN
	db 19, BUBBLEBEAM
	db 21, LOW_KICK
	db 23, HEADBUTT
	db 24, ICE_PUNCH
	db 25, SUBMISSION
	db 27, MUD_SLAP
	db 29, RAIN_DANCE
	db 32, BODY_SLAM
	db 34, WATER_PULSE
	db 38, TAKE_DOWN
	db 40, AMNESIA
	db 44, HYDRO_PUMP
	db 48, HAZE
	db 51, DOUBLE_EDGE
	db 55, LOVELY_KISS
	db 58, SCALD
	db 0

PoliwrathEvosMoves:
; Evolutions
	db 0
; Learnset
	db 6, POUND
	db 9, DEFENSE_CURL
	db 12, MUD_SHOT
	db 15, HYPNOSIS
	db 17, WATER_GUN
	db 19, BUBBLEBEAM
	db 21, LOW_KICK
	db 23, HEADBUTT
	db 24, ICE_PUNCH
	db 25, SUBMISSION
	db 27, MUD_SLAP
	db 29, RAIN_DANCE
	db 32, BODY_SLAM
	db 34, WATER_PULSE
	db 40, TAKE_DOWN
	db 42, ROCK_TOMB
	db 44, AMNESIA
	db 46, HYDRO_PUMP
	db 49, HAZE
	db 53, DRAIN_PUNCH
	db 55, DOUBLE_EDGE
	db 58, SUPERPOWER
	db 61, LOVELY_KISS
	db 64, SCALD
	db 0

PolitoedEvosMoves:
; Evolutions
	db 0
; Learnset
	db 6, POUND
	db 9, DEFENSE_CURL
	db 12, MUD_SHOT
	db 15, HYPNOSIS
	db 17, WATER_GUN
	db 19, BUBBLEBEAM
	db 21, LOW_KICK
	db 23, HEADBUTT
	db 24, ICE_PUNCH
	db 25, SUBMISSION
	db 27, MUD_SLAP
	db 29, RAIN_DANCE
	db 32, BODY_SLAM
	db 34, WATER_PULSE
	db 40, TAKE_DOWN
	db 42, AMNESIA
	db 44, HYDRO_PUMP
	db 46, HAZE
	db 50, DOUBLE_EDGE
	db 53, LOVELY_KISS
	db 55, SCALD
	db 0

AbraEvosMoves:
; Evolutions
	db EV_LEVEL, 16, KADABRA
	db 0
; Learnset
	db 0

KadabraEvosMoves:
; Evolutions
	db EV_TRADE, 1, ALAKAZAM
	db EV_LEVEL, 42, ALAKAZAM
	db EV_ITEM, LINK_CABLE, 1, ALAKAZAM
	db 0
; Learnset
	db 16, CONFUSION
	db 18, DISABLE
	db 20, BARRIER
	db 23, PSYBEAM
	db 25, SWIFT
	db 27, NIGHT_SHADE
	db 30, RECOVER
	db 32, HYPNOSIS
	db 33, DREAM_EATER
	db 35, REFLECT
	db 37, LIGHT_SCREEN
	db 39, ZEN_HEADBUTT
	db 41, IRON_TAIL
	db 43, THUNDERPUNCH
	db 44, ICE_PUNCH
	db 45, FIRE_PUNCH
	db 47, DRAIN_PUNCH
	db 50, PSYCHIC_M
	db 52, SHOCK_WAVE
	db 55, ENERGY_BALL
	db 57, PSYCHO_CUT
	db 60, ZAP_CANNON
	db 0

AlakazamEvosMoves:
; Evolutions
	db EV_ITEM, MEGA_STONE_X, 1, MALAKAZAM
	db EV_ITEM, MEGA_STONE_Y, 1, MALAKAZAM
	db 0
; Learnset
	db 16, CONFUSION
	db 18, DISABLE
	db 20, BARRIER
	db 23, PSYBEAM
	db 25, SWIFT
	db 27, NIGHT_SHADE
	db 30, RECOVER
	db 32, HYPNOSIS
	db 33, DREAM_EATER
	db 35, REFLECT
	db 37, LIGHT_SCREEN
	db 39, ZEN_HEADBUTT
	db 41, IRON_TAIL
	db 46, THUNDERPUNCH
	db 47, ICE_PUNCH
	db 48, FIRE_PUNCH
	db 50, DRAIN_PUNCH
	db 53, PSYCHIC_M
	db 55, SHOCK_WAVE
	db 57, ENERGY_BALL
	db 60, PSYCHO_CUT
	db 63, ZAP_CANNON
	db 0

MAlakazamEvosMoves:
; Evolutions
	db 0
; Learnset
	db 21, CONFUSION
	db 23, DISABLE
	db 25, BARRIER
	db 28, PSYBEAM
	db 30, SWIFT
	db 32, NIGHT_SHADE
	db 35, RECOVER
	db 37, HYPNOSIS
	db 38, DREAM_EATER
	db 40, REFLECT
	db 42, LIGHT_SCREEN
	db 44, ZEN_HEADBUTT
	db 46, IRON_TAIL
	db 51, THUNDERPUNCH
	db 52, ICE_PUNCH
	db 53, FIRE_PUNCH
	db 55, DRAIN_PUNCH
	db 58, PSYCHIC_M
	db 60, SHOCK_WAVE
	db 62, ENERGY_BALL
	db 65, PSYCHO_CUT
	db 68, ZAP_CANNON
	db 71, NIGHT_SLASH
	db 0

MachopEvosMoves:
; Evolutions
	db EV_LEVEL, 28, MACHOKE
	db 0
; Learnset
	db 4, FOCUS_ENERGY
	db 7, LEER
	db 9, TACKLE
	db 12, LOW_KICK
	db 18, ROCK_TOMB
	db 22, MUD_SLAP
	db 25, HEADBUTT
	db 27, KARATE_CHOP
	db 31, TAKE_DOWN
	db 34, THRASH
	db 37, SEISMIC_TOSS
	db 41, ROLLING_KICK
	db 44, BULLET_PUNCH
	db 45, THUNDERPUNCH
	db 46, FIRE_PUNCH
	db 47, ICE_PUNCH
	db 48, MACH_PUNCH
	db 53, SUBMISSION
	db 55, DOUBLE_EDGE
	db 58, SUPERPOWER
	db 0

MachokeEvosMoves:
; Evolutions
	db EV_TRADE, 1, MACHAMP
	db EV_LEVEL, 42, MACHAMP
	db EV_ITEM, LINK_CABLE, 1, MACHAMP
	db 0
; Learnset
	db 4, FOCUS_ENERGY
	db 7, LEER
	db 9, TACKLE
	db 12, LOW_KICK
	db 18, ROCK_TOMB
	db 22, MUD_SLAP
	db 25, HEADBUTT
	db 27, KARATE_CHOP
	db 34, TAKE_DOWN
	db 37, THRASH
	db 40, SEISMIC_TOSS
	db 44, ROLLING_KICK
	db 48, BULLET_PUNCH
	db 49, THUNDERPUNCH
	db 50, FIRE_PUNCH
	db 51, ICE_PUNCH
	db 52, MACH_PUNCH
	db 56, SUBMISSION
	db 59, DOUBLE_EDGE
	db 62, SUPERPOWER
	db 0

MachampEvosMoves:
; Evolutions
	db 0
; Learnset
	db 4, FOCUS_ENERGY
	db 7, LEER
	db 9, TACKLE
	db 12, LOW_KICK
	db 18, ROCK_TOMB
	db 22, MUD_SLAP
	db 25, HEADBUTT
	db 27, KARATE_CHOP
	db 34, TAKE_DOWN
	db 37, THRASH
	db 40, SEISMIC_TOSS
	db 46, ROLLING_KICK
	db 50, BULLET_PUNCH
	db 51, THUNDERPUNCH
	db 52, FIRE_PUNCH
	db 53, ICE_PUNCH
	db 54, MACH_PUNCH
	db 55, DRAIN_PUNCH
	db 59, SUBMISSION
	db 62, DOUBLE_EDGE
	db 65, SUPERPOWER
	db 0

BellsproutEvosMoves:
; Evolutions
	db EV_LEVEL, 21, WEEPINBELL
	db 0
; Learnset
	db 7, GROWTH
	db 11, WRAP
	db 13, SLEEP_POWDER
	db 15, POISONPOWDER
	db 17, STUN_SPORE
	db 20, MEGA_DRAIN
	db 23, ACID
	db 26, BULLET_SEED
	db 29, MAGICAL_LEAF
	db 32, LEECH_LIFE
	db 35, RAZOR_LEAF
	db 37, SLAM
	db 40, ENERGY_BALL
	db 44, TAKE_DOWN
	db 47, LOVELY_KISS
	db 50, SOLARBEAM
	db 0

WeepinbellEvosMoves:
; Evolutions
	db EV_ITEM, LEAF_STONE, 1, VICTREEBEL
	db 0
; Learnset
	db 7, GROWTH
	db 11, WRAP
	db 13, SLEEP_POWDER
	db 15, POISONPOWDER
	db 17, STUN_SPORE
	db 20, MEGA_DRAIN
	db 26, ACID
	db 29, BULLET_SEED
	db 32, MAGICAL_LEAF
	db 36, LEECH_LIFE
	db 39, RAZOR_LEAF
	db 42, SLAM
	db 45, ENERGY_BALL
	db 48, TAKE_DOWN
	db 51, LOVELY_KISS
	db 55, SOLARBEAM
	db 0

VictreebelEvosMoves:
; Evolutions
	db EV_ITEM, MEGA_STONE_X, 1, MVICTREBEL
	db EV_ITEM, MEGA_STONE_Y, 1, MVICTREBEL
	db 0
; Learnset
	db 7, GROWTH
	db 11, WRAP
	db 13, SLEEP_POWDER
	db 15, POISONPOWDER
	db 17, STUN_SPORE
	db 20, MEGA_DRAIN
	db 26, ACID
	db 29, BULLET_SEED
	db 32, MAGICAL_LEAF
	db 36, LEECH_LIFE
	db 42, RAZOR_LEAF
	db 45, SLAM
	db 49, ENERGY_BALL
	db 52, TAKE_DOWN
	db 54, LOVELY_KISS
	db 58, SOLARBEAM
	db 62, LEAF_STORM
	db 0

MVictrebelEvosMoves:
; Evolutions
	db 0
; Learnset
	db 12, GROWTH
	db 16, WRAP
	db 18, SLEEP_POWDER
	db 20, POISONPOWDER
	db 22, STUN_SPORE
	db 25, MEGA_DRAIN
	db 31, ACID
	db 34, BULLET_SEED
	db 37, MAGICAL_LEAF
	db 41, LEECH_LIFE
	db 47, RAZOR_LEAF
	db 50, SLAM
	db 54, ENERGY_BALL
	db 57, TAKE_DOWN
	db 59, LOVELY_KISS
	db 61, INFESTATION
	db 63, SOLARBEAM
	db 65, GUNK_SHOT
	db 67, LEAF_STORM
	db 72, CRUNCH
	db 0

TentacoolEvosMoves:
; Evolutions
	db EV_LEVEL, 30, TENTACRUEL
	db 0
; Learnset
	db 4, ACID
	db 7, SUPERSONIC
	db 8, WRAP
	db 12, BUBBLE
	db 14, TOXIC_SPIKES
	db 16, WATER_PULSE
	db 18, POISON_STING
	db 20, SCREECH
	db 22, WATER_GUN
	db 24, HAZE
	db 25, RAPID_SPIN
	db 27, BUBBLEBEAM
	db 29, MEGA_DRAIN
	db 32, ACID_ARMOR
	db 35, BARRIER
	db 38, SLUDGE_WAVE
	db 41, TAKE_DOWN
	db 45, AURORA_BEAM
	db 48, HEADBUTT
	db 52, CONFUSE_RAY
	db 55, SCALD
	db 58, INFESTATION
	db 62, HYDRO_PUMP
	db 0

TentacruelEvosMoves:
; Evolutions
	db 0
; Learnset
	db 4, ACID
	db 7, SUPERSONIC
	db 8, WRAP
	db 12, BUBBLE
	db 14, TOXIC_SPIKES
	db 16, WATER_PULSE
	db 18, POISON_STING
	db 20, SCREECH
	db 22, WATER_GUN
	db 24, HAZE
	db 25, RAPID_SPIN
	db 27, BUBBLEBEAM
	db 29, MEGA_DRAIN
	db 35, ACID_ARMOR
	db 38, BARRIER
	db 42, SLUDGE_WAVE
	db 45, TAKE_DOWN
	db 49, AURORA_BEAM
	db 53, HEADBUTT
	db 57, CONFUSE_RAY
	db 60, SCALD
	db 62, INFESTATION
	db 65, HYDRO_PUMP
	db 0

GeodudeEvosMoves:
; Evolutions
	db EV_LEVEL, 25, GRAVELER
	db 0
; Learnset
	db 7, DEFENSE_CURL
	db 10, MUD_SLAP
	db 12, HARDEN
	db 16, ROCK_THROW
	db 20, SELFDESTRUCT
	db 24, HEADBUTT
	db 27, SANDSTORM
	db 28, STEALTH_ROCK
	db 29, EARTHQUAKE
	db 31, RAPID_SPIN
	db 33, ROCK_TOMB
	db 36, TAKE_DOWN
	db 39, EXPLOSION
	db 43, DOUBLE_EDGE
	db 45, STONE_EDGE
	db 49, SUBMISSION
	db 52, SUPERPOWER
	db 0

GravelerEvosMoves:
; Evolutions
	db EV_TRADE, 1, GOLEM
	db EV_LEVEL, 42, GOLEM
	db EV_ITEM, LINK_CABLE, 1, GOLEM
	db 0
; Learnset
	db 7, DEFENSE_CURL
	db 10, MUD_SLAP
	db 12, HARDEN
	db 16, ROCK_THROW
	db 20, SELFDESTRUCT
	db 24, HEADBUTT
	db 31, SANDSTORM
	db 32, STEALTH_ROCK
	db 33, EARTHQUAKE
	db 35, RAPID_SPIN
	db 37, ROCK_TOMB
	db 39, TAKE_DOWN
	db 43, EXPLOSION
	db 46, DOUBLE_EDGE
	db 49, STONE_EDGE
	db 52, SUBMISSION
	db 55, SUPERPOWER
	db 0

GolemEvosMoves:
; Evolutions
	db 0
; Learnset
	db 7, DEFENSE_CURL
	db 10, MUD_SLAP
	db 12, HARDEN
	db 16, ROCK_THROW
	db 20, SELFDESTRUCT
	db 24, HEADBUTT
	db 31, SANDSTORM
	db 32, STEALTH_ROCK
	db 33, EARTHQUAKE
	db 35, RAPID_SPIN
	db 37, ROCK_TOMB
	db 39, TAKE_DOWN
	db 46, EXPLOSION
	db 48, ROAR
	db 50, DOUBLE_EDGE
	db 52, STONE_EDGE
	db 55, SUBMISSION
	db 59, SUPERPOWER
	db 0

PonytaEvosMoves:
; Evolutions
	db EV_LEVEL, 40, RAPIDASH
	db 0
; Learnset
	db 5, LEER
	db 7, TACKLE
	db 10, EMBER
	db 12, QUICK_ATTACK
	db 15, GROWL
	db 17, DOUBLE_KICK
	db 19, SWIFT
	db 22, AGILITY
	db 25, LOW_KICK
	db 27, HEADBUTT
	db 30, STOMP
	db 32, IRON_TAIL
	db 35, FIRE_SPIN
	db 38, THRASH
	db 41, TAKE_DOWN
	db 43, PLAY_ROUGH
	db 45, FLAMETHROWER
	db 48, HYPNOSIS
	db 50, FIRE_BLAST
	db 52, SOLARBEAM
	db 55, HEAT_WAVE
	db 57, FLARE_BLITZ
	db 60, HORN_DRILL
	db 0

RapidashEvosMoves:
; Evolutions
	db 0
; Learnset
	db 5, LEER
	db 7, TACKLE
	db 10, EMBER
	db 12, QUICK_ATTACK
	db 15, GROWL
	db 17, DOUBLE_KICK
	db 19, SWIFT
	db 22, AGILITY
	db 25, LOW_KICK
	db 27, HEADBUTT
	db 30, STOMP
	db 32, IRON_TAIL
	db 35, FIRE_SPIN
	db 38, THRASH
	db 41, FURY_ATTACK
	db 44, TAKE_DOWN
	db 47, PLAY_ROUGH
	db 50, MEGAHORN
	db 52, FLAMETHROWER
	db 54, HYPNOSIS
	db 56, FIRE_BLAST
	db 58, SOLARBEAM
	db 60, HEAT_WAVE
	db 62, FLARE_BLITZ
	db 64, HORN_DRILL
	db 66, DRILL_RUN
	db 0

SlowpokeEvosMoves:
; Evolutions
	db EV_LEVEL, 37, SLOWBRO
	db EV_ITEM, KINGS_ROCK, 1, SLOWKING
	db 0
; Learnset
	db 3, GROWL
	db 5, TACKLE
	db 7, WATER_GUN
	db 9, DEFENSE_CURL
	db 12, CONFUSION
	db 14, MUD_SLAP
	db 16, DISABLE
	db 19, WATER_PULSE
	db 22, HEADBUTT
	db 24, ZEN_HEADBUTT
	db 26, STOMP
	db 29, AMNESIA
	db 33, MUD_SHOT
	db 36, PSYCHIC_M
	db 39, TAKE_DOWN
	db 41, LIGHT_SCREEN
	db 44, IRON_TAIL
	db 47, SCALD
	db 49, RAIN_DANCE
	db 51, DREAM_EATER
	db 54, FLAMETHROWER
	db 59, ZAP_CANNON
	db 0

SlowbroEvosMoves:
; Evolutions
	db EV_ITEM, MEGA_STONE_X, 1, MSLOWBRO
	db EV_ITEM, MEGA_STONE_Y, 1, MSLOWBRO
	db 0
; Learnset
	db 3, GROWL
	db 5, TACKLE
	db 7, WATER_GUN
	db 9, DEFENSE_CURL
	db 12, CONFUSION
	db 14, MUD_SLAP
	db 16, DISABLE
	db 19, WATER_PULSE
	db 22, HEADBUTT
	db 24, ZEN_HEADBUTT
	db 26, STOMP
	db 29, AMNESIA
	db 33, MUD_SHOT
	db 36, PSYCHIC_M
	db 38, SUBMISSION
	db 43, TAKE_DOWN
	db 45, LIGHT_SCREEN
	db 47, ICE_PUNCH
	db 49, IRON_TAIL
	db 51, SCALD
	db 53, DRAIN_PUNCH
	db 55, RAIN_DANCE
	db 59, DREAM_EATER
	db 60, FLAMETHROWER
	db 63, ZAP_CANNON
	db 0

MSlowbroEvosMoves:
; Evolutions
	db 0
; Learnset
	db 8, GROWL
	db 10, TACKLE
	db 12, WATER_GUN
	db 14, DEFENSE_CURL
	db 17, CONFUSION
	db 19, MUD_SLAP
	db 21, DISABLE
	db 24, WATER_PULSE
	db 27, HEADBUTT
	db 29, ZEN_HEADBUTT
	db 31, STOMP
	db 34, AMNESIA
	db 38, MUD_SHOT
	db 41, PSYCHIC_M
	db 43, SUBMISSION
	db 45, SPIKE_CANNON
	db 48, TAKE_DOWN
	db 50, LIGHT_SCREEN
	db 52, ICE_PUNCH
	db 54, IRON_TAIL
	db 56, SCALD
	db 57, PIN_MISSILE
	db 58, DRAIN_PUNCH
	db 60, RAIN_DANCE
	db 62, CLAMP
	db 64, DREAM_EATER
	db 63, AURORA_BEAM
	db 65, FLAMETHROWER
	db 66, HYPNOSIS
	db 68, ZAP_CANNON
	db 0

SlowkingEvosMoves:
; Evolutions
	db 0
; Learnset
	db 2, GROWL
	db 4, TACKLE
	db 6, CONFUSION
	db 8, DEFENSE_CURL
	db 10, WATER_GUN
	db 12, DISABLE
	db 15, MUD_SLAP
	db 18, WATER_PULSE
	db 20, HEADBUTT
	db 22, ZEN_HEADBUTT
	db 24, AMNESIA
	db 26, STOMP
	db 28, PSYCHIC_M
	db 30, MUD_SHOT
	db 34, PSYCHIC_M
	db 36, SUBMISSION
	db 38, DREAM_EATER
	db 40, LIGHT_SCREEN
	db 42, FLAMETHROWER
	db 44, IRON_TAIL
	db 46, SCALD
	db 48, DRAIN_PUNCH
	db 50, TAKE_DOWN
	db 52, RAIN_DANCE
	db 54, ICE_PUNCH
	db 56, ZAP_CANNON
	db 0

MagnemiteEvosMoves:
; Evolutions
	db EV_LEVEL, 30, MAGNETON
	db 0
; Learnset
	db 5, THUNDERSHOCK
	db 8, TACKLE
	db 11, THUNDER_WAVE
	db 13, LIGHT_SCREEN
	db 18, SONICBOOM
	db 23, HEADBUTT
	db 26, SHOCK_WAVE
	db 29, SUPERSONIC
	db 31, AGILITY
	db 33, SWIFT
	db 36, THUNDERBOLT
	db 38, FLASH_CANNON
	db 41, SCREECH
	db 45, TAKE_DOWN
	db 47, THUNDER
	db 51, ZAP_CANNON
	db 0

MagnetonEvosMoves:
; Evolutions
	db EV_ITEM, THUNDER_STONE, 1, MAGNEZONE
	db 0
; Learnset
	db 5, THUNDERSHOCK
	db 8, TACKLE
	db 11, THUNDER_WAVE
	db 13, LIGHT_SCREEN
	db 18, SONICBOOM
	db 23, HEADBUTT
	db 26, SHOCK_WAVE
	db 29, SUPERSONIC
	db 32, ELECTRIC_TERRAIN
	db 35, AGILITY
	db 37, SWIFT
	db 40, THUNDERBOLT
	db 43, FLASH_CANNON
	db 46, SCREECH
	db 49, TAKE_DOWN
	db 52, THUNDER
	db 55, ZAP_CANNON
	db 0

MagnezoneEvosMoves:
; Evolutions
	db 0
; Learnset
	db 5, THUNDERSHOCK
	db 8, TACKLE
	db 11, THUNDER_WAVE
	db 13, LIGHT_SCREEN
	db 18, SONICBOOM
	db 23, HEADBUTT
	db 26, SHOCK_WAVE
	db 29, SUPERSONIC
	db 32, ELECTRIC_TERRAIN
	db 35, AGILITY
	db 37, SWIFT
	db 40, THUNDERBOLT
	db 46, FLASH_CANNON
	db 49, SCREECH
	db 52, TAKE_DOWN
	db 56, THUNDER
	db 59, ZAP_CANNON
	db 0

FarfetchdEvosMoves:
; Evolutions
	db 0
; Learnset
	db 7, LEER
	db 9, FURY_ATTACK
	db 13, GUST
	db 15, MUD_SLAP
	db 18, QUICK_ATTACK
	db 21, AERIAL_ACE
	db 24, SLASH
	db 27, AIR_CUTTER
	db 30, SWORDS_DANCE
	db 32, AGILITY
	db 34, SWIFT
	db 36, TWISTER
	db 38, NIGHT_SLASH
	db 40, FOCUS_ENERGY
	db 42, WHIRLWIND
	db 44, HEADBUTT
	db 46, MIRROR_MOVE
	db 49, AIR_SLASH
	db 51, ROOST
	db 53, IRON_TAIL
	db 55, TAKE_DOWN
	db 58, STEEL_WING
	db 60, HEAT_WAVE
	db 62, RAZOR_LEAF
	db 66, BRAVE_BIRD
	db 0

DoduoEvosMoves:
; Evolutions
	db EV_LEVEL, 31, DODRIO
	db 0
; Learnset
	db 5, QUICK_ATTACK
	db 8, LEER
	db 12, FURY_ATTACK
	db 15, WING_ATTACK
	db 20, GROWL
	db 23, SUPERSONIC
	db 26, AGILITY
	db 28, SWIFT
	db 30, DRILL_PECK
	db 33, HAZE
	db 36, SWORDS_DANCE
	db 39, LOW_KICK
	db 41, WHIRLWIND
	db 43, TAKE_DOWN
	db 46, ROOST
	db 49, AERIAL_ACE
	db 53, STEEL_WING
	db 56, THRASH
	db 0

DodrioEvosMoves:
; Evolutions
	db 0
; Learnset
	db 5, QUICK_ATTACK
	db 8, LEER
	db 12, FURY_ATTACK
	db 15, WING_ATTACK
	db 20, GROWL
	db 23, SUPERSONIC
	db 26, AGILITY
	db 28, SWIFT
	db 30, DRILL_PECK
	db 36, HAZE
	db 39, SWORDS_DANCE
	db 42, LOW_KICK
	db 44, WHIRLWIND
	db 46, TAKE_DOWN
	db 49, ROOST
	db 53, AERIAL_ACE
	db 55, STEEL_WING
	db 60, THRASH
	db 65, DRILL_RUN
	db 0

SeelEvosMoves:
; Evolutions
	db EV_LEVEL, 34, DEWGONG
	db 0
; Learnset
	db 3, GROWL
	db 7, WATER_GUN
	db 10, LICK
	db 15, DISABLE
	db 18, PECK
	db 21, REST
	db 24, WATER_PULSE
	db 27, AURORA_BEAM
	db 31, AQUA_JET
	db 35, IRON_TAIL
	db 37, TAKE_DOWN
	db 39, ICICLE_SPEAR
	db 42, SLAM
	db 45, HAIL
	db 47, ICE_BEAM
	db 52, HORN_DRILL
	db 60, DRILL_RUN
	db 0

DewgongEvosMoves:
; Evolutions
	db 0
; Learnset
	db 3, GROWL
	db 7, WATER_GUN
	db 10, LICK
	db 15, DISABLE
	db 18, PECK
	db 21, REST
	db 24, WATER_PULSE
	db 27, AURORA_BEAM
	db 31, AQUA_JET
	db 38, IRON_TAIL
	db 40, TAKE_DOWN
	db 42, ICICLE_SPEAR
	db 46, SLAM
	db 47, HAIL
	db 51, ICE_BEAM
	db 55, HORN_DRILL
	db 63, DRILL_RUN
	db 0

GrimerEvosMoves:
; Evolutions
	db EV_LEVEL, 38, MUK
	db 0
; Learnset
	db 4, HARDEN
	db 7, MUD_SLAP
	db 10, DISABLE
	db 12, HAZE
	db 15, SLUDGE
	db 18, SMOG
	db 21, MINIMIZE
	db 24, LICK
	db 27, ROCK_TOMB
	db 30, POISON_GAS
	db 32, SLUDGE_WAVE
	db 34, MEGA_DRAIN
	db 36, SCREECH
	db 39, GUNK_SHOT
	db 42, SHADOW_PUNCH
	db 43, THUNDERPUNCH
	db 44, FIRE_PUNCH
	db 45, ICE_PUNCH
	db 49, ACID_ARMOR
	db 53, FLAMETHROWER
	db 56, SHOCK_WAVE
	db 0

MukEvosMoves:
; Evolutions
	db 0
; Learnset
	db 4, HARDEN
	db 7, MUD_SLAP
	db 10, DISABLE
	db 12, HAZE
	db 15, SLUDGE
	db 18, SMOG
	db 21, MINIMIZE
	db 24, LICK
	db 27, ROCK_TOMB
	db 30, POISON_GAS
	db 32, SLUDGE_WAVE
	db 34, MEGA_DRAIN
	db 36, SCREECH
	db 42, GUNK_SHOT
	db 46, SHADOW_PUNCH
	db 47, THUNDERPUNCH
	db 48, FIRE_PUNCH
	db 49, ICE_PUNCH
	db 53, ACID_ARMOR
	db 57, FLAMETHROWER
	db 60, SHOCK_WAVE
	db 0

ShellderEvosMoves:
; Evolutions
	db EV_ITEM, ICE_STONE, 1, CLOYSTER
	db 0
; Learnset
	db 4, DEFENSE_CURL
	db 11, LEER
	db 13, TACKLE
	db 15, WATER_GUN
	db 18, SUPERSONIC
	db 20, SCREECH
	db 22, CLAMP
	db 24, AURORA_BEAM
	db 26, MUD_SHOT
	db 28, RAPID_SPIN
	db 30, BARRIER
	db 32, SWIFT
	db 35, WATER_PULSE
	db 37, TAKE_DOWN
	db 40, ICE_BEAM
	db 43, HEADBUTT
	db 45, TWINEEDLE
	db 48, ICICLE_SPEAR
	db 51, HYDRO_PUMP
	db 0

CloysterEvosMoves:
; Evolutions
	db 0
; Learnset
	db 4, DEFENSE_CURL
	db 11, LEER
	db 13, TACKLE
	db 15, WATER_GUN
	db 18, SUPERSONIC
	db 20, SCREECH
	db 22, CLAMP
	db 24, AURORA_BEAM
	db 26, MUD_SHOT
	db 28, RAPID_SPIN
	db 30, BARRIER
	db 31, LIGHT_SCREEN
	db 32, SWIFT
	db 34, WATER_PULSE
	db 36, SPIKES
	db 37, TOXIC_SPIKES
	db 38, TAKE_DOWN
	db 40, ICE_BEAM
	db 42, ICICLE_CRASH
	db 45, HEADBUTT
	db 47, TWINEEDLE
	db 48, PIN_MISSILE
	db 50, SPIKE_CANNON
	db 54, ICICLE_SPEAR
	db 57, HYDRO_PUMP
	db 62, DRILL_RUN
	db 0

GastlyEvosMoves:
; Evolutions
	db EV_LEVEL, 25, HAUNTER
	db 0
; Learnset
	db 4, HYPNOSIS
	db 7, POISON_GAS
	db 9, SMOG
	db 12, DISABLE
	db 16, HEADBUTT
	db 19, HAZE
	db 23, INFESTATION
	db 28, NIGHT_SHADE
	db 33, MEGA_DRAIN
	db 35, DREAM_EATER
	db 38, DARK_PULSE
	db 40, SHADOW_BALL
	db 43, SLUDGE_WAVE
	db 46, ENERGY_BALL
	db 49, ZAP_CANNON
	db 0

HaunterEvosMoves:
; Evolutions
	db EV_TRADE, 1, GENGAR
	db EV_LEVEL, 42, GENGAR
	db EV_ITEM, LINK_CABLE, 1, GENGAR
	db 0
; Learnset
	db 4, HYPNOSIS
	db 7, POISON_GAS
	db 9, SMOG
	db 12, DISABLE
	db 16, HEADBUTT
	db 19, HAZE
	db 23, INFESTATION
	db 25, SHADOW_PUNCH
	db 30, NIGHT_SHADE
	db 33, SHADOW_CLAW
	db 35, MEGA_DRAIN
	db 38, DREAM_EATER
	db 40, THUNDERPUNCH
	db 41, ICE_PUNCH
	db 42, FIRE_PUNCH
	db 45, DARK_PULSE
	db 47, SHADOW_BALL
	db 50, SLUDGE_WAVE
	db 52, ENERGY_BALL
	db 55, ZAP_CANNON
	db 0

GengarEvosMoves:
; Evolutions
	db EV_ITEM, MEGA_STONE_X, 1, MGENGAR
	db EV_ITEM, MEGA_STONE_Y, 1, MGENGAR
	db 0
; Learnset
	db 4, HYPNOSIS
	db 7, POISON_GAS
	db 9, SMOG
	db 12, DISABLE
	db 16, HEADBUTT
	db 19, HAZE
	db 23, INFESTATION
	db 25, SHADOW_PUNCH
	db 30, NIGHT_SHADE
	db 33, SHADOW_CLAW
	db 35, MEGA_DRAIN
	db 38, DREAM_EATER
	db 40, THUNDERPUNCH
	db 41, ICE_PUNCH
	db 42, FIRE_PUNCH
	db 45, TAKE_DOWN
	db 48, DARK_PULSE
	db 50, SHADOW_BALL
	db 53, DRAIN_PUNCH
	db 55, SLUDGE_WAVE
	db 57, SUBMISSION
	db 59, ENERGY_BALL
	db 63, ZAP_CANNON
	db 0

MGengarEvosMoves:
; Evolutions
	db 0
; Learnset
	db 9, HYPNOSIS
	db 12, POISON_GAS
	db 14, SMOG
	db 17, DISABLE
	db 21, HEADBUTT
	db 24, HAZE
	db 28, INFESTATION
	db 30, SHADOW_PUNCH
	db 35, NIGHT_SHADE
	db 38, SHADOW_CLAW
	db 40, MEGA_DRAIN
	db 43, DREAM_EATER
	db 44, CRUNCH
	db 45, THUNDERPUNCH
	db 46, ICE_PUNCH
	db 47, FIRE_PUNCH
	db 49, SHADOW_SNEAK
	db 50, TAKE_DOWN
	db 53, DARK_PULSE
	db 55, SHADOW_BALL
	db 56, NIGHT_SLASH
	db 57, GUNK_SHOT
	db 58, DRAIN_PUNCH
	db 60, SLUDGE_WAVE
	db 61, FEINT_ATTACK
	db 62, SUBMISSION
	db 64, ENERGY_BALL
	db 68, ZAP_CANNON
	db 0

OnixEvosMoves:
; Evolutions
	db EV_ITEM, METAL_COAT, 1, STEELIX
	db 0
; Learnset
	db 6, DEFENSE_CURL
	db 9, MUD_SLAP
	db 12, DRAGONBREATH
	db 15, WRAP
	db 19, ROCK_THROW
	db 21, ROCK_SLIDE
	db 23, HARDEN
	db 26, SCREECH
	db 28, TWISTER
	db 29, SANDSTORM
	db 30, STEALTH_ROCK
	db 31, ROCK_TOMB
	db 33, SLAM
	db 37, TAKE_DOWN
	db 40, HEADBUTT
	db 42, DIG
	db 44, IRON_TAIL
	db 46, ROAR
	db 48, STONE_EDGE
	db 50, DRAGON_DANCE
	db 52, DOUBLE_EDGE
	db 54, HEAD_SMASH
	db 0

SteelixEvosMoves:
; Evolutions
	db EV_ITEM, MEGA_STONE_X, 1, MSTEELIX
	db EV_ITEM, MEGA_STONE_Y, 1, MSTEELIX
	db 0
; Learnset
	db 6, DEFENSE_CURL
	db 9, MUD_SLAP
	db 12, DRAGONBREATH
	db 15, WRAP
	db 19, ROCK_THROW
	db 21, ROCK_SLIDE
	db 23, HARDEN
	db 28, SCREECH
	db 30, TWISTER
	db 31, SANDSTORM
	db 32, STEALTH_ROCK
	db 33, ROCK_TOMB
	db 35, SLAM
	db 37, CRUNCH
	db 39, TAKE_DOWN
	db 42, HEADBUTT
	db 44, DIG
	db 46, IRON_TAIL
	db 48, ROAR
	db 50, STONE_EDGE
	db 52, DRAGON_DANCE
	db 54, DOUBLE_EDGE
	db 56, HEAD_SMASH
	db 0

MSteelixEvosMoves:
; Evolutions
	db 0
; Learnset
	db 11, DEFENSE_CURL
	db 14, MUD_SLAP
	db 16, DRAGONBREATH
	db 20, WRAP
	db 24, ROCK_THROW
	db 26, ROCK_SLIDE
	db 28, HARDEN
	db 30, SAND_ATTACK
	db 33, SCREECH
	db 35, TWISTER
	db 36, SANDSTORM
	db 37, STEALTH_ROCK
	db 38, ROCK_TOMB
	db 39, MUD_SHOT
	db 40, SLAM
	db 42, CRUNCH
	db 44, TAKE_DOWN
	db 47, HEADBUTT
	db 49, DIG
	db 51, IRON_TAIL
	db 53, ROAR
	db 54, PLAY_ROUGH
	db 55, STONE_EDGE
	db 57, DRAGON_DANCE
	db 59, DOUBLE_EDGE
	db 61, HEAD_SMASH
	db 0

DrowzeeEvosMoves:
; Evolutions
	db EV_LEVEL, 26, HYPNO
	db 0
; Learnset
	db 5, DISABLE
	db 7, TELEPORT
	db 9, CONFUSION
	db 11, BARRIER
	db 13, HEADBUTT
	db 15, LIGHT_SCREEN
	db 17, POISON_GAS
	db 21, HYPNOSIS
	db 22, TAKE_DOWN
	db 25, PSYBEAM
	db 29, DREAM_EATER
	db 32, PSYCHIC_M
	db 35, AMNESIA
	db 42, ICE_PUNCH
	db 43, FIRE_PUNCH
	db 44, THUNDERPUNCH
	db 45, DRAIN_PUNCH
	db 50, ZEN_HEADBUTT
	db 52, PSYCHO_CUT
	db 0

HypnoEvosMoves:
; Evolutions
	db 0
; Learnset
	db 5, DISABLE
	db 7, TELEPORT
	db 9, CONFUSION
	db 11, BARRIER
	db 13, HEADBUTT
	db 15, LIGHT_SCREEN
	db 17, POISON_GAS
	db 21, HYPNOSIS
	db 22, TAKE_DOWN
	db 25, PSYBEAM
	db 30, DREAM_EATER
	db 35, PSYCHIC_M
	db 38, AMNESIA
	db 46, ICE_PUNCH
	db 47, FIRE_PUNCH
	db 48, THUNDERPUNCH
	db 49, DRAIN_PUNCH
	db 53, ZEN_HEADBUTT
	db 57, PSYCHO_CUT
	db 0

KrabbyEvosMoves:
; Evolutions
	db EV_LEVEL, 28, KINGLER
	db 0
; Learnset
	db 4, HARDEN
	db 6, MUD_SLAP
	db 8, METAL_CLAW
	db 10, WATER_GUN
	db 12, MUD_SHOT
	db 15, LEER
	db 17, HAZE
	db 20, BUBBLEBEAM
	db 23, STOMP
	db 25, GUILLOTINE
	db 28, SLASH
	db 30, ROCK_TOMB
	db 32, HEADBUTT
	db 35, CRABHAMMER
	db 37, SLAM
	db 39, SWORDS_DANCE
	db 42, AGILITY
	db 45, AMNESIA
	db 47, WATER_PULSE
	db 49, TAKE_DOWN
	db 52, NIGHT_SLASH
	db 55, SUPERPOWER
	db 58, SCALD
	db 0

KinglerEvosMoves:
; Evolutions
	db 0
; Learnset
	db 4, HARDEN
	db 6, MUD_SLAP
	db 8, METAL_CLAW
	db 10, WATER_GUN
	db 12, MUD_SHOT
	db 15, LEER
	db 17, HAZE
	db 20, BUBBLEBEAM
	db 23, STOMP
	db 25, GUILLOTINE
	db 28, SLASH
	db 32, ROCK_TOMB
	db 35, HEADBUTT
	db 37, CRABHAMMER
	db 39, SLAM
	db 42, SWORDS_DANCE
	db 44, AGILITY
	db 47, AMNESIA
	db 50, WATER_PULSE
	db 52, TAKE_DOWN
	db 55, NIGHT_SLASH
	db 57, SUPERPOWER
	db 61, SCALD
	db 0

VoltorbEvosMoves:
; Evolutions
	db EV_LEVEL, 30, ELECTRODE
	db 0
; Learnset
	db 4, THUNDERSHOCK
	db 7, TACKLE
	db 13, SCREECH
	db 17, SONICBOOM
	db 20, SWIFT
	db 22, SELFDESTRUCT
	db 25, SHOCK_WAVE
	db 29, LIGHT_SCREEN
	db 31, TAKE_DOWN
	db 34, AGILITY
	db 37, HEADBUTT
	db 41, EXPLOSION
	db 45, ZAP_CANNON
	db 48, MIRROR_COAT
	db 0

ElectrodeEvosMoves:
; Evolutions
	db 0
; Learnset
	db 4, THUNDERSHOCK
	db 7, TACKLE
	db 13, SCREECH
	db 17, SONICBOOM
	db 20, SWIFT
	db 22, SELFDESTRUCT
	db 25, SHOCK_WAVE
	db 29, LIGHT_SCREEN
	db 33, TAKE_DOWN
	db 37, AGILITY
	db 40, HEADBUTT
	db 45, EXPLOSION
	db 47, ZAP_CANNON
	db 51, MIRROR_COAT
	db 0

ExeggcuteEvosMoves:
; Evolutions
	db EV_ITEM, LEAF_STONE, 1, EXEGGUTOR
	db 0
; Learnset
	db 5, REFLECT
	db 8, ABSORB
	db 12, LEECH_SEED
	db 14, MEGA_DRAIN
	db 17, LIGHT_SCREEN
	db 20, TAKE_DOWN
	db 22, ENERGY_BALL
	db 25, CONFUSION
	db 27, BULLET_SEED
	db 29, STUN_SPORE
	db 31, HYPNOSIS
	db 32, DREAM_EATER
	db 35, GIGA_DRAIN
	db 37, HEADBUTT
	db 39, POISONPOWDER
	db 42, PSYBEAM
	db 45, SLEEP_POWDER
	db 49, INFESTATION
	db 53, SOLARBEAM
	db 55, LEAF_STORM
	db 0

ExeggutorEvosMoves:
; Evolutions
	db 0
; Learnset
	db 5, REFLECT
	db 8, ABSORB
	db 12, LEECH_SEED
	db 14, MEGA_DRAIN
	db 17, LIGHT_SCREEN
	db 20, TAKE_DOWN
	db 22, ENERGY_BALL
	db 25, CONFUSION
	db 27, BULLET_SEED
	db 28, STOMP
	db 32, STUN_SPORE
	db 37, HYPNOSIS
	db 38, DREAM_EATER
	db 40, MAGICAL_LEAF
	db 42, GIGA_DRAIN
	db 44, LOW_KICK
	db 46, HEADBUTT
	db 48, POISONPOWDER
	db 52, PSYBEAM
	db 54, SLEEP_POWDER
	db 56, PSYCHO_CUT
	db 58, INFESTATION
	db 60, ZEN_HEADBUTT
	db 62, SOLARBEAM
	db 68, LEAF_STORM
	db 0

CuboneEvosMoves:
; Evolutions
	db EV_LEVEL, 28, MAROWAK
	db 0
; Learnset
	db 4, LEER
	db 6, GROWL
	db 8, MUD_SLAP
	db 10, BONE_CLUB
	db 12, HEADBUTT
	db 15, SCREECH
	db 18, SWIFT
	db 21, FURY_ATTACK
	db 23, WATER_GUN
	db 25, ROCK_TOMB
	db 27, LOW_KICK
	db 29, DOUBLE_KICK
	db 31, FOCUS_ENERGY
	db 33, TAKE_DOWN
	db 35, AERIAL_ACE
	db 38, THRASH
	db 40, BONEMERANG
	db 42, IRON_TAIL
	db 45, FIRE_PUNCH
	db 46, THUNDERPUNCH
	db 50, DOUBLE_EDGE
	db 56, SUBMISSION
	db 0

MarowakEvosMoves:
; Evolutions
	db 0
; Learnset
	db 4, LEER
	db 6, GROWL
	db 8, MUD_SLAP
	db 10, BONE_CLUB
	db 12, HEADBUTT
	db 15, SCREECH
	db 18, SWIFT
	db 21, FURY_ATTACK
	db 23, WATER_GUN
	db 25, ROCK_TOMB
	db 27, LOW_KICK
	db 32, DOUBLE_KICK
	db 34, FOCUS_ENERGY
	db 37, TAKE_DOWN
	db 38, AERIAL_ACE
	db 41, THRASH
	db 43, BONEMERANG
	db 45, IRON_TAIL
	db 48, FIRE_PUNCH
	db 49, THUNDERPUNCH
	db 52, DOUBLE_EDGE
	db 59, SUBMISSION
	db 0

TyrogueEvosMoves:
; Evolutions
	db EV_LEVEL, 10, TYROGUE
	db 0
; Learnset
	db 10, TACKLE
	db 11, DOUBLE_KICK
	db 12, LOW_KICK
	db 13, QUICK_ATTACK
	db 14, LOW_KICK
	db 15, LEER
	db 16, MUD_SLAP
	db 17, SWIFT
	db 18, HEADBUTT
	db 19, ROCK_TOMB
	db 20, TWISTER
	db 21, TAKE_DOWN
	db 22, AERIAL_ACE
	db 23, AGILITY
	db 24, ROLLING_KICK
	db 25, DRAIN_PUNCH
	db 26, DIG
	db 27, MACH_PUNCH
	db 28, CLOSE_COMBAT
	db 29, DIZZY_PUNCH
	db 30, COUNTER
	db 31, FOCUS_ENERGY
	db 32, HI_JUMP_KICK
	db 33, MEGA_PUNCH
	db 34, ROLLING_KICK
	db 35, THUNDERPUNCH
	db 36, FIRE_PUNCH
	db 37, ICE_PUNCH
	db 38, SUBMISSION
	db 40, BULLET_PUNCH
	db 41, SUPERPOWER
	db 42, STONE_EDGE
	db 0

HitmonleeEvosMoves:
; Evolutions
	db 0
; Learnset
	db 4, TACKLE
	db 6, DOUBLE_KICK
	db 8, LOW_KICK
	db 13, MUD_SLAP
	db 17, SWIFT
	db 19, HEADBUTT
	db 23, ROCK_TOMB
	db 26, TAKE_DOWN
	db 30, ROLLING_KICK
	db 33, MACH_PUNCH
	db 36, CLOSE_COMBAT
	db 40, FOCUS_ENERGY
	db 44, HI_JUMP_KICK
	db 47, SUBMISSION
	db 49, BULLET_PUNCH
	db 53, SUPERPOWER
	db 0

HitmonchanEvosMoves:
; Evolutions
	db 0
; Learnset
	db 4, TACKLE
	db 6, LOW_KICK
	db 8, LEER
	db 13, MUD_SLAP
	db 17, SWIFT
	db 19, HEADBUTT
	db 23, ROCK_TOMB
	db 26, TAKE_DOWN
	db 30, DRAIN_PUNCH
	db 33, MACH_PUNCH
	db 36, CLOSE_COMBAT
	db 39, DIZZY_PUNCH
	db 41, FOCUS_ENERGY
	db 44, MEGA_PUNCH
	db 46, THUNDERPUNCH
	db 48, FIRE_PUNCH
	db 50, ICE_PUNCH
	db 53, AGILITY
	db 55, COUNTER
	db 57, BULLET_PUNCH
	db 60, SUBMISSION
	db 62, HI_JUMP_KICK
	db 0

HitmontopEvosMoves:
; Evolutions
	db 0
; Learnset
	db 4, TACKLE
	db 6, QUICK_ATTACK
	db 8, LOW_KICK
	db 13, MUD_SLAP
	db 17, SWIFT
	db 19, HEADBUTT
	db 23, TWISTER
	db 26, AERIAL_ACE
	db 28, AGILITY
	db 30, DIG
	db 33, MACH_PUNCH
	db 36, CLOSE_COMBAT
	db 39, COUNTER
	db 41, FOCUS_ENERGY
	db 44, ROLLING_KICK
	db 47, BULLET_PUNCH
	db 50, HI_JUMP_KICK
	db 60, DRILL_RUN
	db 0

LickitungEvosMoves:
; Evolutions
	db EV_LEVEL, 36, LICKILICKY
	db 0
; Learnset
	db 5, LICK
	db 7, TACKLE
	db 9, WATER_GUN
	db 11, SUPERSONIC
	db 13, STOMP
	db 15, DISABLE
	db 17, WRAP
	db 19, MUD_SLAP
	db 21, DEFENSE_CURL
	db 23, SCREECH
	db 25, SLAM
	db 28, ACID
	db 31, ROCK_TOMB
	db 33, THRASH
	db 35, AMNESIA
	db 38, ICE_PUNCH
	db 39, FIRE_PUNCH
	db 40, THUNDERPUNCH
	db 43, TAKE_DOWN
	db 45, SHOCK_WAVE
	db 47, WATER_PULSE
	db 49, ZEN_HEADBUTT
	db 51, IRON_TAIL
	db 53, HEADBUTT
	db 55, SUBMISSION
	db 57, DREAM_EATER
	db 59, FLAMETHROWER
	db 62, SOLARBEAM
	db 0

LickilickyEvosMoves:
; Evolutions
	db 0
; Learnset
	db 5, LICK
	db 7, TACKLE
	db 9, WATER_GUN
	db 11, SUPERSONIC
	db 13, STOMP
	db 15, DISABLE
	db 17, WRAP
	db 19, MUD_SLAP
	db 21, DEFENSE_CURL
	db 23, SCREECH
	db 25, SLAM
	db 28, ACID
	db 31, ROCK_TOMB
	db 33, THRASH
	db 35, AMNESIA
	db 41, ICE_PUNCH
	db 42, FIRE_PUNCH
	db 43, THUNDERPUNCH
	db 46, TAKE_DOWN
	db 48, SHOCK_WAVE
	db 50, WATER_PULSE
	db 52, ZEN_HEADBUTT
	db 54, IRON_TAIL
	db 56, HEADBUTT
	db 58, SUBMISSION
	db 60, DREAM_EATER
	db 62, FLAMETHROWER
	db 65, SOLARBEAM
	db 0

KoffingEvosMoves:
; Evolutions
	db EV_LEVEL, 35, WEEZING
	db 0
; Learnset
	db 3, POISON_GAS
	db 5, SMOG
	db 8, SMOKESCREEN
	db 12, INFESTATION
	db 15, SCREECH
	db 18, HEADBUTT
	db 20, SLUDGE
	db 24, HAZE
	db 28, SELFDESTRUCT
	db 32, SLUDGE_WAVE
	db 36, TOXIC
	db 39, FLAMETHROWER
	db 44, EXPLOSION
	db 47, SHOCK_WAVE
	db 49, PSYBEAM
	db 53, ZAP_CANNON
	db 0

WeezingEvosMoves:
; Evolutions
	db 0
; Learnset
	db 3, POISON_GAS
	db 5, SMOG
	db 8, SMOKESCREEN
	db 12, INFESTATION
	db 15, SCREECH
	db 18, HEADBUTT
	db 20, SLUDGE
	db 24, HAZE
	db 28, SELFDESTRUCT
	db 32, SLUDGE_WAVE
	db 35, HEAT_WAVE
	db 39, TOXIC
	db 43, FLAMETHROWER
	db 47, EXPLOSION
	db 50, SHOCK_WAVE
	db 54, PSYBEAM
	db 58, ZAP_CANNON
	db 0

RhyhornEvosMoves:
; Evolutions
	db EV_LEVEL, 42, RHYDON
	db 0
; Learnset
	db 5, TACKLE
	db 7, MUD_SLAP
	db 10, LEER
	db 12, STOMP
	db 15, SAND_ATTACK
	db 17, ROCK_THROW
	db 19, FURY_ATTACK
	db 21, CRUNCH
	db 23, STEALTH_ROCK
	db 25, TAKE_DOWN
	db 27, IRON_TAIL
	db 29, HEADBUTT
	db 31, ROCK_TOMB
	db 33, THRASH
	db 35, MUD_SHOT
	db 40, EARTHQUAKE
	db 42, SUPERPOWER
	db 45, STONE_EDGE
	db 47, MEGAHORN
	db 51, ROAR
	db 55, HORN_DRILL
	db 60, DRILL_RUN
	db 0

RhydonEvosMoves:
; Evolutions
	db EV_LEVEL, 54, RHYPERIOR
	db 0
; Learnset
	db 5, TACKLE
	db 7, MUD_SLAP
	db 10, LEER
	db 12, STOMP
	db 15, SAND_ATTACK
	db 17, ROCK_THROW
	db 19, FURY_ATTACK
	db 21, CRUNCH
	db 23, STEALTH_ROCK
	db 25, TAKE_DOWN
	db 27, IRON_TAIL
	db 29, HEADBUTT
	db 31, ROCK_TOMB
	db 33, THRASH
	db 35, MUD_SHOT
	db 40, EARTHQUAKE
	db 42, IRON_TAIL
	db 45, SUPERPOWER
	db 47, STONE_EDGE
	db 50, THUNDERPUNCH
	db 51, FIRE_PUNCH
	db 52, ICE_PUNCH
	db 53, ROAR
	db 55, MEGAHORN
	db 57, SHADOW_CLAW
	db 60, HORN_DRILL
	db 65, DRILL_RUN
	db 0

RhyperiorEvosMoves:
; Evolutions
	db 0
; Learnset
	db 5, TACKLE
	db 7, MUD_SLAP
	db 10, LEER
	db 12, STOMP
	db 15, SAND_ATTACK
	db 17, ROCK_THROW
	db 19, FURY_ATTACK
	db 21, CRUNCH
	db 23, STEALTH_ROCK
	db 25, TAKE_DOWN
	db 27, IRON_TAIL
	db 29, HEADBUTT
	db 31, ROCK_TOMB
	db 33, THRASH
	db 35, MUD_SHOT
	db 40, EARTHQUAKE
	db 42, IRON_TAIL
	db 45, SUPERPOWER
	db 47, STONE_EDGE
	db 50, THUNDERPUNCH
	db 51, FIRE_PUNCH
	db 52, ICE_PUNCH
	db 55, ROAR
	db 57, MEGAHORN
	db 60, SHADOW_CLAW
	db 64, HORN_DRILL
	db 70, DRILL_RUN
	db 0

HappinyEvosMoves:
; Evolutions
	db EV_LEVEL, 10, CHANSEY
	db 0
; Learnset
	db 10, LEER
	db 11, TELEPORT
	db 12, TACKLE
	db 13, SING
	db 14, GROWL
	db 15, MUD_SLAP
	db 16, WATER_GUN
	db 17, HEADBUTT
	db 18, ROCK_TOMB
	db 19, TAKE_DOWN
	db 20, ZEN_HEADBUTT
	db 21, LIGHT_SCREEN
	db 22, SHOCK_WAVE
	db 23, MINIMIZE
	db 24, SOFTBOILED
	db 25, DEFENSE_CURL
	db 26, IRON_TAIL
	db 27, DREAM_EATER
	db 28, DRAIN_PUNCH
	db 29, DOUBLE_EDGE
	db 30, WATER_PULSE
	db 31, ICE_PUNCH
	db 32, FIRE_PUNCH
	db 33, THUNDERPUNCH
	db 34, SUBMISSION
	db 35, FLAMETHROWER
	db 36, ZAP_CANNON
	db 37, SOLARBEAM
	db 38, PLAY_ROUGH
	db 0

ChanseyEvosMoves:
; Evolutions
	db EV_LEVEL, 42, BLISSEY
	db 0
; Learnset
	db 4, LEER
	db 6, TELEPORT
	db 8, TACKLE
	db 10, SING
	db 12, GROWL
	db 14, MUD_SLAP
	db 16, WATER_GUN
	db 18, HEADBUTT
	db 20, ROCK_TOMB
	db 22, TAKE_DOWN
	db 24, ZEN_HEADBUTT
	db 26, LIGHT_SCREEN
	db 28, SHOCK_WAVE
	db 30, MINIMIZE
	db 32, SOFTBOILED
	db 34, DEFENSE_CURL
	db 36, IRON_TAIL
	db 38, DREAM_EATER
	db 40, DRAIN_PUNCH
	db 42, DOUBLE_EDGE
	db 44, WATER_PULSE
	db 47, ICE_PUNCH
	db 48, FIRE_PUNCH
	db 49, THUNDERPUNCH
	db 52, SUBMISSION
	db 54, FLAMETHROWER
	db 57, ZAP_CANNON
	db 60, SOLARBEAM
	db 0

BlisseyEvosMoves:
; Evolutions
	db 0
; Learnset
	db 4, LEER
	db 6, TELEPORT
	db 8, TACKLE
	db 10, SING
	db 12, GROWL
	db 14, MUD_SLAP
	db 16, WATER_GUN
	db 18, HEADBUTT
	db 20, ROCK_TOMB
	db 22, TAKE_DOWN
	db 24, ZEN_HEADBUTT
	db 26, LIGHT_SCREEN
	db 28, SHOCK_WAVE
	db 30, MINIMIZE
	db 32, SOFTBOILED
	db 34, DEFENSE_CURL
	db 39, IRON_TAIL
	db 41, DREAM_EATER
	db 43, DRAIN_PUNCH
	db 45, DOUBLE_EDGE
	db 47, WATER_PULSE
	db 50, ICE_PUNCH
	db 51, FIRE_PUNCH
	db 52, THUNDERPUNCH
	db 55, SUBMISSION
	db 57, FLAMETHROWER
	db 60, ZAP_CANNON
	db 63, SOLARBEAM
	db 0

TangelaEvosMoves:
; Evolutions
	db EV_ITEM, LEAF_STONE, 1, TANGROWTH
	db 0
; Learnset
	db 4, STUN_SPORE
	db 7, GROWTH
	db 9, BULLET_SEED
	db 12, ABSORB
	db 14, LEECH_SEED
	db 17, VINE_WHIP
	db 21, POISONPOWDER
	db 24, WRAP
	db 27, MEGA_DRAIN
	db 30, INFESTATION
	db 32, GIGA_DRAIN
	db 35, SLEEP_POWDER
	db 37, AMNESIA
	db 40, SLAM
	db 42, ENERGY_BALL
	db 44, GRASSY_TERRAIN
	db 46, HEADBUTT
	db 48, TAKE_DOWN
	db 50, CONFUSION
	db 52, SOLARBEAM
	db 56, LEAF_STORM
	db 0

TangrowthEvosMoves:
; Evolutions
	db 0
; Learnset
	db 4, STUN_SPORE
	db 7, GROWTH
	db 9, BULLET_SEED
	db 12, ABSORB
	db 14, LEECH_SEED
	db 17, VINE_WHIP
	db 21, POISONPOWDER
	db 24, WRAP
	db 27, MEGA_DRAIN
	db 30, INFESTATION
	db 32, GIGA_DRAIN
	db 35, SLEEP_POWDER
	db 37, MUD_SLAP
	db 40, AMNESIA
	db 42, ROCK_TOMB
	db 45, SLAM
	db 47, AERIAL_ACE
	db 49, ENERGY_BALL
	db 51, GRASSY_TERRAIN
	db 53, HEADBUTT
	db 55, TAKE_DOWN
	db 57, CONFUSION
	db 60, SOLARBEAM
	db 63, LEAF_STORM
	db 0

KangaskhanEvosMoves:
; Evolutions
	db EV_ITEM, MEGA_STONE_X, 1, MKANGASKAN
	db EV_ITEM, MEGA_STONE_Y, 1, MKANGASKAN
	db 0
; Learnset
	db 4, GROWL
	db 6, POUND
	db 8, BITE
	db 10, MUD_SLAP
	db 12, STOMP
	db 14, LOW_KICK
	db 16, DISABLE
	db 18, WATER_GUN
	db 20, FOCUS_ENERGY
	db 22, IRON_TAIL
	db 24, LEER
	db 26, HEADBUTT
	db 28, ROCK_TOMB
	db 30, TAKE_DOWN
	db 32, MEGA_PUNCH
	db 33, THUNDERPUNCH
	db 34, FIRE_PUNCH
	db 35, ICE_PUNCH
	db 36, DRAIN_PUNCH
	db 38, CRUNCH
	db 40, DIZZY_PUNCH
	db 43, OUTRAGE
	db 45, SHADOW_CLAW
	db 47, ROAR
	db 49, AERIAL_ACE
	db 52, SUBMISSION
	db 55, SING
	db 0

MKangaskanEvosMoves:
; Evolutions
	db 0
; Learnset
	db 9, GROWL
	db 11, POUND
	db 13, BITE
	db 15, MUD_SLAP
	db 17, STOMP
	db 19, LOW_KICK
	db 21, DISABLE
	db 23, WATER_GUN
	db 25, FOCUS_ENERGY
	db 27, IRON_TAIL
	db 29, LEER
	db 31, HEADBUTT
	db 33, ROCK_TOMB
	db 35, TAKE_DOWN
	db 36, LOVELY_KISS
	db 37, MEGA_PUNCH
	db 38, THUNDERPUNCH
	db 39, FIRE_PUNCH
	db 40, ICE_PUNCH
	db 41, DRAIN_PUNCH
	db 43, CRUNCH
	db 45, DIZZY_PUNCH
	db 48, OUTRAGE
	db 50, DOUBLE_KICK
	db 50, SHADOW_CLAW
	db 52, ROAR
	db 53, FURY_ATTACK
	db 54, AERIAL_ACE
	db 56, PIN_MISSILE
	db 57, SUBMISSION
	db 58, SPIKE_CANNON
	db 60, SING
	db 61, FURY_SWIPES
	db 62, BULLET_SEED
	db 64, ICICLE_SPEAR
	db 66, BONEMERANG
	db 0

HorseaEvosMoves:
; Evolutions
	db EV_LEVEL, 32, SEADRA
	db 0
; Learnset
	db 4, BUBBLE
	db 6, SMOKESCREEN
	db 10, TWISTER
	db 13, FOCUS_ENERGY
	db 18, SWIFT
	db 21, DISABLE
	db 24, WATER_GUN
	db 27, LEER
	db 30, BUBBLEBEAM
	db 32, DRAGONBREATH
	db 34, DRAGON_RAGE
	db 37, AGILITY
	db 40, DRAGON_DANCE
	db 43, WATER_PULSE
	db 45, HEADBUTT
	db 48, SCALD
	db 50, AURORA_BEAM
	db 53, RAIN_DANCE
	db 55, HYDRO_PUMP
	db 0

SeadraEvosMoves:
; Evolutions
	db EV_LEVEL, 42, KINGDRA
	db 0
; Learnset
	db 4, BUBBLE
	db 6, SMOKESCREEN
	db 10, TWISTER
	db 13, FOCUS_ENERGY
	db 18, SWIFT
	db 21, DISABLE
	db 24, WATER_GUN
	db 27, LEER
	db 30, BUBBLEBEAM
	db 34, DRAGONBREATH
	db 36, DRAGON_RAGE
	db 39, AGILITY
	db 43, DRAGON_DANCE
	db 45, WATER_PULSE
	db 47, HEADBUTT
	db 50, SCALD
	db 52, AURORA_BEAM
	db 57, RAIN_DANCE
	db 59, HYDRO_PUMP
	db 0

KingdraEvosMoves:
; Evolutions
	db 0
; Learnset
	db 4, BUBBLE
	db 6, SMOKESCREEN
	db 10, TWISTER
	db 13, FOCUS_ENERGY
	db 18, SWIFT
	db 21, DISABLE
	db 24, WATER_GUN
	db 27, LEER
	db 30, BUBBLEBEAM
	db 34, DRAGONBREATH
	db 36, DRAGON_RAGE
	db 39, AGILITY
	db 46, DRAGON_DANCE
	db 48, WATER_PULSE
	db 50, HURRICANE
	db 52, HEADBUTT
	db 54, SCALD
	db 57, AURORA_BEAM
	db 59, RAIN_DANCE
	db 62, HYDRO_PUMP
	db 0

GoldeenEvosMoves:
; Evolutions
	db EV_LEVEL, 33, SEAKING
	db 0
; Learnset
	db 5, SUPERSONIC
	db 7, WATER_GUN
	db 10, WATER_PULSE
	db 12, SWIFT
	db 15, HORN_ATTACK
	db 17, QUICK_ATTACK
	db 20, AGILITY
	db 23, MUD_SLAP
	db 25, HAZE
	db 28, HEADBUTT
	db 30, FURY_ATTACK
	db 34, MUD_SHOT
	db 39, TAKE_DOWN
	db 42, HORN_DRILL
	db 45, MEGAHORN
	db 49, PSYBEAM
	db 53, SCALD
	db 59, DRILL_RUN
	db 0

SeakingEvosMoves:
; Evolutions
	db 0
; Learnset
	db 5, SUPERSONIC
	db 7, WATER_GUN
	db 10, WATER_PULSE
	db 12, SWIFT
	db 15, HORN_ATTACK
	db 17, QUICK_ATTACK
	db 20, AGILITY
	db 23, MUD_SLAP
	db 25, HAZE
	db 28, HEADBUTT
	db 30, FURY_ATTACK
	db 37, MUD_SHOT
	db 43, TAKE_DOWN
	db 46, HORN_DRILL
	db 49, MEGAHORN
	db 53, PSYBEAM
	db 55, SCALD
	db 62, DRILL_RUN
	db 0

StaryuEvosMoves:
; Evolutions
	db EV_ITEM, WATER_STONE, 1, STARMIE
	db 0
; Learnset
	db 7, WATER_GUN
	db 12, CONFUSE_RAY
	db 15, MINIMIZE
	db 20, SWIFT
	db 22, HARDEN
	db 24, PSYBEAM
	db 27, RECOVER
	db 30, TWISTER
	db 32, LIGHT_SCREEN
	db 34, HEADBUTT
	db 36, RAPID_SPIN
	db 38, WATER_PULSE
	db 41, PSYCHIC_M
	db 45, TAKE_DOWN
	db 49, ZAP_CANNON
	db 53, SCALD
	db 55, HYDRO_PUMP
	db 0

StarmieEvosMoves:
; Evolutions
	db EV_ITEM, MEGA_STONE_X, 1, MSTARMIE
	db EV_ITEM, MEGA_STONE_Y, 1, MSTARMIE
	db 0
; Learnset
	db 7, WATER_GUN
	db 12, CONFUSE_RAY
	db 15, MINIMIZE
	db 20, SWIFT
	db 22, HARDEN
	db 24, PSYBEAM
	db 27, RECOVER
	db 30, TWISTER
	db 32, LIGHT_SCREEN
	db 34, HEADBUTT
	db 36, AGILITY
	db 38, RAPID_SPIN
	db 40, WATER_PULSE
	db 44, PSYCHIC_M
	db 48, DREAM_EATER
	db 50, TAKE_DOWN
	db 54, PSYCHO_CUT
	db 57, ZAP_CANNON
	db 59, SCALD
	db 62, HYDRO_PUMP
	db 0

MStarmieEvosMoves:
; Evolutions
	db 0
; Learnset
	db 12, WATER_GUN
	db 17, CONFUSE_RAY
	db 20, MINIMIZE
	db 25, SWIFT
	db 27, HARDEN
	db 29, PSYBEAM
	db 32, RECOVER
	db 35, TWISTER
	db 37, LIGHT_SCREEN
	db 39, HEADBUTT
	db 41, AGILITY
	db 43, RAPID_SPIN
	db 45, WATER_PULSE
	db 49, PSYCHIC_M
	db 53, DREAM_EATER
	db 55, TAKE_DOWN
	db 57, MACH_PUNCH
	db 59, PSYCHO_CUT
	db 62, ZAP_CANNON
	db 64, SCALD
	db 67, HYDRO_PUMP
	db 72, MOONBLAST
	db 0

MimeJrEvosMoves:
; Evolutions
	db EV_LEVEL, 10, MR_MIME
	db 0
; Learnset
	db 10, TELEPORT
	db 11, POUND
	db 12, MUD_SLAP
	db 13, SCREECH
	db 14, CONFUSE_RAY
	db 15, INFESTATION
	db 16, HEADBUTT
	db 17, LIGHT_SCREEN
	db 18, TAKE_DOWN
	db 19, AERIAL_ACE
	db 20, PSYBEAM
	db 21, MIMIC
	db 22, ENERGY_BALL
	db 24, SHOCK_WAVE
	db 25, HYPNOSIS
	db 26, DREAM_EATER
	db 27, ZEN_HEADBUTT
	db 28, ICE_PUNCH
	db 29, FIRE_PUNCH
	db 30, THUNDERPUNCH
	db 31, DRAIN_PUNCH
	db 32, SUBSTITUTE
	db 33, SUBMISSION
	db 34, ZAP_CANNON
	db 35, PSYCHIC_M
	db 36, PLAY_ROUGH
	db 0

MrMimeEvosMoves:
; Evolutions
	db 0
; Learnset
	db 5, TELEPORT
	db 8, POUND
	db 10, MUD_SLAP
	db 12, SCREECH
	db 15, CONFUSE_RAY
	db 17, INFESTATION
	db 20, HEADBUTT
	db 22, LIGHT_SCREEN
	db 24, TAKE_DOWN
	db 26, AERIAL_ACE
	db 28, PSYBEAM
	db 30, MIMIC
	db 32, MISTY_TERRAIN
	db 34, ENERGY_BALL
	db 37, SHOCK_WAVE
	db 39, HYPNOSIS
	db 40, DREAM_EATER
	db 43, ZEN_HEADBUTT
	db 45, ICE_PUNCH
	db 46, FIRE_PUNCH
	db 47, THUNDERPUNCH
	db 48, DRAIN_PUNCH
	db 51, SUBSTITUTE
	db 54, SUBMISSION
	db 57, ZAP_CANNON
	db 60, PSYCHIC_M
	db 0

ScytherEvosMoves:
; Evolutions
	db EV_ITEM, METAL_COAT, 1, SCIZOR
	db EV_ITEM, KINGS_ROCK, 1, KLEAVOR
	db 0
; Learnset
	db 7, WING_ATTACK
	db 12, LEER
	db 15, FOCUS_ENERGY
	db 18, DOUBLE_TEAM
	db 20, SWIFT
	db 23, SONICBOOM
	db 25, SLASH
	db 28, LIGHT_SCREEN
	db 30, AGILITY
	db 32, TAKE_DOWN
	db 34, SWORDS_DANCE
	db 37, HEADBUTT
	db 40, X_SCISSOR
	db 42, AIR_SLASH
	db 45, ROOST
	db 47, STEEL_WING
	db 50, AERIAL_ACE
	db 53, PSYCHO_CUT
	db 55, NIGHT_SLASH
	db 0

ScizorEvosMoves:
; Evolutions
	db EV_ITEM, MEGA_STONE_X, 1, MSCIZOR
	db EV_ITEM, MEGA_STONE_Y, 1, MSCIZOR
	db 0
; Learnset
	db 7, WING_ATTACK
	db 12, LEER
	db 15, FOCUS_ENERGY
	db 18, DOUBLE_TEAM
	db 20, SWIFT
	db 23, SONICBOOM
	db 25, SLASH
	db 28, LIGHT_SCREEN
	db 30, AGILITY
	db 32, TAKE_DOWN
	db 34, SWORDS_DANCE
	db 36, BULLET_PUNCH
	db 41, HEADBUTT
	db 43, X_SCISSOR
	db 46, METAL_CLAW
	db 48, AIR_SLASH
	db 50, ROOST
	db 53, SUPERPOWER
	db 55, STEEL_WING
	db 57, AERIAL_ACE
	db 59, PSYCHO_CUT
	db 61, NIGHT_SLASH
	db 0

MScizorEvosMoves:
; Evolutions
	db 0
; Learnset
	db 12, WING_ATTACK
	db 17, LEER
	db 20, FOCUS_ENERGY
	db 23, DOUBLE_TEAM
	db 25, SWIFT
	db 28, SONICBOOM
	db 30, SLASH
	db 33, LIGHT_SCREEN
	db 35, AGILITY
	db 37, TAKE_DOWN
	db 39, SWORDS_DANCE
	db 41, BULLET_PUNCH
	db 46, HEADBUTT
	db 48, X_SCISSOR
	db 51, METAL_CLAW
	db 53, AIR_SLASH
	db 55, ROOST
	db 56, HARDEN
	db 58, SUPERPOWER
	db 59, HEAT_WAVE
	db 60, STEEL_WING
	db 62, AERIAL_ACE
	db 64, PSYCHO_CUT
	db 66, NIGHT_SLASH
	db 68, LEECH_LIFE
	db 0

KleavorEvosMoves:
; Evolutions
	db 0
; Learnset
	db 11, AERIAL_ACE
	db 13, FOCUS_ENERGY
	db 16, DOUBLE_TEAM
	db 20, SWIFT
	db 22, AIR_SLASH
	db 24, SLASH
	db 26, SWORDS_DANCE
	db 28, LIGHT_SCREEN
	db 30, AGILITY
	db 32, TAKE_DOWN
	db 34, X_SCISSOR
	db 36, ROCK_SLIDE
	db 41, HEADBUTT
	db 43, CLOSE_COMBAT,
	db 60, NIGHT_SLASH
	db 0

SmoochumEvosMoves:
; Evolutions
	db EV_LEVEL, 10, JYNX
	db 0
; Learnset
	db 10, CONFUSION
	db 11, SCREECH
	db 12, LICK
	db 13, SING
	db 14, LIGHT_SCREEN
	db 15, WATER_PULSE
	db 16, ICE_PUNCH
	db 17, DRAIN_PUNCH
	db 18, PSYCHIC_M
	db 19, ICICLE_SPEAR
	db 20, BODY_SLAM
	db 21, LOVELY_KISS
	db 22, DREAM_EATER
	db 23, THRASH
	db 24, ZEN_HEADBUTT
	db 25, ENERGY_BALL
	db 26, PSYCHO_CUT
	db 27, BLIZZARD
	db 28, AMNESIA
	db 0

JynxEvosMoves:
; Evolutions
	db 0
; Learnset
	db 12, CONFUSION
	db 14, SCREECH
	db 18, LICK
	db 20, SING
	db 23, LIGHT_SCREEN
	db 26, WATER_PULSE
	db 28, ICE_PUNCH
	db 31, DRAIN_PUNCH
	db 34, PSYCHIC_M
	db 37, ICICLE_SPEAR
	db 39, BODY_SLAM
	db 41, LOVELY_KISS
	db 42, DREAM_EATER
	db 47, THRASH
	db 50, ZEN_HEADBUTT
	db 53, ENERGY_BALL
	db 55, PSYCHO_CUT
	db 58, BLIZZARD
	db 0

ElekidEvosMoves:
; Evolutions
	db EV_LEVEL, 10, ELECTABUZZ
	db 0
; Learnset
	db 10, SWIFT
	db 11, MUD_SLAP
	db 12, SHOCK_WAVE
	db 13, THUNDER_WAVE
	db 14, BARRIER
	db 15, SCREECH
	db 16, HEADBUTT
	db 17, THUNDERPUNCH
	db 18, FIRE_PUNCH
	db 19, ICE_PUNCH
	db 20, ROCK_TOMB
	db 21, THUNDERSHOCK
	db 22, LOW_KICK
	db 23, IRON_TAIL
	db 24, THUNDERBOLT
	db 25, KARATE_CHOP
	db 26, LIGHT_SCREEN
	db 27, THUNDER
	db 28, FLAMETHROWER
	db 29, ZAP_CANNON
	db 30, CLOSE_COMBAT
	db 0

ElectabuzzEvosMoves:
; Evolutions
	db EV_ITEM, THUNDER_STONE, 1, ELECTIVIRE
	db 0
; Learnset
	db 12, SWIFT
	db 14, MUD_SLAP
	db 16, SHOCK_WAVE
	db 19, THUNDER_WAVE
	db 22, BARRIER
	db 24, SCREECH
	db 26, HEADBUTT
	db 28, THUNDERPUNCH
	db 29, FIRE_PUNCH
	db 30, ICE_PUNCH
	db 35, THUNDERSHOCK
	db 39, LOW_KICK
	db 43, IRON_TAIL
	db 46, THUNDERBOLT
	db 49, KARATE_CHOP
	db 51, LIGHT_SCREEN
	db 54, THUNDER
	db 58, ZAP_CANNON
	db 0

ElectivireEvosMoves:
; Evolutions
	db 0
; Learnset
	db 12, SWIFT
	db 14, MUD_SLAP
	db 16, SHOCK_WAVE
	db 19, THUNDER_WAVE
	db 22, BARRIER
	db 24, SCREECH
	db 26, HEADBUTT
	db 28, THUNDERPUNCH
	db 29, FIRE_PUNCH
	db 30, ICE_PUNCH
	db 33, ROCK_TOMB
	db 35, THUNDERSHOCK
	db 39, LOW_KICK
	db 46, IRON_TAIL
	db 49, THUNDERBOLT
	db 52, KARATE_CHOP
	db 54, LIGHT_SCREEN
	db 57, THUNDER
	db 60, FLAMETHROWER
	db 63, ZAP_CANNON
	db 0

MagbyEvosMoves:
; Evolutions
	db EV_LEVEL, 10, MAGMAR
	db 0
; Learnset
	db 10, MUD_SLAP
	db 11, POISON_GAS
	db 12, LEER
	db 13, FOCUS_ENERGY
	db 14, CONFUSE_RAY
	db 15, LOW_KICK
	db 16, HEADBUTT
	db 17, FIRE_PUNCH
	db 18, THUNDERPUNCH
	db 19, ROCK_TOMB
	db 20, FIRE_SPIN
	db 21, KARATE_CHOP
	db 22, SMOG
	db 23, IRON_TAIL
	db 24, FLAMETHROWER
	db 25, SMOKESCREEN
	db 26, MACH_PUNCH
	db 27, HEAT_WAVE
	db 28, SOLARBEAM
	db 29, FLARE_BLITZ
	db 30, FIRE_BLAST
	db 31, HYPER_BEAM
	db 32, BULLET_SEED
	db 0

MagmarEvosMoves:
; Evolutions
	db EV_ITEM, FIRE_STONE, 1, MAGMORTAR
	db 0
; Learnset
	db 7, MUD_SLAP
	db 10, POISON_GAS
	db 13, LEER
	db 17, FOCUS_ENERGY
	db 20, CONFUSE_RAY
	db 23, LOW_KICK
	db 26, HEADBUTT
	db 28, FIRE_PUNCH
	db 29, THUNDERPUNCH
	db 33, FIRE_SPIN
	db 36, KARATE_CHOP
	db 38, SUNNY_DAY
	db 40, SMOG
	db 43, IRON_TAIL
	db 46, FLAMETHROWER
	db 48, SMOKESCREEN
	db 51, MACH_PUNCH
	db 54, HEAT_WAVE
	db 56, FLARE_BLITZ
	db 58, ROAR
	db 60, FIRE_BLAST
	db 63, HYPER_BEAM
	db 0

MagmortarEvosMoves:
; Evolutions
	db 0
; Learnset
	db 7, MUD_SLAP
	db 10, POISON_GAS
	db 13, LEER
	db 17, FOCUS_ENERGY
	db 20, CONFUSE_RAY
	db 23, LOW_KICK
	db 26, HEADBUTT
	db 28, FIRE_PUNCH
	db 29, THUNDERPUNCH
	db 31, ROCK_TOMB
	db 33, FIRE_SPIN
	db 36, KARATE_CHOP
	db 38, SUNNY_DAY
	db 40, SMOG
	db 46, IRON_TAIL
	db 49, FLAMETHROWER
	db 51, SMOKESCREEN
	db 54, MACH_PUNCH
	db 57, HEAT_WAVE
	db 59, SOLARBEAM
	db 60, ROAR
	db 61, FLARE_BLITZ
	db 63, FIRE_BLAST
	db 66, HYPER_BEAM
	db 0

PinsirEvosMoves:
; Evolutions
	db EV_ITEM, MEGA_STONE_X, 1, MPINSIR
	db EV_ITEM, MEGA_STONE_Y, 1, MPINSIR
	db 0
; Learnset
	db 4, FOCUS_ENERGY
	db 8, WRAP
	db 12, SEISMIC_TOSS
	db 17, FURY_ATTACK
	db 21, ROCK_THROW
	db 24, HARDEN
	db 28, HEADBUTT
	db 32, X_SCISSOR
	db 37, ROCK_TOMB
	db 42, SWORDS_DANCE
	db 45, TAKE_DOWN
	db 47, SUBMISSION
	db 49, THRASH
	db 52, SLASH
	db 55, SUPERPOWER
	db 58, GUILLOTINE
	db 0

MPinsirEvosMoves:
; Evolutions
	db 0
; Learnset
	db 9, FOCUS_ENERGY
	db 13, WRAP
	db 17, SEISMIC_TOSS
	db 22, FURY_ATTACK
	db 26, ROCK_THROW
	db 29, HARDEN
	db 33, HEADBUTT
	db 37, X_SCISSOR
	db 42, ROCK_TOMB
	db 47, SWORDS_DANCE
	db 50, TAKE_DOWN
	db 51, AIR_CUTTER
	db 52, SUBMISSION
	db 53, ROOST
	db 54, THRASH
	db 56, LEECH_LIFE
	db 57, SLASH
	db 60, SUPERPOWER
	db 62, AIR_SLASH
	db 63, GUILLOTINE
	db 0

TaurosEvosMoves:
; Evolutions
	db 0
; Learnset
	db 10, FOCUS_ENERGY
	db 14, HORN_ATTACK
	db 18, QUICK_ATTACK
	db 21, STOMP
	db 24, LEER
	db 27, ROCK_TOMB
	db 30, ZEN_HEADBUTT
	db 34, TAKE_DOWN
	db 37, HEADBUTT
	db 40, REST
	db 43, IRON_TAIL
	db 48, THRASH
	db 51, DOUBLE_EDGE
	db 61, HORN_DRILL
	db 70, DRILL_RUN
	db 0

MagikarpEvosMoves:
; Evolutions
	db EV_LEVEL, 20, GYARADOS
	db 0
; Learnset
	db 10, TACKLE
	db 20, FLAIL
	db 50, DRAGON_RAGE
	db 0

GyaradosEvosMoves:
; Evolutions
	db EV_ITEM, MEGA_STONE_X, 1, MGYARADOS
	db EV_ITEM, MEGA_STONE_Y, 1, MGYARADOS
	db 0
; Learnset
	db 20, BITE
	db 21, BUBBLE
	db 22, LEER
	db 23, TWISTER
	db 24, FOCUS_ENERGY
	db 25, WATER_GUN
	db 27, DRAGON_RAGE
	db 30, WRAP
	db 32, CRUNCH
	db 34, ROAR
	db 35, DRAGONBREATH
	db 37, HEADBUTT
	db 38, WATER_PULSE
	db 40, TAKE_DOWN
	db 42, DRAGON_DANCE
	db 44, IRON_TAIL
	db 45, RAIN_DANCE
	db 46, FLAMETHROWER
	db 50, HURRICANE
	db 52, THRASH
	db 54, SCALD
	db 56, ZAP_CANNON
	db 58, HYDRO_PUMP
	db 60, HYPER_BEAM
	db 0

MGyaradosEvosMoves:
; Evolutions
	db 0
; Learnset
	db 25, BITE
	db 26, BUBBLE
	db 27, LEER
	db 28, TWISTER
	db 29, FOCUS_ENERGY
	db 30, WATER_GUN
	db 32, DRAGON_RAGE
	db 35, WRAP
	db 37, CRUNCH
	db 39, ROAR
	db 40, DRAGONBREATH
	db 42, HEADBUTT
	db 43, WATER_PULSE
	db 45, TAKE_DOWN
	db 47, DRAGON_DANCE
	db 48, NIGHT_SLASH
	db 49, IRON_TAIL
	db 50, RAIN_DANCE
	db 51, FLAMETHROWER
	db 53, ZEN_HEADBUTT
	db 55, HURRICANE
	db 57, THRASH
	db 59, SCALD
	db 60, PLAY_ROUGH
	db 61, ZAP_CANNON
	db 63, HYDRO_PUMP
	db 65, HYPER_BEAM
	db 0

LaprasEvosMoves:
; Evolutions
	db 0
; Learnset
	db 5, SING
	db 12, BITE
	db 17, CONFUSE_RAY
	db 21, SHOCK_WAVE
	db 25, DRAGON_RAGE
	db 28, BODY_SLAM
	db 30, WATER_PULSE
	db 35, DRAGONBREATH
	db 38, AURORA_BEAM
	db 40, IRON_TAIL
	db 43, RAIN_DANCE
	db 45, DRAGON_DANCE
	db 47, HEADBUTT
	db 49, ZEN_HEADBUTT
	db 53, ICE_BEAM
	db 57, HYDRO_PUMP
	db 60, ZAP_CANNON
	db 62, ROAR
	db 64, HORN_DRILL
	db 69, DRILL_RUN
	db 0

DittoEvosMoves:
; Evolutions
	db 0
; Learnset
	db 0

EeveeEvosMoves:
; Evolutions
	db EV_ITEM, FIRE_STONE, 1, FLAREON
	db EV_ITEM, THUNDER_STONE, 1, JOLTEON
	db EV_ITEM, WATER_STONE, 1, VAPOREON
	db EV_ITEM, SUN_STONE, 1, ESPEON
	db EV_ITEM, MOON_STONE, 1, UMBREON
	db EV_ITEM, LEAF_STONE, 1, LEAFEON
	db EV_ITEM, ICE_STONE, 1, GLACEON
	db EV_ITEM, KINGS_ROCK, 1, SYLVEON
	db 0
; Learnset
	db 5, SAND_ATTACK
	db 10, QUICK_ATTACK
	db 14, GROWL
	db 17, MUD_SLAP
	db 20, SWIFT
	db 25, BITE
	db 31, DOUBLE_KICK
	db 36, FOCUS_ENERGY
	db 40, TAKE_DOWN
	db 45, HEADBUTT
	db 50, IRON_TAIL
	db 54, DOUBLE_EDGE
	db 0

VaporeonEvosMoves:
; Evolutions
	db 0
; Learnset
	db 5, SAND_ATTACK
	db 10, QUICK_ATTACK
	db 13, MUD_SLAP
	db 16, WATER_GUN
	db 20, HAZE
	db 24, BUBBLE
	db 27, FOCUS_ENERGY
	db 30, BITE
	db 34, HEADBUTT
	db 37, WATER_PULSE
	db 40, AURORA_BEAM
	db 42, IRON_TAIL
	db 45, DOUBLE_KICK
	db 51, ACID_ARMOR
	db 56, SCALD
	db 58, ROAR
	db 60, HYDRO_PUMP
	db 0

JolteonEvosMoves:
; Evolutions
	db 0
; Learnset
	db 5, SAND_ATTACK
	db 10, QUICK_ATTACK
	db 13, MUD_SLAP
	db 16, THUNDERSHOCK
	db 20, THUNDER_WAVE
	db 24, DOUBLE_KICK
	db 27, FOCUS_ENERGY
	db 30, BITE
	db 34, HEADBUTT
	db 37, SHOCK_WAVE
	db 40, PIN_MISSILE
	db 42, IRON_TAIL
	db 46, AGILITY
	db 50, LIGHT_SCREEN
	db 55, ZAP_CANNON
	db 58, ROAR
	db 60, THUNDER
	db 0

FlareonEvosMoves:
; Evolutions
	db 0
; Learnset
	db 5, SAND_ATTACK
	db 10, QUICK_ATTACK
	db 13, MUD_SLAP
	db 16, EMBER
	db 20, SMOG
	db 24, BITE
	db 27, FOCUS_ENERGY
	db 30, DOUBLE_KICK
	db 34, HEADBUTT
	db 38, FIRE_SPIN
	db 42, IRON_TAIL
	db 46, FLAMETHROWER
	db 49, SUPERPOWER
	db 53, HEAT_WAVE
	db 57, ZAP_CANNON
	db 58, ROAR
	db 60, FLARE_BLITZ
	db 0

EspeonEvosMoves:
; Evolutions
	db 0
; Learnset
	db 5, SAND_ATTACK
	db 10, QUICK_ATTACK
	db 13, MUD_SLAP
	db 17, FOCUS_ENERGY
	db 20, SWIFT
	db 23, DOUBLE_KICK
	db 25, PSYBEAM
	db 29, LIGHT_SCREEN
	db 33, BITE
	db 36, CONFUSION
	db 40, PSYCHIC_M
	db 44, HEADBUTT
	db 48, IRON_TAIL
	db 52, ZEN_HEADBUTT
	db 55, ZAP_CANNON
	db 58, ROAR
	db 0

UmbreonEvosMoves:
; Evolutions
	db 0
; Learnset
	db 5, SAND_ATTACK
	db 10, QUICK_ATTACK
	db 13, MUD_SLAP
	db 17, FOCUS_ENERGY
	db 20, CONFUSE_RAY
	db 25, DOUBLE_KICK
	db 29, HEADBUTT
	db 33, CRUNCH
	db 37, IRON_TAIL
	db 40, DARK_PULSE
	db 45, SCREECH
	db 50, ZAP_CANNON
	db 58, ROAR
	db 0

LeafeonEvosMoves:
; Evolutions
	db 0
; Learnset
	db 5, SAND_ATTACK
	db 10, QUICK_ATTACK
	db 13, MUD_SLAP
	db 17, FOCUS_ENERGY
	db 20, LEECH_SEED
	db 23, DOUBLE_KICK
	db 25, MAGICAL_LEAF
	db 29, HEADBUTT
	db 32, BULLET_SEED
	db 34, SUNNY_DAY
	db 36, IRON_TAIL
	db 39, RAZOR_LEAF
	db 42, GIGA_DRAIN
	db 44, SWORDS_DANCE
	db 48, ENERGY_BALL
	db 51, AERIAL_ACE
	db 53, SOLARBEAM
	db 57, LEAF_STORM
	db 58, ROAR
	db 0

GlaceonEvosMoves:
; Evolutions
	db 0
; Learnset
	db 5, SAND_ATTACK
	db 10, QUICK_ATTACK
	db 13, MUD_SLAP
	db 17, FOCUS_ENERGY
	db 20, WATER_PULSE
	db 25, BITE
	db 27, HEADBUTT
	db 30, DOUBLE_KICK
	db 35, ICICLE_SPEAR
	db 37, HAIL
	db 40, IRON_TAIL
	db 42, ICE_BEAM
	db 45, MIRROR_COAT
	db 49, ICICLE_CRASH
	db 53, BARRIER
	db 57, BLIZZARD
	db 58, ROAR
	db 0

SylveonEvosMoves:
; Evolutions
	db 0
; Learnset
	db 5, SAND_ATTACK
	db 10, QUICK_ATTACK
	db 13, MUD_SLAP
	db 17, FOCUS_ENERGY
	db 20, SWIFT
	db 25, DOUBLE_KICK
	db 30, LIGHT_SCREEN
	db 33, HEADBUTT
	db 35, MISTY_TERRAIN
	db 39, DRAININGKISS
	db 42, IRON_TAIL
	db 47, PLAY_ROUGH
	db 51, MAGICAL_LEAF
	db 55, MOONBLAST
	db 58, ROAR
	db 0

PorygonEvosMoves:
; Evolutions
	db EV_ITEM, UPGRADE, 1, PORYGON2
	db 0
; Learnset
	db 10, TELEPORT
	db 15, THUNDERSHOCK
	db 20, BARRIER
	db 25, SWIFT
	db 28, RECOVER
	db 31, AGILITY
	db 33, PSYBEAM
	db 35, TRI_ATTACK
	db 37, SHOCK_WAVE
	db 39, HEADBUTT
	db 42, TRI_ATTACK
	db 45, ZEN_HEADBUTT
	db 47, AMNESIA
	db 50, IRON_TAIL
	db 52, AERIAL_ACE
	db 55, ZAP_CANNON
	db 58, SOLARBEAM
	db 60, HYPER_BEAM
	db 0

Porygon2EvosMoves:
; Evolutions
	db EV_ITEM, DUBIOUS_DISK, 1, PORYGONZ
	db 0
; Learnset
	db 10, TELEPORT
	db 15, THUNDERSHOCK
	db 20, BARRIER
	db 25, SWIFT
	db 28, RECOVER
	db 31, AGILITY
	db 33, PSYBEAM
	db 35, TRI_ATTACK
	db 40, SHOCK_WAVE
	db 43, HEADBUTT
	db 45, TRI_ATTACK
	db 48, ZEN_HEADBUTT
	db 50, AMNESIA
	db 53, IRON_TAIL
	db 55, AERIAL_ACE
	db 58, ZAP_CANNON
	db 61, SOLARBEAM
	db 63, HYPER_BEAM
	db 0

PorygonZEvosMoves:
; Evolutions
	db 0
; Learnset
	db 10, TELEPORT
	db 15, THUNDERSHOCK
	db 20, BARRIER
	db 25, SWIFT
	db 28, RECOVER
	db 31, AGILITY
	db 33, PSYBEAM
	db 35, TRI_ATTACK
	db 38, TRICK_ROOM
	db 40, SHOCK_WAVE
	db 45, HEADBUTT
	db 47, TRI_ATTACK
	db 50, ZEN_HEADBUTT
	db 52, AMNESIA
	db 55, IRON_TAIL
	db 57, AERIAL_ACE
	db 60, ZAP_CANNON
	db 63, SOLARBEAM
	db 65, HYPER_BEAM
	db 0

OmanyteEvosMoves:
; Evolutions
	db EV_LEVEL, 40, OMASTAR
	db 0
; Learnset
	db 7, WRAP
	db 10, SAND_ATTACK
	db 13, WATER_GUN
	db 17, SUPERSONIC
	db 20, LEER
	db 22, HAZE
	db 25, MUD_SHOT
	db 27, SLAM
	db 30, ROCK_TOMB
	db 32, BITE
	db 34, HORN_ATTACK
	db 37, ROCK_THROW
	db 39, HEADBUTT
	db 42, WATER_PULSE
	db 45, SPIKE_CANNON
	db 48, AURORA_BEAM
	db 51, SCALD
	db 53, HYDRO_PUMP
	db 0

OmastarEvosMoves:
; Evolutions
	db 0
; Learnset
	db 7, WRAP
	db 10, SAND_ATTACK
	db 13, WATER_GUN
	db 17, SUPERSONIC
	db 20, LEER
	db 22, HAZE
	db 25, MUD_SHOT
	db 27, SLAM
	db 30, ROCK_TOMB
	db 32, BITE
	db 34, HORN_ATTACK
	db 37, ROCK_THROW
	db 39, HEADBUTT
	db 40, PIN_MISSILE
	db 43, CRUNCH
	db 46, WATER_PULSE
	db 48, SPIKE_CANNON
	db 51, SUBMISSION
	db 54, AURORA_BEAM
	db 56, SCALD
	db 58, HYDRO_PUMP
	db 61, HORN_DRILL
	db 0

KabutoEvosMoves:
; Evolutions
	db EV_LEVEL, 40, KABUTOPS
	db 0
; Learnset
	db 9, WATER_GUN
	db 11, ABSORB
	db 14, MUD_SLAP
	db 17, AQUA_JET
	db 20, ROCK_THROW
	db 22, SCREECH
	db 24, RAPID_SPIN
	db 26, MUD_SHOT
	db 29, CONFUSE_RAY
	db 32, MEGA_DRAIN
	db 35, SLASH
	db 38, LEECH_LIFE
	db 41, ROCK_TOMB
	db 43, AERIAL_ACE
	db 45, WATER_PULSE
	db 49, AURORA_BEAM
	db 53, SCALD
	db 57, HYDRO_PUMP
	db 61, STONE_EDGE
	db 0

KabutopsEvosMoves:
; Evolutions
	db 0
; Learnset
	db 9, WATER_GUN
	db 11, ABSORB
	db 14, MUD_SLAP
	db 17, AQUA_JET
	db 19, LOW_KICK
	db 20, ROCK_THROW
	db 22, SCREECH
	db 24, RAPID_SPIN
	db 26, MUD_SHOT
	db 29, CONFUSE_RAY
	db 32, MEGA_DRAIN
	db 35, SLASH
	db 38, LEECH_LIFE
	db 44, ROCK_TOMB
	db 46, AERIAL_ACE
	db 48, WATER_PULSE
	db 50, SUBMISSION
	db 52, AURORA_BEAM
	db 54, PSYCHO_CUT
	db 56, SCALD
	db 58, SUPERPOWER
	db 60, HYDRO_PUMP
	db 62, NIGHT_SLASH
	db 64, STONE_EDGE
	db 0

AerodactylEvosMoves:
; Evolutions
	db EV_ITEM, MEGA_STONE_X, 1, MARODACTYL
	db EV_ITEM, MEGA_STONE_Y, 1, MARODACTYL
	db 0
; Learnset
	db 5, SUPERSONIC
	db 7, WING_ATTACK
	db 10, ROCK_THROW
	db 13, BITE
	db 17, AERIAL_ACE
	db 20, ROCK_SLIDE
	db 23, DRAGON_RAGE
	db 27, CRUNCH
	db 30, STEEL_WING
	db 33, IRON_TAIL
	db 35, ROCK_TOMB
	db 38, DRAGONBREATH
	db 40, ROOST
	db 42, DRAGON_DANCE
	db 44, WHIRLWIND
	db 46, FLAMETHROWER
	db 48, ROAR
	db 50, AIR_CUTTER
	db 53, STONE_EDGE
	db 57, HURRICANE
	db 60, HYPER_BEAM
	db 0

MArodactylEvosMoves:
; Evolutions
	db 0
; Learnset
	db 10, SUPERSONIC
	db 12, WING_ATTACK
	db 15, ROCK_THROW
	db 18, BITE
	db 22, AERIAL_ACE
	db 25, ROCK_SLIDE
	db 28, DRAGON_RAGE
	db 30, SUPER_FANG
	db 32, CRUNCH
	db 35, STEEL_WING
	db 38, IRON_TAIL
	db 40, ROCK_TOMB
	db 43, DRAGONBREATH
	db 45, ROOST
	db 47, DRAGON_DANCE
	db 49, WHIRLWIND
	db 51, FLAMETHROWER
	db 52, EXTREMESPEED
	db 53, ROAR
	db 55, AIR_CUTTER
	db 58, STONE_EDGE
	db 62, HURRICANE
	db 65, HYPER_BEAM
	db 67, HEAD_SMASH
	db 0

MunchlaxEvosMoves:
; Evolutions
	db EV_LEVEL, 10, SNORLAX
	db 0
; Learnset
	db 10, LICK
	db 11, BITE
	db 12, MUD_SLAP
	db 13, CRUNCH
	db 14, AMNESIA
	db 15, REST
	db 16, BODY_SLAM
	db 17, METRONOME
	db 18, HARDEN
	db 19, SCREECH
	db 20, ROCK_TOMB
	db 21, LOVELY_KISS
	db 22, FIRE_PUNCH
	db 23, ICE_PUNCH
	db 24, THUNDERPUNCH
	db 25, DOUBLE_EDGE
	db 26, SUBMISSION
	db 27, ZEN_HEADBUTT
	db 28, SUPERPOWER
	db 29, HYPER_BEAM
	db 30, PLAY_ROUGH
	db 0

SnorlaxEvosMoves:
; Evolutions
	db 0
; Learnset
	db 10, LICK
	db 13, BITE
	db 15, MUD_SLAP
	db 18, CRUNCH
	db 21, AMNESIA
	db 27, REST
	db 30, BODY_SLAM
	db 33, METRONOME
	db 35, HARDEN
	db 37, SCREECH
	db 39, ROCK_TOMB
	db 42, LOVELY_KISS
	db 46, FIRE_PUNCH
	db 47, ICE_PUNCH
	db 48, THUNDERPUNCH
	db 50, DOUBLE_EDGE
	db 52, SUBMISSION
	db 55, ZEN_HEADBUTT
	db 58, SUPERPOWER
	db 61, HYPER_BEAM
	db 0

ArticunoEvosMoves:
; Evolutions
	db 0
; Learnset
	db 5, GUST
	db 10, REFLECT
	db 15, LEER
	db 20, AGILITY
	db 25, SWIFT
	db 30, HAZE
	db 34, TWISTER
	db 37, WATER_PULSE
	db 40, STEEL_WING
	db 42, AIR_CUTTER
	db 45, WHIRLWIND
	db 47, ROOST
	db 50, ICE_BEAM
	db 52, AIR_SLASH
	db 54, ICICLE_SPEAR
	db 55, HAIL
	db 58, AERIAL_ACE
	db 61, BLIZZARD
	db 64, ROAR
	db 65, HURRICANE
	db 0

ZapdosEvosMoves:
; Evolutions
	db 0
; Learnset
	db 5, THUNDERSHOCK
	db 7, LEER
	db 10, LIGHT_SCREEN
	db 15, PECK
	db 20, AGILITY
	db 25, SWIFT
	db 30, THUNDER_WAVE
	db 31, SHOCK_WAVE
	db 34, TWISTER
	db 37, AERIAL_ACE
	db 40, STEEL_WING
	db 42, DRILL_PECK
	db 45, WHIRLWIND
	db 47, ROOST
	db 50, HEAT_WAVE
	db 53, ZAP_CANNON
	db 57, RAIN_DANCE
	db 60, AIR_CUTTER
	db 63, THUNDER
	db 64, ROAR
	db 65, HURRICANE
	db 0

MoltresEvosMoves:
; Evolutions
	db 0
; Learnset
	db 5, EMBER
	db 7, GUST
	db 10, LEER
	db 15, WING_ATTACK
	db 20, AGILITY
	db 25, SWIFT
	db 30, TWISTER
	db 35, AIR_SLASH
	db 40, STEEL_WING
	db 42, AERIAL_ACE
	db 45, WHIRLWIND
	db 47, ROOST
	db 50, FIRE_SPIN
	db 52, FLAMETHROWER
	db 53, AIR_CUTTER
	db 55, SUNNY_DAY
	db 57, HEAT_WAVE
	db 60, SOLARBEAM
	db 63, FLARE_BLITZ
	db 64, ROAR
	db 65, HURRICANE
	db 0

DratiniEvosMoves:
; Evolutions
	db EV_LEVEL, 30, DRAGONAIR
	db 0
; Learnset
	db 5, TWISTER
	db 7, SUPERSONIC
	db 10, THUNDER_WAVE
	db 12, LIGHT_SCREEN
	db 16, SWIFT
	db 18, SHOCK_WAVE
	db 20, AGILITY
	db 22, DRAGONBREATH
	db 25, SLAM
	db 28, HEADBUTT
	db 31, FIRE_SPIN
	db 33, WATER_PULSE
	db 35, FLAMETHROWER
	db 38, DRAGON_RAGE
	db 42, DRAGON_DANCE
	db 44, IRON_TAIL
	db 46, RAIN_DANCE
	db 48, AQUA_JET
	db 50, HYPER_BEAM
	db 53, ZAP_CANNON
	db 55, OUTRAGE
	db 0

DragonairEvosMoves:
; Evolutions
	db EV_LEVEL, 55, DRAGONITE
	db 0
; Learnset
	db 5, TWISTER
	db 7, SUPERSONIC
	db 10, THUNDER_WAVE
	db 12, LIGHT_SCREEN
	db 16, SWIFT
	db 18, SHOCK_WAVE
	db 20, AGILITY
	db 22, DRAGONBREATH
	db 25, SLAM
	db 28, HEADBUTT
	db 34, FIRE_SPIN
	db 36, WATER_PULSE
	db 38, FLAMETHROWER
	db 41, DRAGON_RAGE
	db 45, DRAGON_DANCE
	db 47, IRON_TAIL
	db 48, RAIN_DANCE
	db 49, SUNNY_DAY
	db 50, HAIL
	db 51, SANDSTORM
	db 52, AQUA_JET
	db 53, HYPER_BEAM
	db 55, HORN_DRILL
	db 57, ZAP_CANNON
	db 59, OUTRAGE
	db 0

DragoniteEvosMoves:
; Evolutions
	db EV_ITEM, MEGA_STONE_X, 1, MDRAGONITE
	db EV_ITEM, MEGA_STONE_Y, 1, MDRAGONITE
	db 0
; Learnset
	db 5, TWISTER
	db 7, SUPERSONIC
	db 10, THUNDER_WAVE
	db 12, LIGHT_SCREEN
	db 16, SWIFT
	db 17, AERIAL_ACE
	db 18, SHOCK_WAVE
	db 20, AGILITY
	db 22, DRAGONBREATH
	db 25, SLAM
	db 28, HEADBUTT
	db 34, FIRE_SPIN
	db 36, WATER_PULSE
	db 39, FLAMETHROWER
	db 42, DRAGON_RAGE
	db 44, DRAGON_DANCE
	db 46, WHIRLWIND
	db 48, IRON_TAIL
	db 49, RAIN_DANCE
	db 51, AQUA_JET
	db 53, HYPER_BEAM
	db 55, HURRICANE
	db 56, STEEL_WING
	db 57, ROOST
	db 58, AIR_SLASH
	db 59, HORN_DRILL
	db 60, AIR_CUTTER
	db 61, ZAP_CANNON
	db 62, SUPERPOWER
	db 63, FIRE_PUNCH
	db 64, THUNDERPUNCH
	db 65, ICE_PUNCH
	db 66, ROCK_TOMB
	db 67, HEAT_WAVE
	db 68, ROAR
	db 70, OUTRAGE
	db 75, BARRIER
	db 80, EXTREMESPEED
	db 0

MDragoniteEvosMoves:
; Evolutions
	db 0
; Learnset
	db 10, TWISTER
	db 12, SUPERSONIC
	db 15, THUNDER_WAVE
	db 17, LIGHT_SCREEN
	db 21, SWIFT
	db 22, AERIAL_ACE
	db 23, SHOCK_WAVE
	db 25, AGILITY
	db 27, DRAGONBREATH
	db 30, SLAM
	db 33, HEADBUTT
	db 39, FIRE_SPIN
	db 41, WATER_PULSE
	db 44, FLAMETHROWER
	db 47, DRAGON_RAGE
	db 49, DRAGON_DANCE
	db 51, WHIRLWIND
	db 53, IRON_TAIL
	db 54, RAIN_DANCE
	db 56, AQUA_JET
	db 58, HYPER_BEAM
	db 60, HURRICANE
	db 61, STEEL_WING
	db 62, ROOST
	db 63, AIR_SLASH
	db 64, HORN_DRILL
	db 65, AIR_CUTTER
	db 66, ZAP_CANNON
	db 67, SUPERPOWER
	db 68, FIRE_PUNCH
	db 69, THUNDERPUNCH
	db 70, ICE_PUNCH
	db 71, ROCK_TOMB
	db 72, HEAT_WAVE
	db 73, ROAR
	db 75, OUTRAGE
	db 77, DRAININGKISS
	db 80, BARRIER
	db 83, MOONBLAST
	db 85, EXTREMESPEED
	db 87, LOVELY_KISS
	db 90, PLAY_ROUGH
	db 0

ArmMewtwoEvosMoves:
; Evolutions
	db 0
; Learnset
	db 19, PSYCHO_CUT
	db 23, FIRE_PUNCH
	db 24, ICE_PUNCH
	db 25, THUNDERPUNCH
	db 35, LIGHT_SCREEN
	db 39, SHOCK_WAVE
	db 43, AGILITY
	db 51, AMNESIA
	db 63, BARRIER
	db 67, PSYCHIC_M
	db 73, RECOVER
	db 74, DRAIN_PUNCH
	db 75, ZEN_HEADBUTT
	db 76, IRON_TAIL
	db 77, CONFUSE_RAY
	db 78, ROCK_TOMB
	db 79, WATER_PULSE
	db 80, HURRICANE
	db 81, ENERGY_BALL
	db 82, FLAMETHROWER
	db 83, PSYSTRIKE
	db 0

MewtwoEvosMoves:
; Evolutions
	db EV_ITEM, MEGA_STONE_X, 1, MMEWTWOX
	db EV_ITEM, MEGA_STONE_Y, 1, MMEWTWOY
	db 0
; Learnset
	db 16, PSYCHO_CUT
	db 20, FIRE_PUNCH
	db 21, ICE_PUNCH
	db 22, THUNDERPUNCH
	db 32, LIGHT_SCREEN
	db 36, SHOCK_WAVE
	db 40, AGILITY
	db 48, AMNESIA
	db 60, BARRIER
	db 64, PSYCHIC_M
	db 70, RECOVER
	db 71, DRAIN_PUNCH
	db 72, ZEN_HEADBUTT
	db 73, IRON_TAIL
	db 74, CONFUSE_RAY
	db 75, ROCK_TOMB
	db 76, WATER_PULSE
	db 77, HURRICANE
	db 78, ENERGY_BALL
	db 79, FLAMETHROWER
	db 80, PSYSTRIKE
	db 0

MMewtwoXEvosMoves:
; Evolutions
	db 0
; Learnset
	db 21, PSYCHO_CUT
	db 25, FIRE_PUNCH
	db 26, ICE_PUNCH
	db 27, THUNDERPUNCH
	db 37, LIGHT_SCREEN
	db 41, SHOCK_WAVE
	db 45, AGILITY
	db 53, AMNESIA
	db 65, BARRIER
	db 69, PSYCHIC_M
	db 70, SUPERPOWER
	db 71, SLUDGE_WAVE
	db 72, SHADOW_CLAW
	db 73, HI_JUMP_KICK
	db 74, LEECH_LIFE
	db 75, RECOVER
	db 76, DRAIN_PUNCH
	db 77, ZEN_HEADBUTT
	db 78, IRON_TAIL
	db 79, CONFUSE_RAY
	db 80, ROCK_TOMB
	db 81, WATER_PULSE
	db 82, HURRICANE
	db 83, ENERGY_BALL
	db 84, PLAY_ROUGH
	db 84, FLAMETHROWER
	db 85, PSYSTRIKE
	db 0

MMewtwoYEvosMoves:
; Evolutions
	db 0
; Learnset
	db 21, PSYCHO_CUT
	db 25, FIRE_PUNCH
	db 26, ICE_PUNCH
	db 27, THUNDERPUNCH
	db 37, LIGHT_SCREEN
	db 41, SHOCK_WAVE
	db 45, AGILITY
	db 53, AMNESIA
	db 65, BARRIER
	db 69, PSYCHIC_M
	db 70, NIGHT_SLASH
	db 71, PLAY_ROUGH
	db 72, DRAGONBREATH
	db 73, HEAT_WAVE
	db 74, RAZOR_LEAF
	db 75, RECOVER
	db 76, DRAIN_PUNCH
	db 77, ZEN_HEADBUTT
	db 78, IRON_TAIL
	db 79, CONFUSE_RAY
	db 80, ROCK_TOMB
	db 81, WATER_PULSE
	db 82, HURRICANE
	db 83, ENERGY_BALL
	db 84, X_SCISSOR
	db 84, FLAMETHROWER
	db 85, PSYSTRIKE
	db 0

MewEvosMoves:
; Evolutions
	db 0
; Learnset
	db 2, POUND
	db 3, WHIRLWIND
	db 4, LIGHT_SCREEN
	db 5, DEFENSE_CURL
	db 6, REFLECT
	db 7, HYPNOSIS
	db 8, AGILITY
	db 9, CHARM
	db 10, TRANSFORM
	db 11, MUD_SLAP
	db 12, ABSORB
	db 13, WRAP
	db 14, TWISTER
	db 15, WATER_GUN
	db 16, STRING_SHOT
	db 17, FOCUS_ENERGY
	db 18, PIN_MISSILE
	db 19, NIGHT_SHADE
	db 20, MEGA_PUNCH
	db 21, MUD_SHOT
	db 22, MEGA_DRAIN
	db 23, CONFUSION
	db 24, SUPER_FANG
	db 25, ROCK_THROW
	db 26, DREAM_EATER
	db 27, BULLET_SEED
	db 28, LOW_KICK
	db 29, DRAGON_RAGE
	db 30, METRONOME
	db 31, DIG
	db 32, GIGA_DRAIN
	db 33, SHADOW_CLAW
	db 34, AERIAL_ACE
	db 35, ROCK_TOMB
	db 36, CONFUSE_RAY
	db 37, SWIFT
	db 38, LEECH_SEED
	db 39, BULLET_PUNCH
	db 40, PSYCHIC_M
	db 41, EARTHQUAKE
	db 42, SWORDS_DANCE
	db 43, MAGICAL_LEAF
	db 44, TAKE_DOWN
	db 45, ROCK_SLIDE
	db 46, CRUNCH
	db 47, WATER_PULSE
	db 48, LEECH_LIFE
	db 49, DRAIN_PUNCH
	db 50, AMNESIA
	db 51, FISSURE
	db 52, SUBMISSION
	db 53, HEADBUTT
	db 54, MACH_PUNCH
	db 55, STONE_EDGE
	db 56, HYPER_FANG
	db 57, THUNDERPUNCH
	db 58, FIRE_PUNCH
	db 59, ICE_PUNCH
	db 60, DRAGONBREATH
	db 61, AIR_CUTTER
	db 62, ICICLE_SPEAR
	db 63, STEEL_WING
	db 64, SCALD
	db 65, HEAD_SMASH
	db 66, ZEN_HEADBUTT
	db 67, SHOCK_WAVE
	db 68, FLAMETHROWER
	db 69, ROOST
	db 70, SLUDGE_WAVE
	db 71, FLARE_BLITZ
	db 72, HURRICANE
	db 73, IRON_TAIL
	db 74, PSYCHO_CUT
	db 75, FIRE_SPIN
	db 76, ENERGY_BALL
	db 78, AIR_SLASH
	db 79, RAPID_SPIN
	db 80, SUPERPOWER
	db 81, LEAF_STORM
	db 82, PLAY_ROUGH
	db 83, INFESTATION
	db 84, HEAT_WAVE
	db 85, HORN_DRILL
	db 86, GUILLOTINE
	db 87, EXTREMESPEED
	db 88, SOLARBEAM
	db 89, ZAP_CANNON
	db 90, PAY_DAY
	db 91, SOFTBOILED
	db 92, BONEMERANG
	db 93, CRABHAMMER
	db 94, VOLT_TACKLE
	db 95, FRENZY_PLANT
	db 96, HYDRO_CANNON
	db 97, BLAST_BURN
	db 98, TRI_ATTACK
	db 99, MIRROR_MOVE
	db 100, SPLASH
	db 0

MeltanEvosMoves:
; Evolutions
	db EV_ITEM, METAL_COAT, 1, MELMETAL
	db 0
; Learnset
	db 9, LEER
	db 18, THUNDER_WAVE
	db 27, THUNDERSHOCK
	db 36, ACID_ARMOR
	db 40, THUNDERSHOCK
	db 45, FLASH_CANNON
	db 0

MelmetalEvosMoves:
; Evolutions
	db 0
; Learnset
	db 9, LEER
	db 18, THUNDER_WAVE
	db 23, MUD_SLAP
	db 27, THUNDERSHOCK
	db 31, SHOCK_WAVE
	db 36, ACID_ARMOR
	db 39, ZEN_HEADBUTT
	db 41, DRAIN_PUNCH
	db 45, FLASH_CANNON
	db 50, ROCK_TOMB
	db 54, MEGA_PUNCH
	db 60, ICE_PUNCH
	db 70, SOLARBEAM
	db 75, SUPERPOWER
	db 80, HYPER_BEAM
	db 85, ZAP_CANNON
	db 90, METEOR_MASH
	db 95, THUNDERPUNCH
	db 100, DOUBLE_IRON_BASH
	db 0

VenustoiseEvosMoves:
; Evolutions
	db 0
; Learnset
	db 7, LEECH_SEED
	db 8, BUBBLE
	db 9, VINE_WHIP
	db 12, BITE
	db 13, POISONPOWDER
	db 14, WATER_GUN
	db 15, SLEEP_POWDER
	db 18, HAZE
	db 20, RAZOR_LEAF
	db 23, MUD_SLAP
	db 24, MAGICAL_LEAF
	db 25, WATER_PULSE
	db 28, GROWTH
	db 29, DEFENSE_CURL
	db 30, HEADBUTT
	db 31, DOUBLE_EDGE
	db 32, CRUNCH
	db 33, PETAL_DANCE
	db 34, SUBMISSION
	db 36, FLASH_CANNON
	db 38, SLUDGE_WAVE
	db 41, IRON_TAIL
	db 42, LEAF_STORM
	db 43, ICE_PUNCH
	db 45, SCALD
	db 47, ENERGY_BALL
	db 48, AQUA_JET
	db 50, ZEN_HEADBUTT
	db 52, SOLARBEAM
	db 53, ROCK_TOMB
	db 55, HYDRO_PUMP
	db 60, FRENZY_PLANT
	db 61, HYDRO_CANNON
	db 0

ThuFiZerEvosMoves:
; Evolutions
	db 0
; Learnset
	db 6, GUST
	db 8, THUNDERSHOCK
	db 10, EMBER
	db 12, LEER
	db 14, LIGHT_SCREEN
	db 16, REFLECT
	db 18, PECK
	db 20, WING_ATTACK
	db 22, LEER
	db 24, AGILITY
	db 26, SWIFT
	db 28, THUNDER_WAVE
	db 30, AIR_SLASH
	db 32, HAZE
	db 34, SHOCK_WAVE
	db 36, TWISTER
	db 38, WATER_PULSE
	db 40, STEEL_WING
	db 42, AIR_CUTTER
	db 44, DRILL_PECK
	db 46, ROOST
	db 48, ZAP_CANNON
	db 50, ICE_BEAM
	db 52, FIRE_SPIN
	db 54, AIR_SLASH
	db 56, HEAT_WAVE
	db 58, ICICLE_SPEAR
	db 60, FLAMETHROWER
	db 62, WHIRLWIND
	db 63, ROAR
	db 64, HAIL
	db 66, RAIN_DANCE
	db 68, SUNNY_DAY
	db 70, AERIAL_ACE
	db 72, SOLARBEAM
	db 74, BLIZZARD
	db 76, THUNDER
	db 78, FLARE_BLITZ
	db 80, HURRICANE
	db 0

MZygardeEvosMoves:
; Evolutions
	db 0
; Learnset
	db 8, HAZE
	db 12, SWIFT
	db 16, DIG
	db 23, IRON_TAIL
	db 28, ZEN_HEADBUTT
	db 32, CRUNCH
	db 38, SHOCK_WAVE
	db 43, SLUDGE_WAVE
	db 50, DRAGON_DANCE
	db 56, GLARE
	db 63, EXTREMESPEED
	db 72, SUPERPOWER
	db 80, EARTHQUAKE
	db 88, OUTRAGE
	db 95, THOUSANDARROWS
	db 0

UNecrozmaEvosMoves:
; Evolutions
	db 0
; Learnset
	db 16, SLASH
	db 24, NIGHT_SLASH
	db 32, PSYCHO_CUT
	db 40, ROCK_TOMB
	db 48, LIGHT_SCREEN
	db 56, SHOCK_WAVE
	db 64, AERIAL_ACE
	db 70, SHADOW_CLAW
	db 76, SOLARBEAM
	db 84, HEAT_WAVE
	db 90, DRAGON_DANCE
	db 95, PHOTONGEYSER
	db 0

MRayquazaEvosMoves:
; Evolutions
	db 0
; Learnset
	db 9, CRUNCH
	db 12, SWIFT
	db 15, MUD_SLAP
	db 18, DRAGON_DANCE
	db 21, WRAP
	db 24, ROCK_TOMB
	db 27, EXTREMESPEED
	db 31, WATER_PULSE
	db 36, AERIAL_ACE
	db 40, SHOCK_WAVE
	db 44, HEADBUTT
	db 47, SHADOW_CLAW
	db 54, REST
	db 58, IRON_TAIL
	db 63, FLY
	db 67, ENERGY_BALL
	db 72, HURRICANE
	db 76, SOLARBEAM
	db 81, OUTRAGE
	db 85, FLAMETHROWER
	db 90, HYPER_BEAM
	db 95, DRAGONASCENT
	db 0

EEternatusEvosMoves:
; Evolutions
	db 0
; Learnset
	db 8, TOXIC
	db 16, SCREECH
	db 24, DRAGON_DANCE
	db 30, LIGHT_SCREEN
	db 38, SLUDGE_WAVE
	db 48, FLAMETHROWER
	db 58, SOLARBEAM
	db 72, RECOVER
	db 80, HYPER_BEAM
	db 88, ETERNABEAM
	db 0

ArceusEvosMoves:
; Evolutions
	db 0
; Learnset
	db 10, HAZE
	db 20, EARTHQUAKE
	db 30, SONICBOOM
	db 40, EXTREMESPEED
	db 50, LIGHT_SCREEN
	db 60, FLY
	db 70, RECOVER
	db 80, HYPER_BEAM
	db 90, EXPLOSION
	db 100, JUDGMENT
	db 0

MissingNoEvosMoves:
; Evolutions
	db 0
; Learnset
	db 0

; rearranged to have all the proxies together
FossilKabutopsEvosMoves:
FossilAerodactylEvosMoves:
MonGhostEvosMoves:
BluesDadEvosMoves:
BluesMomEvosMoves:
DadEvosMoves:
MapPiecesEvosMoves: ; new
; Evolutions
	db 0
; Learnset
	db 0
